...
if [ "$IS_CMAKE_COMPILE_FLAG" -eq 0 ]; then
    if [ "$IS_DEBUG" -eq 1 ]; then
        COMPLIE_FLAG="-DCMAKE_C_COMPILER=mpicc -DCMAKE_CXX_COMPILER=mpicxx -DSFEM_ENABLE_CUDA=ON -DSFEM_ENABLE_PYTHON=OFF -DSFEM_ENABLE_OPENMP=ON -DCMAKE_CUDA_ARCHITECTURES=${CUDA_ARCHITECTURES} -DSFEM_CUDA_ARCH=${CUDA_ARCHITECTURES} -DCMAKE_BUILD_TYPE=Debug"
    else
