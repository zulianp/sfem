...
$9.25\times$ one assembly.  We show that the $m$ evaluations share almost all of
their work, and that what they share can be hoisted out of the trial-step loop
exactly, not approximately, because the state enters the element kernel through a
quantity that is affine in $\alpha$.  The result is a single pass over the mesh
...
entire output is a buffer of $m$ scalars.  For comparison, the same reorganization applied
to the energy merit --- where it is already in place --- costs $1.17\times$ one
step at $m=12$.
\end{abstract}
...
$m\in\{1,2,4,8,12,20\}$ reported as $T(m)/T(1)$: the residual arm reads $9.25$ at
$m=12$ with the per-step implementation, against $1.17$ for the energy arm, which
is the same reorganization already applied one layer up.
