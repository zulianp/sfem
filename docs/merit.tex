...
within that one node's patch, first by its incident elements and then by the
trial steps --- two sequential vector loops, never nested, the first
precomputing everything the second would otherwise recompute.  Normalizing each
element's orientation so the node is always the same local node makes its basis
function the same in every lane and at every step, which is what allows those
terms to leave both loops.  The second loop's lanes are the trial
steps, because $m$ is a quantity the line search chooses and is therefore set to
the hardware's vector length --- unlike the number of elements incident on a
node, which the mesh fixes.  It is the only arrangement built.  What this is worth is bounded by the constitutive evaluation, which
...
\subsection*{Two vectorization axes, one after the other}

The lanes are filled from within one node's patch: the elements of
$\mathcal{E}(n)$, and the trial steps $\alpha_1,\dots,\alpha_m$.  What is
deliberately \emph{not} done is to fill lanes by processing several patches at
once; the lanes belong to one node.

The two axes are two \emph{sequential} vector loops, never one inside the other.
Nesting them is not an arrangement the hardware has: a lane loop containing
another loop is no longer innermost and does not vectorize at all, which is the
same measured fact the stepped objective already records from the other
direction.  So the first loop is vectorized over the incident elements and
computes everything independent of the trial step --- the gathers, the geometry,
the reference gradients of the state and of the direction, the physical gradient
of the fixed basis function, and $w_q\det J_e$ --- writing them to per-element
buffers.  The second loop reads those buffers and does only what the trial step
changes: the combination, the constitutive law, and the contraction.

\subsection*{The orientation that pays for it}

A node sits at a different local index in each of its incident elements, so the
basis function paired with it --- the test function of \eqref{eq:formsum} ---
would differ from lane to lane, and nothing about it could be hoisted.  Each
element is therefore presented with its local numbering permuted so that $n$ is
always local node $0$.  Then $\varphi_0$ is the same basis function in every lane
and at every trial step, and everything belonging to it leaves both loops: for a
low-order simplex $\widehat\nabla\varphi_0$ is a constant the generator emits as
a literal rather than a table read, and $J_e^{-\top}\widehat\nabla\varphi_0$ is
computed in the first loop and merely read in the second.
...
\begin{algorithm}
\caption{Residual merit at $m$ trial steps: one node, two vector loops}
\label{alg:kernel}
\begin{algorithmic}[1]
\State $s \gets 0 \in \R^{m}$ \Comment{thread-private, the only output}
\State $\widehat\nabla\varphi_0$ \Comment{a literal: same for every node, lane and step}
\For{each owned node $n$ (threads)}
  \State $\rho_k \gets g_n \in \R^{N_c}$, $k=1,\dots,m$ \Comment{registers, not memory}
  \Statex \textbf{loop 1 --- lanes are the elements of $\mathcal{E}(n)$}
  \For{$e \in \mathcal{E}(n)$}
    \State present $e$ with $n$ at local $0$, by an even permutation
    \State gather $x^e,h^e$; form $\det J_e$, $\mathrm{adj}\,J_e$
    \For{$q = 1,\dots,N_q$}
      \State $G^e_q \gets \widehat\nabla u_q$, \; $H^e_q \gets \widehat\nabla w_q$
      \State $T^e_q \gets J_e^{-\top}\widehat\nabla\varphi_0$, \;
             $\omega^e_q \gets w_q \det J_e$
    \EndFor
  \EndFor
  \Statex \textbf{loop 2 --- lanes are the trial steps $\alpha_k$}
  \For{$k = 1,\dots,m$}
    \For{$e \in \mathcal{E}(n)$, $q = 1,\dots,N_q$}
      \State $\rho_k \mathrel{+}= \omega^e_q \,
             P\bigl(J_e^{-\top}(G^e_q + \alpha_k H^e_q)\bigr)\, T^e_q$
    \EndFor
...
The second loop's lanes are the trial steps, and the reason is that $m$ is ours
to choose.  The objection that would otherwise count against that axis --- a
runtime count shorter than the vector width, leaving lanes idle --- does not
apply to a quantity the line search sets: $m$ is rounded to the hardware's
vector length, or a multiple of it.  The lanes are then uniform, always full and
identical for every node, whereas the number of elements incident on a node is a
property of the mesh, varies from node to node, and leaves a ragged tail no
choice of ours can remove.

Sampling more step lengths than a classical search would ask for is close to
free in this arrangement, because the extra samples occupy lanes that would
otherwise idle.  Rounding $m$ up to the vector width buys a finer picture of
$\phi$ along the direction at no cost, which is a pleasant inversion of the
usual trade.

The alternatives --- lanes over the incident elements with the steps serial, and
lanes over the flattened element-by-step product --- are not built.  Competing
loop arrangements are normally settled here by measurement rather than by
argument, and this one is the exception because the comparison has no contested
regime: the step count is chosen and the incident count is not, so the
mesh-determined axis is never the better-behaved one and building it would
confirm that at the price of a second kernel to keep correct.

What the shape costs is that an element is visited once per node it carries, so
the constitutive evaluation $P(\nabla u)$ is performed about $N_s$ times where an
element-centric pass performs it once and contracts it against every test
function.  That cost is measured rather than argued.  On TET4 Mooney--Rivlin
Kelvin--Voigt at $768{,}000$ elements on one Grace node at $72$ threads, against
the implementation that assembles the whole residual once per trial step and
fitted to $T(m)=a+bm$:

\begin{center}
\begin{tabular}{lrrrr}
\toprule
arm & $a$ [s] & $b$ [s] & $a/(a{+}b)$ & $T(12)$ [s] \\
\midrule
assemble per step   & $2.33\times10^{-4}$ & $3.01\times10^{-3}$ & 7.2\%  & $3.63\times10^{-2}$ \\
patch, node-centric & $5.60\times10^{-3}$ & $1.40\times10^{-3}$ & 80.0\% & $2.22\times10^{-2}$ \\
\bottomrule
\end{tabular}
\end{center}

The step-independent fraction rises from $7\%$ to $80\%$, which is the quantity
this work exists to move and is past the $57\%$ an already-sampled energy kernel
of comparable constitutive cost reaches.  At twelve trial steps the patch kernel
is $1.63\times$ faster.

The redundancy is in the same numbers, and it is the honest half of the result:
at a \emph{single} step the patch kernel is $2.16\times$ slower, which is what
visiting each element once per node costs.  That it is $2.16$ rather than
$N_s=4$ is the scatter and the accumulator traffic it no longer pays.  The two
cross between three and four trial steps, so the arrangement is a loss for a
search sampling one or two points and a gain for anything sampling more --- the
regime the interface was built for.
