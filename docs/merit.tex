...
assemblies.  Measured on a Mooney--Rivlin Kelvin--Voigt solid, $m=12$ costs
$11.8\times$ one assembly at $m=12$, measured at $512{,}000$ elements on a
Grace node at $72$ threads.  We show that the $m$ evaluations share a
substantial part of their work, and that what they share can be hoisted out of
the trial-step loop exactly, not approximately, because the state enters the
element kernel through a quantity that is affine in $\alpha$.  The result is a single pass over the mesh
that produces all $m$ merit values, whose cost model is one gather, one
trial-function accumulation per field role, and $m$ repetitions of the
constitutive evaluation alone.  No per-step residual vector is materialized at any
point: the contraction is closed inside a patch of elements, where the node sums
are complete, so each trial step leaves the patch as a single number and a thread's
entire output is a buffer of $m$ scalars.  How much that is worth is bounded by the
constitutive evaluation, which repeats per trial step and cannot be hoisted.
The same reorganization is already in place for the energy merit, and measured
on the same node it costs $5.7\times$ one step at $m=12$ for a Mooney-Rivlin
class material and $3.7\times$ for linear elasticity --- a real saving against
$11.8$, and not the near-flat cost a reading of "sampled" might suggest.
\end{abstract}
