...
static SFEM_INLINE void tet4_linear_elasticity_apply_adj(
        const real_t mu,
        const real_t lambda,
    ...
    real_t disp_grad[9];
    {
        const real_t x0 = (real_t)1.0 / jacobian_determinant;
    ...
    real_t *P = disp_grad;
    {
        const real_t x0 = (real_t)(1.0 / 3.0) * mu;
        const real_t x1 = (real_t)(1.0 / 12.0) * lambda *
                            (2 * disp_grad[0] + 2 * disp_grad[4] + 2 * disp_grad[8]);
        const real_t x2 = (real_t)(1.0 / 6.0) * mu;
