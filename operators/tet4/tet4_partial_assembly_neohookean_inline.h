...
static SFEM_INLINE void tet4_partial_assembly_neohookean(const scalar_t *const SFEM_RESTRICT S_iklm,
    ...
    // mundane ops: 176 divs: 0 sqrts: 0 
// total ops: 176

const scalar_t x0 = S_iklm[0]*inc_grad[0] + S_iklm[12]*inc_grad[4] + S_iklm[15]*inc_grad[5] + S_iklm[18]*inc_grad[6] + S_iklm[21]*inc_grad[7] + S_iklm[24]*inc_grad[8] + S_iklm[3]*inc_grad[1] + 
S_iklm[6]*inc_grad[2] + S_iklm[9]*inc_grad[3];
const scalar_t x1 = S_iklm[10]*inc_grad[3] + S_iklm[13]*inc_grad[4] + S_iklm[16]*inc_grad[5] + S_iklm[19]*inc_grad[6] + S_iklm[1]*inc_grad[0] + S_iklm[22]*inc_grad[7] + S_iklm[25]*inc_grad[8] + 
S_iklm[4]*inc_grad[1] + S_iklm[7]*inc_grad[2];
const scalar_t x2 = S_iklm[11]*inc_grad[3] + S_iklm[14]*inc_grad[4] + S_iklm[17]*inc_grad[5] + S_iklm[20]*inc_grad[6] + S_iklm[23]*inc_grad[7] + S_iklm[26]*inc_grad[8] + S_iklm[2]*inc_grad[0] + 
S_iklm[5]*inc_grad[1] + S_iklm[8]*inc_grad[2];
const scalar_t x3 = (1.0/6.0)*jacobian_determinant*qw;
const scalar_t x4 = S_iklm[27]*inc_grad[0] + S_iklm[30]*inc_grad[1] + S_iklm[33]*inc_grad[2] + S_iklm[36]*inc_grad[3] + S_iklm[39]*inc_grad[4] + S_iklm[42]*inc_grad[5] + S_iklm[45]*inc_grad[6] + 
S_iklm[48]*inc_grad[7] + S_iklm[51]*inc_grad[8];
const scalar_t x5 = S_iklm[28]*inc_grad[0] + S_iklm[31]*inc_grad[1] + S_iklm[34]*inc_grad[2] + S_iklm[37]*inc_grad[3] + S_iklm[40]*inc_grad[4] + S_iklm[43]*inc_grad[5] + S_iklm[46]*inc_grad[6] + 
S_iklm[49]*inc_grad[7] + S_iklm[52]*inc_grad[8];
const scalar_t x6 = S_iklm[29]*inc_grad[0] + S_iklm[32]*inc_grad[1] + S_iklm[35]*inc_grad[2] + S_iklm[38]*inc_grad[3] + S_iklm[41]*inc_grad[4] + S_iklm[44]*inc_grad[5] + S_iklm[47]*inc_grad[6] + 
S_iklm[50]*inc_grad[7] + S_iklm[53]*inc_grad[8];
const scalar_t x7 = S_iklm[54]*inc_grad[0] + S_iklm[57]*inc_grad[1] + S_iklm[60]*inc_grad[2] + S_iklm[63]*inc_grad[3] + S_iklm[66]*inc_grad[4] + S_iklm[69]*inc_grad[5] + S_iklm[72]*inc_grad[6] + 
S_iklm[75]*inc_grad[7] + S_iklm[78]*inc_grad[8];
const scalar_t x8 = S_iklm[55]*inc_grad[0] + S_iklm[58]*inc_grad[1] + S_iklm[61]*inc_grad[2] + S_iklm[64]*inc_grad[3] + S_iklm[67]*inc_grad[4] + S_iklm[70]*inc_grad[5] + S_iklm[73]*inc_grad[6] + 
S_iklm[76]*inc_grad[7] + S_iklm[79]*inc_grad[8];
const scalar_t x9 = S_iklm[56]*inc_grad[0] + S_iklm[59]*inc_grad[1] + S_iklm[62]*inc_grad[2] + S_iklm[65]*inc_grad[3] + S_iklm[68]*inc_grad[4] + S_iklm[71]*inc_grad[5] + S_iklm[74]*inc_grad[6] + 
S_iklm[77]*inc_grad[7] + S_iklm[80]*inc_grad[8];

element_outx[0] = -x3*(x0 + x1 + x2);
element_outx[1] = x0*x3;
element_outx[2] = x1*x3;
element_outx[3] = x2*x3;

element_outy[0] = -x3*(x4 + x5 + x6);
element_outy[1] = x3*x4;
element_outy[2] = x3*x5;
element_outy[3] = x3*x6;

element_outz[0] = -x3*(x7 + x8 + x9);
element_outz[1] = x3*x7;
element_outz[2] = x3*x8;
element_outz[3] = x3*x9;
}
