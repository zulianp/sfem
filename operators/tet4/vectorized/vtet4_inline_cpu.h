...
static SFEM_INLINE void vtet4_fff(const vec_t px0,
                                  const vec_t px1,
                                  const vec_t px2,
                                  const vec_t px3,
                                  const vec_t py0,
    ...
    const vec_t x20 = -(scalar_t)(1.0 / 6.0) * x12 + (scalar_t)(1.0 / 6.0) * x15 +
                          (scalar_t)(1.0 / 6.0) * x17 - (scalar_t)(1.0 / 6.0) * x19 +
                          (scalar_t)(1.0 / 6.0) * x4 - (scalar_t)(1.0 / 6.0) * x8;
    const vec_t x21 = x14 - x18;
    const vec_t x22 = (scalar_t)1. / POW2(-x12 + x15 + x17 - x19 + x4 - x8);
...
static SFEM_INLINE vec_t vtet4_det_fff(const vec_t *const fff) {
    return fff[0] * fff[3] * fff[5] - fff[0] * POW2(fff[4]) - POW2(fff[1]) * fff[5] +
           2 * fff[1] * fff[2] * fff[4] - POW2(fff[2]) * fff[3];
}

static SFEM_INLINE void vtet4_adjugate_and_det(const vec_t px0,
                                               const vec_t px1,
                                               const vec_t px2,
                                               const vec_t px3,
                                               const vec_t py0,
                                               ...
                                               const vec_t pz2,
                                               const vec_t pz3,
                                               vec_t *const SFEM_RESTRICT adjugate,
    ...
    const vec_t x0 = jacobian[4] * jacobian[8];
    const vec_t x1 = jacobian[5] * jacobian[7];
    const vec_t x2 = jacobian[1] * jacobian[8];
    const vec_t x3 = jacobian[1] * jacobian[5];
    const vec_t x4 = jacobian[2] * jacobian[4];
