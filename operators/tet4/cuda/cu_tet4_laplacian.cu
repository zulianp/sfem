...
static int cu_tet4_laplacian_apply_tpl(const ptrdiff_t                          nelements,
    ...
    {
        int min_grid_size;
        cudaOccupancyMaxPotentialBlockSize(&min_grid_size, &block_size, cu_tet4_laplacian_apply_kernel<T>, 0, 0);
    }
    ...
    if (stream) {
        ...
        cu_tet4_laplacian_apply_kernel<<<n_blocks, block_size, 0, s>>>(nelements, elements, fff_stride, fff, x, y);
    } else {
        cu_tet4_laplacian_apply_kernel<<<n_blocks, block_size, 0>>>(nelements, elements, fff_stride, fff, x, y);
    }
...
static int cu_tet4_laplacian_diag_tpl(const ptrdiff_t                          nelements,
    ...
    {
        int min_grid_size;
        cudaOccupancyMaxPotentialBlockSize(&min_grid_size, &block_size, cu_tet4_laplacian_diag_kernel<T>, 0, 0);
    }
#endif  // SFEM_USE_OCCUPANCY_MAX_POTENTIAL
    ...
    if (stream) {
        cudaStream_t s = *static_cast<cudaStream_t *>(stream);
        cu_tet4_laplacian_diag_kernel<<<n_blocks, block_size, 0, s>>>(nelements, elements, fff_stride, fff, diag);
    } else {
        cu_tet4_laplacian_diag_kernel<<<n_blocks, block_size, 0>>>(nelements, elements, fff_stride, fff, diag);
    }
...
extern int cu_tet4_laplacian_diag(const ptrdiff_t                 nelements,
    ...
    switch (real_type_diag) {
        case SFEM_REAL_DEFAULT: {
            return cu_tet4_laplacian_diag_tpl(nelements, elements, fff_stride, (cu_jacobian_t *)fff, (real_t *)diag, stream);
        }
        case SFEM_FLOAT32: {
            return cu_tet4_laplacian_diag_tpl(nelements, elements, fff_stride, (cu_jacobian_t *)fff, (float *)diag, stream);
        }
        case SFEM_FLOAT64: {
            return cu_tet4_laplacian_diag_tpl(nelements, elements, fff_stride, (cu_jacobian_t *)fff, (double *)diag, stream);
        }
        default: {
            SFEM_ERROR("[Error] cu_tet4_laplacian_diag: not implemented for type %s (code %d)\n",
...
#pragma unroll(6)
        ...
        cu_tet4_laplacian_matrix_fff(fffe, element_matrix);
...
int cu_tet4_laplacian_crs_tpl(const ptrdiff_t                          nelements,
    ...
    {
        int min_grid_size;
        cudaOccupancyMaxPotentialBlockSize(&min_grid_size, &block_size, cu_tet4_laplacian_crs_kernel<T>, 0, 0);
    }
#endif  // SFEM_USE_OCCUPANCY_MAX_POTENTIAL
    ...
    if (stream) {
        ...
        cu_tet4_laplacian_crs_kernel<<<n_blocks, block_size, 0>>>(nelements, elements, fff_stride, fff, rowptr, colidx, values);
    }
...
extern int cu_tet4_laplacian_crs(const ptrdiff_t                    nelements,
    ...
    switch (real_type) {
        case SFEM_REAL_DEFAULT: {
            return cu_tet4_laplacian_crs_tpl(
        ...
        default: {
            SFEM_ERROR("[Error] cu_tet4_laplacian_crs: not implemented for type %s (code %d)\n",
