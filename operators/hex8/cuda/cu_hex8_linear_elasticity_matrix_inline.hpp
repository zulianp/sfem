...
template <typename scalar_t>
static inline __device__ __host__ void cu_linear_elasticity_matrix_block(
        const scalar_t mu,
    ...
    block[4] += x19 * (trial_grad[0] * (adjugate[1] * x27 + x26 + x9) +
                       trial_grad[1] * (adjugate[4] * x27 + x16 + x28) +
                       trial_grad[2] * (adjugate[7] * x27 + x18 + x29));
    block[5] += x19 * (trial_grad[0] * (adjugate[1] * x8 + adjugate[2] * x24) +
                       trial_grad[1] * (adjugate[4] * x8 + adjugate[5] * x24) +
    ...
    block[7] += x19 * (trial_grad[0] * (adjugate[1] * x32 + adjugate[2] * x5) +
                       trial_grad[1] * (adjugate[4] * x32 + adjugate[5] * x5) +
                       trial_grad[2] * (adjugate[7] * x32 + adjugate[8] * x5));
    block[8] += x19 * (trial_grad[0] * (adjugate[2] * x33 + x26 + x6) +
                       trial_grad[1] * (adjugate[5] * x33 + x15 + x28) +
                       trial_grad[2] * (adjugate[8] * x33 + x17 + x29));
}

template <typename scalar_t>
static inline __device__ __host__ void cu_linear_elasticity_matrix_diag_block(
        const scalar_t mu,
        const scalar_t lambda,
        const scalar_t *const SFEM_RESTRICT adjugate,
        const scalar_t jacobian_determinant,
        ...
        scalar_t *const SFEM_RESTRICT block) {
    const scalar_t x0 =
            adjugate[1] * test_grad[0] + adjugate[4] * test_grad[1] + adjugate[7] * test_grad[2];
    const scalar_t x1 = mu * x0;
    const scalar_t x2 = adjugate[1] * x1;
    const scalar_t x3 =
            adjugate[2] * test_grad[0] + adjugate[5] * test_grad[1] + adjugate[8] * test_grad[2];
    const scalar_t x4 = mu * x3;
    const scalar_t x5 = adjugate[2] * x4;
    const scalar_t x6 = lambda + 2 * mu;
    const scalar_t x7 =
            adjugate[0] * test_grad[0] + adjugate[3] * test_grad[1] + adjugate[6] * test_grad[2];
    ...
    block[0] += x13 * (test_grad[0] * (adjugate[0] * x8 + x2 + x5) +
                       test_grad[1] * (adjugate[3] * x8 + x10 + x9) +
                       test_grad[2] * (adjugate[6] * x8 + x11 + x12));
    block[1] += x13 * (test_grad[0] * (adjugate[0] * x1 + adjugate[1] * x14) +
                       test_grad[1] * (adjugate[3] * x1 + adjugate[4] * x14) +
                       test_grad[2] * (adjugate[6] * x1 + adjugate[7] * x14));
    block[2] += x13 * (test_grad[0] * (adjugate[0] * x4 + adjugate[2] * x14) +
                       test_grad[1] * (adjugate[3] * x4 + adjugate[5] * x14) +
                       test_grad[2] * (adjugate[6] * x4 + adjugate[8] * x14));
    block[3] += x13 * (test_grad[0] * (adjugate[0] * x15 + adjugate[1] * x16) +
                       test_grad[1] * (adjugate[3] * x15 + adjugate[4] * x16) +
                       test_grad[2] * (adjugate[6] * x15 + adjugate[7] * x16));
    block[4] += x13 * (test_grad[0] * (adjugate[1] * x18 + x17 + x5) +
                       test_grad[1] * (adjugate[4] * x18 + x10 + x19) +
                       test_grad[2] * (adjugate[7] * x18 + x12 + x20));
    block[5] += x13 * (test_grad[0] * (adjugate[1] * x4 + adjugate[2] * x15) +
                       test_grad[1] * (adjugate[4] * x4 + adjugate[5] * x15) +
                       test_grad[2] * (adjugate[7] * x4 + adjugate[8] * x15));
    block[6] += x13 * (test_grad[0] * (adjugate[0] * x21 + adjugate[2] * x16) +
                       test_grad[1] * (adjugate[3] * x21 + adjugate[5] * x16) +
                       test_grad[2] * (adjugate[6] * x21 + adjugate[8] * x16));
    block[7] += x13 * (test_grad[0] * (adjugate[1] * x21 + adjugate[2] * x1) +
                       test_grad[1] * (adjugate[4] * x21 + adjugate[5] * x1) +
                       test_grad[2] * (adjugate[7] * x21 + adjugate[8] * x1));
    block[8] += x13 * (test_grad[0] * (adjugate[2] * x22 + x17 + x2) +
                       test_grad[1] * (adjugate[5] * x22 + x19 + x9) +
