...
#include "cu_hex8_kelvin_voigt_inline.hpp"

#include <stdio.h>

template <typename T>
__global__ void cu_affine_hex8_kelvin_voigt_apply_kernel(const ptrdiff_t                          nelements,
        ...
        for (int kz = 0; kz < n_qp; kz++) {
            for (int ky = 0; ky < n_qp; ky++) {
                for (int kx = 0; kx < n_qp; kx++) {
                    cu_hex8_kelvin_voigt_apply_adj<T, T>(k,
                                                              K,
                                                              eta,
                                                              rho,
                                                              adjugate,
...
template <typename T>
int cu_affine_hex8_kelvin_voigt_apply_tpl(const ptrdiff_t                          nelements,
                                               idx_t **const SFEM_RESTRICT              elements,
                                               const ptrdiff_t                          jacobian_stride,
    ...
    if (stream) {
        ...
        cu_affine_hex8_kelvin_voigt_apply_kernel<T><<<n_blocks, block_size, 0, s>>>(nelements,
                                                                                         elements,
                                                                                         jacobian_stride,
                                                                                         jacobian_adjugate,
                                                                                         jacobian_determinant,
        ...
        cu_affine_hex8_kelvin_voigt_apply_kernel<T><<<n_blocks, block_size, 0>>>(nelements,
                                                                                      elements,
                                                                                      jacobian_stride,
                                                                                      jacobian_adjugate,
                                                                                      jacobian_determinant,
...
extern int cu_affine_hex8_kelvin_voigt_apply(const ptrdiff_t                 nelements,
                                                  idx_t **const SFEM_RESTRICT     elements,
                                                  const ptrdiff_t                 jacobian_stride,
    ...
    switch (real_type) {
        case SFEM_REAL_DEFAULT: {
            return cu_affine_hex8_kelvin_voigt_apply_tpl(nelements,
                                                              elements,
                                                              jacobian_stride, 
        ...
        default: {
            SFEM_ERROR(
                    "[Error] cu_affine_hex8_kelvin_voigt_apply: not implemented for type %s "
                    "(code %d)\n",
