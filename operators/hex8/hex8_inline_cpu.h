...
static SFEM_INLINE void hex8_fff(const scalar_t *const SFEM_RESTRICT x,
    ...
    const scalar_t x12 =
            qx * qy * x[6] + qx * x3 * x[5] + qy * x9 * x[7] - x10 * x[3] - x11 * x[0] + x3 * x9 * x[4] - x7 * x[2] - x8 * x[1];
    const scalar_t x13 = qx * qz;
    const scalar_t x14 = qx * x1;
    const scalar_t x15 = qz * x9;
    const scalar_t x16 = x1 * x9;
    const scalar_t x17 =
            qx * qz * y[6] + qx * x1 * y[2] + qz * x9 * y[7] + x1 * x9 * y[3] - x13 * y[5] - x14 * y[1] - x15 * y[4] - x16 * y[0];
    const scalar_t x18 = x12 * x17;
    const scalar_t x19 = x0 * x[6] - x0 * x[7] + x2 * x[2] - x2 * x[3] - x4 * x[4] + x4 * x[5] - x5 * x[0] + x5 * x[1];
    const scalar_t x20 =
            qx * qy * y[6] + qx * x3 * y[5] + qy * x9 * y[7] - x10 * y[3] - x11 * y[0] + x3 * x9 * y[4] - x7 * y[2] - x8 * y[1];
    const scalar_t x21 =
            qx * qz * z[6] + qx * x1 * z[2] + qz * x9 * z[7] + x1 * x9 * z[3] - x13 * z[5] - x14 * z[1] - x15 * z[4] - x16 * z[0];
    const scalar_t x22 = x20 * x21;
    const scalar_t x23 = x0 * y[6] - x0 * y[7] + x2 * y[2] - x2 * y[3] - x4 * y[4] + x4 * y[5] - x5 * y[0] + x5 * y[1];
    const scalar_t x24 =
            qx * qy * z[6] + qx * x3 * z[5] + qy * x9 * z[7] - x10 * z[3] - x11 * z[0] + x3 * x9 * z[4] - x7 * z[2] - x8 * z[1];
    const scalar_t x25 =
            qx * qz * x[6] + qx * x1 * x[2] + qz * x9 * x[7] + x1 * x9 * x[3] - x13 * x[5] - x14 * x[1] - x15 * x[4] - x16 * x[0];
...
static SFEM_INLINE void hex8_l2_project(const scalar_t                     jacobian_determinant,
    ...
    const scalar_t x2 = (qw * jacobian_determinant) * v;
    const scalar_t x3 = x2 * (1 - qz);
    const scalar_t x4 = x1 * x3;
    const scalar_t x5 = qy * x3;
    const scalar_t x6 = qz * x2;
    const scalar_t x7 = x1 * x6;
    const scalar_t x8 = qy * x6;
    values[0] += x0 * x4;
    values[1] += qx * x4;
    values[2] += qx * x5;
    values[3] += x0 * x5;
