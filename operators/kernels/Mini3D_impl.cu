...
SFEM_DEVICE_KERNEL void Mini3D_jacobian_kernel(
...
const real_t px3 = this_xyz[(0 * fe_subparam_n_nodes + 3) * nelements];
// y-coordinates
...
const real_t py3 = this_xyz[(1 * fe_subparam_n_nodes + 3) * nelements];
// z-coordinates
const real_t pz0 = this_xyz[(2 * fe_subparam_n_nodes + 0) * nelements];
const real_t pz1 = this_xyz[(2 * fe_subparam_n_nodes + 1) * nelements];
const real_t pz2 = this_xyz[(2 * fe_subparam_n_nodes + 2) * nelements];
const real_t pz3 = this_xyz[(2 * fe_subparam_n_nodes + 3) * nelements];
...
SFEM_DEVICE_KERNEL void Mini3D_jacobian_inverse_kernel(
const ptrdiff_t nelements,
const geom_t *const SFEM_RESTRICT xyz,
real_t *const SFEM_RESTRICT jacobian)
{
...
static void Mini3D_host_pack_elements
(
const ptrdiff_t n,
...
{
    SFEM_NVTX_SCOPE("Mini3D_host_pack_elements");
    {
...
static void Mini3D_host_pack_vector
(
const ptrdiff_t n,
const ptrdiff_t element_offset,
idx_t **const SFEM_RESTRICT elems,
...
{
    SFEM_NVTX_SCOPE("Mini3D_host_pack_vector");
    {
...
template <typename T>
SFEM_DEVICE_KERNEL void Mini3D_pack_kernel(
const ptrdiff_t nelements,
...
SFEM_DEVICE_FUNCTION int Mini3D_linear_search
(
const idx_t target,
...
SFEM_DEVICE_FUNCTION int Mini3D_find_col
(
const idx_t key,
...
{
    return Mini3D_linear_search(key, row, lenrow);
}

SFEM_DEVICE_FUNCTION void Mini3D_find_cols
(
const idx_t *targets,
const idx_t *const row,
const int lenrow,
...
)
{
    if (lenrow > 32)
    {
        ...
        {
            ks[d] = Mini3D_find_col(targets[d], row, lenrow);
        }
    } else {
...
SFEM_DEVICE_KERNEL void Mini3D_vector_local_to_global_kernel
(
const ptrdiff_t nelements,
idx_t **const SFEM_RESTRICT elems,
...
SFEM_DEVICE_KERNEL void Mini3D_block_vector_local_to_global_kernel
(
const ptrdiff_t nelements,
idx_t **const SFEM_RESTRICT elems,
const int stride_element_vector,
...
SFEM_DEVICE_KERNEL void Mini3D_matrix_local_to_global_kernel
(
const ptrdiff_t nelements,
idx_t **const SFEM_RESTRICT elems,
...
#pragma unroll(fe_n_nodes)
        ...
        for (int edof_i = 0; edof_i < fe_n_nodes; ++edof_i) 
        {
            ...
            Mini3D_find_cols(ev, row, lenrow, ks);
...
SFEM_DEVICE_KERNEL void Mini3D_block_matrix_local_to_global_kernel
(
const ptrdiff_t nelements,
idx_t **const SFEM_RESTRICT elems,
const int stride_element_matrix,
