...
template <typename T, int LEVEL>
int cu_affine_sshex8_kelvin_voigt_newmark_apply_warp_tpl(const ptrdiff_t                          nelements,
                                                         idx_t **const SFEM_RESTRICT              elements,
                                                         const ptrdiff_t                          jacobian_stride,
                                                         const cu_jacobian_t *const SFEM_RESTRICT jacobian_adjugate,
                                                         const cu_jacobian_t *const SFEM_RESTRICT jacobian_determinant,
    ...
    if (stream) {
        ...
        cu_affine_sshex8_kelvin_voigt_newmark_apply_warp_kernel<T, LEVEL><<<n_blocks, block_size, 0, s>>>(nelements,
                                                                                                          elements,
                                                                                                          jacobian_stride,
                                                                                                          jacobian_adjugate,
        ...
        cu_affine_sshex8_kelvin_voigt_newmark_apply_warp_kernel<T, LEVEL><<<n_blocks, block_size, 0>>>(nelements,
                                                                                                       elements,
                                                                                                       jacobian_stride,
                                                                                                       jacobian_adjugate,
                                                                                                       jacobian_determinant,
...
#define my_kernel cu_affine_sshex8_kelvin_voigt_newmark_apply_warp_tpl
// #define my_kernel_large cu_affine_sshex8_kelvin_voigt_newmark_apply_warp_tpl
// #define my_kernel cu_affine_sshex8_kelvin_voigt_newmark_apply_local_mem_tpl
#define my_kernel_large cu_affine_sshex8_kelvin_voigt_newmark_apply_local_mem_tpl
...
template <typename T, int LEVEL>
__global__ void cu_affine_sshex8_kelvin_voigt_newmark_apply_local_mem_segmented_kernel(
        const ptrdiff_t                          nelements,
        idx_t **const SFEM_RESTRICT              elements,
        const ptrdiff_t                          jacobian_stride,
        const cu_jacobian_t *const SFEM_RESTRICT g_jacobian_adjugate,
    ...
    for (ptrdiff_t e = blockIdx.x * blockDim.x + threadIdx.x; e < nelements; e += blockDim.x * gridDim.x) {
        ...
        // X
        {
            ...
            SEGEMENTED_QUADRATURE_LOOP(cu_hex8_kelvin_voigt_newmark_matrix_block_0_0);
#endif
            ...
            SEGEMENTED_QUADRATURE_LOOP(cu_hex8_kelvin_voigt_newmark_matrix_block_1_0);
#endif
            ...
            SEGEMENTED_QUADRATURE_LOOP(cu_hex8_kelvin_voigt_newmark_matrix_block_2_0);
#endif
        ...
        // Y
        {
            ...
            SEGEMENTED_QUADRATURE_LOOP(cu_hex8_kelvin_voigt_newmark_matrix_block_0_1);
#endif
            ...
            SEGEMENTED_QUADRATURE_LOOP(cu_hex8_kelvin_voigt_newmark_matrix_block_1_1);
#endif
            ...
            SEGEMENTED_QUADRATURE_LOOP(cu_hex8_kelvin_voigt_newmark_matrix_block_2_1);
#endif
        ...
        // Z
        {
            ...
            SEGEMENTED_QUADRATURE_LOOP(cu_hex8_kelvin_voigt_newmark_matrix_block_0_2);
#endif
            ...
            SEGEMENTED_QUADRATURE_LOOP(cu_hex8_kelvin_voigt_newmark_matrix_block_1_2);
#endif
            ...
            SEGEMENTED_QUADRATURE_LOOP(cu_hex8_kelvin_voigt_newmark_matrix_block_2_2);
#endif
...
#define local_mem_kernel cu_affine_sshex8_kelvin_voigt_newmark_apply_local_mem_segmented_kernel
// #define local_mem_kernel cu_affine_sshex8_kelvin_voigt_newmark_apply_local_mem_kernel

template <typename T, int LEVEL>
int cu_affine_sshex8_kelvin_voigt_newmark_apply_local_mem_tpl(const ptrdiff_t                          nelements,
                                                              idx_t **const SFEM_RESTRICT              elements,
                                                              const ptrdiff_t                          jacobian_stride,
                                                              const cu_jacobian_t *const SFEM_RESTRICT jacobian_adjugate,
                                                              const cu_jacobian_t *const SFEM_RESTRICT jacobian_determinant,
...
static int cu_affine_sshex8_kelvin_voigt_newmark_apply_tpl(const int                                level,
                                                           const ptrdiff_t                          nelements,
                                                           idx_t **const SFEM_RESTRICT              elements,
                                                           const ptrdiff_t                          jacobian_stride,
                                                           const cu_jacobian_t *const SFEM_RESTRICT jacobian_adjugate,
    ...
    switch (level) {
        ...
        default: {
            SFEM_ERROR(
                    "cu_affine_sshex8_kelvin_voigt_newmark_apply_tpl: level %d not "
                    "supported!\n",
                    level);
            return SFEM_FAILURE;
        }
...
extern int cu_affine_sshex8_kelvin_voigt_newmark_diag(const int                       level,
    ...
    switch (real_type) {
        ...
        default: {
            SFEM_ERROR(
                    "[Error] cu_affine_sshex8_kelvin_voigt_newmark_diag: not implemented "
                    "for "
                    "type %s "
                    "(code %d)\n",
...
static int cu_affine_sshex8_kelvin_voigt_newmark_diag_tpl(const int                                level,
...
#ifdef SFEM_USE_OCCUPANCY_MAX_POTENTIAL
    {
        ...
        cudaOccupancyMaxPotentialBlockSize(
                &min_grid_size, &block_size, cu_affine_sshex8_kelvin_voigt_newmark_diag_kernel<T>, 0, 0);
    }
#endif  // SFEM_USE_OCCUPANCY_MAX_POTENTIAL
    ...
    if (stream) {
        cudaStream_t s = *static_cast<cudaStream_t *>(stream);
        cu_affine_sshex8_kelvin_voigt_newmark_diag_kernel<T><<<n_blocks, block_size, 0, s>>>(level,
                                                                                             nelements,
                                                                                             elements,
                                                                                             jacobian_stride,
                                                                                             jacobian_adjugate,
        ...
        cu_affine_sshex8_kelvin_voigt_newmark_diag_kernel<T><<<n_blocks, block_size, 0>>>(level,
                                                                                          nelements,
                                                                                          elements,
                                                                                          jacobian_stride,
                                                                                          jacobian_adjugate,
...
__global__ void cu_affine_sshex8_kelvin_voigt_newmark_diag_kernel(const int                                level,
    ...
    for (ptrdiff_t e = blockIdx.x * blockDim.x + threadIdx.x; e < nelements; e += blockDim.x * gridDim.x) {
        T kelvin_voigt_newmark_diag[3 * 8];
        // Build operator
        {
            T sub_adjugate[9];
            ...
            cu_sshex8_kelvin_voigt_newmark_diag<T>(
                    k, K, eta, rho, dt, gamma, beta, sub_adjugate, sub_determinant, kelvin_voigt_newmark_diag);
        }
        ...
        for (int zi = 0; zi < level; zi++) {
            for (int yi = 0; yi < level; yi++) {
                for (int xi = 0; xi < level; xi++) {
                    ...
                    for (int d = 0; d < 3; d++) {
                        for (int v = 0; v < 8; v++) {
                            assert(kelvin_voigt_newmark_diag[d * 8 + v] == kelvin_voigt_newmark_diag[d * 8 + v]);

                            atomicAdd(&out[d][ev[v] * out_stride], kelvin_voigt_newmark_diag[d * 8 + v]);
                        }
                    }
                }
            }
