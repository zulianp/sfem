...
#ifndef NDEBUG
            {
                accumulator_t *test = (accumulator_t *)calloc(nxe, sizeof(accumulator_t));
...
#pragma omp parallel
    {
        ...
        idx_t    *ev   = (idx_t *)malloc(nxe * sizeof(idx_t));
        scalar_t *eu   = (scalar_t *)malloc(nxe * sizeof(scalar_t));
        scalar_t *erhs = (scalar_t *)malloc(nxe * sizeof(scalar_t));
        scalar_t *r    = (scalar_t *)malloc(nxe * sizeof(scalar_t));
        scalar_t *p    = (scalar_t *)malloc(nxe * sizeof(scalar_t));
        scalar_t *Ap   = (scalar_t *)malloc(nxe * sizeof(scalar_t));
...
#pragma omp parallel
    {
        // Allocation per thread
        idx_t    *ev    = (idx_t *)malloc(nxe * sizeof(idx_t));
        scalar_t *eu    = (scalar_t *)calloc(gnxe, sizeof(scalar_t));
        scalar_t *erhs  = (scalar_t *)calloc(gnxe, sizeof(scalar_t));
        scalar_t *r     = (scalar_t *)calloc(gnxe, sizeof(scalar_t));
        scalar_t *p     = (scalar_t *)calloc(gnxe, sizeof(scalar_t));
        scalar_t *Ap    = (scalar_t *)calloc(gnxe, sizeof(scalar_t));
        int      *emask = (int *)calloc(gnxe, sizeof(int));
                ...
                for (int zi = 1; zi < levelp2; zi++) {
                    for (int yi = 1; yi < levelp2; yi++) {
                        for (int xi = 1; xi < levelp2; xi++) {
                            ...
                            const idx_t gidx  = ev[lidx];
                            emask[glidx]      = !!(mask[(gidx) / (sizeof(mask_t) * 8)] & (1 << ((gidx) % (sizeof(mask_t) * 8))));
                            erhs[glidx]       = rhs[gidx];
                        }
                    }
                }
