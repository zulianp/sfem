...
int sshex8_macro_hex8_adjugate_and_det_fill(const int                       level,
    ...
    const geom_t *const px = points[0];
    const geom_t *const py = points[1];
    const geom_t *const pz = points[2];
...
#pragma omp parallel for
    for (ptrdiff_t e = 0; e < nelements; ++e) {
        ...
        for (int v = 0; v < 8; v++) {
            ev_corner[v] = elements[proteus_to_std_hex8_corners[v]][e];
            lx[v]        = (scalar_t)px[ev_corner[v]];
            ly[v]        = (scalar_t)py[ev_corner[v]];
            lz[v]        = (scalar_t)pz[ev_corner[v]];
        }
        ...
        for (int d = 0; d < 9; d++) {
            adjugate[e * 9 + d] = (jacobian_t)l_adjugate[d];
        }
        determinant[e] = (geom_t)l_determinant;
    }

    return SFEM_SUCCESS;
}

int affine_sshex8_kelvin_voigt_newmark_apply_adjugate(const int                             level,
                                                      const ptrdiff_t                       nelements,
...
#pragma omp for
        for (ptrdiff_t e = 0; e < nelements; ++e) {
            ...
            scalar_t macro_adj[9];
            for (int d = 0; d < 9; d++) {
                macro_adj[d] = (scalar_t)jacobian_adjugate[e * 9 + d];
            }
            const scalar_t macro_det = (scalar_t)jacobian_determinant[e];

            scalar_t sub_adjugate[9];
            scalar_t sub_determinant;
            hex8_sub_adj_0(macro_adj, macro_det, h, sub_adjugate, &sub_determinant);
...
#pragma omp for
        for (ptrdiff_t e = 0; e < nelements; ++e) {
            ...
            for (int zi = 0; zi < level; zi++) {
                for (int yi = 0; yi < level; yi++) {
                    for (int xi = 0; xi < level; xi++) {
                        // Convert to standard smesh::HEX8 local ordering (see 3-4 and 6-7)
                        int lev[8] = {// Bottom
                                      sshex8_lidx(level, xi, yi, zi),
                                      sshex8_lidx(level, xi + 1, yi, zi),
                                      sshex8_lidx(level, xi + 1, yi + 1, zi),
...
int affine_sshex8_kelvin_voigt_newmark_gradient_adjugate(const int                             level,
                                                         const ptrdiff_t                       nelements,
                                                         const ptrdiff_t                       nnodes,
                                                         idx_t **const SFEM_RESTRICT           elements,
                                                         const jacobian_t *const SFEM_RESTRICT jacobian_adjugate,
...
#pragma omp parallel
    {
        ...
        for (int d = 0; d < 3; d++) {
            eu[d]   = (scalar_t *)malloc(nxe * sizeof(scalar_t));
            evel[d] = (scalar_t *)malloc(nxe * sizeof(scalar_t));
            eax[d]  = (scalar_t *)malloc(nxe * sizeof(scalar_t));
            v[d]    = (accumulator_t *)malloc(nxe * sizeof(accumulator_t));
        }
