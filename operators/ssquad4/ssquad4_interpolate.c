...
int ssquad4_prolongate(const ptrdiff_t                   nelements,
    ...
    if (to_level % from_level != 0) {
        SFEM_ERROR("Level must be divisible by from_level and to_level");
        return SFEM_FAILURE;
    }
...
#pragma omp parallel
    {
        ...
        idx_t *to_lidx = malloc(to_nxe * sizeof(idx_t));
        idx_t *to_gidx = malloc(to_nxe * sizeof(idx_t));

        {  // Generate local indices for "to" coefficients
            int k = 0;
            for (int yi = 0; yi <= to_level; yi += to_level_stride) {
                for (int xi = 0; xi <= to_level; xi += to_level_stride) {
                    to_lidx[k++] = ssquad4_lidx(to_level * to_level_stride, xi * to_level_stride, yi * to_level_stride);
                }
            }
        }
...
#pragma omp for
        for (ptrdiff_t e = 0; e < nelements; e++) {
            {  // Gather elemental data
                for (int v = 0; v < to_nxe; v++) {
                    to_gidx[v] = to_elements[to_lidx[v]][e];
                }
                ...
                for (int d = 0; d < vec_size; d++) {
                    ...
                    for (int yi = 0; yi <= from_level; yi++) {
                        for (int xi = 0; xi < from_level; xi++) {
                            scalar_t c0 = c[ssquad4_lidx(to_level, xi * from_to_step, yi * from_to_step)];
                            scalar_t c1 = c[ssquad4_lidx(to_level, (xi + 1) * from_to_step, yi * from_to_step)];
                    ...
                    for (int yi = 0; yi < from_level; yi++) {
                        for (int xi = 0; xi <= from_level; xi++) {
                            scalar_t c0 = c[ssquad4_lidx(to_level, xi * from_to_step, yi * from_to_step)];
                            scalar_t c1 = c[ssquad4_lidx(to_level, xi * from_to_step, (yi + 1) * from_to_step)];
