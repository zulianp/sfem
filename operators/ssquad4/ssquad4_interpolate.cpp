...
#pragma omp parallel
    {
        ...
        scalar_t **e_from = (scalar_t **)malloc(vec_size * sizeof(scalar_t *));
        scalar_t **e_to   = (scalar_t **)malloc(vec_size * sizeof(scalar_t *));
        uint16_t  *weight = (uint16_t *)malloc(nxe * sizeof(uint16_t));

        for (int d = 0; d < vec_size; d++) {
            e_from[d] = (scalar_t *)malloc(nxe * sizeof(scalar_t));
            e_to[d]   = (scalar_t *)malloc(4 * sizeof(scalar_t));
        }
...
#pragma omp parallel
    {
        ...
        for (int d = 0; d < vec_size; d++) {
            e_from[d] = (scalar_t *)malloc(4 * sizeof(scalar_t));
            e_to[d]   = (scalar_t *)malloc(nxe * sizeof(scalar_t));
        }
