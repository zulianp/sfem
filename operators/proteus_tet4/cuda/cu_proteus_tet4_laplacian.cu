...
static /*inline*/ __device__ __host__ void cu_proteus_tet4_laplacian_apply_fff(
...
static inline __device__ __host__ int proteus_tet4_nxe(int level) {
    int num_nodes = 0;
    if (level % 2 == 0) {
...
static inline __device__ __host__ int proteus_tet4_txe(int level) { return (int)POW3(level); }

template <typename fff_t, typename scalar_t>
static inline __device__ __host__ void cu_proteus_tet4_sub_fff_0(
        const int L,
        const fff_t *const SFEM_RESTRICT fff,
...
template <typename fff_t, typename scalar_t>
static inline __device__ __host__ void cu_proteus_tet4_sub_fff_1(
        const int L,
        const fff_t *const SFEM_RESTRICT fff,
        scalar_t *const SFEM_RESTRICT sub_fff) {
...
template <typename fff_t, typename scalar_t>
static inline __device__ __host__ void cu_proteus_tet4_sub_fff_2(
        const int L,
        const fff_t *const SFEM_RESTRICT fff,
        scalar_t *const SFEM_RESTRICT sub_fff) {
    const scalar_t x0 = 1. / POW3(L);
...
template <typename fff_t, typename scalar_t>
static inline __device__ __host__ void cu_proteus_tet4_sub_fff_3(
        const int L,
        const fff_t *const SFEM_RESTRICT fff,
        scalar_t *const SFEM_RESTRICT sub_fff) {
    const scalar_t x0 = 1. / POW3(L);
...
template <typename fff_t, typename scalar_t>
static inline __device__ __host__ void cu_proteus_tet4_sub_fff_4(
        const int L,
        const fff_t *const SFEM_RESTRICT fff,
        scalar_t *const SFEM_RESTRICT sub_fff) {
    const scalar_t x0 = 1.0 / L;
...
template <typename fff_t, typename scalar_t>
static inline __device__ __host__ void cu_proteus_tet4_sub_fff_5(
        const int L,
        const fff_t *const SFEM_RESTRICT fff,
        scalar_t *const SFEM_RESTRICT sub_fff) {
    const scalar_t x0 = 1. / POW3(L);
...
__global__ void cu_proteus_tet4_laplacian_apply_kernel(const int level,
                                                       const ptrdiff_t nelements,
                                                       const ptrdiff_t stride,
                                                       const cu_jacobian_t *const SFEM_RESTRICT g_fff,
                                                       const real_t *const SFEM_RESTRICT u,
                                                       real_t *const SFEM_RESTRICT values) {
    const int nxe = proteus_tet4_nxe(level);
    const int txe = proteus_tet4_txe(level);
...
#pragma unroll(6)
        ...
        ///////////////////////////////////
        {
            cu_proteus_tet4_sub_fff_0(level, offf, fff);
            ...
            for (int i = 0; i < level - 1; i++) {
                ...
                for (int j = 0; j < level - i - 1; j++) {
                    for (int k = 0; k < level - i - j - 1; k++) {
                        ...
                        cu_proteus_tet4_laplacian_apply_fff(fff,
        ...
        ///////////////////////////////////
        {
            cu_proteus_tet4_sub_fff_1(level, offf, fff);
        ...
        ///////////////////////////////////
        {
            cu_proteus_tet4_sub_fff_2(level, offf, fff);
        ...
        ///////////////////////////////////
        {
            cu_proteus_tet4_sub_fff_3(level, offf, fff);
        ...
        ///////////////////////////////////
        {
            cu_proteus_tet4_sub_fff_4(level, offf, fff);
        ...
        ///////////////////////////////////
        {
            cu_proteus_tet4_sub_fff_5(level, offf, fff);
...
template <typename T>
static int cu_proteus_tet4_laplacian_apply_tpl(const int level,
                                               const ptrdiff_t nelements,
                                               const ptrdiff_t stride,
...
#ifdef SFEM_USE_OCCUPANCY_MAX_POTENTIAL
    {
        ...
        cudaOccupancyMaxPotentialBlockSize(
                &min_grid_size, &block_size, cu_proteus_tet4_laplacian_apply_kernel<T>, 0, 0);
    }
    ...
    if (stream) {
        ...
        cu_proteus_tet4_laplacian_apply_kernel<T>
                <<<n_blocks, block_size, 0, s>>>(level, nelements, stride, fff, x, y);
    } else {
        cu_proteus_tet4_laplacian_apply_kernel<T>
                <<<n_blocks, block_size, 0>>>(level, nelements, stride, fff, x, y);
    }
...
extern int cu_proteus_tet4_laplacian_apply(const int level,
                                           const ptrdiff_t nelements,
                                           const ptrdiff_t stride,
    ...
    switch (real_type_xy) {
        case SFEM_REAL_DEFAULT: {
            return cu_proteus_tet4_laplacian_apply_tpl(level,
                                                       nelements,
                                                       stride,
                                                       (cu_jacobian_t *)fff,
                                                       (real_t *)x,
        ...
        case SFEM_FLOAT32: {
            return cu_proteus_tet4_laplacian_apply_tpl(
                    level, nelements, stride, (cu_jacobian_t *)fff, (float *)x, (float *)y, stream);
        }
        case SFEM_FLOAT64: {
            return cu_proteus_tet4_laplacian_apply_tpl(level,
                                                       nelements,
        ...
        default: {
            fprintf(stderr,
                    "[Error] cu_proteus_tet4_laplacian_apply: not implemented for type %s (code "
                    "%d)\n",
