...
static inline __device__ __host__ void trial_operand(const real_t qx,
                                                     const real_t qy,
                                                     const real_t qz,
    ...
    out[0] = fff[0 * stride] * x7 + fff[1 * stride] * x10 + fff[2 * stride] * x11;
    out[1] = fff[1 * stride] * x7 + fff[3 * stride] * x10 + fff[4 * stride] * x11;
    out[2] = fff[2 * stride] * x7 + fff[4 * stride] * x10 + fff[5 * stride] * x11;
}

static inline __device__ __host__ void ref_shape_grad_x(const real_t qx,
                                                        const real_t qy,
                                                        const real_t qz,
                                                        real_t *const gx) {
...
static inline __device__ __host__ void ref_shape_grad_y(const real_t qx,
                                                        const real_t qy,
                                                        const real_t qz,
                                                        real_t *const gx) {
    const real_t x0 = 4 * qz;
...
static inline __device__ __host__ void ref_shape_grad_z(const real_t qx,
                                                        const real_t qy,
                                                        const real_t qz,
                                                        real_t *const gx) {
    const real_t x0 = 4 * qz;
...
static inline __device__ __host__ void lapl_apply_micro_kernel(
    ...
    trial_operand(qx, qy, qz, fff, u, grad_u);

    {  // X-components
        ref_shape_grad_x(qx, qy, qz, ref_grad);
#pragma unroll(10)
        for (int i = 0; i < 10; i++) {
            element_vector[i] += ref_grad[i] * grad_u[0] * qw;
        }
    }
    {  // Y-components
...
#pragma unroll(10)
        for (int i = 0; i < 10; i++) {
            element_vector[i] += ref_grad[i] * grad_u[1] * qw;
        }
    }
...
#pragma unroll(10)
        for (int i = 0; i < 10; i++) {
            element_vector[i] += ref_grad[i] * grad_u[2] * qw;
        }
    }
}
...
#pragma unroll(10)
        ...
        {  // Numerical quadrature
            lapl_apply_micro_kernel(0, 0, 0, 0.025, &fff[e], nelements, ex, ey);
            lapl_apply_micro_kernel(1, 0, 0, 0.025, &fff[e], nelements, ex, ey);
            lapl_apply_micro_kernel(0, 1, 0, 0.025, &fff[e], nelements, ex, ey);
            lapl_apply_micro_kernel(0, 0, 1, 0.025, &fff[e], nelements, ex, ey);

            static const real_t athird = 1. / 3;
            lapl_apply_micro_kernel(athird, athird, 0., 0.225, &fff[e], nelements, ex, ey);
            lapl_apply_micro_kernel(athird, 0., athird, 0.225, &fff[e], nelements, ex, ey);
            lapl_apply_micro_kernel(0.3, athird, athird, 0.225, &fff[e], nelements, ex, ey);
            lapl_apply_micro_kernel(athird, athird, athird, 0.225, &fff[e], nelements, ex, ey);
        }
