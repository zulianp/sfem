...
int main(int argc, char* argv[]) {
            ...
            if (size == 1) {
                ...
                if (size > 1) {
                    ...
                    auto e_add = [&](real_t* const SFEM_RESTRICT inout) {
                        exchange_add(comm, mesh->n_nodes(), n_owned_nodes, &slave_to_master, inout, real_buffer);
                    };
    ...
    // Write result to disk
    {
        ...
        mesh_write_nodal_field(comm, n_owned_nodes, node_mapping->data(), path, SFEM_MPI_REAL_T, g);

        snprintf(path, sizeof(path), "%s/xnormal.float64.raw", output_folder);
        mesh_write_nodal_field(comm, n_owned_nodes, node_mapping->data(), path, SFEM_MPI_REAL_T, xnormal);

        snprintf(path, sizeof(path), "%s/ynormal.float64.raw", output_folder);
        mesh_write_nodal_field(comm, n_owned_nodes, node_mapping->data(), path, SFEM_MPI_REAL_T, ynormal);

        snprintf(path, sizeof(path), "%s/znormal.float64.raw", output_folder);
        mesh_write_nodal_field(comm, n_owned_nodes, node_mapping->data(), path, SFEM_MPI_REAL_T, znormal);
