...
// void compute_pseudo_normals(smesh::ElemType                element_type,
...
//     assert(element_type == smesh::EDGE2 && "IMPLEMENT OTHER CASES");
...
//     auto boundary_surface = std::make_shared<sfem::Mesh>(mesh->comm(), st, sides, mesh->points());

//     boundary_surface->write(smesh::Path((output_folder + "/surface")));
//     normals->to_files(smesh::Path((output_folder + "/surface/pseudo_normals.%d.raw")));
...
//         auto              surface         = sfem::Mesh::create_from_file(sfem::Communicator::wrap(comm), smesh::Path(surface_path.c_str()));
//         auto              surface_normals = sfem::create_host_buffer<real_t>(dim, surface->n_nodes());
//         compute_pseudo_normals(surface->element_type(0),
//                                surface->n_elements(),
//                                surface->n_nodes(),
...
//         surface_normals->to_files(smesh::Path((surface_path + "/normals.%d.raw")));
...
//     distance->to_file(smesh::Path((output_folder + "/input_distance.raw")));
//     normals->to_files(smesh::Path((output_folder + "/input_normals.%d.raw")));
...
//     tri3_div_apply(
//             mesh->n_elements(), mesh->n_nodes(), mesh->elements(0)->data(), mesh->points()->data(), n[0], n[1], div->data());
...
//     distance->to_file(smesh::Path((output_folder + "/distance.raw")));
//     normals->to_files(smesh::Path((output_folder + "/normals.%d.raw")));

//     blas->zeros(correction->size(), correction->data());

//     apply_inv_lumped_mass(mesh->element_type(0),
//                           mesh->n_elements(),
//                           mesh->n_nodes(),
//                           mesh->elements(0)->data(),
//                           mesh->points()->data(),
...
//     correction->to_file(smesh::Path((output_folder + "/divergence.raw")));
//     return MPI_Finalize();
// }
