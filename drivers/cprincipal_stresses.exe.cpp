...
int principal_stresses(const std::shared_ptr<sfem::Communicator> &comm, int argc, char *argv[]) {
    if (comm->size() != 1) {
    ...
    std::shared_ptr<sfem::Buffer<real_t>> stress_bufs[3];
    real_t                               *stress[3];
    for (int d = 0; d < spatial_dim; ++d) {
        stress_bufs[d] = sfem::create_host_buffer<real_t>(n_elements);
    ...
    if (is_AoS) {
        ...
        neohookean_principal_stresses_aos(
                n_elements, n_nodes, mesh->elements(0)->data(), mesh->points()->data(), mu, lambda, u->data(), stress);
    } else {
        ...
        neohookean_principal_stresses_soa(
                n_elements, n_nodes, mesh->elements(0)->data(), mesh->points()->data(), mu, lambda, u, stress);
    }
...
int main(int argc, char *argv[]) {
    ...
    return principal_stresses(ctx->communicator(), argc, argv);
}
