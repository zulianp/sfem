...
int main(int argc, char *argv[]) {
    sfem::Context context(argc, argv);
    {
        ...
        smesh::LocalSideTable lst_fill;
        lst_fill.fill(static_cast<smesh::ElemType>(element_type_hack));
        memcpy(local_side_table->data(), lst_fill.table, nsxe * nnxs * sizeof(int));
...
#pragma omp critical
        ...
        if (dim == 2) {
            // Build node -> surface-edge incidence to traverse boundary by shared nodes
            std::unordered_map<idx_t, std::vector<ptrdiff_t>> node_to_faces;
            node_to_faces.reserve(n_surf_elements * 2);

            for (ptrdiff_t e = 0; e < n_surf_elements; ++e) {
                const element_idx_t sp = surf_parents[e];
                const int16_t       s  = surf_idx[e];
                const idx_t         n0 = elements[lst[s * nnxs + 0]][sp];
                const idx_t         n1 = elements[lst[s * nnxs + 1]][sp];
                node_to_faces[n0].push_back(e);
                node_to_faces[n1].push_back(e);
            }
            ...
            for (ptrdiff_t q = 0; equeue[q] >= 0; q = (q + 1) % size_queue) {
                ...
                real_t n2[2];
                {
                    ...
                    real_t p0[2], p1[2];
                    for (int d = 0; d < 2; ++d) { p0[d] = points[d][idx0]; p1[d] = points[d][idx1]; }
                    normal2(p0, p1, n2);
                }

                const idx_t en0 = elements[lst[s * nnxs + 0]][sp];
                const idx_t en1 = elements[lst[s * nnxs + 1]][sp];
                for (idx_t vtx : {en0, en1}) {
                    auto it = node_to_faces.find(vtx);
                    if (it == node_to_faces.end()) continue;
                    for (ptrdiff_t neigh_e : it->second) {
                        if (neigh_e == e || eselect[neigh_e]) continue;
                        const element_idx_t nsp = surf_parents[neigh_e];
                        const int16_t       ns  = surf_idx[neigh_e];
                        const idx_t         nidx0 = elements[lst[ns * nnxs + 0]][nsp];
                        const idx_t         nidx1 = elements[lst[ns * nnxs + 1]][nsp];
                        real_t p0a[2], p1a[2], na2[2];
                        for (int d = 0; d < 2; ++d) { p0a[d] = points[d][nidx0]; p1a[d] = points[d][nidx1]; }
                        normal2(p0a, p1a, na2);
            ...
            // Build edge -> surface-face incidence to traverse boundary by shared edges
            auto edge_key = [](idx_t a, idx_t b) -> unsigned long long {
                if (a > b) std::swap(a, b);
                return (static_cast<unsigned long long>(static_cast<uint32_t>(b)) << 32) |
                       static_cast<uint32_t>(a);
            };

            std::unordered_map<unsigned long long, std::vector<ptrdiff_t>> edge_to_faces;
            edge_to_faces.reserve(n_surf_elements * 3);

            for (ptrdiff_t e = 0; e < n_surf_elements; ++e) {
                const element_idx_t sp = surf_parents[e];
                const int16_t       s  = surf_idx[e];
                idx_t v[4];
                for (int k = 0; k < nnxs; ++k) v[k] = elements[lst[s * nnxs + k]][sp];
                for (int i = 0; i < nnxs; ++i) {
                    for (int j = i + 1; j < nnxs; ++j) {
                        edge_to_faces[edge_key(v[i], v[j])].push_back(e);
                    }
                }
            }
            ...
            for (ptrdiff_t q = 0; equeue[q] >= 0; q = (q + 1) % size_queue) {
                ...
                idx_t vtx[4];
                for (int k = 0; k < nnxs; ++k) vtx[k] = elements[lst[s * nnxs + k]][sp];
                for (int i = 0; i < nnxs; ++i) {
                    for (int j = i + 1; j < nnxs; ++j) {
                        auto it = edge_to_faces.find(edge_key(vtx[i], vtx[j]));
                        ...
                        for (ptrdiff_t neigh_e : it->second) {
                            ...
                            const idx_t         nidx0 = elements[lst[ns * nnxs + 0]][nsp];
                            const idx_t         nidx1 = elements[lst[ns * nnxs + 1]][nsp];
                            const idx_t         nidx2 = elements[lst[ns * nnxs + 2]][nsp];
                            real_t ua[3], va[3], na[3];
                            for (int d = 0; d < 3; ++d) {
                                ua[d] = points[d][nidx1] - points[d][nidx0];
                                va[d] = points[d][nidx2] - points[d][nidx0];
                            }
                            normal(ua, va, na);
        ...
        if (SFEM_DEBUG) {
            ...
            if (smesh::extract_surface_from_sideset(mesh->element_type(0),
                                                    elements,
                                                    n_selected,
                                                    sideset_parent->data(),
                                                    sideset_lfi->data(),
