...
//     auto mesh = sfem::Mesh::create_from_file(sfem::Communicator::wrap(comm), smesh::Path(mesh_folder));
...
//         int  nnxs             = elem_num_nodes(side_type(mesh->element_type(0)));
//         int *local_side_table = (int *)malloc(elem_num_sides(mesh->element_type(0)) * nnxs * sizeof(int));
//         surf_elems            = (idx_t **)malloc(nnxs * sizeof(idx_t *));
//         for (int d = 0; d < nnxs; d++) {
//             surf_elems[d] = (idx_t *)malloc(n_surf_elements * sizeof(idx_t));
//         }

//         fill_local_side_table(mesh->element_type(0), local_side_table);
...
//         correct_side_orientation(n_surf_elements, surf_elems, parent, mesh->elements(0)->data(), mesh->points()->data());
//     }
...
//     surf.element_type = side_type(mesh->element_type(0));
