...
// FIXME
    ...
    // ============================================================================
    // BRANCH: SFEM_ELEMENT_REFINE_LEVEL > 1 (use SSGMG)
    // ============================================================================
    if (SFEM_ELEMENT_REFINE_LEVEL > 1) {
        if (!comm->rank() && verbose) {
            printf("\n=== Using SSGMG solver (refine level > 1) ===\n");
        }
    ...
    // ============================================================================
    // BRANCH: SFEM_ELEMENT_REFINE_LEVEL <= 1 (use CG, solver created in loop)
    // ============================================================================
    } else {
        if (!comm->rank() && verbose) {
            printf("\n=== Using CG solver (refine level <= 1) ===\n");
        }
        ...
        // Create material operator (OUTSIDE loop)
        auto material_op = sfem::create_linear_operator("MF", f, solution, es);
