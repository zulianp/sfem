...
int solve_lumped_mass_inv(const std::shared_ptr<sfem::Communicator> &comm, int argc, char *argv[]) {
    if (comm->size() != 1) {
    ...
    apply_inv_lumped_mass(mesh->element_type(0), n_elements, n_nodes, mesh->elements(0)->data(), mesh->points()->data(), input->data(), output);
...
int main(int argc, char *argv[]) {
    ...
    return solve_lumped_mass_inv(ctx->communicator(), argc, argv);
}
