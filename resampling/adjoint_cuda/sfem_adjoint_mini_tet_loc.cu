...
call_sfem_adjoint_mini_tet_buffer_cluster_info_kernel_gpu(const ptrdiff_t             start_element,   // Mesh
    ...
    {  // BEGIN: Compute local grid sizes for each element
        ...
        sfem_make_local_data_tets_kernel_gpu<real_t, int>  //
                <<<blocks_per_grid,                        //
                   threads_per_block,                      //
                   0,                                      //
                   cuda_stream>>>(start_element,           // Mesh
    ...
    {  // BEGIN: Compute local grid sizes for each element
        ...
        for (ptrdiff_t start_element_local = start_element;  //
            ...
            sfem_adjoint_mini_tet_buffer_cluster_loc_kernel_gpu<real_t, int>  //
                    <<<blocks_per_grid,                                       //
        ...
        real_t* alpha_data = (real_t*)malloc(nelements * sizeof(real_t));
        cudaMemcpy((void*)alpha_data, (void*)tet_properties_info.tet_alpha, nelements * sizeof(real_t), cudaMemcpyDeviceToHost);

        FILE* fp_alpha = fopen("alpha_data.dat", "w");
