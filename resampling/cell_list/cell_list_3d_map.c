...
// coord_to_grid_index
////////////////////////////////////////////////
static inline int coord_to_grid_index(real_t coord, real_t origin, real_t delta) { return (int)floor((coord - origin) / delta); }
...
static inline void coord_to_grid_indices(const real_t coord[2], const real_t origin[2], const real_t delta[2], int indices[2]) {
    ...
    for (int ii = 0; ii < 2; ii++) {
        indices[ii] = (int)floor((coord[ii] - origin[ii]) * inv_delta[ii]);
    }
}
...
int query_cell_list_3d_2d_map(const cell_list_3d_2d_map_t *map, const boxes_t *boxes, const real_t x, const real_t y,
    ...
    if (num_boxes_local > 0) {
        *box_indices = (int *)malloc(num_boxes_local * sizeof(int));
        ...
        int lower_bound_index = lower_bound_float(&map->upper_bounds_z[start_index], (size_t)num_boxes_local, z);

        int upper_bound_index = upper_bound_float(&map->lower_bounds_z[start_index], (size_t)num_boxes_local, z);

        lower_bound_index =
                lower_bound_index < 0 ? 0 : (lower_bound_index > num_boxes_local ? num_boxes_local : lower_bound_index);
        upper_bound_index = upper_bound_index < lower_bound_index
                                    ? lower_bound_index
                                    : (upper_bound_index > num_boxes_local ? num_boxes_local : upper_bound_index);

        if (upper_bound_index <= lower_bound_index) {
            lower_bound_index = 0;
            upper_bound_index = num_boxes_local;
        }  // END if (upper_bound_index <= lower_bound_index)
