...
real_t random_interval(real_t min, real_t max) { return min + ((real_t)rand() / (real_t)RAND_MAX) * (max - min); }

////////////////////////////////////////////////
// make_boxes_interleaved_t
////////////////////////////////////////////////
boxes_interleaved_t make_boxes_interleaved_t(void) {
    boxes_interleaved_t boxes;
    init_boxes_interleaved_t(&boxes);
    return boxes;
}

////////////////////////////////////////////////
// init_boxes_interleaved_t
////////////////////////////////////////////////
void init_boxes_interleaved_t(boxes_interleaved_t *boxes) {
    boxes->min_max_xyz = NULL;
    boxes->num_boxes   = 0;
}

////////////////////////////////////////////////
// allocate_boxes_interleaved_t
////////////////////////////////////////////////
boxes_interleaved_t *allocate_boxes_interleaved_t(const int num_boxes) {
    boxes_interleaved_t *boxes = (boxes_interleaved_t *)malloc(sizeof(boxes_interleaved_t));
    boxes->min_max_xyz         = (real_t *)malloc(6 * num_boxes * sizeof(real_t));
    boxes->num_boxes           = num_boxes;
    return boxes;
}

////////////////////////////////////////////////
// free_boxes_interleaved_t
////////////////////////////////////////////////
void free_boxes_interleaved_t(boxes_interleaved_t *boxes) {
    if (boxes != NULL) {
        if (boxes->min_max_xyz != NULL) free(boxes->min_max_xyz);
        free(boxes);
    }
}
...
bool check_box_contains_pt(const boxes_t *boxes, const int box_index, const real_t x, const real_t y, const real_t z) {
    return (x >= boxes->min_x[box_index] && x <= boxes->max_x[box_index] && y >= boxes->min_y[box_index] &&
            y <= boxes->max_y[box_index] && z >= boxes->min_z[box_index] && z <= boxes->max_z[box_index]);
}
...
void free_boxes_t(boxes_t *boxes) {
    if (boxes != NULL) {
        if (boxes->min_x != NULL) free(boxes->min_x);
        if (boxes->min_y != NULL) free(boxes->min_y);
        if (boxes->min_z != NULL) free(boxes->min_z);
        if (boxes->max_x != NULL) free(boxes->max_x);
        if (boxes->max_y != NULL) free(boxes->max_y);
        if (boxes->max_z != NULL) free(boxes->max_z);
        free(boxes);
    }
}

////////////////////////////////////////////////
// copy_boxes_to_interleaved
////////////////////////////////////////////////
void                                                           //
copy_boxes_to_interleaved(const boxes_t       *boxes,          //
                          boxes_interleaved_t *interleaved) {  //

    if (boxes == NULL || interleaved == NULL) {
        return;
    }

    if (interleaved->min_max_xyz != NULL) {
        free(interleaved->min_max_xyz);
    }

    interleaved->min_max_xyz = (real_t *)malloc(boxes->num_boxes * 6 * sizeof(real_t));
    interleaved->num_boxes   = boxes->num_boxes;

    for (int i = 0; i < boxes->num_boxes; i++) {
        interleaved->min_max_xyz[6 * i + 0] = boxes->min_x[i];
        interleaved->min_max_xyz[6 * i + 1] = boxes->min_y[i];
        interleaved->min_max_xyz[6 * i + 2] = boxes->min_z[i];
        interleaved->min_max_xyz[6 * i + 3] = boxes->max_x[i];
        interleaved->min_max_xyz[6 * i + 4] = boxes->max_y[i];
        interleaved->min_max_xyz[6 * i + 5] = boxes->max_z[i];
    }
}
...
////////////////////////////////////////////////
void                                                     //
get_box_bounds_il(const boxes_interleaved_t *boxes,      //
                  const int                  box_index,  //
                  real_t                     bounds[6]) {
    memcpy(bounds, &boxes->min_max_xyz[6 * box_index], 6 * sizeof(real_t));
}
...
int query_linear_search_boxes(const boxes_t *boxes, const real_t x, const real_t y, const real_t z, int **box_indices,
    ...
    *box_indices              = (int *)malloc(delta_num_boxes * sizeof(int));
    ...
    for (int i = 0; i < boxes->num_boxes; i++) {
        if (x >= boxes->min_x[i] && x <= boxes->max_x[i] && y >= boxes->min_y[i] && y <= boxes->max_y[i] &&
