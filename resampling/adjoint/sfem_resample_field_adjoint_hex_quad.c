...
compute_tet_bounding_box_a(const real_t                   x0,            //
    ...
    *min_grid_x = floor((x_min - origin0) * inv_delta0) - 1;
    *min_grid_y = floor((y_min - origin1) * inv_delta1) - 1;
    *min_grid_z = floor((z_min - origin2) * inv_delta2) - 1;

    // Using ceil for maximum indices (with safety margin of +1)
    *max_grid_x = ceil((x_max - origin0) * inv_delta0) + 1;
    *max_grid_y = ceil((y_max - origin1) * inv_delta1) + 1;
    *max_grid_z = ceil((z_max - origin2) * inv_delta2) + 1;
...
sfem_quad_rule_3D(const tet_quad_midpoint_nqp_t rule,  //
    ...
    double nodes_4[4] = {6.943184420297371373e-02,   //
                         3.300094782075718713e-01,   //
                         6.699905217924281287e-01,   //
                         9.305681557970262308e-01};  //

    double weights_4[4] = {1.739274225687267861e-01,   //
                           3.260725774312732139e-01,   //
                           3.260725774312732139e-01,   //
...
tet4_resample_field_adjoint_tet_quad_d(const real_t                x0_n,       // Tet vertices //
    ...
    const int off5 = stride0 + stride2;
    const int off6 = stride0 + stride1 + stride2;
    ...
    compute_tet_bounding_box_a(x0_n,          //
                               x1_n,          //
                               x2_n,          //
                               x3_n,          //
                               y0_n,          //
    ...
    for (int k_grid_z = min_grid_z; k_grid_z < max_grid_z; k_grid_z++) {
        const real_t z_hex_min = ((real_t)k_grid_z) * delta2 + origin2;
        const real_t z_hex_max = z_hex_min + delta2;

        for (int j_grid_y = min_grid_y; j_grid_y < max_grid_y; j_grid_y++) {
            const real_t y_hex_min = ((real_t)j_grid_y) * delta1 + origin1;
            const real_t y_hex_max = y_hex_min + delta1;

            for (int i_grid_x = min_grid_x; i_grid_x < max_grid_x; i_grid_x++) {
                const real_t x_hex_min         = ((real_t)i_grid_x) * delta0 + origin0;
                const real_t x_hex_max         = x_hex_min + delta0;
                const real_t hex_vertices_x[8] = {x_hex_min,
                                                  x_hex_max,
                                                  x_hex_max,
                                                  x_hex_min,  //
