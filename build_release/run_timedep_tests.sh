...
echo "Starting Time-dependent Neumann Kelvin-Voigt tests..."
...
make -j12 && \
rm -rf test_newmark_timedep_neumann_kv_eta_* test_newmark_timedep_neumann_kv u_eta_*.xmf u.xmf u_eta_*.h5 u.h5 && \
echo "Build and cleanup successful"

# Run Kelvin-Voigt time-dependent Neumann tests
counter=1
for eta in "${ETA_VALUES[@]}"; do
    echo "Running Time-dependent Neumann KV eta = $eta"
    
    SFEM_DAMPING_RATIO=$eta ./sfem_TimedepNeumannKVTest && \
    mv test_newmark_timedep_neumann_kv "test_newmark_timedep_neumann_kv_eta_$eta" && \
    
    python ../python/sfem/mesh/raw_to_db.py "test_newmark_timedep_neumann_kv_eta_$eta" "u_eta_${counter}.xmf" \
        --transient \
        --time_whole_txt="test_newmark_timedep_neumann_kv_eta_$eta/time.txt" \
        --point_data="test_newmark_timedep_neumann_kv_eta_$eta/disp.0.*.raw,test_newmark_timedep_neumann_kv_eta_$eta/disp.1.*.raw,test_newmark_timedep_neumann_kv_eta_$eta/disp.2.*.raw" && \
    
    echo "Generated: u_eta_${counter}.xmf (eta = $eta)"
    ((counter++))
done

echo "All time-dependent Neumann tests completed!"
