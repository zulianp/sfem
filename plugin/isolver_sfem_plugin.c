...
int ISOLVER_EXPORT isolver_function_value(const isolver_function_t *info,
    ...
    laplacian_assemble_value(mesh->element_type, mesh->nelements, mesh->nnodes, mesh->elements, mesh->points, x, out);
    return ISOLVER_FUNCTION_SUCCESS;
}
...
int ISOLVER_EXPORT isolver_function_apply(const isolver_function_t *info,
    ...
    laplacian_assemble_gradient(mesh->element_type, mesh->nelements, mesh->nnodes, mesh->elements, mesh->points, h, out);
    return ISOLVER_FUNCTION_SUCCESS;
}
