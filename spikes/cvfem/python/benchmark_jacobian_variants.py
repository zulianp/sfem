...
DEFAULT_BENCH = SPIKE_ROOT / "build" / "cvfem_tet4_ns_upwind_bench"
DEFAULT_KERNELS = (
    "current",
    "current_slots",
    "sympy",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--build-dir", type=Path, default=SPIKE_ROOT / "build", help="CMake build directory for --build")
...
def maybe_build(args: argparse.Namespace) -> None:
    ...
    subprocess.run(cmd, cwd=SPIKE_ROOT, check=True)
...
def run_one(args: argparse.Namespace, driver: str, kernel: str, trial: int) -> RunResult:
    ...
    proc = subprocess.run(cmd, cwd=SPIKE_ROOT, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
