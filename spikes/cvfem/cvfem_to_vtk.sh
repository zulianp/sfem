...
#
# The driver writes mesh/ plus one SoA file per field (x.0 x.1 x.2 = velocity, x.3 =
# pressure). On a semi-structured run mesh/ is the *expanded* fine mesh, written by
...
shopt -s nullglob
fields=("$OUT"/x.*.float64 "$OUT"/x.*.float32)
shopt -u nullglob
[[ ${#fields[@]} -gt 0 ]] || { echo "no x.* field files in $OUT" >&2; exit 1; }
