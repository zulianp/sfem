...
# The CMake source directory is the spike root, one level above this script.
# BASH_SOURCE is unset under zsh, where a sourced file gets its path in $0 instead;
# without the fallback the "/.." below would resolve one directory too high.
_cvfem_env_file="${BASH_SOURCE[0]:-$0}"
export CVFEM_SRC="${CVFEM_SRC:-$(cd "$(dirname "$_cvfem_env_file")/.." && pwd)}"
unset _cvfem_env_file
