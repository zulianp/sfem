...
BENCH="${SPIKE_ROOT}/build/cvfem_tet4_ns_upwind_bench"
OUT_DIR="${SPIKE_ROOT}/traces"
...
Build first, e.g.:
  cmake -S $SPIKE_ROOT -B $SPIKE_ROOT/build -DSFEM_DIR=<sfem-build>
  cmake --build $SPIKE_ROOT/build -j"
...
if [[ "$ANALYZE" -eq 1 ]]; then
    ...
    ANALYZER="${SPIKE_ROOT}/python/analyze_xctrace_export.py"
    ...
    echo
    echo "Analyze later:"
    echo "  ${REPO_ROOT}/venv/bin/python ${SPIKE_ROOT}/python/analyze_xctrace_export.py ${OUT_DIR}/cvfem_*_${run_tag}.trace"
fi
