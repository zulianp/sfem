...
#include "packed_elements.hpp"   // packed_elements_matmul: BLAS gemm, with a loop fallback
...
//
// packed_elements_matmul is the shared wrapper -- BLAS dgemm where SFEM_ENABLE_BLAS is on,
// a blocked loop otherwise. X and Y are element-major, 32 contiguous dofs per element.
...
#pragma omp for schedule(static)
        for (ptrdiff_t e = 0; e < d.nmacro; ++e) {
            ...
            // used L^3 times. packed_elements_matmul_nonsym rather than the plain routine,
            // because a Navier-Stokes element matrix is not symmetric and the plain one's
            // two branches disagree for such a matrix -- see the note on it.
