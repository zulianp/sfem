...
calling `make_packed` (`drivers/cvfem_hex8_ns_upwind_bench.cpp`, `smesh::SFC::create_from_env`).
...
node id can be found by binary search over `owned_nodes_ptr`. `src/core/cvfem_pack_coloring.hpp`
relies on this.
...
`drivers/cvfem_hex8_ns_steady.cpp` defines its own `MeshData` and `BSR4`, distinct from the
...
Only the **pack machinery** is unified (`src/hex8/cvfem_hex8_pack_common.hpp`). Unifying
