...
```bash
cd spikes/cvfem
sbatch jobs/bench_hex8_alps.sbatch
```
...
```bash
SIZES="48 64 96 128" sbatch jobs/bench_hex8_alps.sbatch     # add bigger meshes
SWEEPS="layout threads" sbatch jobs/bench_hex8_alps.sbatch  # just those two
TRIALS=5 sbatch jobs/bench_hex8_alps.sbatch                 # more interleaved passes
THREADS="1 2 4 9 18 36 72" sbatch jobs/bench_hex8_alps.sbatch
```
...
```bash
sbatch jobs/perf_hex8_alps.sbatch                    # counters
PERF_RECORD=1 sbatch jobs/perf_hex8_alps.sbatch      # + sampled profile
```
...
ssh alps
cd $SCRATCH/sfem/spikes/cvfem && source scripts/alps_env.sh
...
(`src/op/cvfem_hex8_ns_op.cpp`, and `assemble_block_diag` does the same). This benchmark hoists
...
CVFEM_CPUS=72 cvfem_run ./scripts/run_sshex8_sweep.sh
...
```bash
CVFEM_CPUS=72 cvfem_run ./scripts/run_block_assess.sh          # the sweep above
...
stagnates -- so `src/core/cvfem_fgmres.hpp` adds flexible GMRES, selected automatically whenever the
...
3. *The weights depend only on the offset class* `(xi%q, yi%q, zi%q)`. There are `q^3` of them,
   ...
   indexed per entry -- the principle `src/ss/cvfem_ss_transfer.hpp` applies to the prolongation.
