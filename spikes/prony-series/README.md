...
| `epdm_torsion_newmark.yaml` | The same Newmark release scenario driven by a *measured* material: the EPDM + 30 phr carbon black of Felhos et al., eXPRESS Polymer Letters 2(3) 157–164 (2008) — a 15-term Maxwell fit to a DMTA master curve, with the Mooney-Rivlin backbone taken from its glassy `E0 = 3.7 GPa`, in SI units. Eleven of the fifteen terms relax inside any step the driver can take and are filtered, leaving `tau = 1610, 12.9, 0.153, 9.89e-4 s` and `gamma = 4.09e-3`, i.e. a 15 MPa rubber. Peak torque `1079 N·m` at the end of the ramp, relaxing 8.9% over the 4.5 s hold; after the release the freed face carries `2.5e-9 N·m` and the control point recovers 99.3% of its twist over the remaining 25 s. 60000 steps, ndof 1200, 19.77M CG iterations. The terms that are filtered still cannot be deleted from the file: `g_inf = 1 - sum(g_i)` is formed over all fifteen, and dropping them would turn the rubber back into a glass. |
