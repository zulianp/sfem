...
do
	...
	strain_prefix=$p0"/strain"
	strain_postfix=`printf ".%0."$padding"d" $i`

	echo $strain_prefix
	echo $strain_postfix

	SFEM_OUTPUT_POSTFIX=$strain_postfix cstrain $mesh_folder $ux $uy $uz $strain_prefix

	for(( d=0; d < 6; d++ ))
	do
		p0_file=$strain_prefix".$d"$strain_postfix".raw"
		p1_file=$p1"/strain.$d"$strain_postfix".raw"
...
strain_selector_0="$p1/strain.0.*.raw,$p1/strain.1.*.raw,$p1/strain.2.*.raw"
strain_selector_1="$p1/strain.3.*.raw,$p1/strain.4.*.raw,$p1/strain.5.*.raw"
