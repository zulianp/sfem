...
SFEM_OUTPUT="${SFEM_OUTPUT:-${SFEM_DISTANCE_GRADIENT_OUTPUT:-distance_gradients}}"
SFEM_STEPS="${SFEM_STEPS:-${SFEM_DISTANCE_GRADIENT_STEPS:-8}}"
...
SFEM_OUTPUT="$SFEM_OUTPUT" \
SFEM_STEPS="$SFEM_STEPS" \
    $LAUNCH "$TEST_BIN"

python3 "$HERE/write_xdmf.py" "$SFEM_OUTPUT" "$((SFEM_STEPS + 1))"
