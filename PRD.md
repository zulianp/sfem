...
- **Matrix-free parallel apply**: Support phased assembly via `ElementScope` (`ALL`, `OWNED_NOT_SHARED`, `SHARED_AND_AURA`). Ranges are **block-local** `[begin,end)`; on distributed meshes smesh currently exposes owned/shared/aura counts only for a **single** `"default"` block (multi-block distributed partitioning not yet in smesh).
