...
int read_mapped_field(MPI_Comm           comm,
    ...
    idx_t *req_count = (idx_t *)calloc((size_t)size, sizeof(idx_t));
    if (!req_count) {
        free(local_chunk);
        return SFEM_FAILURE;
    }
    ...
    idx_t *recv_req_count = (idx_t *)malloc((size_t)size * sizeof(idx_t));
    if (!recv_req_count) {
        free(req_count);
        free(local_chunk);
        return SFEM_FAILURE;
    ...
    MPI_CATCH_ERROR(MPI_Alltoall(req_count, 1, SFEM_MPI_IDX_T, recv_req_count, 1, SFEM_MPI_IDX_T, comm));

    idx_t *req_displs  = (idx_t *)malloc((size_t)size * sizeof(idx_t));
    idx_t *recv_displs = (idx_t *)malloc((size_t)size * sizeof(idx_t));
    ...
    MPI_CATCH_ERROR(MPI_Alltoallv(req_list,
                                  req_count,
                                  req_displs,
                                  SFEM_MPI_IDX_T,
                                  recv_req_list,
    ...
    MPI_CATCH_ERROR(MPI_Alltoallv(send_resp,
                                  recv_req_count,
                                  recv_displs,
                                  data_type,
                                  recv_resp,
