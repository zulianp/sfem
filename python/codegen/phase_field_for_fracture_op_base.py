...
class PhaseFieldForFractureOpBase(FEMaterial):
    ...
    def initialize(self):
        ...
        eval_hessian_c += self.vector_diff_tensor_x(
            eval_grad, s_gradc, trial_grad_phase
        )
        eval_hessian[0:nnodes, 0:ndofs] = eval_hessian_c[:, :]

        # Displacement
        eval_hessian[nnodes:ndofs, 0:ndofs] = self.vector_diff_tensor_x(
            eval_grad, s_disp_grad, trial_grad_disp
        )
