...
def mass_matrix(level, interpolation):
	...
	if not interpolation:
		...
		for l in range(0, 8):
			for m in range(0, 8):
				D_e[l, m] = sp.integrate(f[l] * f[m] * h3, (q[2], 0, 1), (q[1], 0, 1), (q[0], 0, 1))
				sum_D_e += D_e[l, m] 
		...
		for yi in range(0, level):
			for xi in range(0, level):
				...
				if interpolation:
					...
					for l in range(0, 8):
						q_m = sp.Matrix(3, 1, [tx + x[l]*h, ty + y[l]*h, tz + z[l]*h])
						f_m = fe.fun(q_m)
					...
					for l in range(0, 8):
						for m in range(0, len(f_m)):
							B_e[l, m] = sp.integrate(f[l] * f_m[m] * h3, (q[2], 0, 1), (q[1], 0, 1), (q[0], 0, 1))
							sum_Be += B_e[l, m]
