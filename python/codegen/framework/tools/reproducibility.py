...
def _bind(params, element, components):
    ...
    for ctype, name, extent in params:
        if name == "element_type":
            ...
            # A matrix value array, not a field: how many entries each node
            # owns is the assembly format's decision, and this harness does not
            # model it.  Block-diagonal-symmetric writes dim*(dim+1)/2 per node,
            # so sizing it one-per-node overran the buffer -- silently at small
            # meshes, and by corrupting the driver's own strings at refine 5.
