...
| --- | --- | --- | --- | --- |
| Generated SFEM `laplace_proteus_hex64_residual_element_soa_float` vs fused Metal local apply | 1 element, 64 shape functions, 64 quadrature points, identity geometry | max abs diff `2.85065539e-07` with tolerance `5e-4` | generated SFEM CPU `4.899160 us` avg over 4096 calls; Metal GPU `264.529297 us` avg over 64 dispatches in the comparison harness | `/private/tmp/sfem_laplace_proteus_hex64_raw_spirv_binary/manifest.json`, `generated_sfem_comparison` |
| SFEM-IR fused Metal local apply smoke | 1 element, 64 shape functions, 64 quadrature points | pass against SFEM IR reference | `241.117839 us` avg GPU time over 64 dispatches | same manifest, `metal_smoke.local_apply` |
| SFEM-IR Metal EBE map plus reduce smoke | 1 element, 65-node bound, node degree 1 | pass against SFEM IR EBE reference | `368.774088 us` avg GPU time over 64 map+reduce command buffers | same manifest, `metal_smoke.ebe_map_reduce` |
| SFEM-IR Metal sum-factor stage smoke | 18 generated stage kernels | each stage pass against dense stage reference | `37.843388 us` mean avg GPU time per stage, range `9.213541-66.515625 us`; serial stage-sum `681.180983 us` | same manifest, `metal_smoke.sum_factor_stages` |
...
| Raw generic GPU to SPIR-V probe | sum-factor, form, EBE map, EBE full GPU dialect artifacts | extracted `spirv.module` kernels verify and serialize | 4 raw kernel SPIR-V binaries totaling 27712 bytes; not runnable yet because host launch lowering remains open | same manifest, `mlir_gpu_to_spirv` |
