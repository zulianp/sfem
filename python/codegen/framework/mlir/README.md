...
Current PROTEUS_HEX64 Apple GPU measurements from the strict artifact gate on 2026-07-03:

| Kernel path | Element/work | Output check | Timing | Evidence |
| --- | --- | --- | --- | --- |
| Generated SFEM `laplace_proteus_hex64_residual_element_soa_float` vs fused Metal local apply | 1 element, 64 shape functions, 64 quadrature points, identity geometry | max abs diff `2.85065539e-07` with tolerance `5e-4` | generated SFEM CPU `1.802928 us` avg over 4096 calls; Metal GPU `216.904298 us` avg over 64 dispatches in the comparison harness | `/private/tmp/sfem_laplace_proteus_hex64_host_wrapper_spirv/manifest.json`, `generated_sfem_comparison` |
| SFEM-IR fused Metal local apply smoke | 1 element, 64 shape functions, 64 quadrature points | pass against SFEM IR reference | `227.050782 us` avg GPU time over 64 dispatches | same manifest, `metal_smoke.local_apply` |
| SFEM-IR Metal EBE map plus reduce smoke | 1 element, 65-node bound, node degree 1 | pass against SFEM IR EBE reference | `349.203777 us` avg GPU time over 64 map+reduce command buffers | same manifest, `metal_smoke.ebe_map_reduce` |
| SFEM-IR Metal sum-factor stage smoke | 18 generated stage kernels | each stage pass against dense stage reference | `30.336625 us` mean avg GPU time per stage, range `16.548827-44.608073 us`; serial stage-sum `546.059242 us` | same manifest, `metal_smoke.sum_factor_stages` |
| IREE Metal linalg pipelines | sum-factor and full-form `.linalg_pipeline.mlir` VMFBs | runtime pass against SFEM reference values | timing not recorded yet; backend dump has 8 MSL files and 8 SPIR-V binaries totaling 18272 bytes | same manifest, `iree_metal_runtime`, `iree_metal_executable_files` |
| Raw generic GPU to SPIR-V probe | sum-factor, form, EBE map, EBE full GPU dialect artifacts | host `func.func` wrappers preserved; extracted `spirv.module` kernels verify and serialize | 4 raw kernel SPIR-V binaries totaling 27712 bytes; not runnable yet because `gpu.launch_func` lowering remains open | same manifest, `mlir_gpu_to_spirv` |
...
Use `--probe-mlir-gpu-to-spirv` to also run the standard MLIR GPU-module-to-SPIR-V pass pipeline on those raw generic GPU artifacts. This probe records kernel lowering, host wrapper preservation, extracted kernel-module verification, binary serialization, and host launch lowering separately: `raw_gpu_spirv_kernel_ok` is true once raw GPU kernels lower to SPIR-V with OpenCL storage classes and no remaining memref loads/stores, `raw_gpu_spirv_host_wrapper_ok` is true when the host side remains a normal `func.func` wrapper around `gpu.launch_func`, `raw_gpu_spirv_kernel_module_ok` is true once the nested `spirv.module` verifies as standalone SPIR-V dialect, and `raw_gpu_spirv_kernel_binary_ok` is true once the extracted kernel modules serialize to nonempty SPIR-V binaries with `mlir-translate --serialize-spirv`. Unresolved host `gpu.launch_func`/`gpu.module` wiring keeps `raw_gpu_spirv_host_launch_ok` false and is classified as `mlir_gpu_to_spirv_host_launch_boundary`.
...
Use `--require-pipeline-evidence` when the summarized SFEM IR to IREE/Metal path must be complete according to `pipeline_evidence["metal_pipeline_ok"]`.
