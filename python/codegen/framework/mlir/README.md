...
Use `--probe-iree-metal-gpu` to record whether the generated generic GPU dialect artifacts currently compile directly to IREE Metal VMFBs. This probe covers the sum-factor stage kernels, local form apply, EBE map, and EBE map/reduce GPU artifacts. Known IREE VM conversion failures around `gpu.launch_func` and SCF/index materialization are classified as `iree_vm_generic_gpu_index_conversion`; add `--require-iree-metal-gpu` when those boundaries must pass.
