...
4. The specialization of Laplace for TET4 is acceptable only if the result is as tight as `tet4_laplacian_apply_fff` the plan should still allow to fully manipulate and merge expressions to reach such results (my baseline is 2x faster than the geneerated code. The generated code needs to be the same or faster). The generated kernels are quite "dirty" which is a symptom of too much reliance on string templates rather than proper model emission separation. Also reproduce tightness and performance of `tet4_linear_elasticity_apply_adj` through proper model manipulation to emission (this should generalize to other models as well). 
5. Guarantee the proper layered architectural separation. Have objects for tensor products and sum factorization with proper model abstractions to be used to lower the IR in the lower layers.
6. Make sure to make the generated codes as lean and mean as possible, no usless copies, no usless indices. 
