...
def bspline(i, order, t):
    if order == 1:
        return sp.Piecewise((1, (t >= i)), (0, True)) * sp.Piecewise(
            (1, (t < i + 1)), (0, True)
        )
    else:
        return (t - i) / (i + order - 1 - i) * bspline(i, order - 1, t) + (
            i + order - t
        ) / (i + order - (i + 1)) * bspline(i + 1, order - 1, t)
...
def bspline5(i, order, t):
    if order == 1:
        ...
        return (t - i) / (i + order - 1 - i) * bspline5(i, order - 1, t) + (
            i + order - t
        ) / (i + order - (i + 1)) * bspline5(i + 1, order - 1, t)
