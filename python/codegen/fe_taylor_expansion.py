...
def try_laplacian(fe):
    ...
    for i in range(0, n):
        for j in range(0, n):
            ...
            taylor_mass_matrix.append(
                ast.Assignment(sp.symbols(f"element_matrix[{i*n+j}]"), M[i, j])
            )
            taylor_lapl_matrix.append(
                ast.Assignment(sp.symbols(f"element_matrix[{i*n+j}]"), L[i, j])
            )
