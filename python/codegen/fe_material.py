...
class FEFunction:
    def __init__(self, name, fe, ncomp):
        ...
        self.symbols_grad_ = sp.Matrix(
            fe.spatial_dim(), ncomp, coeffs(f"s_grad_{name}", fe.spatial_dim() * ncomp)
        )
        ...
        self.symbols_trial_grad_ = sp.Matrix(
            fe.spatial_dim(), 1, coeffs(f"s_trial_grad_{name}", fe.spatial_dim())
        )
        self.symbols_test_grad_ = sp.Matrix(
            fe.spatial_dim(), 1, coeffs(f"s_test_grad_{name}", fe.spatial_dim())
        )
