...
def fff_code_basic(FFF):
    ...
    code0to2 = tpl.format(NUM="0to2", CODE=c_gen(assign_matrix("sub_fff", FFF[0])))

    code3 = tpl.format(NUM="3", CODE=c_gen(assign_matrix("sub_fff", FFF[3])))
...
def fff_code(name, FFF):
    funs = []
    tpl = read_file("tpl/macro_sub_jacobian_tpl.c")
    code = tpl.format(NUM=name, CODE=c_gen(assign_matrix("sub_fff", FFF)))
...
class MacroTri3:
    def __init__(self):
        self.x0 = vec2(0.0, 0.0)
        self.x1 = vec2(1.0, 0.0)
        self.x2 = vec2(0.0, 1.0)
    ...
    def fff_level_1(self):
        ...
        FFF = [FFFA0toA2, FFFA0toA2, FFFA0toA2, subJ([x3, x4, x5])]
