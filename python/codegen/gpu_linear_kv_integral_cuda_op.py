...
class GPULinearKVOp:
    ...
    def init_opt(self, fe, q):
        ...
        # GPU-friendly optimized gradient (SoA, reference gradient form)
        # Define symbolic placeholders to mirror linear elasticity codegen:
        #   out_i += <Pk_tXJinv_t_sym + Pd_tXJinv_t_sym, tgrad_ref[i]> + <M_load_sym, tfun[i]>
        Pk_tXJinv_t_sym = matrix_coeff("Pk_tXJinv_t", dims, dims)
        Pd_tXJinv_t_sym = matrix_coeff("Pd_tXJinv_t", dims, dims)
        M_load_sym      = matrix_coeff("M_load",      dims, 1)

        self.Pk_tXJinv_t_sym = Pk_tXJinv_t_sym
        self.Pd_tXJinv_t_sym = Pd_tXJinv_t_sym
        self.M_load_sym      = M_load_sym

        self.eval_grad_opt = sp.zeros(rows, 1)
        for i in range(0, fe.n_nodes() * dims):
            self.eval_grad_opt[i] = inner(Pk_tXJinv_t_sym + Pd_tXJinv_t_sym, shape_grad_ref[i]) \
                                    + inner(M_load_sym, shape_vec[i])
    ...
    def Koperand(self):
        ...
        K_tXJinv_t = stiffness * jac_inv.T * dV

        dims = self.fe.manifold_dim()
        for i in range(0, dims):
            for j in range(0, dims):
                K_tXJinv_t[i, j] = simplify(K_tXJinv_t[i, j])

        expr = []
        expr.extend(assign_matrix("K_tXJinv_t", K_tXJinv_t))
    ...
    def Coperand(self):
        ...
        C_tXJinv_t = damping * jac_inv.T * dV

        dims = self.fe.manifold_dim()
        for i in range(0, dims):
            for j in range(0, dims):
                C_tXJinv_t[i, j] = simplify(C_tXJinv_t[i, j])

        expr = []
        expr.extend(assign_matrix("C_tXJinv_t", C_tXJinv_t))
    ...
    def Moperand(self):
        ...
        dims = self.fe.manifold_dim()
        for i in range(0, dims):
            M_load[i, 0] = simplify(M_load[i, 0])

        expr = []
        expr.extend(assign_matrix("M_load", M_load))
...
def main():
    ...
    # c_log("//--------------------------")
    # c_log("// gradient_opt (SoA, ref-grad, CUDA-friendly)")
    # c_log("//--------------------------")
