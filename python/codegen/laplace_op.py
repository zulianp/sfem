...
class LaplaceOp:
    ...
    def trial_operand_expr(self):
        ...
        for d in range(0, self.fe.spatial_dim()):
            ...
            val = (
                self.trial_operand[d] * self.qw
            )  # NOTE that quadrature weight is used here
            expr.append(ast.Assignment(var, val))
        return expr
    ...
    def apply(self):
        ...
        if self.symbolic_integration:
            ...
            trial_operand = (
                self.trial_operand_symbolic
            )  # NOTE that quadrature weight is included here
    ...
    def value(self):
        ...
        for d in range(0, fe.manifold_dim()):
            if self.symbolic_integration:
                gsquared = (
                    fe.integrate(q, (trial_operand[d] * self.ref_grad_interp[d])) / 2
                )
            else:
