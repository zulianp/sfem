...
class NavierStokesOp:
	def __init__(self, fe_vel, fe_pressure):
		...
		if integrate_implicit:
			...
			for i in range(0,  n_vel):
				for j in range(0,  n_vel):
					# dt * nu * < grad(u), grad(v) >
					integr = dt * nu * fe_vel.integrate(qp, inner(grad_vel[i], grad_vel[j])) *  fe_vel.symbol_jacobian_determinant()
					self.form2_diffusion[i, j] = integr

			for i in range(0,  n_vel):
				for j in range(0,  n_vel):
					integr = fe_vel.integrate(qp, inner(fun_vel[i], fun_vel[j])) *  fe_vel.symbol_jacobian_determinant()
		...
		for i in range(0, n_vel):
			...
			for d1 in range(0, fe_vel.spatial_dim()):
				for d2 in range(0, fe_vel.spatial_dim()):
					print(f"\t{d1 * fe_vel.spatial_dim() + d2 + 1}/{fe_vel.spatial_dim()*fe_vel.spatial_dim()}")
					integr += -dt * nu * fe_vel.integrate(qp, grad_uh[d1,d2] * grad_vel[i][d1,d2]) * fe_vel.jacobian_determinant(qp)
		...
		if True:
			...
			for i in range(0, n_vel):
				...
				for d in range(0, fe_vel.spatial_dim()):
					...
					integr += -dt * fe_vel.integrate(qp, conv[d] * fun_vel[i][d]) * fe_vel.jacobian_determinant(qp)
		...
		for i in range(0, n_vel):
			...
			for d in range(0, fe_vel.spatial_dim()):
				print(f"\t{d+1}/{fe_vel.spatial_dim()}")
				integr += (-(dt/rho) * fe_vel.integrate(qp, grad_ph[d] * fun_vel[i][d])) * fe_vel.jacobian_determinant(qp)
