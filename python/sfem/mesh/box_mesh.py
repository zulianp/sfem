...
def create_sidesets(cell_type, nx, ny, nz):
    ...
    # FIXME
    return {
        "left": {"parent": parent_left, "lfi": lfi_left},
        "right": {"parent": parent_right, "lfi": lfi_right},
        "back": {"parent": parent_bottom, "lfi": lfi_bottom},
        "front": {"parent": parent_top, "lfi": lfi_top},
        "top": {"parent": parent_front, "lfi": lfi_front},
        "bottom": {"parent": parent_back, "lfi": lfi_back},
    }
    ...
    if not skip_boundary:
        ...
        for k, v in boundary_faces.items():
            ...
            for d in range(0, nxf):
                ...
                surf_meta["elements"].append(
                    {f"i{d}": f"i{d}.{str(idx_t.__name__)}.raw"}
                )
        ...
        if cell_type == "HEX8":
            ...
            for k, v in sidesets.items():
                ...
                sideset_meta = {}
                sideset_meta["element_type"] = side_type(
                    to_sfem_element_type(cell_type)
                )
