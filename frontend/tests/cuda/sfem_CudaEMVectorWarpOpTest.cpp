...
std::shared_ptr<FunctionSpace> create_sshex8_space(const int base_resolution, const int refine_level) {
    auto mesh = Mesh::create_hex8_cube(Communicator::world(),
                                       base_resolution,
                                       base_resolution,
                                       base_resolution,
                                       0,
...
void device_zeros(const std::shared_ptr<Buffer<real_t>> &v) {
    d_memset(v->data(), 0, v->size() * sizeof(real_t));
}
...
int compare_device_vectors(const char                             *label,
    ...
    if (max_abs > tol && max_rel > tol) {
        fprintf(stderr,
                "[Error] %s: max_abs=%g max_rel=%g tol=%g\n",
                label,
                (double)max_abs,
                (double)max_rel,
                (double)tol);
...
int solve_cg(const std::shared_ptr<FunctionSpace>                   &fs,
    ...
    const auto &block_name = fs->mesh_ptr()->block(0)->name();
    op->set_value_in_block(block_name, "mu", (real_t)1);
    op->set_value_in_block(block_name, "lambda", (real_t)1);
    f->add_operator(op);
...
int test_em_vector_warp_apply_vs_linear_elasticity() {
    ...
    const int    base_resolution = smesh::Env::read<int>("SFEM_BASE_RESOLUTION", 2);
    const int    refine_level    = smesh::Env::read<int>("SFEM_ELEMENT_REFINE_LEVEL", 2);
    const real_t tol             = (real_t)smesh::Env::read<double>("SFEM_TEST_TOL", 1e-8);
    ...
    const real_t     mu         = (real_t)1;
    const real_t     lambda     = (real_t)1;
    const auto      &block_name = fs->mesh_ptr()->block(0)->name();
    op_em->set_value_in_block(block_name, "mu", mu);
    op_em->set_value_in_block(block_name, "lambda", lambda);
    op_le->set_value_in_block(block_name, "mu", mu);
    op_le->set_value_in_block(block_name, "lambda", lambda);
    ...
    SFEM_TEST_ASSERT(compare_device_vectors("apply EMVectorWarpOp vs LinearElasticity", y_em, y_le, tol) ==
                     SFEM_TEST_SUCCESS);
...
int test_em_vector_warp_cg_vs_linear_elasticity() {
    ...
    auto left_ss = Sideset::create_from_selector(
            fs->mesh_ptr(),
            [](const geom_t x, const geom_t /*y*/, const geom_t /*z*/) -> bool { return x > -1e-5 && x < 1e-5; });
    auto right_ss = Sideset::create_from_selector(
            fs->mesh_ptr(),
            [](const geom_t x, const geom_t /*y*/, const geom_t /*z*/) -> bool { return x > (1 - 1e-5) && x < (1 + 1e-5); });

    std::vector<DirichletConditions::Condition> bcs = {
            {.sidesets = {left_ss}, .value = 0, .component = 0},
            {.sidesets = {left_ss}, .value = 0, .component = 1},
            {.sidesets = {left_ss}, .value = 0, .component = 2},
            {.sidesets = {right_ss}, .value = 0.01, .component = 0},
            {.sidesets = {right_ss}, .value = 0, .component = 1},
            {.sidesets = {right_ss}, .value = 0, .component = 2},
    };
