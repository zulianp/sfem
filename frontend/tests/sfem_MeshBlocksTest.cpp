...
int test_mesh_blocks_basic() {
    ...
    auto mesh = smesh::Mesh::create_hex8_cube(sfem::Communicator::self(), 2, 2, 2);
    
    // Test basic block functionality
    SFEM_TEST_EQ(mesh->n_blocks(), (size_t)1);
    SFEM_TEST_EQ(mesh->block(0)->name(), std::string("default"));
    SFEM_TEST_EQ(mesh->block(0)->element_type(), smesh::HEX8);
    SFEM_TEST_EQ(mesh->n_elements(), (ptrdiff_t)8);
    
    SFEM_TEST_EQ(mesh->elements(0)->extent(1), (size_t)8);
...
int test_mesh_blocks_add_remove() {
    ...
    mesh->add_block("quad4_block", smesh::QUAD4, elements_buffer);
    
    // Test adding block
    SFEM_TEST_EQ(mesh->n_blocks(), (size_t)2);
    SFEM_TEST_EQ(mesh->block(1)->name(), std::string("quad4_block"));
    SFEM_TEST_EQ(mesh->block(1)->element_type(), smesh::QUAD4);
...
int test_mesh_blocks_checkerboard() {
    ...
    auto mesh = smesh::Mesh::create_hex8_checkerboard_cube(sfem::Communicator::self(), 2, 2, 2);
    ...
    // Test element types
    SFEM_TEST_EQ(mesh->block(0)->element_type(), smesh::HEX8);
    SFEM_TEST_EQ(mesh->block(1)->element_type(), smesh::HEX8);
    ...
    SFEM_TEST_EQ(mesh->elements(0)->extent(1), (size_t)4);
    SFEM_TEST_EQ(mesh->element_type(0), smesh::HEX8);
