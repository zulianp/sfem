...
namespace sfem {
    ...
    void SemiStructuredKelvinVoigtNewmark::set_field(const char* name, const std::shared_ptr<Buffer<real_t>>& vel, int component) {
    ...
    std::unique_ptr<Op> SemiStructuredKelvinVoigtNewmark::create(const std::shared_ptr<FunctionSpace> &space) {
        ...
        if (!space->has_semi_structured_mesh()) {
            fprintf(stderr,
                    "[Error] SemiStructuredKelvinVoigtNewmark::create requires space with semi_structured_mesh!\n");
            return nullptr;
        }
    ...
    SemiStructuredKelvinVoigtNewmark::SemiStructuredKelvinVoigtNewmark(const std::shared_ptr<FunctionSpace> &space) : space(space) {}

    int SemiStructuredKelvinVoigtNewmark::hessian_crs(const real_t *const, const count_t *const, const idx_t *const, real_t *const) {
        return SFEM_FAILURE;
    }

    int SemiStructuredKelvinVoigtNewmark::hessian_bsr(const real_t *const, const count_t *const, const idx_t *const, real_t *const) {
        return SFEM_FAILURE;
    }

    int SemiStructuredKelvinVoigtNewmark::hessian_crs_sym(const real_t *const, const count_t *const, const idx_t *const, real_t *const, real_t *const) {
        return SFEM_FAILURE;
    }

    int SemiStructuredKelvinVoigtNewmark::hessian_bcrs_sym(const real_t *const, const count_t *const, const idx_t *const, const ptrdiff_t, real_t **const, real_t **const) {
        return SFEM_FAILURE;
    }

    int SemiStructuredKelvinVoigtNewmark::hessian_block_diag_sym(const real_t *const, real_t *const) { return SFEM_FAILURE; }
    ...
    int SemiStructuredKelvinVoigtNewmark::value(const real_t *x, real_t *const out) { return SFEM_FAILURE; }
    ...
    SemiStructuredKelvinVoigtNewmark::~SemiStructuredKelvinVoigtNewmark() {
        if (SFEM_PRINT_THROUGHPUT && calls) {
            printf("SemiStructuredKelvinVoigtNewmark[%d]::apply called %ld times. Total: %g [s], Avg: %g [s]\n",
                   space->semi_structured_mesh().level(), calls, total_time, total_time / calls);
        }
    }
