...
// // Optimized variant with software prefetching, fewer copies, and unrolling.
// template <typename pack_idx_t, int NXE, typename MicroKernel>
// struct PackedLaplacianOpt {
//     static int apply(const ptrdiff_t                       n_packs,
//                      const ptrdiff_t                       n_elements_per_pack,
//                      const ptrdiff_t                       n_elements,
//                      const ptrdiff_t                       max_nodes_per_pack,
...
//                      const real_t *const SFEM_RESTRICT     u,
//                      real_t *const SFEM_RESTRICT           values) {
//         // Prefetch distance configurable via env; default tuned per-arch
//         int prefetch_distance = 0;
// #if defined(__aarch64__)
//         prefetch_distance = smesh::Env::read("SFEM_PACKED_PREFETCH_DISTANCE", 0);
// #else
//         prefetch_distance = smesh::Env::read("SFEM_PACKED_PREFETCH_DISTANCE", 1);
// #endif
// #pragma omp parallel
//         {
//             // Align buffers for better SIMD/cacheline behavior.
//             real_t *in  = nullptr;
//             real_t *out = nullptr;
// #if defined(_POSIX_C_SOURCE)
//             (void)posix_memalign((void **)&in, 64, max_nodes_per_pack * sizeof(real_t));
//             (void)posix_memalign((void **)&out, 64, max_nodes_per_pack * sizeof(real_t));
//             memset(out, 0, (size_t)max_nodes_per_pack * sizeof(real_t));
// #else
//             in  = (real_t *)aligned_alloc(64, max_nodes_per_pack * sizeof(real_t));
//             out = (real_t *)aligned_alloc(64, max_nodes_per_pack * sizeof(real_t));
//             memset(out, 0, (size_t)max_nodes_per_pack * sizeof(real_t));
...
//                 // Gather input once per pack
//                 memcpy(in, &u[owned_nodes_ptr[p]], (size_t)n_owned * sizeof(real_t));
//                 for (ptrdiff_t k = 0; k < n_ghost; ++k) {
//                     in[n_owned + k] = u[ghosts[k]];
//                 }

//                 // Element loop with prefetch (read)
//                 for (ptrdiff_t e = e_start; e < e_end; ++e) {
//                     // Prefetch next iteration streams and gathers
//                     const ptrdiff_t pe = e + prefetch_distance;
//                     if (pe < e_end) {
//                         SFEM_PREFETCH_R(&fff[pe * 6], 3);
//                         for (int v = 0; v < NXE; ++v) {
//                             SFEM_PREFETCH_R(&elements[v][pe], 0);
//                         }
//                         // Prefetch next gather addresses
//                         alignas(64) pack_idx_t evp[NXE];
// // #pragma unroll
//                         for (int v = 0; v < NXE; ++v) {
//                             evp[v] = elements[v][pe];
//                         }
// // #pragma unroll
//                         for (int v = 0; v < NXE; ++v) {
//                             SFEM_PREFETCH_R(&in[evp[v]], 3);
//                         }
//                     }

//                     // Gather indices and values for current element
//                     alignas(64) pack_idx_t ev[NXE];
// // #pragma unroll
//                     for (int v = 0; v < NXE; ++v) {
//                         ev[v] = elements[v][e];
//                     }

//                     alignas(64) scalar_t element_u[NXE];
// // #pragma unroll
//                     for (int v = 0; v < NXE; ++v) {
//                         element_u[v] = in[ev[v]];
//                     }

//                     alignas(64) accumulator_t element_out[NXE] = {0};
//                     alignas(64) scalar_t      fff_i[6];
// // #pragma unroll
//                     for (int d = 0; d < 6; ++d) {
//                         fff_i[d] = (scalar_t)fff[e * 6 + d];
//                     }
//                     MicroKernel::apply(fff_i, element_u, element_out);
...
//                 // Flush accumulators with tiny clears (keeps cachelines hot)
//                 for (ptrdiff_t k = 0; k < n_owned; ++k) {
// #if defined(__GNUC__) || defined(__clang__)
//                     SFEM_PREFETCH_W(&values[owned_nodes_ptr[p] + k], 1);
// #endif
// #pragma omp atomic update
//                     values[owned_nodes_ptr[p] + k] += out[k];
//                     out[k] = 0;  // tiny clear while hot
//                 }

//                 scalar_t *const SFEM_RESTRICT g_out = &out[n_owned];
//                 for (ptrdiff_t k = 0; k < n_ghost; ++k) {
// #if defined(__GNUC__) || defined(__clang__)
//                     SFEM_PREFETCH_W(&values[ghosts[k]], 1);
// #endif
// #pragma omp atomic update
//                     values[ghosts[k]] += g_out[k];
//                     g_out[k] = 0;  // tiny clear while hot
//                 }
//             }
...
// template <typename pack_idx_t>
// static int packed_laplacian_apply_opt(smesh::ElemType                         element_type,
//                                       const ptrdiff_t                       n_packs,
//                                       const ptrdiff_t                       n_elements_per_pack,
//                                       const ptrdiff_t                       n_elements,
//                                       const ptrdiff_t                       max_nodes_per_pack,
...
//     switch (element_type) {
//         case smesh::TET4:
//             return PackedLaplacianOpt<PackedIdxType, 4, Tet4MicroKernel>::apply(n_packs,
//                                                                                 n_elements_per_pack,
//                                                                                 n_elements,
//                                                                                 max_nodes_per_pack,
//                                                                                 elements,
...
//                                                                                 values);
//         case smesh::HEX8:
//             return PackedLaplacianOpt<PackedIdxType, 8, Hex8MicroKernel>::apply(n_packs,
//                                                                                 n_elements_per_pack,
//                                                                                 n_elements,
//                                                                                 max_nodes_per_pack,
//                                                                                 elements,
...
//                                                                                 values);
//         case smesh::TET10:
//             return PackedLaplacianOpt<PackedIdxType, 10, Tet10MicroKernel>::apply(n_packs,
//                                                                                   n_elements_per_pack,
//                                                                                   n_elements,
//                                                                                   max_nodes_per_pack,
//                                                                                   elements,
...
//                                                                                   values);
//         default: {
//             SFEM_ERROR("packed_laplacian_apply_opt not implemented for type %s\n", type_to_string(element_type));
//             return SFEM_FAILURE;
//         }
//     }
// }
...
//             if (domain->second.element_type == smesh::HEX8 || domain->second.element_type == SSHEX8) {
...
//             const bool use_opt = smesh::Env::read("SFEM_PACKED_LAPLACIAN_OPT", true);
//             if (use_opt) {
//                 return packed_laplacian_apply_opt<PackedIdxType>(domain.element_type,
