...
namespace sfem {
    ...
    class NewmarkInertiaPotential final : public Op {
    public:
        static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

        explicit NewmarkInertiaPotential(const std::shared_ptr<FunctionSpace> &space);
        ~NewmarkInertiaPotential() override;

        const char *name() const override { return "NewmarkInertiaPotential"; }
