...
namespace sfem {
    namespace {
        ...
        static const char *const MATERIAL_PARAMETER_NAMES[N_MATERIAL_PARAMETERS] = {"mu"};
        ...
        smesh::block_idx_t block_id_for_domain(const smesh::Mesh &mesh,
            ...
            SFEM_ERROR("GeneratedStokes: mesh block pointer not found in mesh.blocks()\n");
            return 0;
        }
    ...
    std::unique_ptr<Op> GeneratedStokes::create(const std::shared_ptr<FunctionSpace> &space) {
        ...
        if (space->block_size() != expected_block_size) {
            SFEM_ERROR("GeneratedStokes requires block_size=%ld\n",
                       static_cast<long>(expected_block_size));
            return nullptr;
        }
        auto op = std::make_unique<GeneratedStokes>(space);
        op->initialize();
        return op;
    }

    GeneratedStokes::GeneratedStokes(const std::shared_ptr<FunctionSpace> &space)
        : impl_(std::make_unique<Impl>(space)) {}
    GeneratedStokes::~GeneratedStokes() = default;

    ptrdiff_t GeneratedStokes::n_dofs_domain() const { return impl_->space->n_dofs(); }
    ptrdiff_t GeneratedStokes::n_dofs_image() const { return impl_->space->n_dofs(); }

    double GeneratedStokes::flops_value() const {
        double total = 0;
        if (!impl_->domains) {
            return total;
        }
    ...
    size_t GeneratedStokes::memory_traffic_bytes_value() const {
        size_t total = 0;
        if (!impl_->domains) {
            return total;
        }
    ...
    double GeneratedStokes::flops_gradient() const {
        double total = 0;
        if (!impl_->domains) {
            return total;
        }
        ...
        impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics = stokes_residual_element_2d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
            ...
            if (dim == 3) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics = stokes_residual_element_3d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total += impl_->residual_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
                    }
                }
    ...
    size_t GeneratedStokes::memory_traffic_bytes_gradient() const {
        size_t total = 0;
        if (!impl_->domains) {
            return total;
        }
    ...
    double GeneratedStokes::flops_apply() const {
        double total = 0;
        if (!impl_->domains) {
            return total;
        }
        ...
        impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics = stokes_jacobian_action_element_2d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
            ...
            if (dim == 3) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics = stokes_jacobian_action_element_3d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total += impl_->jacobian_action_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
                    }
                }
    ...
    size_t GeneratedStokes::memory_traffic_bytes_apply() const {
        size_t total = 0;
        if (!impl_->domains) {
            return total;
        }
    ...
    int GeneratedStokes::initialize(const std::vector<std::string> &block_names) {
        SFEM_TRACE_SCOPE("GeneratedStokes::initialize");
        impl_->domains = std::make_shared<MultiDomainOp>(impl_->space, block_names);
        {
            ...
            if (!dof_graph ||
                ...
                SFEM_ERROR("GeneratedStokes::initialize: the dof graph is malformed; the assembly kernels assume it is not\n");
                return SFEM_FAILURE;
            }
        }
    ...
    int GeneratedStokes::update(const real_t *const x) {
        SFEM_TRACE_SCOPE("GeneratedStokes::update");
        impl_->current = x;
        return SFEM_SUCCESS;
    }

    int GeneratedStokes::update(const real_t *const previous,
    ...
    int GeneratedStokes::gradient(const real_t *const state, real_t *const out) {
        SFEM_TRACE_SCOPE("GeneratedStokes::gradient");
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (impl_->residual_uses_affine) {
                auto cache = std::static_pointer_cast<AffineGeometryCache>(
                        domain.user_data);
                if (!cache) {
                    SFEM_ERROR("GeneratedStokes affine residual requires cached geometry\n");
                    return SFEM_FAILURE;
                }
                if (true) {
                    if (!cache->jacobian) {
                        SFEM_ERROR("GeneratedStokes affine residual requires cached jacobian geometry\n");
                        return SFEM_FAILURE;
                    }
                ...
                if (false) {
                    if (!cache->metric_soa) {
                        SFEM_ERROR("GeneratedStokes affine residual requires cached SoA metric geometry\n");
                        return SFEM_FAILURE;
                    }
                ...
                if (false) {
                    if (!cache->metric_aos) {
                        SFEM_ERROR("GeneratedStokes affine residual requires cached AoS metric geometry\n");
                        return SFEM_FAILURE;
                    }
            ...
            if (dim == 2) {
                ...
                if (impl_->residual_uses_affine) {
                    return stokes_residual_2d_affine_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, storage[0], FIELD_STRIDE, (const void *const *)u_data, p_data, FIELD_STRIDE, (void *const *)u_out, p_out);
                }
                return stokes_residual_2d_isoparametric_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], FIELD_STRIDE, (const void *const *)u_data, p_data, FIELD_STRIDE, (void *const *)u_out, p_out);
            }
            else if (dim == 3) {
                ...
                if (impl_->residual_uses_affine) {
                    return stokes_residual_3d_affine_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], adjugate[4], adjugate[5], adjugate[6], adjugate[7], adjugate[8], determinant, storage[0], FIELD_STRIDE, (const void *const *)u_data, p_data, FIELD_STRIDE, (void *const *)u_out, p_out);
                }
                return stokes_residual_3d_isoparametric_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], FIELD_STRIDE, (const void *const *)u_data, p_data, FIELD_STRIDE, (void *const *)u_out, p_out);
            }
            SFEM_ERROR("stokes residual does not support spatial dimension %d\n", dim);
            return SFEM_FAILURE;
        });
    }

    int GeneratedStokes::apply(const real_t *const state,
                      const real_t *const direction,
                      real_t *const out) {
        SFEM_TRACE_SCOPE("GeneratedStokes::apply");
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (impl_->jacobian_action_uses_affine) {
                auto cache = std::static_pointer_cast<AffineGeometryCache>(
                        domain.user_data);
                if (!cache) {
                    SFEM_ERROR("GeneratedStokes affine jacobian action requires cached geometry\n");
                    return SFEM_FAILURE;
                }
                if (true) {
                    if (!cache->jacobian) {
                        SFEM_ERROR("GeneratedStokes affine jacobian action requires cached jacobian geometry\n");
                        return SFEM_FAILURE;
                    }
                    adjugate = reinterpret_cast<const geom_t *const *>(
                            cache->jacobian->jacobian_adjugate_SoA()->data());
                ...
                if (false) {
                    if (!cache->metric_soa) {
                        SFEM_ERROR("GeneratedStokes affine jacobian action requires cached SoA metric geometry\n");
                        return SFEM_FAILURE;
                    }
                    geom_metric = reinterpret_cast<const geom_t *const *>(
                            cache->metric_soa->fff_SoA()->data());
                ...
                if (false) {
                    if (!cache->metric_aos) {
                        SFEM_ERROR("GeneratedStokes affine jacobian action requires cached AoS metric geometry\n");
                        return SFEM_FAILURE;
                    }
                    geom_metric_aos = reinterpret_cast<const geom_t *>(
                            cache->metric_aos->fff_AoS()->data());
            ...
            if (dim == 2) {
                static constexpr ptrdiff_t FIELD_STRIDE = 3;
                    ...
                    real_t *const SFEM_RESTRICT p_out = out + 2;
                if (impl_->jacobian_action_uses_affine) {
                    return stokes_jacobian_action_2d_affine_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, storage[0], FIELD_STRIDE, (const void *const *)u_direction_data, p_direction_data, FIELD_STRIDE, (void *const *)u_out, p_out);
                }
                return stokes_jacobian_action_2d_isoparametric_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], FIELD_STRIDE, (const void *const *)u_direction_data, p_direction_data, FIELD_STRIDE, (void *const *)u_out, p_out);
            }
            else if (dim == 3) {
                static constexpr ptrdiff_t FIELD_STRIDE = 4;
                    ...
                    real_t *const SFEM_RESTRICT p_out = out + 3;
                if (impl_->jacobian_action_uses_affine) {
                    return stokes_jacobian_action_3d_affine_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], adjugate[4], adjugate[5], adjugate[6], adjugate[7], adjugate[8], determinant, storage[0], FIELD_STRIDE, (const void *const *)u_direction_data, p_direction_data, FIELD_STRIDE, (void *const *)u_out, p_out);
                }
                return stokes_jacobian_action_3d_isoparametric_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], FIELD_STRIDE, (const void *const *)u_direction_data, p_direction_data, FIELD_STRIDE, (void *const *)u_out, p_out);
            }
            SFEM_ERROR("stokes jacobian_action does not support spatial dimension %d\n", dim);
            return SFEM_FAILURE;
        });
    }

    void GeneratedStokes::set_field(const char *name,
                           const std::shared_ptr<Buffer<real_t>> &values,
                           const int component) {
        SFEM_TRACE_SCOPE("GeneratedStokes::set_field");
        if (component != 0 || std::strcmp(name, "previous") != 0) {
            SFEM_ERROR("GeneratedStokes supports set_field(\"previous\", buffer, 0)\n");
            return;
        }
    ...
    void GeneratedStokes::set_value_in_block(const std::string &block_name,
                                    const std::string &var_name,
                                    const real_t value) {
        SFEM_TRACE_SCOPE("GeneratedStokes::set_value_in_block");
        impl_->domains->set_value_in_block(block_name, var_name, value);
    }

    void GeneratedStokes::set_option(const std::string &name, const bool val) {
        SFEM_TRACE_SCOPE("GeneratedStokes::set_option");
        ...
        if (matched && val && impl_->domains) {
            ...
            if (cache_affine_geometry(impl_->space,
                ...
                SFEM_ERROR("GeneratedStokes failed to cache affine geometry\n");
            }
        }
    }

#ifdef SFEM_ENABLE_RYAML
    std::shared_ptr<Op> GeneratedStokes::create_from_yaml(const std::shared_ptr<FunctionSpace> &space,
                                                 const ryml::ConstNodeRef             &node) {
        SFEM_TRACE_SCOPE("GeneratedStokes::create_from_yaml");
        auto ret = std::make_shared<GeneratedStokes>(space);
    ...
    int GeneratedStokes::hessian_crs(const real_t *const state,
                            const count_t *const rowptr,
                            const idx_t *const colidx,
                            real_t *const values) {
        SFEM_TRACE_SCOPE("GeneratedStokes::hessian_crs");
        return SFEM_FAILURE;
    }

    int GeneratedStokes::hessian_bsr(const real_t *const state,
                            const count_t *const rowptr,
                            const idx_t *const colidx,
                            real_t *const values) {
        SFEM_TRACE_SCOPE("GeneratedStokes::hessian_bsr");
        return SFEM_FAILURE;
    }

    int GeneratedStokes::hessian_dia(const real_t *const state,
                            const int *const diag_offsets,
                            const ptrdiff_t ndiag,
                            real_t *const values) {
        SFEM_TRACE_SCOPE("GeneratedStokes::hessian_dia");
        return SFEM_FAILURE;
    }


    int GeneratedStokes::value_steps(const real_t *state,
                            const real_t *h,
                            const int nsteps,
                            const real_t *const steps,
                            real_t *const out) {
        SFEM_TRACE_SCOPE("GeneratedStokes::value_steps");
        if (nsteps <= 0) {
            return SFEM_SUCCESS;
        }
...
#pragma omp simd reduction(+ : sum)
    ...
    int GeneratedStokes::value(const real_t *state, real_t *const out) {
        SFEM_TRACE_SCOPE("GeneratedStokes::value");
