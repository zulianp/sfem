...
namespace sfem {
  class GPUGeneratedBodyForce final : public Op {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GPUGeneratedBodyForce(const std::shared_ptr<FunctionSpace> &space);
    ~GPUGeneratedBodyForce() override;

    const char *name() const override { return "GPUGeneratedBodyForce"; }
