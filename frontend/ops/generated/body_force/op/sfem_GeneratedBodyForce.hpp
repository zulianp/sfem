...
namespace sfem {
  class GeneratedBodyForce final : public Op {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GeneratedBodyForce(const std::shared_ptr<FunctionSpace> &space);
    ~GeneratedBodyForce() override;

    const char *name() const override { return "GeneratedBodyForce"; }
