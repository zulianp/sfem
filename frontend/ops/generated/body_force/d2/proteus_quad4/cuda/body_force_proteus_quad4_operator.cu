...
__global__ void body_force_proteus_quad4_residual_a_msoa_impl(
...
extern "C" int cu_body_force_proteus_quad4_residual_a_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        ...
        sfem::codegen::body_force_proteus_quad4_residual_a_msoa_impl<double, geom_t><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_det0, density, g0, g1, out_stride, (double *)u0_out, (double *)u1_out);
        return sfem::codegen::launch_status("body_force_proteus_quad4_residual_a_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::body_force_proteus_quad4_residual_a_msoa_impl<float, geom_t><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_det0, density, g0, g1, out_stride, (float *)u0_out, (float *)u1_out);
        return sfem::codegen::launch_status("body_force_proteus_quad4_residual_a_msoa_impl");
    }
    default:
      break;
...
__global__ void body_force_proteus_quad4_residual_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
...
extern "C" int cu_body_force_proteus_quad4_residual_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::body_force_proteus_quad4_residual_i_msoa_impl<double><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, density, g0, g1, out_stride, (double *)u0_out, (double *)u1_out);
        return sfem::codegen::launch_status("body_force_proteus_quad4_residual_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::body_force_proteus_quad4_residual_i_msoa_impl<float><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, density, g0, g1, out_stride, (float *)u0_out, (float *)u1_out);
        return sfem::codegen::launch_status("body_force_proteus_quad4_residual_i_msoa_impl");
    }
    default:
      break;
...
extern "C" int cu_body_force_proteus_quad4_residual_i_maos(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return cu_body_force_proteus_quad4_residual_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const double *)parameters)[0], ((const double *)parameters)[1], ((const double *)parameters)[2], 2, (double *)output + 0, (double *)output + 1, stream);
    }
    case (int)sizeof(float): {
        return cu_body_force_proteus_quad4_residual_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const float *)parameters)[0], ((const float *)parameters)[1], ((const float *)parameters)[2], 2, (float *)output + 0, (float *)output + 1, stream);
    }
    default:
      break;
  }
