#ifndef BODY_FORCE_D3_TENSOR_PRODUCT_LOCAL_HPP
#define BODY_FORCE_D3_TENSOR_PRODUCT_LOCAL_HPP
...
static SFEM_INLINE void body_force_d3_tensor_product_residual_block(
    const int ne,
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    for (int lane = 0; lane < ne; ++lane) {
      ...
      value_coeff_q2[lane] = qw * det * value_coeff2;
    }
  }
...
template <typename s_t, int NQ, int NS, int VS>
static SFEM_INLINE void body_force_d3_tensor_product_residual_block_contiguous(
    const int ne,
    const ptrdiff_t geometry_stride,
    const s_t *const RSTR determinant,
    const s_t *const RSTR shape_1d,
