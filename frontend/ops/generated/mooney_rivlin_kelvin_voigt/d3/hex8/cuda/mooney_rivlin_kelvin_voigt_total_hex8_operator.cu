...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_hex8_residual_esoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_hex8_residual_esoa", -1, (int)scalar_bytes);
}
...
__global__ void mooney_rivlin_kelvin_voigt_total_hex8_residual_a_msoa_impl(
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_hex8_residual_a_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_hex8_residual_a_msoa", -1, (int)scalar_bytes);
}
...
__global__ void mooney_rivlin_kelvin_voigt_total_hex8_residual_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_hex8_residual_i_maos(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return cu_mooney_rivlin_kelvin_voigt_total_hex8_residual_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const double *)parameters)[0], ((const double *)parameters)[1], ((const double *)parameters)[2], ((const double *)parameters)[3], ((const double *)parameters)[4], 3, (const double *)current + 0, (const double *)current + 1, (const double *)current + 2, 3, (const double *)previous + 0, (const double *)previous + 1, (const double *)previous + 2, 3, (double *)output + 0, (double *)output + 1, (double *)output + 2, stream);
    }
    case (int)sizeof(float): {
        return cu_mooney_rivlin_kelvin_voigt_total_hex8_residual_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const float *)parameters)[0], ((const float *)parameters)[1], ((const float *)parameters)[2], ((const float *)parameters)[3], ((const float *)parameters)[4], 3, (const float *)current + 0, (const float *)current + 1, (const float *)current + 2, 3, (const float *)previous + 0, (const float *)previous + 1, (const float *)previous + 2, 3, (float *)output + 0, (float *)output + 1, (float *)output + 2, stream);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_hex8_residual_i_maos", -1, (int)scalar_bytes);
}

extern "C" int cu_mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_esoa(
    const int scalar_bytes,
    const int ne,
    const ptrdiff_t geometry_stride,
    const void *const RSTR determinant,
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_esoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t>
__global__ void mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_a_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const RSTR g_adj0,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_a_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
__global__ void mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const geom_t *const *const RSTR points,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_i_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_i_msoa", -1, (int)scalar_bytes);
}
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_i_maos(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return cu_mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const double *)parameters)[0], ((const double *)parameters)[1], ((const double *)parameters)[2], ((const double *)parameters)[3], ((const double *)parameters)[4], 3, (const double *)current + 0, (const double *)current + 1, (const double *)current + 2, 3, (const double *)previous + 0, (const double *)previous + 1, (const double *)previous + 2, 3, (const double *)direction + 0, (const double *)direction + 1, (const double *)direction + 2, 3, (double *)output + 0, (double *)output + 1, (double *)output + 2, stream);
    }
    case (int)sizeof(float): {
        return cu_mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const float *)parameters)[0], ((const float *)parameters)[1], ((const float *)parameters)[2], ((const float *)parameters)[3], ((const float *)parameters)[4], 3, (const float *)current + 0, (const float *)current + 1, (const float *)current + 2, 3, (const float *)previous + 0, (const float *)previous + 1, (const float *)previous + 2, 3, (const float *)direction + 0, (const float *)direction + 1, (const float *)direction + 2, 3, (float *)output + 0, (float *)output + 1, (float *)output + 2, stream);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_hex8_jacobian_action_i_maos", -1, (int)scalar_bytes);
}
