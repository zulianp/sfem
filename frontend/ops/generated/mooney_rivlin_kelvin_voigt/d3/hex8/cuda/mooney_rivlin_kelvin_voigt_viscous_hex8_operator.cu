...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_hex8_residual_esoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_viscous_hex8_residual_esoa", -1, (int)scalar_bytes);
}
...
__global__ void mooney_rivlin_kelvin_voigt_viscous_hex8_residual_a_msoa_impl(
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_hex8_residual_a_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_viscous_hex8_residual_a_msoa", -1, (int)scalar_bytes);
}
...
__global__ void mooney_rivlin_kelvin_voigt_viscous_hex8_residual_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_hex8_residual_i_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_viscous_hex8_residual_i_msoa", -1, (int)scalar_bytes);
}
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_hex8_residual_i_maos(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_viscous_hex8_residual_i_maos", -1, (int)scalar_bytes);
}

extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_hex8_jacobian_action_esoa(
    const int scalar_bytes,
    const int ne,
    const ptrdiff_t geometry_stride,
    const void *const RSTR determinant,
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_viscous_hex8_jacobian_action_esoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t>
__global__ void mooney_rivlin_kelvin_voigt_viscous_hex8_jacobian_action_a_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const RSTR g_adj0,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_hex8_jacobian_action_a_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_viscous_hex8_jacobian_action_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
__global__ void mooney_rivlin_kelvin_voigt_viscous_hex8_jacobian_action_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const geom_t *const *const RSTR points,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_hex8_jacobian_action_i_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_viscous_hex8_jacobian_action_i_msoa", -1, (int)scalar_bytes);
}
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_hex8_jacobian_action_i_maos(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_viscous_hex8_jacobian_action_i_maos", -1, (int)scalar_bytes);
}

namespace sfem {
namespace codegen {

__host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_crs_i_msoa_find_cols(
...
template <typename s_t>
__host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_crs_i_msoa_scatter_crs(
  ...
  for (int i = 0; i < NS; ++i) {
    ...
    mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_crs_i_msoa_find_cols(ev, cols, lenrow, ks);
    for (int j = 0; j < NS; ++j) {
...
template <typename s_t>
__global__ void mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_crs_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const geom_t *const *const RSTR points,
  ...
  for (ptrdiff_t element = (ptrdiff_t)blockIdx.x * blockDim.x + threadIdx.x; element < nelements; element += (ptrdiff_t)blockDim.x * gridDim.x) {
    ...
    mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_crs_i_msoa_scatter_crs(ev, element_matrix, rowptr, colidx, values);
  }
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_bsr_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
      const int block_size = 256;
      const int grid_size = (int)((nelements + block_size - 1) / block_size);
      sfem::codegen::mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_crs_i_msoa_impl<double><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, eta_b, eta_s, u_dt_shift, current_stride, (const double *)u0, (const double *)u1, (const double *)u2, previous_stride, (const double *)u0_old, (const double *)u1_old, (const double *)u2_old, rowptr, colidx, (double *)values);
      return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_crs_i_msoa_impl");
    }
    case (int)sizeof(float): {
      const int block_size = 256;
      const int grid_size = (int)((nelements + block_size - 1) / block_size);
      sfem::codegen::mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_crs_i_msoa_impl<float><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, eta_b, eta_s, u_dt_shift, current_stride, (const float *)u0, (const float *)u1, (const float *)u2, previous_stride, (const float *)u0_old, (const float *)u1_old, (const float *)u2_old, rowptr, colidx, (float *)values);
      return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_crs_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_viscous_hex8_hessian_bsr_i_msoa", -1, (int)scalar_bytes);
}
