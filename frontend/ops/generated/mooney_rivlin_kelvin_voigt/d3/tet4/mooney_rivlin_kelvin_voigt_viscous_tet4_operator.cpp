...
extern "C" int mooney_rivlin_kelvin_voigt_viscous_tet4_residual_esoa(
...
static SFEM_INLINE int mooney_rivlin_kelvin_voigt_viscous_tet4_residual_a_msoa_impl(
...
extern "C" int mooney_rivlin_kelvin_voigt_viscous_tet4_jacobian_action_esoa(
    const int scalar_bytes,
    const int ne,
    const ptrdiff_t geometry_stride,
    const void *const RSTR determinant,
...
template <typename s_t, typename g_t>
static SFEM_INLINE int mooney_rivlin_kelvin_voigt_viscous_tet4_jacobian_action_a_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const RSTR g_adj0,
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_viscous_tet4_hessian_crs_i_msoa_find_cols(
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_viscous_tet4_hessian_crs_i_msoa_scatter_crs(
...
template <typename s_t>
static SFEM_INLINE int mooney_rivlin_kelvin_voigt_viscous_tet4_hessian_crs_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
...
extern "C" int mooney_rivlin_kelvin_voigt_viscous_tet4_hessian_bsr_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
      return sfem::codegen::mooney_rivlin_kelvin_voigt_viscous_tet4_hessian_crs_i_msoa_impl<double>(nelements, nnodes, elements, points, eta_b, eta_s, u_dt_shift, current_stride, (const double *)u0, (const double *)u1, (const double *)u2, previous_stride, (const double *)u0_old, (const double *)u1_old, (const double *)u2_old, rowptr, colidx, (double *)values);
    }
    case (int)sizeof(float): {
      return sfem::codegen::mooney_rivlin_kelvin_voigt_viscous_tet4_hessian_crs_i_msoa_impl<float>(nelements, nnodes, elements, points, eta_b, eta_s, u_dt_shift, current_stride, (const float *)u0, (const float *)u1, (const float *)u2, previous_stride, (const float *)u0_old, (const float *)u1_old, (const float *)u2_old, rowptr, colidx, (float *)values);
    }
    default:
      break;
  }
