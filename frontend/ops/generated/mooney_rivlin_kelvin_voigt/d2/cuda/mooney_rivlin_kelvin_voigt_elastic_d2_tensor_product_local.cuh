#ifndef MOONEY_RIVLIN_KELVIN_VOIGT_ELASTIC_D2_TENSOR_PRODUCT_LOCAL_CUH
#define MOONEY_RIVLIN_KELVIN_VOIGT_ELASTIC_D2_TENSOR_PRODUCT_LOCAL_CUH
#include <math.h>
#include <stddef.h>
...
static __host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_elastic_d2_tensor_product_objective_block(
        const int ne,
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    const s_t weak_obj_tmp0 = gu[0]*gu[3];
    const s_t weak_obj_tmp1 = gu[1]*gu[2];
    const s_t weak_obj_tmp2 = s_t(2)*weak_obj_tmp0;
    const s_t weak_obj_tmp3 = pow_2(gu[0]);
    const s_t weak_obj_tmp4 = pow_2(gu[1]);
    const s_t weak_obj_tmp5 = pow_2(gu[2]);
    const s_t weak_obj_tmp6 = pow_2(gu[3]);
    const s_t weak_obj_tmp7 = s_t(2)*gu[0];
    const s_t weak_obj_tmp8 = s_t(2)*gu[3];
    value[step * value_stride + 0] += qw * det_value0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(gu[0] + gu[3] + weak_obj_tmp0 - weak_obj_tmp1) + mu*(-weak_obj_tmp1*weak_obj_tmp2 - weak_obj_tmp1*weak_obj_tmp7 - weak_obj_tmp1*weak_obj_tmp8 + s_t(4)*weak_obj_tmp1 - weak_obj_tmp2 + weak_obj_tmp3*weak_obj_tmp6 + weak_obj_tmp3*weak_obj_tmp8 + s_t(3)*weak_obj_tmp3 + weak_obj_tmp4*weak_obj_tmp5 + s_t(2)*weak_obj_tmp4 + s_t(2)*weak_obj_tmp5 + weak_obj_tmp6*weak_obj_tmp7 + s_t(3)*weak_obj_tmp6));
      }
    }
  }
}
...
template <typename s_t, int NQ, int NS, int VS>
static __host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_elastic_d2_tensor_product_gradient_block(
        const int ne,
        const ptrdiff_t geometry_stride,
        const s_t *const RSTR adj0,
        const s_t *const RSTR adj1,
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    const s_t weak_mat_tmp3 = lmbda*(gu[0] + gu[3] + weak_mat_tmp1 - weak_mat_tmp2);
    const s_t weak_mat_tmp4 = s_t(2)*gu[3];
    const s_t weak_mat_tmp5 = s_t(2)*pow_2(gu[3]);
    const s_t weak_mat_tmp6 = s_t(4)*weak_mat_tmp1 - s_t(2)*weak_mat_tmp2;
    const s_t weak_mat_tmp7 = pow_2(gu[1]) + pow_2(weak_mat_tmp0);
    const s_t weak_mat_tmp8 = s_t(2)*gu[1];
    const s_t weak_mat_tmp9 = gu[0] + s_t(1);
    const s_t weak_mat_tmp10 = s_t(2)*gu[1]*weak_mat_tmp9 + s_t(2)*gu[2]*weak_mat_tmp0;
    const s_t weak_mat_tmp11 = pow_2(gu[2]) + pow_2(weak_mat_tmp9);
...
template <typename s_t, int NQ, int NS, int VS>
static __host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_elastic_d2_tensor_product_apply_block(
        const int ne,
        const ptrdiff_t geometry_stride,
        const s_t *const RSTR adj0,
        const s_t *const RSTR adj1,
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    const s_t weak_mat_tmp4 = -gu[2]*weak_mat_tmp1 - gu[2]*weak_mat_tmp3;
    const s_t weak_mat_tmp5 = -gu[1]*weak_mat_tmp1 - gu[1]*weak_mat_tmp3;
    const s_t weak_mat_tmp6 = pow_2(weak_mat_tmp0);
    const s_t weak_mat_tmp7 = gu[0] + s_t(1);
    const s_t weak_mat_tmp8 = gu[0]*gu[3];
    const s_t weak_mat_tmp9 = gu[1]*gu[2];
    const s_t weak_mat_tmp10 = lmbda*(gu[0] + gu[3] + weak_mat_tmp8 - weak_mat_tmp9);
    const s_t weak_mat_tmp11 = mu*(s_t(4)*gu[0] + s_t(4)*gu[3] + s_t(4)*weak_mat_tmp8 - s_t(2)*weak_mat_tmp9 + s_t(-2)) + weak_mat_tmp1*weak_mat_tmp7 + weak_mat_tmp10;
    const s_t weak_mat_tmp12 = pow_2(gu[2]);
    const s_t weak_mat_tmp13 = lmbda*weak_mat_tmp7;
    const s_t weak_mat_tmp14 = weak_mat_tmp2*weak_mat_tmp7;
    const s_t weak_mat_tmp15 = -gu[2]*weak_mat_tmp13 - gu[2]*weak_mat_tmp14;
    const s_t weak_mat_tmp16 = lmbda*weak_mat_tmp9 + mu*(-s_t(2)*gu[0] + s_t(4)*gu[1]*gu[2] - s_t(2)*gu[3] - s_t(2)*weak_mat_tmp8 + s_t(4)) - weak_mat_tmp10;
