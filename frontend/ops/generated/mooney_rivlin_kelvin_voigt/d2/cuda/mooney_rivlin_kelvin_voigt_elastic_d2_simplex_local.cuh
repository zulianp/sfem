#ifndef MOONEY_RIVLIN_KELVIN_VOIGT_ELASTIC_D2_SIMPLEX_LOCAL_CUH
#define MOONEY_RIVLIN_KELVIN_VOIGT_ELASTIC_D2_SIMPLEX_LOCAL_CUH
#include <math.h>
#include <stddef.h>
...
static __host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_elastic_d2_simplex_objective_block(
        const int ne,
  ...
  static_assert(VS > 0, "VS must be positive");
    ...
    const s_t weak_obj_tmp0 = gu0*gu3;
    const s_t weak_obj_tmp1 = gu1*gu2;
    const s_t weak_obj_tmp2 = s_t(2)*weak_obj_tmp0;
    const s_t weak_obj_tmp3 = pow_2(gu0);
    const s_t weak_obj_tmp4 = pow_2(gu1);
    const s_t weak_obj_tmp5 = pow_2(gu2);
    const s_t weak_obj_tmp6 = pow_2(gu3);
    const s_t weak_obj_tmp7 = s_t(2)*gu0;
    const s_t weak_obj_tmp8 = s_t(2)*gu3;
    value[step * value_stride + 0] += qw * det_value0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(gu0 + gu3 + weak_obj_tmp0 - weak_obj_tmp1) + mu*(-weak_obj_tmp1*weak_obj_tmp2 - weak_obj_tmp1*weak_obj_tmp7 - weak_obj_tmp1*weak_obj_tmp8 + s_t(4)*weak_obj_tmp1 - weak_obj_tmp2 + weak_obj_tmp3*weak_obj_tmp6 + weak_obj_tmp3*weak_obj_tmp8 + s_t(3)*weak_obj_tmp3 + weak_obj_tmp4*weak_obj_tmp5 + s_t(2)*weak_obj_tmp4 + s_t(2)*weak_obj_tmp5 + weak_obj_tmp6*weak_obj_tmp7 + s_t(3)*weak_obj_tmp6));
        }
      }
    }
}
...
template <typename s_t, int NQ, int NS, int VS>
static __host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_elastic_d2_simplex_tri3_objective_block(
        const int ne,
        const ptrdiff_t geometry_stride,
        const s_t *const RSTR adj0,
        const s_t *const RSTR adj1,
...
template <typename s_t, int NQ, int NS, int VS>
static __host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_elastic_d2_simplex_gradient_block(
        const int ne,
        const ptrdiff_t geometry_stride,
        const s_t *const RSTR adj0,
        const s_t *const RSTR adj1,
  ...
  static_assert(VS > 0, "VS must be positive");
    ...
    const s_t weak_mat_tmp1 = gu0*gu3;
    const s_t weak_mat_tmp2 = gu1*gu2;
    const s_t weak_mat_tmp3 = lmbda*(gu0 + gu3 + weak_mat_tmp1 - weak_mat_tmp2);
    const s_t weak_mat_tmp4 = s_t(2)*gu3;
    const s_t weak_mat_tmp5 = s_t(2)*pow_2(gu3);
    const s_t weak_mat_tmp6 = s_t(4)*weak_mat_tmp1 - s_t(2)*weak_mat_tmp2;
    const s_t weak_mat_tmp7 = pow_2(gu1) + pow_2(weak_mat_tmp0);
    const s_t weak_mat_tmp8 = s_t(2)*gu1;
    const s_t weak_mat_tmp9 = gu0 + s_t(1);
    const s_t weak_mat_tmp10 = s_t(2)*gu1*weak_mat_tmp9 + s_t(2)*gu2*weak_mat_tmp0;
    const s_t weak_mat_tmp11 = pow_2(gu2) + pow_2(weak_mat_tmp9);
    const s_t weak_mat_tmp12 = weak_mat_tmp11 + weak_mat_tmp7;
    const s_t weak_mat_tmp13 = s_t(2)*gu2;
    const s_t weak_mat_tmp14 = s_t(2)*gu0;
    const s_t weak_mat_tmp15 = pow_2(gu0);
    const s_t material0 = mu*(gu0*weak_mat_tmp5 + s_t(6)*gu0 - weak_mat_tmp2*weak_mat_tmp4 - weak_mat_tmp4 + weak_mat_tmp5 + weak_mat_tmp6) + weak_mat_tmp0*weak_mat_tmp3;
    const s_t material1 = -gu2*weak_mat_tmp3 + mu*(s_t(4)*gu1 + s_t(6)*gu2 - weak_mat_tmp10*weak_mat_tmp9 + weak_mat_tmp12*weak_mat_tmp8 - weak_mat_tmp7*weak_mat_tmp8);
    const s_t material2 = -gu1*weak_mat_tmp3 + mu*(s_t(6)*gu1 + s_t(4)*gu2 - weak_mat_tmp0*weak_mat_tmp10 - weak_mat_tmp11*weak_mat_tmp13 + weak_mat_tmp12*weak_mat_tmp13);
    const s_t material3 = mu*(s_t(6)*gu3 - weak_mat_tmp14*weak_mat_tmp2 - weak_mat_tmp14 + weak_mat_tmp15*weak_mat_tmp4 + s_t(2)*weak_mat_tmp15 + weak_mat_tmp6) + weak_mat_tmp3*weak_mat_tmp9;
...
template <typename s_t, int NQ, int NS, int VS>
static __host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_elastic_d2_simplex_tri3_gradient_block(
        const int ne,
        const ptrdiff_t geometry_stride,
        const s_t *const RSTR adj0,
        const s_t *const RSTR adj1,
...
template <typename s_t, int NQ, int NS, int VS>
static __host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_elastic_d2_simplex_apply_block(
        const int ne,
        const ptrdiff_t geometry_stride,
        const s_t *const RSTR adj0,
        const s_t *const RSTR adj1,
  ...
  static_assert(VS > 0, "VS must be positive");
    ...
    const s_t weak_mat_tmp8 = gu0*gu3;
    const s_t weak_mat_tmp9 = gu1*gu2;
    const s_t weak_mat_tmp10 = lmbda*(gu0 + gu3 + weak_mat_tmp8 - weak_mat_tmp9);
    const s_t weak_mat_tmp11 = mu*(s_t(4)*gu0 + s_t(4)*gu3 + s_t(4)*weak_mat_tmp8 - s_t(2)*weak_mat_tmp9 + s_t(-2)) + weak_mat_tmp1*weak_mat_tmp7 + weak_mat_tmp10;
    ...
    const s_t weak_mat_tmp16 = lmbda*weak_mat_tmp9 + mu*(-s_t(2)*gu0 + s_t(4)*gu1*gu2 - s_t(2)*gu3 - s_t(2)*weak_mat_tmp8 + s_t(4)) - weak_mat_tmp10;
...
template <typename s_t, int NQ, int NS, int VS>
static __host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_elastic_d2_simplex_tri3_apply_block(
        const int ne,
        const ptrdiff_t geometry_stride,
        const s_t *const RSTR adj0,
        const s_t *const RSTR adj1,
