...
#include <cstdlib>
#include <string.h>
#include "../../cuda/mooney_rivlin_kelvin_voigt_viscous_d2_tensor_product_local.cuh"
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_residual_esoa(
...
__global__ void mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_residual_a_msoa_impl(
...
__global__ void mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_residual_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_residual_i_maos(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return cu_mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_residual_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const double *)parameters)[0], ((const double *)parameters)[1], ((const double *)parameters)[2], 2, (const double *)current + 0, (const double *)current + 1, 2, (const double *)previous + 0, (const double *)previous + 1, 2, (double *)output + 0, (double *)output + 1, stream);
    }
    case (int)sizeof(float): {
        return cu_mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_residual_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const float *)parameters)[0], ((const float *)parameters)[1], ((const float *)parameters)[2], 2, (const float *)current + 0, (const float *)current + 1, 2, (const float *)previous + 0, (const float *)previous + 1, 2, (float *)output + 0, (float *)output + 1, stream);
    }
    default:
      break;
  }
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_jacobian_action_esoa(
    const int scalar_bytes,
    const int ne,
    const ptrdiff_t geometry_stride,
    const void *const RSTR determinant,
...
template <typename s_t, typename g_t>
__global__ void mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_jacobian_action_a_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const RSTR g_adj0,
...
template <typename s_t>
__global__ void mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_jacobian_action_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const geom_t *const *const RSTR points,
...
__host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_hessian_crs_i_msoa_find_cols(
...
template <typename s_t>
__host__ __device__ __forceinline__ void mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_hessian_crs_i_msoa_scatter_crs(
...
template <typename s_t>
__global__ void mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_hessian_crs_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const geom_t *const *const RSTR points,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_hessian_bsr_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
      const int block_size = 256;
      const int grid_size = (int)((nelements + block_size - 1) / block_size);
      sfem::codegen::mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_hessian_crs_i_msoa_impl<double><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, eta_b, eta_s, u_dt_shift, current_stride, (const double *)u0, (const double *)u1, previous_stride, (const double *)u0_old, (const double *)u1_old, rowptr, colidx, (double *)values);
      return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_hessian_crs_i_msoa_impl");
    }
    case (int)sizeof(float): {
      const int block_size = 256;
      const int grid_size = (int)((nelements + block_size - 1) / block_size);
      sfem::codegen::mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_hessian_crs_i_msoa_impl<float><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, eta_b, eta_s, u_dt_shift, current_stride, (const float *)u0, (const float *)u1, previous_stride, (const float *)u0_old, (const float *)u1_old, rowptr, colidx, (float *)values);
      return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_viscous_proteus_quad4_hessian_crs_i_msoa_impl");
    }
    default:
      break;
