...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_esoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_esoa", -1, (int)scalar_bytes);
}
...
__global__ void mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_a_msoa_impl(
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_a_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        ...
        sfem::codegen::mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_a_msoa_impl<double, geom_t><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, eta_b, eta_s, lmbda, mu, u_dt_shift, current_stride, (const double *)u0, (const double *)u1, previous_stride, (const double *)u0_old, (const double *)u1_old, out_stride, (double *)u0_out, (double *)u1_out);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_a_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_a_msoa_impl<float, geom_t><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, eta_b, eta_s, lmbda, mu, u_dt_shift, current_stride, (const float *)u0, (const float *)u1, previous_stride, (const float *)u0_old, (const float *)u1_old, out_stride, (float *)u0_out, (float *)u1_out);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_a_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_a_msoa", -1, (int)scalar_bytes);
}
...
__global__ void mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_i_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_i_msoa", -1, (int)scalar_bytes);
}
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_i_maos(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_proteus_quad4_residual_i_maos", -1, (int)scalar_bytes);
}
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_esoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_esoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t>
__global__ void mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_a_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const RSTR g_adj0,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_a_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
__global__ void mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const geom_t *const *const RSTR points,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_i_msoa_impl<double><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, eta_b, eta_s, lmbda, mu, u_dt_shift, current_stride, (const double *)u0, (const double *)u1, previous_stride, (const double *)u0_old, (const double *)u1_old, direction_stride, (const double *)u0_direction, (const double *)u1_direction, out_stride, (double *)u0_out, (double *)u1_out);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_i_msoa_impl<float><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, eta_b, eta_s, lmbda, mu, u_dt_shift, current_stride, (const float *)u0, (const float *)u1, previous_stride, (const float *)u0_old, (const float *)u1_old, direction_stride, (const float *)u0_direction, (const float *)u1_direction, out_stride, (float *)u0_out, (float *)u1_out);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_i_msoa", -1, (int)scalar_bytes);
}
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_i_maos(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_proteus_quad4_jacobian_action_i_maos", -1, (int)scalar_bytes);
}
