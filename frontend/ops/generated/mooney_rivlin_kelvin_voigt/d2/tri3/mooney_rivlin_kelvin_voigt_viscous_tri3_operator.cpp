...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
    ...
    mooney_rivlin_kelvin_voigt_viscous_d2_simplex_residual_block_contiguous<s_t, NQ, NS, VS>(ne, 0, bageom_streams[4], badjugate, affine_grad_ref_x, affine_grad_ref_y, affine_q_weight, bcurrent, bprevious, eta_b, eta_s, u_dt_shift, boutput);
...
extern "C" int mooney_rivlin_kelvin_voigt_viscous_tri3_jacobian_action_esoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        sfem::codegen::mooney_rivlin_kelvin_voigt_viscous_d2_simplex_jacobian_action_block<double, 1, 3, 16>(ne, geometry_stride, (const double *)determinant, (const double *const *)adjugate, sfem::codegen::ref_tri3_q1<double>::grad_ref_x(), sfem::codegen::ref_tri3_q1<double>::grad_ref_y(), sfem::codegen::quad_tri_q1<double>::q_weight(), (const double *const *)current, (const double *const *)previous, (const double *const *)direction, eta_b, eta_s, u_dt_shift, (double *const *)output);
        return SFEM_SUCCESS;
    }
    case (int)sizeof(float): {
        sfem::codegen::mooney_rivlin_kelvin_voigt_viscous_d2_simplex_jacobian_action_block<float, 1, 3, 16>(ne, geometry_stride, (const float *)determinant, (const float *const *)adjugate, sfem::codegen::ref_tri3_q1<float>::grad_ref_x(), sfem::codegen::ref_tri3_q1<float>::grad_ref_y(), sfem::codegen::quad_tri_q1<float>::q_weight(), (const float *const *)current, (const float *const *)previous, (const float *const *)direction, eta_b, eta_s, u_dt_shift, (float *const *)output);
        return SFEM_SUCCESS;
    }
    default:
      break;
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
    ...
    mooney_rivlin_kelvin_voigt_viscous_d2_simplex_jacobian_action_block_contiguous<s_t, NQ, NS, VS>(ne, 0, bageom_streams[4], badjugate, affine_grad_ref_x, affine_grad_ref_y, affine_q_weight, bcurrent, bprevious, bdirection, eta_b, eta_s, u_dt_shift, boutput);
