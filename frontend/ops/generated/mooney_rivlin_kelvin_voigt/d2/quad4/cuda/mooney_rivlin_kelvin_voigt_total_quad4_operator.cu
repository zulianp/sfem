...
#include <cstdlib>
#include <string.h>
#include "../../cuda/mooney_rivlin_kelvin_voigt_total_d2_tensor_product_local.cuh"
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_quad4_residual_esoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_quad4_residual_esoa", -1, (int)scalar_bytes);
}
...
__global__ void mooney_rivlin_kelvin_voigt_total_quad4_residual_a_msoa_impl(
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_quad4_residual_a_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_quad4_residual_a_msoa", -1, (int)scalar_bytes);
}
...
__global__ void mooney_rivlin_kelvin_voigt_total_quad4_residual_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_quad4_residual_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_total_quad4_residual_i_msoa_impl<double><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, eta_b, eta_s, lmbda, mu, u_dt_shift, current_stride, (const double *)u0, (const double *)u1, previous_stride, (const double *)u0_old, (const double *)u1_old, out_stride, (double *)u0_out, (double *)u1_out);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_total_quad4_residual_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_total_quad4_residual_i_msoa_impl<float><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, eta_b, eta_s, lmbda, mu, u_dt_shift, current_stride, (const float *)u0, (const float *)u1, previous_stride, (const float *)u0_old, (const float *)u1_old, out_stride, (float *)u0_out, (float *)u1_out);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_total_quad4_residual_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_quad4_residual_i_msoa", -1, (int)scalar_bytes);
}
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_quad4_residual_i_maos(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return cu_mooney_rivlin_kelvin_voigt_total_quad4_residual_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const double *)parameters)[0], ((const double *)parameters)[1], ((const double *)parameters)[2], ((const double *)parameters)[3], ((const double *)parameters)[4], 2, (const double *)current + 0, (const double *)current + 1, 2, (const double *)previous + 0, (const double *)previous + 1, 2, (double *)output + 0, (double *)output + 1, stream);
    }
    case (int)sizeof(float): {
        return cu_mooney_rivlin_kelvin_voigt_total_quad4_residual_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const float *)parameters)[0], ((const float *)parameters)[1], ((const float *)parameters)[2], ((const float *)parameters)[3], ((const float *)parameters)[4], 2, (const float *)current + 0, (const float *)current + 1, 2, (const float *)previous + 0, (const float *)previous + 1, 2, (float *)output + 0, (float *)output + 1, stream);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_quad4_residual_i_maos", -1, (int)scalar_bytes);
}

extern "C" int cu_mooney_rivlin_kelvin_voigt_total_quad4_jacobian_action_esoa(
    const int scalar_bytes,
    const int ne,
    const ptrdiff_t geometry_stride,
    const void *const RSTR determinant,
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_quad4_jacobian_action_esoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t>
__global__ void mooney_rivlin_kelvin_voigt_total_quad4_jacobian_action_a_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const RSTR g_adj0,
...
template <typename s_t>
__global__ void mooney_rivlin_kelvin_voigt_total_quad4_jacobian_action_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const geom_t *const *const RSTR points,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_quad4_jacobian_action_i_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_quad4_jacobian_action_i_msoa", -1, (int)scalar_bytes);
}
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_total_quad4_jacobian_action_i_maos(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return cu_mooney_rivlin_kelvin_voigt_total_quad4_jacobian_action_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const double *)parameters)[0], ((const double *)parameters)[1], ((const double *)parameters)[2], ((const double *)parameters)[3], ((const double *)parameters)[4], 2, (const double *)current + 0, (const double *)current + 1, 2, (const double *)previous + 0, (const double *)previous + 1, 2, (const double *)direction + 0, (const double *)direction + 1, 2, (double *)output + 0, (double *)output + 1, stream);
    }
    case (int)sizeof(float): {
        return cu_mooney_rivlin_kelvin_voigt_total_quad4_jacobian_action_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const float *)parameters)[0], ((const float *)parameters)[1], ((const float *)parameters)[2], ((const float *)parameters)[3], ((const float *)parameters)[4], 2, (const float *)current + 0, (const float *)current + 1, 2, (const float *)previous + 0, (const float *)previous + 1, 2, (const float *)direction + 0, (const float *)direction + 1, 2, (float *)output + 0, (float *)output + 1, stream);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_total_quad4_jacobian_action_i_maos", -1, (int)scalar_bytes);
}
