#ifndef PORO_HYPERELASTICITY_PORO_FORM_1_P_D3_TENSOR_PRODUCT_MIXED_LOCAL_HPP
#define PORO_HYPERELASTICITY_PORO_FORM_1_P_D3_TENSOR_PRODUCT_MIXED_LOCAL_HPP
...
static SFEM_INLINE void poro_hyperelasticity_poro_form_1_p_d3_tensor_product_mixed_residual_block(
        const int nelems,
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            u0_grad_coeff_ref[(q * DIM + 0) * VECTOR_SIZE + lane] = scalar_t(0);
            u0_grad_coeff_ref[(q * DIM + 1) * VECTOR_SIZE + lane] = scalar_t(0);
            u0_grad_coeff_ref[(q * DIM + 2) * VECTOR_SIZE + lane] = scalar_t(0);
            u1_value_coeff[q * VECTOR_SIZE + lane] = scalar_t(0);
            u1_grad_coeff_ref[(q * DIM + 0) * VECTOR_SIZE + lane] = scalar_t(0);
            u1_grad_coeff_ref[(q * DIM + 1) * VECTOR_SIZE + lane] = scalar_t(0);
            u1_grad_coeff_ref[(q * DIM + 2) * VECTOR_SIZE + lane] = scalar_t(0);
            u2_value_coeff[q * VECTOR_SIZE + lane] = scalar_t(0);
            u2_grad_coeff_ref[(q * DIM + 0) * VECTOR_SIZE + lane] = scalar_t(0);
            u2_grad_coeff_ref[(q * DIM + 1) * VECTOR_SIZE + lane] = scalar_t(0);
            u2_grad_coeff_ref[(q * DIM + 2) * VECTOR_SIZE + lane] = scalar_t(0);
...
template <typename scalar_t, int N_QP, int CELL_N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void poro_hyperelasticity_poro_form_1_p_d3_tensor_product_mixed_residual_block_contiguous(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT determinant,
        const scalar_t *const SFEM_RESTRICT adjugate[9],
...
template <typename scalar_t, int N_QP, int CELL_N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void poro_hyperelasticity_poro_form_1_p_d3_tensor_product_mixed_jacobian_action_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT determinant,
        const scalar_t *const SFEM_RESTRICT field_shape_1d[2],
...
template <typename scalar_t, int N_QP, int CELL_N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void poro_hyperelasticity_poro_form_1_p_d3_tensor_product_mixed_jacobian_action_block_contiguous(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT determinant,
        const scalar_t *const SFEM_RESTRICT field_shape_1d[2],
