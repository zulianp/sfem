...
static const KernelDiagnostics modified_mooney_rivlin_hex8_objective_soa_diagnostics_data = {
  "modified_mooney_rivlin_hex8_objective_soa",
  "HEX8",
  3,
  8,
  8,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_modified_mooney_rivlin_hex8_objective_soa_diagnostics(void) {
  return &sfem::codegen::modified_mooney_rivlin_hex8_objective_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
__global__ void modified_mooney_rivlin_hex8_objective_steps_a_msoa_impl(
...
extern "C" int cu_modified_mooney_rivlin_hex8_objective_steps_a_msoa(
...
template <typename s_t, typename g_t, int VS>
__global__ void modified_mooney_rivlin_hex8_objective_steps_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
...
extern "C" int cu_modified_mooney_rivlin_hex8_objective_steps_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::modified_mooney_rivlin_hex8_objective_steps_i_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, nsteps, (const double *)steps, (double *)value);
        return sfem::codegen::launch_status("modified_mooney_rivlin_hex8_objective_steps_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::modified_mooney_rivlin_hex8_objective_steps_i_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, nsteps, (const float *)steps, (float *)value);
        return sfem::codegen::launch_status("modified_mooney_rivlin_hex8_objective_steps_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("modified_mooney_rivlin_hex8_objective_steps_i_msoa", -1, (int)scalar_bytes);
}
...
static const KernelDiagnostics modified_mooney_rivlin_hex8_gradient_soa_diagnostics_data = {
  "modified_mooney_rivlin_hex8_gradient_soa",
  "HEX8",
  3,
  8,
  8,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_modified_mooney_rivlin_hex8_gradient_soa_diagnostics(void) {
  return &sfem::codegen::modified_mooney_rivlin_hex8_gradient_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
__global__ void modified_mooney_rivlin_hex8_gradient_a_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const RSTR g_adj0,
...
extern "C" int cu_modified_mooney_rivlin_hex8_gradient_a_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  return sfem::codegen::unsupported_dispatch("modified_mooney_rivlin_hex8_gradient_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t, int VS>
__global__ void modified_mooney_rivlin_hex8_gradient_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
extern "C" int cu_modified_mooney_rivlin_hex8_gradient_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::modified_mooney_rivlin_hex8_gradient_i_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, out_stride, (double *)outx, (double *)outy, (double *)outz);
        return sfem::codegen::launch_status("modified_mooney_rivlin_hex8_gradient_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::modified_mooney_rivlin_hex8_gradient_i_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, out_stride, (float *)outx, (float *)outy, (float *)outz);
        return sfem::codegen::launch_status("modified_mooney_rivlin_hex8_gradient_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("modified_mooney_rivlin_hex8_gradient_i_msoa", -1, (int)scalar_bytes);
}
...
static const KernelDiagnostics modified_mooney_rivlin_hex8_apply_soa_diagnostics_data = {
  "modified_mooney_rivlin_hex8_apply_soa",
  "HEX8",
  3,
  8,
  8,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_modified_mooney_rivlin_hex8_apply_soa_diagnostics(void) {
  return &sfem::codegen::modified_mooney_rivlin_hex8_apply_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
__global__ void modified_mooney_rivlin_hex8_apply_a_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const RSTR g_adj0,
...
extern "C" int cu_modified_mooney_rivlin_hex8_apply_a_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::modified_mooney_rivlin_hex8_apply_a_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, c1, c2, kappa, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, out_stride, (double *)outx, (double *)outy, (double *)outz);
        return sfem::codegen::launch_status("modified_mooney_rivlin_hex8_apply_a_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::modified_mooney_rivlin_hex8_apply_a_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, c1, c2, kappa, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, out_stride, (float *)outx, (float *)outy, (float *)outz);
        return sfem::codegen::launch_status("modified_mooney_rivlin_hex8_apply_a_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("modified_mooney_rivlin_hex8_apply_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t, int VS>
__global__ void modified_mooney_rivlin_hex8_apply_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
extern "C" int cu_modified_mooney_rivlin_hex8_apply_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::modified_mooney_rivlin_hex8_apply_i_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, out_stride, (double *)outx, (double *)outy, (double *)outz);
        return sfem::codegen::launch_status("modified_mooney_rivlin_hex8_apply_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::modified_mooney_rivlin_hex8_apply_i_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, out_stride, (float *)outx, (float *)outy, (float *)outz);
        return sfem::codegen::launch_status("modified_mooney_rivlin_hex8_apply_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("modified_mooney_rivlin_hex8_apply_i_msoa", -1, (int)scalar_bytes);
}
