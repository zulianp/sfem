...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int modified_mooney_rivlin_tri6_objective_steps_i_msoa_impl(
...
extern "C" int modified_mooney_rivlin_tri6_objective_steps_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::modified_mooney_rivlin_tri6_objective_steps_i_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const double *)ux, (const double *)uy, h_stride, (const double *)hx, (const double *)hy, nsteps, (const double *)steps, (double *)value);
    }
    case (int)sizeof(float): {
        return sfem::codegen::modified_mooney_rivlin_tri6_objective_steps_i_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const float *)ux, (const float *)uy, h_stride, (const float *)hx, (const float *)hy, nsteps, (const float *)steps, (float *)value);
    }
    default:
      break;
  }
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int modified_mooney_rivlin_tri6_gradient_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
extern "C" int modified_mooney_rivlin_tri6_gradient_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::modified_mooney_rivlin_tri6_gradient_i_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const double *)ux, (const double *)uy, out_stride, (double *)outx, (double *)outy);
    }
    case (int)sizeof(float): {
        return sfem::codegen::modified_mooney_rivlin_tri6_gradient_i_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const float *)ux, (const float *)uy, out_stride, (float *)outx, (float *)outy);
    }
    default:
      break;
  }
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int modified_mooney_rivlin_tri6_apply_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
extern "C" int modified_mooney_rivlin_tri6_apply_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::modified_mooney_rivlin_tri6_apply_i_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const double *)ux, (const double *)uy, h_stride, (const double *)hx, (const double *)hy, out_stride, (double *)outx, (double *)outy);
    }
    case (int)sizeof(float): {
        return sfem::codegen::modified_mooney_rivlin_tri6_apply_i_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const float *)ux, (const float *)uy, h_stride, (const float *)hx, (const float *)hy, out_stride, (float *)outx, (float *)outy);
    }
    default:
      break;
  }
...
static SFEM_INLINE void modified_mooney_rivlin_tri6_hessian_i_msoa_find_cols(
...
static SFEM_INLINE void modified_mooney_rivlin_tri6_hessian_i_msoa_scatter_bsr(
  ...
  for (int i = 0; i < NS; ++i) {
    ...
    modified_mooney_rivlin_tri6_hessian_i_msoa_find_cols(ev, cols, lenrow, ks);
    for (int j = 0; j < NS; ++j) {
...
static int modified_mooney_rivlin_tri6_hessian_i_msoa_assemble_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const *const RSTR points,
...
extern "C" int modified_mooney_rivlin_tri6_hessian_bsr_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::modified_mooney_rivlin_tri6_hessian_i_msoa_assemble_impl<double, geom_t, 1>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const double *)ux, (const double *)uy, rowptr, colidx, (double *)values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
    }
    case (int)sizeof(float): {
        return sfem::codegen::modified_mooney_rivlin_tri6_hessian_i_msoa_assemble_impl<float, geom_t, 1>(nelements, nnodes, elements, points, c1, c2, kappa, u_stride, (const float *)ux, (const float *)uy, rowptr, colidx, (float *)values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("modified_mooney_rivlin_tri6_hessian_bsr_i_msoa", -1, (int)scalar_bytes);
}
