...
namespace sfem {
    ...
    double GeneratedModifiedMooneyRivlin::flops_value() const {
        ...
        impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics =
                            modified_mooney_rivlin_objective_2d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total +=
            ...
            if (dim == 3) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics =
                            modified_mooney_rivlin_objective_3d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total +=
                                impl_->objective_uses_affine
                                        ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements)
    ...
    double GeneratedModifiedMooneyRivlin::flops_gradient() const {
        ...
        impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics =
                            modified_mooney_rivlin_gradient_2d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total +=
            ...
            if (dim == 3) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics =
                            modified_mooney_rivlin_gradient_3d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total +=
                                impl_->gradient_uses_affine
                                        ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements)
    ...
    double GeneratedModifiedMooneyRivlin::flops_apply() const {
        ...
        impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics =
                            modified_mooney_rivlin_apply_2d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total +=
            ...
            if (dim == 3) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics =
                            modified_mooney_rivlin_apply_3d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total +=
                                impl_->apply_uses_affine
                                        ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements)
    ...
    int GeneratedModifiedMooneyRivlin::gradient(const real_t *const x, real_t *const out) {
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
                ...
                if (packed_block >= 0) {
                    ...
                    if (dim == 2) {
                        if (impl_->use_packed_two_pass) {
                            return modified_mooney_rivlin_gradient_packed_two_pass_2d_affine_mesh_soa(
                                    domain.element_type,
                                    ...
                                    adjugate[3],
                                    determinant,
                                    domain.parameters->require_real_value("c1"),
                                    domain.parameters->require_real_value("c2"),
                                    domain.parameters->require_real_value("kappa"),
                                    2,
                                    x + 0,
                                    x + 1,
                                    2,
            ...
            if (!impl_->gradient_uses_affine && impl_->space->has_packed_mesh()) {
                ...
                if (packed_block >= 0) {
                    ...
                    if (dim == 2) {
                        ...
                        if (impl_->use_packed_two_pass) {
                            return modified_mooney_rivlin_gradient_packed_two_pass_3d_isoparametric_mesh_soa(
                                    domain.element_type,
                                    packed->n_packs(packed_block),
                                    packed->n_elements_per_pack(packed_block),
                                    domain.block->n_elements(),
            ...
            if (dim == 2) {
                if (impl_->gradient_uses_affine) {
                    return modified_mooney_rivlin_gradient_2d_affine_mesh_soa(domain.element_type,
                                                                              domain.block->n_elements(),
                                                                              mesh->n_nodes(),
                ...
                return modified_mooney_rivlin_gradient_2d_isoparametric_mesh_soa(domain.element_type,
                                                                                 domain.block->n_elements(),
                                                                                 mesh->n_nodes(),
                                                                                 domain.block->elements()->data(),
                                                                                 points,
                ...
                if (impl_->gradient_uses_affine) {
                    return modified_mooney_rivlin_gradient_3d_affine_mesh_soa(domain.element_type,
                                                                              domain.block->n_elements(),
                                                                              mesh->n_nodes(),
                                                                              domain.block->elements()->data(),
                                                                              adjugate[0],
                ...
                return modified_mooney_rivlin_gradient_3d_isoparametric_mesh_soa(domain.element_type,
                                                                                 domain.block->n_elements(),
                                                                                 mesh->n_nodes(),
                                                                                 domain.block->elements()->data(),
                                                                                 points,
    ...
    int GeneratedModifiedMooneyRivlin::apply(const real_t *const x, const real_t *const h, real_t *const out) {
            ...
            if (dim == 2) {
                if (impl_->apply_uses_affine) {
                    if (impl_->space->has_packed_mesh()) {
                        ...
                        if (packed_block >= 0) {
                            ...
                            return modified_mooney_rivlin_apply_packed_2d_affine_mesh_soa(
                                    domain.element_type,
                                    packed->n_packs(packed_block),
                                    packed->n_elements_per_pack(packed_block),
                                    domain.block->n_elements(),
                    ...
                    return modified_mooney_rivlin_apply_2d_affine_mesh_soa(domain.element_type,
                                                                           domain.block->n_elements(),
                                                                           mesh->n_nodes(),
                                                                           domain.block->elements()->data(),
                                                                           adjugate[0],
                ...
                if (impl_->space->has_packed_mesh()) {
                    ...
                    if (packed_block >= 0) {
                        ...
                        return modified_mooney_rivlin_apply_packed_2d_isoparametric_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
                ...
                return modified_mooney_rivlin_apply_2d_isoparametric_mesh_soa(domain.element_type,
                                                                              domain.block->n_elements(),
                                                                              mesh->n_nodes(),
                                                                              domain.block->elements()->data(),
                                                                              points,
                ...
                if (impl_->apply_uses_affine) {
                    if (impl_->space->has_packed_mesh()) {
                        ...
                        if (packed_block >= 0) {
                            ...
                            return modified_mooney_rivlin_apply_packed_3d_affine_mesh_soa(
                                    domain.element_type,
                                    packed->n_packs(packed_block),
                                    packed->n_elements_per_pack(packed_block),
                                    domain.block->n_elements(),
                    ...
                    return modified_mooney_rivlin_apply_3d_affine_mesh_soa(domain.element_type,
                                                                           domain.block->n_elements(),
                                                                           mesh->n_nodes(),
                                                                           domain.block->elements()->data(),
                                                                           adjugate[0],
                ...
                if (impl_->space->has_packed_mesh()) {
                    ...
                    if (packed_block >= 0) {
                        ...
                        auto ghost_reduce_dest = packed->ghost_reduce_dest(packed_block);
                        if (impl_->use_packed_two_pass) {
                            return modified_mooney_rivlin_apply_packed_two_pass_3d_isoparametric_mesh_soa(
                                    domain.element_type,
                                    packed->n_packs(packed_block),
                                    packed->n_elements_per_pack(packed_block),
                                    domain.block->n_elements(),
                        ...
                        return modified_mooney_rivlin_apply_packed_3d_isoparametric_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
                ...
                return modified_mooney_rivlin_apply_3d_isoparametric_mesh_soa(domain.element_type,
                                                                              domain.block->n_elements(),
                                                                              mesh->n_nodes(),
                                                                              domain.block->elements()->data(),
                                                                              points,
    ...
    int GeneratedModifiedMooneyRivlin::value(const real_t *x, real_t *const out) {
        ...
        *out        = 0;
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                if (impl_->objective_uses_affine) {
                    status = modified_mooney_rivlin_objective_2d_affine_mesh_soa(domain.element_type,
                                                                                 nelements,
                                                                                 mesh->n_nodes(),
                                                                                 domain.block->elements()->data(),
                                                                                 adjugate[0],
                    ...
                    status = modified_mooney_rivlin_objective_2d_isoparametric_mesh_soa(
                            domain.element_type,
                            nelements,
                            mesh->n_nodes(),
                            domain.block->elements()->data(),
                ...
                if (impl_->objective_uses_affine) {
                    status = modified_mooney_rivlin_objective_3d_affine_mesh_soa(domain.element_type,
                                                                                 nelements,
                                                                                 mesh->n_nodes(),
                                                                                 domain.block->elements()->data(),
                                                                                 adjugate[0],
                    ...
                    status = modified_mooney_rivlin_objective_3d_isoparametric_mesh_soa(
                            domain.element_type,
                            nelements,
                            mesh->n_nodes(),
                            domain.block->elements()->data(),
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (impl_->objective_uses_affine && impl_->space->has_packed_mesh()) {
                ...
                if (packed_block >= 0) {
                    ...
                    const int dim             = mesh->spatial_dimension();
                    if (dim == 2) {
                        status = modified_mooney_rivlin_objective_steps_packed_2d_affine_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
            ...
            if (!impl_->objective_uses_affine && impl_->space->has_packed_mesh()) {
                ...
                if (packed_block >= 0) {
                    ...
                    const int dim             = mesh->spatial_dimension();
                    if (dim == 2) {
                        status = modified_mooney_rivlin_objective_steps_packed_2d_isoparametric_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
                        ...
                        status = modified_mooney_rivlin_objective_steps_packed_3d_isoparametric_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
            ...
            if (status == SFEM_FAILURE) {
                ...
                if (dim == 2) {
                    if (impl_->objective_uses_affine) {
                        status = modified_mooney_rivlin_objective_steps_2d_affine_mesh_soa(
                                domain.element_type,
                                nelements,
                                mesh->n_nodes(),
                                domain.block->elements()->data(),
                    ...
                    if (impl_->objective_uses_affine) {
                        status = modified_mooney_rivlin_objective_steps_3d_affine_mesh_soa(
                                domain.element_type,
                                nelements,
                                mesh->n_nodes(),
                                domain.block->elements()->data(),
                        ...
                        status = modified_mooney_rivlin_objective_steps_3d_isoparametric_mesh_soa(
                                domain.element_type,
                                nelements,
                                mesh->n_nodes(),
                                domain.block->elements()->data(),
...
#pragma omp simd reduction(+ : sum)
    ...
    int GeneratedModifiedMooneyRivlin::hessian_bsr(const real_t *const  x,
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            const int dim = mesh->spatial_dimension();
            if (dim == 2) {
                return modified_mooney_rivlin_hessian_bsr_2d_isoparametric_mesh_soa(
                        domain.element_type,
                        domain.block->n_elements(),
                        mesh->n_nodes(),
                        domain.block->elements()->data(),
                ...
                return modified_mooney_rivlin_hessian_bsr_3d_isoparametric_mesh_soa(
                        domain.element_type,
                        domain.block->n_elements(),
                        mesh->n_nodes(),
                        domain.block->elements()->data(),
