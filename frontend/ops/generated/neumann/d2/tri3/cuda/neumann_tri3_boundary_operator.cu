...
template <typename s_t>
__host__ __device__ __forceinline__ s_t neumann_tri3_edgeshell2_boundary_residual_soa_measure(
    const int q,
...
__host__ __device__ __forceinline__ const int *neumann_tri3_edgeshell2_boundary_residual_soa_side_nodes() {
...
__host__ __device__ __forceinline__ void neumann_tri3_edgeshell2_boundary_residual_soa_gather_sideset_element(
    const element_idx_t parent_element,
    const int side,
    idx_t **const RSTR elements,
    idx_t *const RSTR ev) {
  const int *const RSTR side_nodes = neumann_tri3_edgeshell2_boundary_residual_soa_side_nodes();
...
template <typename s_t>
__host__ __device__ __forceinline__ void neumann_tri3_edgeshell2_boundary_residual_soa_element(
    const idx_t *const RSTR ev,
...
template <typename s_t>
__host__ __device__ __forceinline__ void neumann_tri3_edgeshell2_boundary_residual_soa_scatter_element(
    const idx_t *const RSTR ev,
...
template <typename s_t>
__global__ void neumann_tri3_edgeshell2_boundary_residual_soa_impl(
...
template <typename s_t>
__global__ void neumann_tri3_edgeshell2_boundary_residual_ss_soa_impl(
...
extern "C" int cu_neumann_tri3_edgeshell2_boundary_residual_soa(
    const ptrdiff_t nelements,
  ...
  sfem::codegen::neumann_tri3_edgeshell2_boundary_residual_soa_impl<real_t><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(
      nelements, nnodes, elements, points, t0, t1, out_stride, out0, out1);
  return sfem::codegen::launch_status("neumann_tri3_edgeshell2_boundary_residual_soa_impl");
}

extern "C" int cu_neumann_tri3_edgeshell2_boundary_residual_soa_float(
    const ptrdiff_t nelements,
    const ptrdiff_t nnodes,
    idx_t **const RSTR elements,
  ...
  const int block_size = 256;
  const int grid_size = (int)((nelements + block_size - 1) / block_size);
  sfem::codegen::neumann_tri3_edgeshell2_boundary_residual_soa_impl<float><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(
      nelements, nnodes, elements, points, t0, t1, out_stride, out0, out1);
  return sfem::codegen::launch_status("neumann_tri3_edgeshell2_boundary_residual_soa_impl");
}
...
extern "C" int cu_neumann_tri3_edgeshell2_boundary_residual_ss_soa(
  ...
  sfem::codegen::neumann_tri3_edgeshell2_boundary_residual_ss_soa_impl<real_t><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(
      nsides, nnodes, elements, parent, side_idx, points, t0, t1, out_stride, out0, out1);
  return sfem::codegen::launch_status("neumann_tri3_edgeshell2_boundary_residual_ss_soa_impl");
}
...
extern "C" int cu_neumann_tri3_edgeshell2_boundary_residual_ss_soa_float(
  ...
  const int block_size = 256;
  const int grid_size = (int)((nsides + block_size - 1) / block_size);
  sfem::codegen::neumann_tri3_edgeshell2_boundary_residual_ss_soa_impl<float><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(
      nsides, nnodes, elements, parent, side_idx, points, t0, t1, out_stride, out0, out1);
  return sfem::codegen::launch_status("neumann_tri3_edgeshell2_boundary_residual_ss_soa_impl");
}
