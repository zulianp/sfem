...
struct neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_reference_data {
...
template <typename s_t>
__host__ __device__ __forceinline__ s_t neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_measure(
    const int qx,
    const int qy,
...
__host__ __device__ __forceinline__ const int *neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_side_nodes() {
...
__host__ __device__ __forceinline__ void neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_gather_sideset_element(
...
template <typename s_t>
__host__ __device__ __forceinline__ void neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_element(
    const idx_t *const RSTR ev,
  ...
  for (int qy = 0; qy < NQ1; ++qy) {
    for (int qx = 0; qx < NQ1; ++qx) {
      const s_t dS = neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_measure<s_t>(qx, qy, ev, points);
...
template <typename s_t>
__host__ __device__ __forceinline__ void neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_scatter_element(
    const idx_t *const RSTR ev,
...
template <typename s_t>
__global__ void neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_impl(
  ...
  for (ptrdiff_t e = (ptrdiff_t)blockIdx.x * blockDim.x + threadIdx.x; e < nelements; e += (ptrdiff_t)blockDim.x * gridDim.x) {
    ...
    neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_element<s_t>(ev, points, t0, t1, t2, element_vector);
    neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_scatter_element<s_t>(ev, element_vector, out_stride, out0, out1, out2);
  }
...
template <typename s_t>
__global__ void neumann_proteus_hex8_proteus_quadshell4_boundary_residual_ss_soa_impl(
  ...
  for (ptrdiff_t s = (ptrdiff_t)blockIdx.x * blockDim.x + threadIdx.x; s < nsides; s += (ptrdiff_t)blockDim.x * gridDim.x) {
    idx_t ev[4];
    s_t element_vector[3][4];
    neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa_gather_sideset_element(parent[s], side_idx[s], elements, ev);
    for (int c = 0; c < 3; ++c) {
      for (int i = 0; i < 4; ++i) {
        element_vector[c][i] = s_t(0);
      }
...
extern "C" int cu_neumann_proteus_hex8_proteus_quadshell4_boundary_residual_soa(
    const ptrdiff_t nelements,
