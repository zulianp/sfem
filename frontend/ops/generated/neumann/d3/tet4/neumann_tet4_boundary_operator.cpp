...
template <typename scalar_t>
static SFEM_INLINE scalar_t neumann_tet4_trishell3_boundary_residual_soa_measure(
        const int q,
        const idx_t *const SFEM_RESTRICT ev,
        const geom_t *const *const SFEM_RESTRICT points) {
    const scalar_t *const grad = neumann_tet4_trishell3_boundary_residual_soa_reference_data<scalar_t>::grad();
    const int n_shape = neumann_tet4_trishell3_boundary_residual_soa_reference_data<scalar_t>::N_SHAPE;
...
static SFEM_INLINE void neumann_tet4_trishell3_boundary_residual_soa_gather_sideset_element(
    ...
    const int *const SFEM_RESTRICT side_nodes = neumann_tet4_trishell3_boundary_residual_soa_side_nodes();
...
static SFEM_INLINE void neumann_tet4_trishell3_boundary_residual_soa_element(
    ...
    const scalar_t *const shape = neumann_tet4_trishell3_boundary_residual_soa_reference_data<scalar_t>::shape();
    const scalar_t *const weight = neumann_tet4_trishell3_boundary_residual_soa_reference_data<scalar_t>::weight();
    const int n_shape = neumann_tet4_trishell3_boundary_residual_soa_reference_data<scalar_t>::N_SHAPE;
    const int n_qp = neumann_tet4_trishell3_boundary_residual_soa_reference_data<scalar_t>::N_QP;
    ...
    for (int q = 0; q < n_qp; ++q) {
        const scalar_t dS = neumann_tet4_trishell3_boundary_residual_soa_measure<scalar_t>(q, ev, points);
...
#pragma omp parallel for
    for (ptrdiff_t e = 0; e < nelements; ++e) {
        ...
        neumann_tet4_trishell3_boundary_residual_soa_element<scalar_t>(ev, points, t0, t1, t2, element_vector);
        neumann_tet4_trishell3_boundary_residual_soa_scatter_element<scalar_t>(ev, element_vector, out_stride, out0, out1, out2);
    }
...
template <typename scalar_t>
static SFEM_INLINE int neumann_tet4_trishell3_boundary_residual_sideset_soa_impl(
...
extern "C" int neumann_tet4_trishell3_boundary_residual_sideset_soa(
        ...
        real_t *const SFEM_RESTRICT out1,
        real_t *const SFEM_RESTRICT out2) {
    return sfem::codegen::neumann_tet4_trishell3_boundary_residual_sideset_soa_impl<real_t>(
...
extern "C" int neumann_tet4_trishell3_boundary_residual_sideset_soa_float(
        ...
        float *const SFEM_RESTRICT out1,
        float *const SFEM_RESTRICT out2) {
    return sfem::codegen::neumann_tet4_trishell3_boundary_residual_sideset_soa_impl<float>(
            nsides, nnodes, elements, parent, side_idx, points, t0, t1, t2, out_stride, out0, out1, out2);
}
