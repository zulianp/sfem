      ...
      for (const auto &condition : impl_->conditions) {
        ...
        switch (domain.element_type) {
          ...
          real_t *const RSTR u_out[2] = {out + 0, out + 1};
            status |= neumann_edgeshell2_boundary_residual_2d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), domain.block->elements()->data(), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], FIELD_STRIDE, u_out[0], u_out[1]);
            break;
          }
                    case smesh::QUAD4: {
            static constexpr ptrdiff_t FIELD_STRIDE = 2;
          ...
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= neumann_trishell3_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), domain.block->elements()->data(), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], FIELD_STRIDE, u_out[0], u_out[1], u_out[2]);
            break;
          }
                    case smesh::TET10: {
            static constexpr ptrdiff_t FIELD_STRIDE = 3;
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= neumann_trishell6_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), domain.block->elements()->data(), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], FIELD_STRIDE, u_out[0], u_out[1], u_out[2]);
            break;
          }
                    case smesh::HEX8: {
            static constexpr ptrdiff_t FIELD_STRIDE = 3;
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= neumann_quadshell4_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), domain.block->elements()->data(), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], FIELD_STRIDE, u_out[0], u_out[1], u_out[2]);
            break;
          }
          ...
          real_t *const RSTR u_out[2] = {out + 0, out + 1};
            ...
            static constexpr ptrdiff_t FIELD_STRIDE = 3;
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= neumann_proteus_quadshell4_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), domain.block->elements()->data(), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], FIELD_STRIDE, u_out[0], u_out[1], u_out[2]);
            break;
          }
          default:
