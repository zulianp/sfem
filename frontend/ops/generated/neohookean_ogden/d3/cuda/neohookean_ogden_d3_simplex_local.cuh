#ifndef NEOHOOKEAN_OGDEN_D3_SIMPLEX_LOCAL_CUH
#define NEOHOOKEAN_OGDEN_D3_SIMPLEX_LOCAL_CUH
#include <math.h>
#include <stddef.h>
...
static __host__ __device__ __forceinline__ void neohookean_ogden_d3_simplex_objective_block(
  ...
  static_assert(VS > 0, "VS must be positive");
    ...
    value[step * value_stride + 0] += qw * det_value0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(weak_obj_tmp3) - mu*weak_obj_tmp3 + ((s_t(1) / s_t(2)))*mu*(pow_2(gu1) + pow_2(gu2) + pow_2(gu3) + pow_2(gu5) + pow_2(gu6) + pow_2(gu7) + pow_2(weak_obj_tmp0) + pow_2(weak_obj_tmp1) + pow_2(weak_obj_tmp2) + s_t(-3)));
        }
      }
    }
}
