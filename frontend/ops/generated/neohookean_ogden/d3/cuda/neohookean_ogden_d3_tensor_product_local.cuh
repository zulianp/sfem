...
static __host__ __device__ __forceinline__ void neohookean_ogden_d3_tensor_product_objective_block(
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    value[0] += qw * det_value0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(weak_obj_tmp3) - mu*weak_obj_tmp3 + ((s_t(1) / s_t(2)))*mu*(pow_2(gu[1]) + pow_2(gu[2]) + pow_2(gu[3]) + pow_2(gu[5]) + pow_2(gu[6]) + pow_2(gu[7]) + pow_2(weak_obj_tmp0) + pow_2(weak_obj_tmp1) + pow_2(weak_obj_tmp2) + s_t(-3)));
    }
  }
}
