...
template <typename scalar_t>
static SFEM_INLINE void neohookean_ogden_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_bsr(
...
#pragma omp parallel for schedule(static) reduction(|:unsupported_matrix_format)
    for (ptrdiff_t element = 0; element < nelements; ++element) {
        ...
        if constexpr (FORMAT == 1) {
            neohookean_ogden_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_bsr(ev, element_matrix, rowptr, colidx, values);
        } else {
