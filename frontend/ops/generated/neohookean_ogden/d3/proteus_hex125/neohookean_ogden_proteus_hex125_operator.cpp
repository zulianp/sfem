...
static SFEM_INLINE void neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_find_cols(
...
template <typename scalar_t>
static SFEM_INLINE int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_bsr(
    ...
    for (int i = 0; i < N_SHAPE; ++i) {
        ...
        neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_find_cols(ev, cols, lenrow, ks);
        for (int j = 0; j < N_SHAPE; ++j) {
            if (ks[j] < 0 || ks[j] >= lenrow || cols[ks[j]] != ev[j]) {
                if (valid_block_graph) {
                    std::fprintf(stderr, "neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_bsr missing block graph entry (%ld, %ld)\n", (long)ev[i], (long)ev[j]);
                }
...
template <typename scalar_t>
static SFEM_INLINE int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_crs(
        const idx_t *const SFEM_RESTRICT ev,
        const scalar_t *const SFEM_RESTRICT element_matrix,
        const count_t *const SFEM_RESTRICT rowptr,
        const idx_t *const SFEM_RESTRICT colidx,
    ...
    for (int i = 0; i < N_SHAPE; ++i) {
        ...
        neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_find_cols(ev, cols, lenrow[i], ks);
        for (int j = 0; j < N_SHAPE; ++j) {
            if (ks[j] < 0 || ks[j] >= lenrow[i] || cols[ks[j]] != ev[j]) {
                if (valid_graph) {
                    std::fprintf(stderr, "neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_crs missing graph entry (%ld, %ld)\n", (long)ev[i], (long)ev[j]);
                }
...
template <typename scalar_t>
static SFEM_INLINE int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_dia(
        const idx_t *const SFEM_RESTRICT ev,
        const scalar_t *const SFEM_RESTRICT element_matrix,
        const ptrdiff_t nnodes,
        ...
        for (int j = 0; j < N_SHAPE; ++j) {
            ...
            if (diagonal == ndiag) {
                if (valid_diagonal_offsets) {
                    std::fprintf(stderr, "neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_dia missing diagonal offset %d\n", offset);
                }
...
template <typename scalar_t>
static SFEM_INLINE int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_coo(
        const idx_t *const SFEM_RESTRICT ev,
        const scalar_t *const SFEM_RESTRICT element_matrix,
        const ptrdiff_t nnz,
        ...
        for (int j = 0; j < N_SHAPE; ++j) {
            ...
            if (lo == nnz || rows[lo] != ev[i] || cols[lo] != ev[j]) {
                if (valid_coo_entries) {
                    std::fprintf(stderr, "neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_coo missing graph entry (%ld, %ld)\n", (long)ev[i], (long)ev[j]);
                }
...
template <typename scalar_t>
static SFEM_INLINE void neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_coo_triplets(
        const idx_t *const SFEM_RESTRICT ev,
        const scalar_t *const SFEM_RESTRICT element_matrix,
...
template <typename scalar_t>
static SFEM_INLINE int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_patch(
        const idx_t *const SFEM_RESTRICT ev,
        const scalar_t *const SFEM_RESTRICT element_matrix,
        const count_t *const SFEM_RESTRICT rowptr,
        const idx_t *const SFEM_RESTRICT colidx,
...
static SFEM_INLINE idx_t neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_packed_global_node(
...
template <typename scalar_t>
static SFEM_INLINE int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_discover_packed_crs_entries(
        const idx_t *const SFEM_RESTRICT ev,
        const count_t *const SFEM_RESTRICT rowptr,
        const idx_t *const SFEM_RESTRICT colidx,
    ...
    for (int i = 0; i < N_SHAPE; ++i) {
        ...
        for (int j = 0; j < N_SHAPE; ++j) {
            if (ks[j] < 0 || ks[j] >= lenrow || cols[ks[j]] != ev[j]) {
                if (valid_graph) {
                    std::fprintf(stderr, "neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_discover_packed_crs_entries missing graph entry (%ld, %ld)\n", (long)ev[i], (long)ev[j]);
                }
                for (int bi = 0; bi < DIM; ++bi) {
                    for (int bj = 0; bj < DIM; ++bj) {
                        const int row = bi * N_SHAPE + i;
...
template <typename scalar_t>
static SFEM_INLINE void neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_packed_crs_entries(
        const scalar_t *const SFEM_RESTRICT element_matrix,
...
#pragma omp parallel for schedule(static) reduction(|:invalid_matrix_graph)
    for (ptrdiff_t element = 0; element < nelements; ++element) {
        ...
        if constexpr (FORMAT == 1) {
            invalid_matrix_graph |= (neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_bsr(ev, element_matrix, rowptr, colidx, values) != SFEM_SUCCESS);
        } else if constexpr (FORMAT == 0) {
            invalid_matrix_graph |= (neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_crs(ev, element_matrix, rowptr, colidx, values) != SFEM_SUCCESS);
        } else if constexpr (FORMAT == 2) {
            invalid_matrix_graph |= (neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_dia(ev, element_matrix, nnodes, diag_offsets, ndiag, values) != SFEM_SUCCESS);
        } else if constexpr (FORMAT == 3) {
            invalid_matrix_graph |= (neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_coo(ev, element_matrix, coo_nnz, coo_rows, coo_cols, values) != SFEM_SUCCESS);
        } else if constexpr (FORMAT == 5) {
            neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_coo_triplets(ev, element_matrix, element, coo_triplet_rows, coo_triplet_cols, values);
        } else {
            invalid_matrix_graph |= (neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_patch(ev, element_matrix, rowptr, colidx, values) != SFEM_SUCCESS);
        }
    }
...
template <typename scalar_t, typename geometry_t>
static int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_packed_discover_impl(
        const ptrdiff_t n_packs,
        const ptrdiff_t n_elements_per_pack,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
...
#pragma omp parallel for schedule(static) reduction(|:invalid_matrix_graph)
    for (ptrdiff_t pack = 0; pack < n_packs; ++pack) {
        ...
        for (ptrdiff_t element = e_start; element < e_end; ++element) {
            idx_t ev[N_SHAPE];
            for (int shape = 0; shape < N_SHAPE; ++shape) {
                ev[shape] = neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_packed_global_node(elements[shape][element], pack, owned_nodes_ptr, ghost_ptr, ghost_idx);
            }
            count_t *const entries = &packed_element_entries[element * (DIM * N_SHAPE) * (DIM * N_SHAPE)];
            invalid_matrix_graph |= (neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_discover_packed_crs_entries<scalar_t>(ev, rowptr, colidx, entries) != SFEM_SUCCESS);
        }
    }
    return invalid_matrix_graph ? SFEM_FAILURE : SFEM_SUCCESS;
}
...
template <typename scalar_t, typename geometry_t>
static int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_packed_fill_impl(
        const ptrdiff_t n_packs,
        const ptrdiff_t n_elements_per_pack,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
...
#pragma omp for schedule(static)
        ...
        geometry_jacobian_adjugate_and_determinant<scalar_t, DIM, N_QP, VECTOR_SIZE>(
            ...
            neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_scatter_packed_crs_entries(element_matrix, entries, values);
            }
        }
    }
    return SFEM_SUCCESS;
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_crs_isoparametric_mesh_soa(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<double, geom_t, 0>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, rowptr, colidx, values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_crs_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<float, geom_t, 0>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, rowptr, colidx, values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_bsr_isoparametric_mesh_soa(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<double, geom_t, 1>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, rowptr, colidx, values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_bsr_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<float, geom_t, 1>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, rowptr, colidx, values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_dia_isoparametric_mesh_soa(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<double, geom_t, 2>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, nullptr, nullptr, values, diag_offsets, ndiag, 0, nullptr, nullptr, nullptr, nullptr);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_dia_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<float, geom_t, 2>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, nullptr, nullptr, values, diag_offsets, ndiag, 0, nullptr, nullptr, nullptr, nullptr);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_coo_isoparametric_mesh_soa(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<double, geom_t, 3>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, nullptr, nullptr, values, nullptr, 0, nnz, rows, cols, nullptr, nullptr);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_coo_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<float, geom_t, 3>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, nullptr, nullptr, values, nullptr, 0, nnz, rows, cols, nullptr, nullptr);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_coo_triplet_isoparametric_mesh_soa(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<double, geom_t, 5>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, nullptr, nullptr, values, nullptr, 0, 0, nullptr, nullptr, rows, cols);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_coo_triplet_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<float, geom_t, 5>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, nullptr, nullptr, values, nullptr, 0, 0, nullptr, nullptr, rows, cols);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_patch_isoparametric_mesh_soa(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<double, geom_t, 4>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, rowptr, colidx, values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_patch_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_assemble_impl<float, geom_t, 4>(nelements, nnodes, elements, points, mu, lmbda, u_stride, ux, uy, uz, rowptr, colidx, values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
}
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_crs_packed_two_pass_isoparametric_mesh_soa(
    ...
    const int graph_status = sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_packed_discover_impl<double, geom_t>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, rowptr, colidx, packed_element_entries);
...
extern "C" int neohookean_ogden_proteus_hex125_proteus_hex125_hessian_crs_packed_two_pass_isoparametric_mesh_soa_float(
    ...
    const int graph_status = sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_packed_discover_impl<float, geom_t>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, rowptr, colidx, packed_element_entries);
    if (graph_status != SFEM_SUCCESS) return graph_status;
    return sfem::codegen::neohookean_ogden_proteus_hex125_proteus_hex125_hessian_isoparametric_mesh_soa_packed_fill_impl<float, geom_t>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, mu, lmbda, u_stride, ux, uy, uz, packed_element_entries, values);
}
