...
#include "../neohookean_ogden_d2_tensor_product_hessian.hpp"
...
static SFEM_INLINE void neohookean_ogden_proteus_quad4_hessian_i_msoa_find_cols(
...
static SFEM_INLINE void neohookean_ogden_proteus_quad4_hessian_i_msoa_scatter_bsr(
  ...
  for (int i = 0; i < NS; ++i) {
    ...
    neohookean_ogden_proteus_quad4_hessian_i_msoa_find_cols(ev, cols, lenrow, ks);
    for (int j = 0; j < NS; ++j) {
...
static int neohookean_ogden_proteus_quad4_hessian_i_msoa_assemble_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const *const RSTR points,
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t element = 0; element < nelements; ++element) {
    ...
    neohookean_ogden_d2_tensor_product_direct_hessian_tensor_product_element_matrix<s_t, NQ, NS, VS>(badj0, badj1, badj2, badj3, bdet0, isoparametric_shape_1d, isoparametric_grad_1d, isoparametric_q_weight_1d, lmbda, mu, bu_data, element_matrix);
