...
extern "C" void neohookean_ogden_quad4_matrix_assembly_print_variant(
        const int variant,
        const ptrdiff_t nelements
) {
    neohookean_ogden_proteus_quad4_matrix_assembly_print_variant(variant, nelements);
}

extern "C" const sfem_MatrixAssemblyDiagnostics * neohookean_ogden_quad4_matrix_assembly_variant(
        const int variant
) {
    return neohookean_ogden_proteus_quad4_matrix_assembly_variant(variant);
}

extern "C" int neohookean_ogden_quad4_matrix_assembly_variant_count(
        void
) {
    return neohookean_ogden_proteus_quad4_matrix_assembly_variant_count();
}
