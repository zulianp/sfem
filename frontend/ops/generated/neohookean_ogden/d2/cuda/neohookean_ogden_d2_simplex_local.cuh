...
static __host__ __device__ __forceinline__ void neohookean_ogden_d2_simplex_objective_block(
  ...
  static_assert(VS > 0, "VS must be positive");
    ...
    value[0] += qw * det_value0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(weak_obj_tmp2) - mu*weak_obj_tmp2 + ((s_t(1) / s_t(2)))*mu*(pow_2(gu1) + pow_2(gu2) + pow_2(weak_obj_tmp0) + pow_2(weak_obj_tmp1) + s_t(-2)));
      }
    }
}
