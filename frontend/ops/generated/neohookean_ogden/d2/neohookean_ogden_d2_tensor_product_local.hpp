#ifndef NEOHOOKEAN_OGDEN_D2_TENSOR_PRODUCT_LOCAL_HPP
#define NEOHOOKEAN_OGDEN_D2_TENSOR_PRODUCT_LOCAL_HPP
#include <math.h>
#include <stddef.h>
...
static SFEM_INLINE void neohookean_ogden_d2_tensor_product_objective_block(
    ...
    for (int q = 0; q < NQ; ++q) {
        ...
        const s_t weak_obj_tmp1 = grad_u[3] + s_t(1);
        const s_t weak_obj_tmp2 = log(-grad_u[1]*grad_u[2] + weak_obj_tmp0*weak_obj_tmp1);
        value[lane] += qw * jacobian_determinant_lane0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(weak_obj_tmp2) - mu*weak_obj_tmp2 + ((s_t(1) / s_t(2)))*mu*(pow_2(grad_u[1]) + pow_2(grad_u[2]) + pow_2(weak_obj_tmp0) + pow_2(weak_obj_tmp1) + s_t(-2)));
        }
    }
}
...
static SFEM_INLINE void neohookean_ogden_d2_tensor_product_gradient_block(
    ...
    for (int q = 0; q < NQ; ++q) {
        ...
        const s_t weak_mat_tmp2 = grad_u[3] + s_t(1);
        const s_t weak_mat_tmp3 = -grad_u[1]*grad_u[2] + weak_mat_tmp0*weak_mat_tmp2;
        ...
        const s_t weak_mat_tmp7 = grad_u[1]*mu;
        const s_t weak_mat_tmp8 = grad_u[2]*mu;
...
static SFEM_INLINE void neohookean_ogden_d2_tensor_product_apply_block(
    ...
    for (int q = 0; q < NQ; ++q) {
        ...
        const s_t weak_mat_tmp1 = grad_u[1]*grad_u[2];
        const s_t weak_mat_tmp2 = grad_u[0] + s_t(1);
        ...
        const s_t weak_mat_tmp6 = grad_u[2]*weak_mat_tmp5;
        const s_t weak_mat_tmp7 = log(weak_mat_tmp3);
        const s_t weak_mat_tmp8 = grad_u[2]*lmbda*weak_mat_tmp0*weak_mat_tmp4*weak_mat_tmp7 - lmbda*weak_mat_tmp6 - mu*weak_mat_tmp6;
        const s_t weak_mat_tmp9 = grad_u[1]*weak_mat_tmp5;
        const s_t weak_mat_tmp10 = grad_u[1]*lmbda*weak_mat_tmp0*weak_mat_tmp4*weak_mat_tmp7 - lmbda*weak_mat_tmp9 - mu*weak_mat_tmp9;
        ...
        const s_t weak_mat_tmp19 = pow_2(grad_u[2])*weak_mat_tmp4;
        const s_t weak_mat_tmp20 = weak_mat_tmp2*weak_mat_tmp4;
        const s_t weak_mat_tmp21 = grad_u[2]*weak_mat_tmp20;
        const s_t weak_mat_tmp22 = grad_u[2]*lmbda*weak_mat_tmp2*weak_mat_tmp4*weak_mat_tmp7 - lmbda*weak_mat_tmp21 - mu*weak_mat_tmp21;
        ...
        const s_t weak_mat_tmp25 = pow_2(grad_u[1])*weak_mat_tmp4;
        const s_t weak_mat_tmp26 = grad_u[1]*weak_mat_tmp20;
        const s_t weak_mat_tmp27 = grad_u[1]*lmbda*weak_mat_tmp2*weak_mat_tmp4*weak_mat_tmp7 - lmbda*weak_mat_tmp26 - mu*weak_mat_tmp26;
