...
static SFEM_INLINE void neohookean_ogden_d2_tensor_product_objective_block(
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    value[lane] += qw * det_lane0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(weak_obj_tmp2) - mu*weak_obj_tmp2 + ((s_t(1) / s_t(2)))*mu*(pow_2(gu[1]) + pow_2(gu[2]) + pow_2(weak_obj_tmp0) + pow_2(weak_obj_tmp1) + s_t(-2)));
    }
  }
}
