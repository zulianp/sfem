...
namespace sfem {
  class GPUGeneratedNeoHookeanOgden final : public Op, public TimeSteppable {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GPUGeneratedNeoHookeanOgden(const std::shared_ptr<FunctionSpace> &space);
    ~GPUGeneratedNeoHookeanOgden() override;

    const char *name() const override { return "GPUGeneratedNeoHookeanOgden"; }
