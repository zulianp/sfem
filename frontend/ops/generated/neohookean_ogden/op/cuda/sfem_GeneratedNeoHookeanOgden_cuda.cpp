...
#include "sfem_GeneratedNeoHookeanOgden_cuda_c_abi.hpp"
...
namespace sfem {
  namespace {
    ...
    smesh::block_idx_t block_id_for_domain(const smesh::Mesh &mesh,
      ...
      SFEM_ERROR("GPUGeneratedNeoHookeanOgden: mesh block pointer not found in mesh.blocks()\n");
      return 0;
    }
  ...
  class GPUGeneratedNeoHookeanOgden::Impl {
  public:
  ...
  std::unique_ptr<Op> GPUGeneratedNeoHookeanOgden::create(const std::shared_ptr<FunctionSpace> &space) {
    ...
    if (space->block_size() != expected_block_size) {
      SFEM_ERROR("GPUGeneratedNeoHookeanOgden requires block_size=%ld\n",
                       static_cast<long>(expected_block_size));
      return nullptr;
    }
    auto op = std::make_unique<GPUGeneratedNeoHookeanOgden>(space);
    op->initialize();
    return op;
  }

  GPUGeneratedNeoHookeanOgden::GPUGeneratedNeoHookeanOgden(const std::shared_ptr<FunctionSpace> &space)
    : impl_(std::make_unique<Impl>(space)) {}
  GPUGeneratedNeoHookeanOgden::~GPUGeneratedNeoHookeanOgden() = default;

  ptrdiff_t GPUGeneratedNeoHookeanOgden::n_dofs_domain() const { return impl_->space->n_dofs(); }
  ptrdiff_t GPUGeneratedNeoHookeanOgden::n_dofs_image() const { return impl_->space->n_dofs(); }

    double GPUGeneratedNeoHookeanOgden::flops_value() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      if (dim == 2) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_neohookean_ogden_objective_2d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
      ...
      if (dim == 3) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_neohookean_ogden_objective_3d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
            total += impl_->objective_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
          }
        }
    ...
    size_t GPUGeneratedNeoHookeanOgden::memory_traffic_bytes_value() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    double GPUGeneratedNeoHookeanOgden::flops_gradient() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      if (dim == 2) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_neohookean_ogden_gradient_2d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
      ...
      if (dim == 3) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_neohookean_ogden_gradient_3d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
            total += impl_->gradient_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
          }
        }
    ...
    size_t GPUGeneratedNeoHookeanOgden::memory_traffic_bytes_gradient() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    double GPUGeneratedNeoHookeanOgden::flops_apply() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      if (dim == 2) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_neohookean_ogden_apply_2d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
      ...
      if (dim == 3) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_neohookean_ogden_apply_3d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
            total += impl_->apply_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
          }
        }
    ...
    size_t GPUGeneratedNeoHookeanOgden::memory_traffic_bytes_apply() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
  ...
  int GPUGeneratedNeoHookeanOgden::initialize(const std::vector<std::string> &block_names) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeoHookeanOgden::initialize");
    impl_->domains = std::make_shared<MultiDomainOp>(impl_->space, block_names);
    {
      ...
      if (!dof_graph ||
        ...
        SFEM_ERROR("GPUGeneratedNeoHookeanOgden::initialize: the dof graph is malformed; the assembly kernels assume it is not\n");
        return SFEM_FAILURE;
      }
    }
    ...
    return impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      const geom_t *determinant = nullptr;
        ...
        if (!cache || !cache->jacobian_soa) {
          SFEM_ERROR("GPUGeneratedNeoHookeanOgden affine gradient requires cached geometry\n");
          return SFEM_FAILURE;
        }
        ...
        if (false) {
          if (!cache->jacobian_aos) {
            SFEM_ERROR("GPUGeneratedNeoHookeanOgden affine gradient requires cached AoS geometry\n");
            return SFEM_FAILURE;
          }
      ...
      if (dim == 2) {
        if (impl_->gradient_uses_affine) {
          return cu_neohookean_ogden_gradient_2d_a_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 2, x + 0, x + 1, 2, out + 0, out + 1, stream);
        }
        return cu_neohookean_ogden_gradient_2d_i_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), points, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 2, x + 0, x + 1, 2, out + 0, out + 1, stream);
      }
      else if (dim == 3) {
        if (impl_->gradient_uses_affine) {
          return cu_neohookean_ogden_gradient_3d_a_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), adjugate[0], adjugate[1], adjugate[2], adjugate[3], adjugate[4], adjugate[5], adjugate[6], adjugate[7], adjugate[8], determinant, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 3, x + 0, x + 1, x + 2, 3, out + 0, out + 1, out + 2, stream);
        }
        return cu_neohookean_ogden_gradient_3d_i_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), points, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 3, x + 0, x + 1, x + 2, 3, out + 0, out + 1, out + 2, stream);
      }
  ...
  int GPUGeneratedNeoHookeanOgden::apply(const real_t *const x,
    ...
    return impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      const geom_t *determinant = nullptr;
        ...
        auto cache = std::static_pointer_cast<AffineGeometryCache>(
            domain.user_data);
        if (!cache || !cache->jacobian_soa) {
          SFEM_ERROR("GPUGeneratedNeoHookeanOgden affine hessian action requires cached geometry\n");
          return SFEM_FAILURE;
        }
        adjugate = reinterpret_cast<const geom_t *const *>(
            cache->jacobian_soa->jacobian_adjugate_SoA()->data());
        ...
        if (false) {
          if (!cache->jacobian_aos) {
            SFEM_ERROR("GPUGeneratedNeoHookeanOgden affine hessian action requires cached AoS geometry\n");
            return SFEM_FAILURE;
          }
          adjugate_aos = reinterpret_cast<const geom_t *>(
              cache->jacobian_aos->jacobian_adjugate_AoS()->data());
      ...
      if (dim == 2) {
        if (impl_->apply_uses_affine) {
          return cu_neohookean_ogden_apply_2d_a_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 2, x + 0, x + 1, 2, h + 0, h + 1, 2, out + 0, out + 1, stream);
        }
        return cu_neohookean_ogden_apply_2d_i_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), points, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 2, x + 0, x + 1, 2, h + 0, h + 1, 2, out + 0, out + 1, stream);
      }
      else if (dim == 3) {
        if (impl_->apply_uses_affine) {
          return cu_neohookean_ogden_apply_3d_a_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), adjugate[0], adjugate[1], adjugate[2], adjugate[3], adjugate[4], adjugate[5], adjugate[6], adjugate[7], adjugate[8], determinant, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 3, x + 0, x + 1, x + 2, 3, h + 0, h + 1, h + 2, 3, out + 0, out + 1, out + 2, stream);
        }
        return cu_neohookean_ogden_apply_3d_i_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), points, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 3, x + 0, x + 1, x + 2, 3, h + 0, h + 1, h + 2, 3, out + 0, out + 1, out + 2, stream);
      }
  ...
  int GPUGeneratedNeoHookeanOgden::value(const real_t *x, real_t *const out) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeoHookeanOgden::value");
  ...
  int GPUGeneratedNeoHookeanOgden::value_steps(const real_t *x,
              const real_t *h,
              const int nsteps,
              const real_t *const steps,
              real_t *const out) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeoHookeanOgden::value_steps");
    auto mesh = impl_->space->mesh_ptr();
    auto points = element_points(mesh);
    if (nsteps <= 0) {
      return SFEM_SUCCESS;
    ...
    return impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      const geom_t *determinant = nullptr;
        ...
        auto cache = std::static_pointer_cast<AffineGeometryCache>(
            domain.user_data);
        if (!cache || !cache->jacobian_soa) {
          SFEM_ERROR("GPUGeneratedNeoHookeanOgden affine objective_steps requires cached geometry\n");
          return SFEM_FAILURE;
        }
        adjugate = reinterpret_cast<const geom_t *const *>(
            cache->jacobian_soa->jacobian_adjugate_SoA()->data());
      ...
      if (status == SFEM_FAILURE) {
        ...
        if (dim == 2) {
          if (impl_->objective_uses_affine) {
            status = cu_neohookean_ogden_objective_steps_2d_a_msoa(domain.element_type, real_type, nelements, mesh->n_nodes(), element_connectivity(domain), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 2, x + 0, x + 1, 2, h + 0, h + 1, nsteps, impl_->step_values->data(), impl_->element_values->data(), stream);
          } else {
            status = cu_neohookean_ogden_objective_steps_2d_i_msoa(domain.element_type, real_type, nelements, mesh->n_nodes(), element_connectivity(domain), points, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 2, x + 0, x + 1, 2, h + 0, h + 1, nsteps, impl_->step_values->data(), impl_->element_values->data(), stream);
          }
        }
        else if (dim == 3) {
          if (impl_->objective_uses_affine) {
            status = cu_neohookean_ogden_objective_steps_3d_a_msoa(domain.element_type, real_type, nelements, mesh->n_nodes(), element_connectivity(domain), adjugate[0], adjugate[1], adjugate[2], adjugate[3], adjugate[4], adjugate[5], adjugate[6], adjugate[7], adjugate[8], determinant, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 3, x + 0, x + 1, x + 2, 3, h + 0, h + 1, h + 2, nsteps, impl_->step_values->data(), impl_->element_values->data(), stream);
          } else {
            status = cu_neohookean_ogden_objective_steps_3d_i_msoa(domain.element_type, real_type, nelements, mesh->n_nodes(), element_connectivity(domain), points, domain.parameters->require_real_value("lmbda"), domain.parameters->require_real_value("mu"), 3, x + 0, x + 1, x + 2, 3, h + 0, h + 1, h + 2, nsteps, impl_->step_values->data(), impl_->element_values->data(), stream);
          }
        }
        if (dim != 2 && dim != 3) {
          SFEM_ERROR("neohookean_ogden objective_steps does not support spatial dimension %d\n", dim);
          return SFEM_FAILURE;
        }
      }
  ...
  int GPUGeneratedNeoHookeanOgden::hessian_crs(const real_t *const x,
  ...
  int GPUGeneratedNeoHookeanOgden::hessian_bsr(const real_t *const x,
              const count_t *const rowptr,
              const idx_t *const colidx,
              real_t *const values) {
  ...
  int GPUGeneratedNeoHookeanOgden::hessian_block_diag_sym(const real_t *const x,
    ...
    return impl_->domains->iterate([&](const OpDomain &domain) {
      const int dim = mesh->spatial_dimension();
      if (dim == 2) {
        SFEM_ERROR("neohookean_ogden hessian_block_diag_sym 2d dispatch was not generated\n");
        return SFEM_FAILURE;
      }
      else if (dim == 3) {
        SFEM_ERROR("neohookean_ogden hessian_block_diag_sym 3d dispatch was not generated\n");
        return SFEM_FAILURE;
      }
      SFEM_ERROR("neohookean_ogden hessian_block_diag_sym does not support spatial dimension %d\n", dim);
      return SFEM_FAILURE;
    });
  }

  void GPUGeneratedNeoHookeanOgden::set_option(const std::string &name, const bool val) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeoHookeanOgden::set_option");
    ...
    if (matched && val && impl_->domains) {
      if (cache_affine_geometry(impl_->space, *impl_->domains) != SFEM_SUCCESS) {
        SFEM_ERROR("GPUGeneratedNeoHookeanOgden failed to cache affine geometry\n");
      }
    }
  }

  void GPUGeneratedNeoHookeanOgden::set_value_in_block(const std::string &block_name,
                  const std::string &var_name,
                  const real_t value) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeoHookeanOgden::set_value_in_block");
    impl_->domains->set_value_in_block(block_name, var_name, value);
  }

#ifdef SFEM_ENABLE_RYAML
  std::shared_ptr<Op> GPUGeneratedNeoHookeanOgden::create_from_yaml(const std::shared_ptr<FunctionSpace> &space,
                                                 const ryml::ConstNodeRef             &node) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeoHookeanOgden::create_from_yaml");
    auto ret = std::make_shared<GPUGeneratedNeoHookeanOgden>(space);
