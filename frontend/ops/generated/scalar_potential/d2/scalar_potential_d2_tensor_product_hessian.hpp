#ifndef SCALAR_POTENTIAL_D2_TENSOR_PRODUCT_HESSIAN_HPP
#define SCALAR_POTENTIAL_D2_TENSOR_PRODUCT_HESSIAN_HPP
#include <math.h>
#include <stddef.h>
...
static SFEM_INLINE void scalar_potential_d2_tensor_product_direct_hessian_tensor_product_element_matrix(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        for (int trial_component = 0; trial_component < N_FIELD_COMPONENTS; ++trial_component) {
            for (int trial_shape = 0; trial_shape < N_SHAPE; ++trial_shape) {
                ...
                const int trial_sy = trial_shape / N_SHAPE_1D;
                const scalar_t trial_grad_ref0 = grad_1d[qx * N_SHAPE_1D + trial_sx] * shape_1d[qy * N_SHAPE_1D + trial_sy];
                const scalar_t trial_grad_ref1 = shape_1d[qx * N_SHAPE_1D + trial_sx] * grad_1d[qy * N_SHAPE_1D + trial_sy];
                ...
                for (int test_component = 0; test_component < N_FIELD_COMPONENTS; ++test_component) {
                    for (int test_shape = 0; test_shape < N_SHAPE; ++test_shape) {
                        ...
                        const int test_sy = test_shape / N_SHAPE_1D;
                        const scalar_t test_grad_ref0 = grad_1d[qx * N_SHAPE_1D + test_sx] * shape_1d[qy * N_SHAPE_1D + test_sy];
                        const scalar_t test_grad_ref1 = shape_1d[qx * N_SHAPE_1D + test_sx] * grad_1d[qy * N_SHAPE_1D + test_sy];
