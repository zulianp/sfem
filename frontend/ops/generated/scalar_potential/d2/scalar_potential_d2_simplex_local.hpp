#ifndef SCALAR_POTENTIAL_D2_SIMPLEX_LOCAL_HPP
#define SCALAR_POTENTIAL_D2_SIMPLEX_LOCAL_HPP
#include <math.h>
#include <stddef.h>
...
static SFEM_INLINE void scalar_potential_d2_simplex_objective_block(
        const int nelems,
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        value[lane] += qw * jacobian_determinant_lane0 * (((scalar_t(1) / scalar_t(2)))*kappa*(pow_2(grad_u0) + pow_2(grad_u1)));
            }
        }
}

template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void scalar_potential_d2_simplex_tri3_objective_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void scalar_potential_d2_simplex_tri3_metric_objective_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            const scalar_t t2 = geom_metric_lane0*t0 + geom_metric_lane1*t1;
            const scalar_t t3 = geom_metric_lane1*t0 + geom_metric_lane2*t1;
            value[lane] += ((scalar_t(1) / scalar_t(2)))*kappa*(t2*(-u_streams[0 * 1 + 0][lane] + u_streams[1 * 1 + 0][lane]) + t3*(-u_streams[0 * 1 + 0][lane] + u_streams[2 * 1 + 0][lane]));
        }
}

template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void scalar_potential_d2_simplex_gradient_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        const scalar_t loperand1 = qw * (material0 * jacobian_adjugate_lane2 + material1 * jacobian_adjugate_lane3);
            ...
            for (int shape = 0; shape < N_SHAPE; ++shape) {
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    out_streams[shape * 1 + 0][lane] += loperand0_values[lane] * grad_ref_x[q * N_SHAPE + shape] + loperand1_values[lane] * grad_ref_y[q * N_SHAPE + shape];
                }
            }
        }
}
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void scalar_potential_d2_simplex_tri3_gradient_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void scalar_potential_d2_simplex_tri3_metric_gradient_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT geom_metric0,
        const scalar_t *const SFEM_RESTRICT geom_metric1,
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void scalar_potential_d2_simplex_apply_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void scalar_potential_d2_simplex_tri3_apply_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void scalar_potential_d2_simplex_tri3_metric_apply_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT geom_metric0,
        const scalar_t *const SFEM_RESTRICT geom_metric1,
