#ifndef SCALAR_POTENTIAL_D3_SIMPLEX_HESSIAN_HPP
#define SCALAR_POTENTIAL_D3_SIMPLEX_HESSIAN_HPP
#include <math.h>
#include <stddef.h>
...
static SFEM_INLINE void scalar_potential_d3_simplex_direct_hessian_reference_element_matrix(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        for (int trial_component = 0; trial_component < N_FIELD_COMPONENTS; ++trial_component) {
            for (int trial_shape = 0; trial_shape < N_SHAPE; ++trial_shape) {
                ...
                const scalar_t trial_grad_ref2 = grad_ref_z[q * N_SHAPE + trial_shape];
                ...
                for (int test_component = 0; test_component < N_FIELD_COMPONENTS; ++test_component) {
                    for (int test_shape = 0; test_shape < N_SHAPE; ++test_shape) {
                        ...
                        const scalar_t test_grad_ref2 = grad_ref_z[q * N_SHAPE + test_shape];
