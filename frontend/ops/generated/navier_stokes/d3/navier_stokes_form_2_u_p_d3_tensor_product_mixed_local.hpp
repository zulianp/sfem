...
static SFEM_INLINE void navier_stokes_form_2_u_p_d3_tensor_product_mixed_jacobian_action_block(
    ...
    for (int q = 0; q < NQ; ++q) {
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            u0_grad_coeff_ref[(q * ND + 0) * VS + lane] = qw * (adj0 * grad_coeff0_0);
            u0_grad_coeff_ref[(q * ND + 1) * VS + lane] = qw * (adj3 * grad_coeff0_0);
            u0_grad_coeff_ref[(q * ND + 2) * VS + lane] = qw * (adj6 * grad_coeff0_0);
            u1_value_coeff[q * VS + lane] = s_t(0);
            u1_grad_coeff_ref[(q * ND + 0) * VS + lane] = qw * (adj1 * grad_coeff1_1);
            u1_grad_coeff_ref[(q * ND + 1) * VS + lane] = qw * (adj4 * grad_coeff1_1);
            u1_grad_coeff_ref[(q * ND + 2) * VS + lane] = qw * (adj7 * grad_coeff1_1);
            u2_value_coeff[q * VS + lane] = s_t(0);
            u2_grad_coeff_ref[(q * ND + 0) * VS + lane] = qw * (adj2 * grad_coeff2_2);
            u2_grad_coeff_ref[(q * ND + 1) * VS + lane] = qw * (adj5 * grad_coeff2_2);
            u2_grad_coeff_ref[(q * ND + 2) * VS + lane] = qw * (adj8 * grad_coeff2_2);
