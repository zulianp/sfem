...
extern "C" int navier_stokes_form_2_u_u_hex27_jacobian_action_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::navier_stokes_form_2_u_u_hex27_jacobian_action_isoparametric_mesh_mixed_impl<double>(nelements, nnodes, elements, points, convection_scale, dt, nu, rho, previous_stride, (const double *const *)u_old_data, direction_stride, (const double *const *)u_direction_data, out_stride, (double *const *)u_out);
    }
    case (int)sizeof(float): {
        return sfem::codegen::navier_stokes_form_2_u_u_hex27_jacobian_action_isoparametric_mesh_mixed_impl<float>(nelements, nnodes, elements, points, convection_scale, dt, nu, rho, previous_stride, (const float *const *)u_old_data, direction_stride, (const float *const *)u_direction_data, out_stride, (float *const *)u_out);
    }
    default:
      break;
  }
