...
extern "C" const sfem::codegen::KernelDiagnostics *cu_navier_stokes_hex27_hex8_residual_esoa_diagnostics(void) {
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_navier_stokes_hex27_hex8_jacobian_action_esoa_diagnostics(void) {
...
__global__ void navier_stokes_hex27_hex8_residual_affine_mesh_mixed_impl(
...
__global__ void navier_stokes_hex27_hex8_residual_isoparametric_mesh_mixed_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
...
template <typename s_t, typename g_t>
__global__ void navier_stokes_hex27_hex8_jacobian_action_affine_mesh_mixed_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const RSTR g_adj0,
...
template <typename s_t>
__global__ void navier_stokes_hex27_hex8_jacobian_action_isoparametric_mesh_mixed_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const geom_t *const *const RSTR points,
...
extern "C" int cu_navier_stokes_hex27_hex8_jacobian_action_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::navier_stokes_hex27_hex8_jacobian_action_isoparametric_mesh_mixed_impl<double><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, convection_scale, dt, nu, rho, previous_stride, (const double *const *)u_old_data, (const double *)p_old_data, direction_stride, (const double *const *)u_direction_data, (const double *)p_direction_data, out_stride, (double *const *)u_out, (double *)p_out);
        return sfem::codegen::launch_status("navier_stokes_hex27_hex8_jacobian_action_isoparametric_mesh_mixed_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
