...
static SFEM_INLINE void navier_stokes_d3_tensor_product_mixed_residual_block(
  ...
  s_t previous_p_value[NQ * VS];
  const s_t *const previous_p_streams[P_NS] = {previous[81], previous[82], previous[83], previous[84], previous[85], previous[86], previous[87], previous[88]};
  tensor_evaluate_value<s_t, NQ, P_NS, VS, ND, 1>(
      ne, field_shape_1d[1], previous_p_streams, previous_p_value);
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    for (int lane = 0; lane < ne; ++lane) {
      ...
      const s_t u0 = current_u0_value[q * VS + lane];
      ...
      const s_t u0_old = previous_u0_value[q * VS + lane];
      const s_t u1 = current_u1_value[q * VS + lane];
      ...
      const s_t u1_old = previous_u1_value[q * VS + lane];
      const s_t u2 = current_u2_value[q * VS + lane];
      ...
      const s_t u2_old = previous_u2_value[q * VS + lane];
      const s_t p = current_p_value[q * VS + lane];
      ...
      const s_t p_old = previous_p_value[q * VS + lane];
      ...
      u0_value_coeff[q * VS + lane] = qw * det * value_coeff0;
      u0_grad_coeff_ref[(q * ND + 0) * VS + lane] = qw * (adj0 * grad_coeff0_0 + adj1 * grad_coeff0_1 + adj2 * grad_coeff0_2);
      u0_grad_coeff_ref[(q * ND + 1) * VS + lane] = qw * (adj3 * grad_coeff0_0 + adj4 * grad_coeff0_1 + adj5 * grad_coeff0_2);
      u0_grad_coeff_ref[(q * ND + 2) * VS + lane] = qw * (adj6 * grad_coeff0_0 + adj7 * grad_coeff0_1 + adj8 * grad_coeff0_2);
      u1_value_coeff[q * VS + lane] = qw * det * value_coeff1;
      u1_grad_coeff_ref[(q * ND + 0) * VS + lane] = qw * (adj0 * grad_coeff1_0 + adj1 * grad_coeff1_1 + adj2 * grad_coeff1_2);
      u1_grad_coeff_ref[(q * ND + 1) * VS + lane] = qw * (adj3 * grad_coeff1_0 + adj4 * grad_coeff1_1 + adj5 * grad_coeff1_2);
      u1_grad_coeff_ref[(q * ND + 2) * VS + lane] = qw * (adj6 * grad_coeff1_0 + adj7 * grad_coeff1_1 + adj8 * grad_coeff1_2);
      u2_value_coeff[q * VS + lane] = qw * det * value_coeff2;
      u2_grad_coeff_ref[(q * ND + 0) * VS + lane] = qw * (adj0 * grad_coeff2_0 + adj1 * grad_coeff2_1 + adj2 * grad_coeff2_2);
      u2_grad_coeff_ref[(q * ND + 1) * VS + lane] = qw * (adj3 * grad_coeff2_0 + adj4 * grad_coeff2_1 + adj5 * grad_coeff2_2);
      u2_grad_coeff_ref[(q * ND + 2) * VS + lane] = qw * (adj6 * grad_coeff2_0 + adj7 * grad_coeff2_1 + adj8 * grad_coeff2_2);
      p_value_coeff[q * VS + lane] = qw * det * value_coeff3;
      p_grad_coeff_ref[(q * ND + 0) * VS + lane] = s_t(0);
      p_grad_coeff_ref[(q * ND + 1) * VS + lane] = s_t(0);
      p_grad_coeff_ref[(q * ND + 2) * VS + lane] = s_t(0);
    }
  }
...
static SFEM_INLINE void navier_stokes_d3_tensor_product_mixed_residual_block_contiguous(
  ...
  tensor_evaluate_value_contiguous<s_t, NQ, P_NS, VS, ND, 1>(
      ne, field_shape_1d[1], previous + 81, previous_p_value);
  s_t u0_value_coeff[NQ * VS];
  s_t u0_grad_coeff_ref[NQ * ND * VS];
  s_t u1_value_coeff[NQ * VS];
  s_t u1_grad_coeff_ref[NQ * ND * VS];
