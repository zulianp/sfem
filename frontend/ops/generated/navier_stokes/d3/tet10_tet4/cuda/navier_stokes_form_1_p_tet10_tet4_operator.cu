#include <type_traits>
#include "../../cuda/navier_stokes_form_1_p_d3_simplex_mixed_local.cuh"
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_navier_stokes_form_1_p_tet10_tet4_residual_esoa_diagnostics(void) {
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_navier_stokes_form_1_p_tet10_tet4_jacobian_action_esoa_diagnostics(void) {
...
__global__ void navier_stokes_form_1_p_tet10_tet4_residual_affine_mesh_mixed_impl(
...
extern "C" int cu_navier_stokes_form_1_p_tet10_tet4_residual_a_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("navier_stokes_form_1_p_tet10_tet4_residual_a_msoa", -1, (int)scalar_bytes);
}
...
__global__ void navier_stokes_form_1_p_tet10_tet4_residual_isoparametric_mesh_mixed_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
...
extern "C" int cu_navier_stokes_form_1_p_tet10_tet4_residual_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::navier_stokes_form_1_p_tet10_tet4_residual_isoparametric_mesh_mixed_impl<double><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, current_stride, (const double *const *)u_data, (const double *)p_data, out_stride, (double *const *)u_out, (double *)p_out);
        return sfem::codegen::launch_status("navier_stokes_form_1_p_tet10_tet4_residual_isoparametric_mesh_mixed_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::navier_stokes_form_1_p_tet10_tet4_residual_isoparametric_mesh_mixed_impl<float><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, current_stride, (const float *const *)u_data, (const float *)p_data, out_stride, (float *const *)u_out, (float *)p_out);
        return sfem::codegen::launch_status("navier_stokes_form_1_p_tet10_tet4_residual_isoparametric_mesh_mixed_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("navier_stokes_form_1_p_tet10_tet4_residual_i_msoa", -1, (int)scalar_bytes);
}
