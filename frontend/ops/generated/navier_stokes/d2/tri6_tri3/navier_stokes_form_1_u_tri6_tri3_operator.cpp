...
extern "C" int navier_stokes_form_1_u_tri6_tri3_residual_a_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::navier_stokes_form_1_u_tri6_tri3_residual_affine_mesh_mixed_impl<double, geom_t>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, convection_scale, dt, f0, f1, nu, rho, current_stride, (const double *const *)u_data, (const double *)p_data, previous_stride, (const double *const *)u_old_data, (const double *)p_old_data, out_stride, (double *const *)u_out, (double *)p_out);
    }
    case (int)sizeof(float): {
        return sfem::codegen::navier_stokes_form_1_u_tri6_tri3_residual_affine_mesh_mixed_impl<float, geom_t>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, convection_scale, dt, f0, f1, nu, rho, current_stride, (const float *const *)u_data, (const float *)p_data, previous_stride, (const float *const *)u_old_data, (const float *)p_old_data, out_stride, (float *const *)u_out, (float *)p_out);
    }
    default:
      break;
  }
