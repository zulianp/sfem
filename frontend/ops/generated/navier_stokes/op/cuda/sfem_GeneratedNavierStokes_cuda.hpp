...
namespace sfem {
  class GPUGeneratedNavierStokes final : public Op {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GPUGeneratedNavierStokes(const std::shared_ptr<FunctionSpace> &space);
    ~GPUGeneratedNavierStokes() override;

    const char *name() const override { return "GPUGeneratedNavierStokes"; }
