...
namespace sfem {
  namespace {
    ...
    smesh::block_idx_t block_id_for_domain(const smesh::Mesh &mesh,
      ...
      SFEM_ERROR("GPUGeneratedNavierStokes: mesh block pointer not found in mesh.blocks()\n");
      return 0;
    }
  ...
  class GPUGeneratedNavierStokes::Impl {
  public:
  ...
  std::unique_ptr<Op> GPUGeneratedNavierStokes::create(const std::shared_ptr<FunctionSpace> &space) {
    ...
    if (space->block_size() != expected_block_size) {
      SFEM_ERROR("GPUGeneratedNavierStokes requires block_size=%ld\n",
                       static_cast<long>(expected_block_size));
      return nullptr;
    }
    auto op = std::make_unique<GPUGeneratedNavierStokes>(space);
    op->initialize();
    return op;
  }

  GPUGeneratedNavierStokes::GPUGeneratedNavierStokes(const std::shared_ptr<FunctionSpace> &space)
    : impl_(std::make_unique<Impl>(space)) {}
  GPUGeneratedNavierStokes::~GPUGeneratedNavierStokes() = default;

  ptrdiff_t GPUGeneratedNavierStokes::n_dofs_domain() const { return impl_->space->n_dofs(); }
  ptrdiff_t GPUGeneratedNavierStokes::n_dofs_image() const { return impl_->space->n_dofs(); }

    double GPUGeneratedNavierStokes::flops_value() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    size_t GPUGeneratedNavierStokes::memory_traffic_bytes_value() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    double GPUGeneratedNavierStokes::flops_gradient() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      if (dim == 2) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_navier_stokes_residual_2d_esoa_diagnostics(domain.element_type);
          if (diagnostics) {
      ...
      if (dim == 3) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_navier_stokes_residual_3d_esoa_diagnostics(domain.element_type);
          if (diagnostics) {
            total += impl_->residual_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
          }
        }
    ...
    size_t GPUGeneratedNavierStokes::memory_traffic_bytes_gradient() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    double GPUGeneratedNavierStokes::flops_apply() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      if (dim == 2) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_navier_stokes_jacobian_action_2d_esoa_diagnostics(domain.element_type);
          if (diagnostics) {
      ...
      if (dim == 3) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_navier_stokes_jacobian_action_3d_esoa_diagnostics(domain.element_type);
          if (diagnostics) {
            total += impl_->jacobian_action_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
          }
        }
    ...
    size_t GPUGeneratedNavierStokes::memory_traffic_bytes_apply() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
  ...
  int GPUGeneratedNavierStokes::initialize(const std::vector<std::string> &block_names) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::initialize");
    impl_->domains = std::make_shared<MultiDomainOp>(impl_->space, block_names);
    {
      ...
      if (!dof_graph ||
        ...
        SFEM_ERROR("GPUGeneratedNavierStokes::initialize: the dof graph is malformed; the assembly kernels assume it is not\n");
        return SFEM_FAILURE;
      }
    }
  ...
  int GPUGeneratedNavierStokes::update(const real_t *const x) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::update");
    impl_->current = x;
    return SFEM_SUCCESS;
  }

  int GPUGeneratedNavierStokes::update(const real_t *const previous,
  ...
  int GPUGeneratedNavierStokes::gradient(const real_t *const state, real_t *const out) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::gradient");
    if (!impl_->previous) {
      SFEM_ERROR("GPUGeneratedNavierStokes requires a previous state\n");
      return SFEM_FAILURE;
    }
    impl_->current = state;
    ...
    return impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      if (impl_->residual_uses_affine) {
        auto cache = std::static_pointer_cast<AffineGeometryCache>(
            domain.user_data);
        if (!cache) {
          SFEM_ERROR("GPUGeneratedNavierStokes affine residual requires cached geometry\n");
          return SFEM_FAILURE;
        }
        if (true) {
          if (!cache->jacobian) {
            SFEM_ERROR("GPUGeneratedNavierStokes affine residual requires cached jacobian geometry\n");
            return SFEM_FAILURE;
          }
        ...
        if (false) {
          if (!cache->metric_soa) {
            SFEM_ERROR("GPUGeneratedNavierStokes affine residual requires cached SoA metric geometry\n");
            return SFEM_FAILURE;
          }
        ...
        if (false) {
          if (!cache->metric_aos) {
            SFEM_ERROR("GPUGeneratedNavierStokes affine residual requires cached AoS metric geometry\n");
            return SFEM_FAILURE;
          }
  ...
  int GPUGeneratedNavierStokes::apply(const real_t *const state,
                      const real_t *const direction,
                      real_t *const out) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::apply");
    ...
    return impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      if (impl_->jacobian_action_uses_affine) {
        auto cache = std::static_pointer_cast<AffineGeometryCache>(
            domain.user_data);
        if (!cache) {
          SFEM_ERROR("GPUGeneratedNavierStokes affine jacobian action requires cached geometry\n");
          return SFEM_FAILURE;
        }
        if (true) {
          if (!cache->jacobian) {
            SFEM_ERROR("GPUGeneratedNavierStokes affine jacobian action requires cached jacobian geometry\n");
            return SFEM_FAILURE;
          }
          adjugate = reinterpret_cast<const geom_t *const *>(
              cache->jacobian->jacobian_adjugate_SoA()->data());
        ...
        if (false) {
          if (!cache->metric_soa) {
            SFEM_ERROR("GPUGeneratedNavierStokes affine jacobian action requires cached SoA metric geometry\n");
            return SFEM_FAILURE;
          }
          geom_metric = reinterpret_cast<const geom_t *const *>(
              cache->metric_soa->fff_SoA()->data());
        ...
        if (false) {
          if (!cache->metric_aos) {
            SFEM_ERROR("GPUGeneratedNavierStokes affine jacobian action requires cached AoS metric geometry\n");
            return SFEM_FAILURE;
          }
          geom_metric_aos = reinterpret_cast<const geom_t *>(
              cache->metric_aos->fff_AoS()->data());
      ...
      if (dim == 2) {
        static constexpr ptrdiff_t FIELD_STRIDE = 3;
          ...
          real_t *const RSTR p_out = out + 2;
        if (impl_->jacobian_action_uses_affine) {
          return cu_navier_stokes_jacobian_action_2d_a_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, storage[0], storage[1], storage[4], storage[5], FIELD_STRIDE, (const void *const *)u_old_data, p_old_data, FIELD_STRIDE, (const void *const *)u_direction_data, p_direction_data, FIELD_STRIDE, (void *const *)u_out, p_out, stream);
        }
        return cu_navier_stokes_jacobian_action_2d_i_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), points, storage[0], storage[1], storage[4], storage[5], FIELD_STRIDE, (const void *const *)u_old_data, p_old_data, FIELD_STRIDE, (const void *const *)u_direction_data, p_direction_data, FIELD_STRIDE, (void *const *)u_out, p_out, stream);
      }
      else if (dim == 3) {
        static constexpr ptrdiff_t FIELD_STRIDE = 4;
          const real_t *const RSTR u_old_data[3] = {previous + 0, previous + 1, previous + 2};
          ...
          real_t *const RSTR p_out = out + 3;
        if (impl_->jacobian_action_uses_affine) {
          return cu_navier_stokes_jacobian_action_3d_a_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), adjugate[0], adjugate[1], adjugate[2], adjugate[3], adjugate[4], adjugate[5], adjugate[6], adjugate[7], adjugate[8], determinant, storage[0], storage[1], storage[5], storage[6], FIELD_STRIDE, (const void *const *)u_old_data, p_old_data, FIELD_STRIDE, (const void *const *)u_direction_data, p_direction_data, FIELD_STRIDE, (void *const *)u_out, p_out, stream);
        }
        return cu_navier_stokes_jacobian_action_3d_i_msoa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), element_connectivity(domain), points, storage[0], storage[1], storage[5], storage[6], FIELD_STRIDE, (const void *const *)u_old_data, p_old_data, FIELD_STRIDE, (const void *const *)u_direction_data, p_direction_data, FIELD_STRIDE, (void *const *)u_out, p_out, stream);
      }
  ...
  void GPUGeneratedNavierStokes::set_field(const char *name,
                           const std::shared_ptr<Buffer<real_t>> &values,
                           const int component) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::set_field");
    if (component != 0 || std::strcmp(name, "previous") != 0) {
      SFEM_ERROR("GPUGeneratedNavierStokes supports set_field(\"previous\", buffer, 0)\n");
      return;
    }
  ...
  void GPUGeneratedNavierStokes::set_value_in_block(const std::string &block_name,
                  const std::string &var_name,
                  const real_t value) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::set_value_in_block");
    impl_->domains->set_value_in_block(block_name, var_name, value);
  }

  void GPUGeneratedNavierStokes::set_option(const std::string &name, const bool val) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::set_option");
    ...
    if (matched && val && impl_->domains) {
      ...
      if (cache_affine_geometry(impl_->space,
        ...
        SFEM_ERROR("GPUGeneratedNavierStokes failed to cache affine geometry\n");
      }
    }
  }

#ifdef SFEM_ENABLE_RYAML
  std::shared_ptr<Op> GPUGeneratedNavierStokes::create_from_yaml(const std::shared_ptr<FunctionSpace> &space,
                                                 const ryml::ConstNodeRef             &node) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::create_from_yaml");
    auto ret = std::make_shared<GPUGeneratedNavierStokes>(space);
  ...
  int GPUGeneratedNavierStokes::hessian_crs(const real_t *const state,
              const count_t *const rowptr,
              const idx_t *const colidx,
              real_t *const values) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::hessian_crs");
    return SFEM_FAILURE;
  }

  int GPUGeneratedNavierStokes::hessian_bsr(const real_t *const state,
              const count_t *const rowptr,
              const idx_t *const colidx,
              real_t *const values) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::hessian_bsr");
    return SFEM_FAILURE;
  }


  int GPUGeneratedNavierStokes::value_steps(const real_t *state,
              const real_t *h,
              const int nsteps,
              const real_t *const steps,
              real_t *const out) {
    SFEM_TRACE_SCOPE("GPUGeneratedNavierStokes::value_steps");
    if (nsteps <= 0) {
      return SFEM_SUCCESS;
    }
...
#pragma omp simd reduction(+ : sum)
  ...
  int GPUGeneratedNavierStokes::value(const real_t *state, real_t *const out) {
