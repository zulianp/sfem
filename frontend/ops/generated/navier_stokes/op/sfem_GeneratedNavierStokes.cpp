...
namespace sfem {
    ...
    int GeneratedNavierStokes::initialize(const std::vector<std::string> &block_names) {
        ...
        impl_->domains = std::make_shared<MultiDomainOp>(impl_->space, block_names);
        {
            ...
            if (!dof_graph ||
                ...
                SFEM_ERROR("GeneratedNavierStokes::initialize: the dof graph is malformed; the assembly kernels assume it is not\n");
                return SFEM_FAILURE;
            }
        }
    ...
    int GeneratedNavierStokes::gradient(const real_t *const state, real_t *const out) {
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                ...
                return navier_stokes_residual_2d_isoparametric_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], storage[1], storage[2], storage[3], storage[4], storage[5], FIELD_STRIDE, (const void *const *)u_data, p_data, FIELD_STRIDE, (const void *const *)u_old_data, p_old_data, FIELD_STRIDE, (void *const *)u_out, p_out);
            }
            else if (dim == 3) {
    ...
    int GeneratedNavierStokes::apply(const real_t *const state,
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                ...
                return navier_stokes_jacobian_action_2d_isoparametric_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], storage[1], storage[4], storage[5], FIELD_STRIDE, (const void *const *)u_old_data, p_old_data, FIELD_STRIDE, (const void *const *)u_direction_data, p_direction_data, FIELD_STRIDE, (void *const *)u_out, p_out);
            }
            else if (dim == 3) {
                static constexpr ptrdiff_t FIELD_STRIDE = 4;
                    const real_t *const SFEM_RESTRICT u_old_data[3] = {previous + 0, previous + 1, previous + 2};
                ...
                return navier_stokes_jacobian_action_3d_isoparametric_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], storage[1], storage[5], storage[6], FIELD_STRIDE, (const void *const *)u_old_data, p_old_data, FIELD_STRIDE, (const void *const *)u_direction_data, p_direction_data, FIELD_STRIDE, (void *const *)u_out, p_out);
            }
    ...
    int GeneratedNavierStokes::value_steps(const real_t *state,
                            ...
                            real_t *const out) {
        SFEM_TRACE_SCOPE("GeneratedNavierStokes::value_steps");
        if (nsteps <= 0) {
            return SFEM_SUCCESS;
        }
...
#pragma omp simd reduction(+ : sum)
    ...
    int GeneratedNavierStokes::value(const real_t *state, real_t *const out) {
