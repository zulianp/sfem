...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_objective_steps_a_msoa_impl(
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_objective_steps_a_msoa(
...
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_objective_steps_packed_a_msoa_impl(
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_objective_steps_packed_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return saint_venant_kirchhoff_proteus_hex8_objective_steps_packed_a_msoa_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, nsteps, (const double *)steps, (double *)value);
    }
    case (int)sizeof(float): {
        return saint_venant_kirchhoff_proteus_hex8_objective_steps_packed_a_msoa_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, nsteps, (const float *)steps, (float *)value);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_objective_steps_packed_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_objective_steps_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_objective_steps_i_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_objective_steps_i_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, nsteps, (const double *)steps, (double *)value);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_objective_steps_i_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, nsteps, (const float *)steps, (float *)value);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_objective_steps_i_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_objective_steps_packed_i_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_objective_steps_packed_i_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_objective_steps_packed_i_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_gradient_a_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const RSTR g_adj0,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_gradient_a_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_gradient_a_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_gradient_a_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_gradient_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_gradient_packed_a_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_gradient_packed_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return saint_venant_kirchhoff_proteus_hex8_gradient_packed_a_msoa_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return saint_venant_kirchhoff_proteus_hex8_gradient_packed_a_msoa_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_gradient_packed_a_msoa", -1, (int)scalar_bytes);
}

template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_gradient_packed_two_pass_a_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_gradient_packed_two_pass_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_gradient_packed_two_pass_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_gradient_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_gradient_i_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_gradient_i_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_gradient_i_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_gradient_i_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_gradient_packed_i_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_gradient_packed_i_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_gradient_packed_i_msoa", -1, (int)scalar_bytes);
}

template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_gradient_packed_two_pass_i_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_gradient_packed_two_pass_i_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_gradient_packed_two_pass_i_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_apply_a_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const RSTR g_adj0,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_apply_a_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_apply_a_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_apply_a_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_apply_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_apply_packed_a_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_apply_packed_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return saint_venant_kirchhoff_proteus_hex8_apply_packed_a_msoa_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return saint_venant_kirchhoff_proteus_hex8_apply_packed_a_msoa_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_apply_packed_a_msoa", -1, (int)scalar_bytes);
}

template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_apply_packed_two_pass_a_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_apply_packed_two_pass_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_apply_packed_two_pass_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_apply_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_apply_i_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_apply_i_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_apply_i_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_apply_i_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_apply_packed_i_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_apply_packed_i_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return saint_venant_kirchhoff_proteus_hex8_apply_packed_i_msoa_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return saint_venant_kirchhoff_proteus_hex8_apply_packed_i_msoa_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_apply_packed_i_msoa", -1, (int)scalar_bytes);
}

template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_proteus_hex8_apply_packed_two_pass_i_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_apply_packed_two_pass_i_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_apply_packed_two_pass_i_msoa", -1, (int)scalar_bytes);
}
...
static SFEM_INLINE void saint_venant_kirchhoff_proteus_hex8_hessian_i_msoa_find_cols(
...
template <typename s_t>
static SFEM_INLINE void saint_venant_kirchhoff_proteus_hex8_hessian_i_msoa_scatter_bsr(
  ...
  for (int i = 0; i < NS; ++i) {
    ...
    saint_venant_kirchhoff_proteus_hex8_hessian_i_msoa_find_cols(ev, cols, lenrow, ks);
    for (int j = 0; j < NS; ++j) {
...
static int saint_venant_kirchhoff_proteus_hex8_hessian_i_msoa_assemble_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const *const RSTR points,
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t element = 0; element < nelements; ++element) {
    ...
    if constexpr (FORMAT == 1) {
      saint_venant_kirchhoff_proteus_hex8_hessian_i_msoa_scatter_bsr(ev, element_matrix, rowptr, colidx, values);
    }
  }
...
extern "C" int saint_venant_kirchhoff_proteus_hex8_hessian_bsr_i_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_hessian_i_msoa_assemble_impl<double, geom_t, 1>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, rowptr, colidx, (double *)values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_proteus_hex8_hessian_i_msoa_assemble_impl<float, geom_t, 1>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, rowptr, colidx, (float *)values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_proteus_hex8_hessian_bsr_i_msoa", -1, (int)scalar_bytes);
}
