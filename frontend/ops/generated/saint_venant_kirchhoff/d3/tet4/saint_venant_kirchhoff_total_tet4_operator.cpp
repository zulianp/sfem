...
static int saint_venant_kirchhoff_total_tet4_merit_patch(
        ...
        for (int lane_e = 0; lane_e < ne; ++lane_e) {
          {
            ...
            for (int lane = 0; lane < nsteps; ++lane) {
              ...
              const s_t residual_tmp3 = ((s_t(1) / s_t(2)))*pow_2(residual_tmp2) + ((s_t(1) / s_t(2)))*pow_2(u0_grad_1) + ((s_t(1) / s_t(2)))*pow_2(u2_grad_1);
              const s_t residual_tmp4 = u2_grad_2 + s_t(1);
              const s_t residual_tmp5 = ((s_t(1) / s_t(2)))*pow_2(residual_tmp4) + ((s_t(1) / s_t(2)))*pow_2(u0_grad_2) + ((s_t(1) / s_t(2)))*pow_2(u1_grad_2);
              const s_t residual_tmp6 = lmbda*(residual_tmp1 + residual_tmp3 + residual_tmp5 + (s_t(-3) / s_t(2)));
              const s_t residual_tmp7 = ((s_t(1) / s_t(2)))*u2_grad_0;
              const s_t residual_tmp8 = ((s_t(1) / s_t(2)))*residual_tmp0;
              const s_t residual_tmp9 = ((s_t(1) / s_t(2)))*u1_grad_0;
              const s_t residual_tmp10 = residual_tmp2*residual_tmp9 + residual_tmp7*u2_grad_1 + residual_tmp8*u0_grad_1;
              const s_t residual_tmp11 = s_t(2)*u0_grad_1;
              const s_t residual_tmp12 = residual_tmp4*residual_tmp7 + residual_tmp8*u0_grad_2 + residual_tmp9*u1_grad_2;
              const s_t residual_tmp13 = s_t(2)*u0_grad_2;
              const s_t residual_tmp14 = residual_tmp1 + (s_t(-1) / s_t(2));
              const s_t residual_tmp15 = s_t(2)*residual_tmp0;
              const s_t residual_tmp16 = ((s_t(1) / s_t(2)))*residual_tmp2*u1_grad_2 + ((s_t(1) / s_t(2)))*residual_tmp4*u2_grad_1 + ((s_t(1) / s_t(2)))*u0_grad_1*u0_grad_2;
              const s_t residual_tmp17 = residual_tmp3 + (s_t(-1) / s_t(2));
              const s_t residual_tmp18 = residual_tmp5 + (s_t(-1) / s_t(2));
              const s_t residual_tmp19 = s_t(2)*u1_grad_2;
              const s_t residual_tmp20 = s_t(2)*u1_grad_0;
              const s_t residual_tmp21 = s_t(2)*residual_tmp2;
              const s_t residual_tmp22 = s_t(2)*u2_grad_1;
              const s_t residual_tmp23 = s_t(2)*u2_grad_0;
              const s_t residual_tmp24 = s_t(2)*residual_tmp4;
              const s_t grad_coeff0_0 = mu*(residual_tmp10*residual_tmp11 + residual_tmp12*residual_tmp13 + residual_tmp14*residual_tmp15) + residual_tmp0*residual_tmp6;
              const s_t grad_coeff0_1 = mu*(residual_tmp10*residual_tmp15 + residual_tmp11*residual_tmp17 + residual_tmp13*residual_tmp16) + residual_tmp6*u0_grad_1;
              const s_t grad_coeff0_2 = mu*(residual_tmp11*residual_tmp16 + residual_tmp12*residual_tmp15 + residual_tmp13*residual_tmp18) + residual_tmp6*u0_grad_2;
              const s_t grad_coeff1_0 = mu*(residual_tmp10*residual_tmp21 + residual_tmp12*residual_tmp19 + residual_tmp14*residual_tmp20) + residual_tmp6*u1_grad_0;
              const s_t grad_coeff1_1 = mu*(residual_tmp10*residual_tmp20 + residual_tmp16*residual_tmp19 + residual_tmp17*residual_tmp21) + residual_tmp2*residual_tmp6;
              const s_t grad_coeff1_2 = mu*(residual_tmp12*residual_tmp20 + residual_tmp16*residual_tmp21 + residual_tmp18*residual_tmp19) + residual_tmp6*u1_grad_2;
...
extern "C" int saint_venant_kirchhoff_total_tet4_merit_patch_a_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_total_tet4_merit_patch<double, geom_t, 1, 4, 16>(n_owned_nodes, n2e_ptr, n2e_idx, n2e_local, elements, points, (const double *const *)grad_ref, (const double *)q_weight, lmbda, mu, nsteps, (const double *)steps, (const double *)x, (const double *)h, (const double *)accumulator, (double *)merit);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_total_tet4_merit_patch<float, geom_t, 1, 4, 16>(n_owned_nodes, n2e_ptr, n2e_idx, n2e_local, elements, points, (const float *const *)grad_ref, (const float *)q_weight, lmbda, mu, nsteps, (const float *)steps, (const float *)x, (const float *)h, (const float *)accumulator, (float *)merit);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_total_tet4_merit_patch_a_msoa", -1, (int)scalar_bytes);
}
