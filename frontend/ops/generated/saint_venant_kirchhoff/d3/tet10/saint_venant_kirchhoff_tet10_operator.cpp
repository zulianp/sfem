...
#include "../saint_venant_kirchhoff_d3_simplex_hessian.hpp"
...
static const KernelDiagnostics saint_venant_kirchhoff_tet10_objective_soa_diagnostics_data = {
  "saint_venant_kirchhoff_tet10_objective_soa",
  "TET10",
  3,
  11,
  10,
...
extern "C" const sfem::codegen::KernelDiagnostics *saint_venant_kirchhoff_tet10_objective_soa_diagnostics(void) {
  return &sfem::codegen::saint_venant_kirchhoff_tet10_objective_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_objective_steps_a_msoa_impl(
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
    ...
    saint_venant_kirchhoff_d3_simplex_objective_block<s_t, NQ, NS, VS>(ne, 0, badj0, badj1, badj2, badj3, badj4, badj5, badj6, badj7, badj8, bdet0, affine_grad_ref_x, affine_grad_ref_y, affine_grad_ref_z, affine_q_weight, lmbda, mu, bu_streams, bh_streams, nsteps, steps, nelements, &value[evb]);
  }
...
extern "C" int saint_venant_kirchhoff_tet10_objective_steps_a_msoa(
...
static SFEM_INLINE int saint_venant_kirchhoff_tet10_objective_steps_packed_a_msoa_impl(
...
extern "C" int saint_venant_kirchhoff_tet10_objective_steps_packed_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return saint_venant_kirchhoff_tet10_objective_steps_packed_a_msoa_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, nsteps, (const double *)steps, (double *)value);
    }
    case (int)sizeof(float): {
        return saint_venant_kirchhoff_tet10_objective_steps_packed_a_msoa_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, nsteps, (const float *)steps, (float *)value);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_objective_steps_packed_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_objective_steps_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
    ...
    saint_venant_kirchhoff_d3_simplex_objective_block<s_t, NQ, NS, VS>(ne, VS, badj0, badj1, badj2, badj3, badj4, badj5, badj6, badj7, badj8, bdet0, isoparametric_grad_ref_x, isoparametric_grad_ref_y, isoparametric_grad_ref_z, isoparametric_q_weight, lmbda, mu, bu_streams, bh_streams, nsteps, steps, nelements, &value[evb]);
  }
...
extern "C" int saint_venant_kirchhoff_tet10_objective_steps_i_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_objective_steps_i_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, nsteps, (const double *)steps, (double *)value);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_objective_steps_i_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, nsteps, (const float *)steps, (float *)value);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_objective_steps_i_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_objective_steps_packed_i_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_tet10_objective_steps_packed_i_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return saint_venant_kirchhoff_tet10_objective_steps_packed_i_msoa_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, nsteps, (const double *)steps, (double *)value);
    }
    case (int)sizeof(float): {
        return saint_venant_kirchhoff_tet10_objective_steps_packed_i_msoa_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, nsteps, (const float *)steps, (float *)value);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_objective_steps_packed_i_msoa", -1, (int)scalar_bytes);
}
...
static const KernelDiagnostics saint_venant_kirchhoff_tet10_gradient_soa_diagnostics_data = {
  "saint_venant_kirchhoff_tet10_gradient_soa",
  "TET10",
  3,
  11,
  10,
...
extern "C" const sfem::codegen::KernelDiagnostics *saint_venant_kirchhoff_tet10_gradient_soa_diagnostics(void) {
  return &sfem::codegen::saint_venant_kirchhoff_tet10_gradient_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_gradient_a_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const RSTR g_adj0,
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
    ...
    saint_venant_kirchhoff_d3_simplex_gradient_block<s_t, NQ, NS, VS>(ne, 0, badj0, badj1, badj2, badj3, badj4, badj5, badj6, badj7, badj8, bdet0, affine_grad_ref_x, affine_grad_ref_y, affine_grad_ref_z, affine_q_weight, lmbda, mu, bu_streams, bout_streams);
...
extern "C" int saint_venant_kirchhoff_tet10_gradient_a_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_gradient_a_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_gradient_a_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_gradient_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_gradient_packed_a_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_tet10_gradient_packed_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return saint_venant_kirchhoff_tet10_gradient_packed_a_msoa_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return saint_venant_kirchhoff_tet10_gradient_packed_a_msoa_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_gradient_packed_a_msoa", -1, (int)scalar_bytes);
}

template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_gradient_packed_two_pass_a_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_tet10_gradient_packed_two_pass_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_gradient_packed_two_pass_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_gradient_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
    ...
    saint_venant_kirchhoff_d3_simplex_gradient_block<s_t, NQ, NS, VS>(ne, VS, badj0, badj1, badj2, badj3, badj4, badj5, badj6, badj7, badj8, bdet0, isoparametric_grad_ref_x, isoparametric_grad_ref_y, isoparametric_grad_ref_z, isoparametric_q_weight, lmbda, mu, bu_streams, bout_streams);
...
extern "C" int saint_venant_kirchhoff_tet10_gradient_i_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_gradient_i_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_gradient_i_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_gradient_i_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_gradient_packed_i_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_tet10_gradient_packed_i_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_gradient_packed_i_msoa", -1, (int)scalar_bytes);
}

template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_gradient_packed_two_pass_i_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_tet10_gradient_packed_two_pass_i_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return saint_venant_kirchhoff_tet10_gradient_packed_two_pass_i_msoa_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, n_ghost_entries, n_ghost_reduce_rows, ghost_reduce_ptr, ghost_reduce_idx, ghost_reduce_dest, (double *)ghost_buf, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return saint_venant_kirchhoff_tet10_gradient_packed_two_pass_i_msoa_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, n_ghost_entries, n_ghost_reduce_rows, ghost_reduce_ptr, ghost_reduce_idx, ghost_reduce_dest, (float *)ghost_buf, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_gradient_packed_two_pass_i_msoa", -1, (int)scalar_bytes);
}
...
static const KernelDiagnostics saint_venant_kirchhoff_tet10_apply_soa_diagnostics_data = {
  "saint_venant_kirchhoff_tet10_apply_soa",
  "TET10",
  3,
  11,
  10,
...
extern "C" const sfem::codegen::KernelDiagnostics *saint_venant_kirchhoff_tet10_apply_soa_diagnostics(void) {
  return &sfem::codegen::saint_venant_kirchhoff_tet10_apply_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_apply_a_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const RSTR g_adj0,
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
    ...
    saint_venant_kirchhoff_d3_simplex_apply_block<s_t, NQ, NS, VS>(ne, 0, badj0, badj1, badj2, badj3, badj4, badj5, badj6, badj7, badj8, bdet0, affine_grad_ref_x, affine_grad_ref_y, affine_grad_ref_z, affine_q_weight, lmbda, mu, bu_streams, bh_streams, bout_streams);
...
extern "C" int saint_venant_kirchhoff_tet10_apply_a_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_apply_a_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_apply_a_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_adj4, g_adj5, g_adj6, g_adj7, g_adj8, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_apply_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_apply_packed_a_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_tet10_apply_packed_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_apply_packed_a_msoa", -1, (int)scalar_bytes);
}

template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_apply_packed_two_pass_a_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_tet10_apply_packed_two_pass_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_apply_packed_two_pass_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t, int VS>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_apply_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
    ...
    saint_venant_kirchhoff_d3_simplex_apply_block<s_t, NQ, NS, VS>(ne, VS, badj0, badj1, badj2, badj3, badj4, badj5, badj6, badj7, badj8, bdet0, isoparametric_grad_ref_x, isoparametric_grad_ref_y, isoparametric_grad_ref_z, isoparametric_q_weight, lmbda, mu, bu_streams, bh_streams, bout_streams);
...
extern "C" int saint_venant_kirchhoff_tet10_apply_i_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_apply_i_msoa_impl<double, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, h_stride, (const double *)hx, (const double *)hy, (const double *)hz, out_stride, (double *)outx, (double *)outy, (double *)outz);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_apply_i_msoa_impl<float, geom_t, 16>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, h_stride, (const float *)hx, (const float *)hy, (const float *)hz, out_stride, (float *)outx, (float *)outy, (float *)outz);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_apply_i_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_apply_packed_i_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_tet10_apply_packed_i_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_apply_packed_i_msoa", -1, (int)scalar_bytes);
}

template <typename s_t>
static SFEM_INLINE int saint_venant_kirchhoff_tet10_apply_packed_two_pass_i_msoa_impl(
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
    const ptrdiff_t,
...
extern "C" int saint_venant_kirchhoff_tet10_apply_packed_two_pass_i_msoa(
    const int scalar_bytes,
    const ptrdiff_t n_packs,
    const ptrdiff_t n_elements_per_pack,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_apply_packed_two_pass_i_msoa", -1, (int)scalar_bytes);
}
...
static SFEM_INLINE void saint_venant_kirchhoff_tet10_hessian_i_msoa_find_cols(
...
template <typename s_t>
static SFEM_INLINE void saint_venant_kirchhoff_tet10_hessian_i_msoa_scatter_bsr(
  ...
  for (int i = 0; i < NS; ++i) {
    ...
    saint_venant_kirchhoff_tet10_hessian_i_msoa_find_cols(ev, cols, lenrow, ks);
    for (int j = 0; j < NS; ++j) {
...
static int saint_venant_kirchhoff_tet10_hessian_i_msoa_assemble_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const *const RSTR points,
...
extern "C" int saint_venant_kirchhoff_tet10_hessian_bsr_i_msoa(
        const int scalar_bytes,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_hessian_i_msoa_assemble_impl<double, geom_t, 1>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (const double *)uz, rowptr, colidx, (double *)values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_tet10_hessian_i_msoa_assemble_impl<float, geom_t, 1>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (const float *)uz, rowptr, colidx, (float *)values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_tet10_hessian_bsr_i_msoa", -1, (int)scalar_bytes);
}
