...
static SFEM_INLINE void saint_venant_kirchhoff_d3_simplex_objective_block(
  ...
  static_assert(VS > 0, "VS must be positive");
    ...
    value[step * value_stride + lane] += qw * det_lane0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(weak_obj_tmp0 + weak_obj_tmp1 + weak_obj_tmp2) + mu*(pow_2(weak_obj_tmp0) + pow_2(weak_obj_tmp1) + pow_2(weak_obj_tmp2) + s_t(2)*pow_2(gu2*weak_obj_tmp3 + gu5*weak_obj_tmp4 + weak_obj_tmp5*weak_obj_tmp6) + s_t(2)*pow_2(((s_t(1) / s_t(2)))*gu2*weak_obj_tmp7 + ((s_t(1) / s_t(2)))*gu3*gu5 + ((s_t(1) / s_t(2)))*gu6*weak_obj_tmp5) + s_t(2)*pow_2(gu3*weak_obj_tmp4 + gu6*weak_obj_tmp6 + weak_obj_tmp3*weak_obj_tmp7)));
        }
      }
    }
}
