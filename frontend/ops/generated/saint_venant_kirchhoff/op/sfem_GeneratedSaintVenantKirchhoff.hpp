...
namespace sfem {
  class GeneratedSaintVenantKirchhoff final : public Op, public TimeSteppable {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GeneratedSaintVenantKirchhoff(const std::shared_ptr<FunctionSpace> &space);
    ~GeneratedSaintVenantKirchhoff() override;

    const char *name() const override { return "GeneratedSaintVenantKirchhoff"; }
