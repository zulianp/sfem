...
namespace sfem {
  namespace {
    ...
    smesh::block_idx_t block_id_for_domain(const smesh::Mesh &mesh,
      ...
      SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff: mesh block pointer not found in mesh.blocks()\n");
      return 0;
    }
  ...
  class GPUGeneratedSaintVenantKirchhoff::Impl {
  public:
  ...
  std::unique_ptr<Op> GPUGeneratedSaintVenantKirchhoff::create(const std::shared_ptr<FunctionSpace> &space) {
    ...
    if (space->block_size() != expected_block_size) {
      SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff requires block_size=%ld\n",
                       static_cast<long>(expected_block_size));
      return nullptr;
    }
    auto op = std::make_unique<GPUGeneratedSaintVenantKirchhoff>(space);
    op->initialize();
    return op;
  }

  GPUGeneratedSaintVenantKirchhoff::GPUGeneratedSaintVenantKirchhoff(const std::shared_ptr<FunctionSpace> &space)
    : impl_(std::make_unique<Impl>(space)) {}
  GPUGeneratedSaintVenantKirchhoff::~GPUGeneratedSaintVenantKirchhoff() = default;

  ptrdiff_t GPUGeneratedSaintVenantKirchhoff::n_dofs_domain() const { return impl_->space->n_dofs(); }
  ptrdiff_t GPUGeneratedSaintVenantKirchhoff::n_dofs_image() const { return impl_->space->n_dofs(); }

    double GPUGeneratedSaintVenantKirchhoff::flops_value() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      if (dim == 2) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_saint_venant_kirchhoff_objective_2d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
      ...
      if (dim == 3) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_saint_venant_kirchhoff_objective_3d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
            total += impl_->objective_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
          }
        }
    ...
    size_t GPUGeneratedSaintVenantKirchhoff::memory_traffic_bytes_value() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    double GPUGeneratedSaintVenantKirchhoff::flops_gradient() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      if (dim == 2) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_saint_venant_kirchhoff_gradient_2d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
      ...
      if (dim == 3) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_saint_venant_kirchhoff_gradient_3d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
            total += impl_->gradient_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
          }
        }
    ...
    size_t GPUGeneratedSaintVenantKirchhoff::memory_traffic_bytes_gradient() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    double GPUGeneratedSaintVenantKirchhoff::flops_apply() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      if (dim == 2) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_saint_venant_kirchhoff_apply_2d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
      ...
      if (dim == 3) {
        {
          const sfem::codegen::KernelDiagnostics *const diagnostics = cu_saint_venant_kirchhoff_apply_3d_soa_diagnostics(domain.element_type);
          if (diagnostics) {
            total += impl_->apply_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
          }
        }
    ...
    size_t GPUGeneratedSaintVenantKirchhoff::memory_traffic_bytes_apply() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
  ...
  int GPUGeneratedSaintVenantKirchhoff::initialize(const std::vector<std::string> &block_names) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::initialize");
    impl_->domains = std::make_shared<MultiDomainOp>(impl_->space, block_names);
    {
      ...
      if (!dof_graph ||
        ...
        SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff::initialize: the dof graph is malformed; the assembly kernels assume it is not\n");
        return SFEM_FAILURE;
      }
    }
  ...
  void GPUGeneratedSaintVenantKirchhoff::set_time_scheme(const std::shared_ptr<TimeScheme> &scheme) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::set_time_scheme");
  ...
  int GPUGeneratedSaintVenantKirchhoff::gradient(const real_t *const x, real_t *const out) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::gradient");
    ...
    return impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      const geom_t *determinant = nullptr;
        ...
        if (!cache || !cache->jacobian_soa) {
          SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff affine gradient requires cached geometry\n");
          return SFEM_FAILURE;
        }
        ...
        if (false) {
          if (!cache->jacobian_aos) {
            SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff affine gradient requires cached AoS geometry\n");
            return SFEM_FAILURE;
          }
  ...
  int GPUGeneratedSaintVenantKirchhoff::apply(const real_t *const x,
                      const real_t *const h,
                      real_t *const out) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::apply");
    if (auto term = time_scheme_term(impl_->time_scheme)) {
    ...
    return impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      const geom_t *determinant = nullptr;
        ...
        auto cache = std::static_pointer_cast<AffineGeometryCache>(
            domain.user_data);
        if (!cache || !cache->jacobian_soa) {
          SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff affine hessian action requires cached geometry\n");
          return SFEM_FAILURE;
        }
        adjugate = reinterpret_cast<const geom_t *const *>(
            cache->jacobian_soa->jacobian_adjugate_SoA()->data());
        ...
        if (false) {
          if (!cache->jacobian_aos) {
            SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff affine hessian action requires cached AoS geometry\n");
            return SFEM_FAILURE;
          }
          adjugate_aos = reinterpret_cast<const geom_t *>(
              cache->jacobian_aos->jacobian_adjugate_AoS()->data());
  ...
  int GPUGeneratedSaintVenantKirchhoff::value(const real_t *x, real_t *const out) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::value");
  ...
  int GPUGeneratedSaintVenantKirchhoff::value_steps(const real_t *x,
              const real_t *h,
              const int nsteps,
              const real_t *const steps,
              real_t *const out) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::value_steps");
    auto mesh = impl_->space->mesh_ptr();
    auto points = element_points(mesh);
    if (nsteps <= 0) {
      return SFEM_SUCCESS;
    ...
    return impl_->domains->iterate([&](const OpDomain &domain) {
      ...
      const geom_t *determinant = nullptr;
        ...
        auto cache = std::static_pointer_cast<AffineGeometryCache>(
            domain.user_data);
        if (!cache || !cache->jacobian_soa) {
          SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff affine objective_steps requires cached geometry\n");
          return SFEM_FAILURE;
        }
        adjugate = reinterpret_cast<const geom_t *const *>(
            cache->jacobian_soa->jacobian_adjugate_SoA()->data());
  ...
  int GPUGeneratedSaintVenantKirchhoff::hessian_crs(const real_t *const x,
              const count_t *const rowptr,
              const idx_t *const colidx,
              real_t *const values) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::hessian_crs");
    if (auto term = time_scheme_term(impl_->time_scheme)) {
    ...
    if (!current) {
      SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff::hessian_crs requires a current state\n");
      return SFEM_FAILURE;
    }
    auto mesh = impl_->space->mesh_ptr();
    auto points = element_points(mesh);
  ...
  int GPUGeneratedSaintVenantKirchhoff::hessian_bsr(const real_t *const x,
              const count_t *const rowptr,
              const idx_t *const colidx,
              real_t *const values) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::hessian_bsr");
    if (auto term = time_scheme_term(impl_->time_scheme)) {
    ...
    const real_t *const current = x;
    if (!current) {
      SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff::hessian_bsr requires a current state\n");
      return SFEM_FAILURE;
    }
    auto mesh = impl_->space->mesh_ptr();
    auto points = element_points(mesh);
    return impl_->domains->iterate([&](const OpDomain &domain) {
      const int dim = mesh->spatial_dimension();
      if (dim == 2) {
        SFEM_ERROR("saint_venant_kirchhoff hessian_bsr 2d dispatch was not generated\n");
        return SFEM_FAILURE;
      }
      else if (dim == 3) {
        SFEM_ERROR("saint_venant_kirchhoff hessian_bsr 3d dispatch was not generated\n");
        return SFEM_FAILURE;
      }
  ...
  int GPUGeneratedSaintVenantKirchhoff::hessian_block_diag_sym(const real_t *const x,
                                       real_t *const values) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::hessian_block_diag_sym");
    if (auto term = time_scheme_term(impl_->time_scheme)) {
    ...
    const real_t *const current = x;
    if (!current) {
      SFEM_ERROR("GPUGeneratedSaintVenantKirchhoff::hessian_block_diag_sym requires a current state\n");
      return SFEM_FAILURE;
    }
    auto mesh = impl_->space->mesh_ptr();
    auto points = element_points(mesh);
  ...
  void GPUGeneratedSaintVenantKirchhoff::set_option(const std::string &name, const bool val) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::set_option");
  ...
  void GPUGeneratedSaintVenantKirchhoff::set_value_in_block(const std::string &block_name,
                  const std::string &var_name,
                  const real_t value) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::set_value_in_block");
    impl_->domains->set_value_in_block(block_name, var_name, value);
  }

#ifdef SFEM_ENABLE_RYAML
  std::shared_ptr<Op> GPUGeneratedSaintVenantKirchhoff::create_from_yaml(const std::shared_ptr<FunctionSpace> &space,
                                                 const ryml::ConstNodeRef             &node) {
    SFEM_TRACE_SCOPE("GPUGeneratedSaintVenantKirchhoff::create_from_yaml");
    auto ret = std::make_shared<GPUGeneratedSaintVenantKirchhoff>(space);
