...
namespace sfem {
  class GPUGeneratedSaintVenantKirchhoff final : public Op, public TimeSteppable {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GPUGeneratedSaintVenantKirchhoff(const std::shared_ptr<FunctionSpace> &space);
    ~GPUGeneratedSaintVenantKirchhoff() override;

    const char *name() const override { return "GPUGeneratedSaintVenantKirchhoff"; }
