#ifndef SAINT_VENANT_KIRCHHOFF_D2_TENSOR_PRODUCT_LOCAL_HPP
#define SAINT_VENANT_KIRCHHOFF_D2_TENSOR_PRODUCT_LOCAL_HPP
#include <math.h>
#include <stddef.h>
...
static SFEM_INLINE void saint_venant_kirchhoff_d2_tensor_product_objective_block(
        const int nelems,
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        const scalar_t weak_obj_tmp0 = grad_u[3] + scalar_t(1);
        const scalar_t weak_obj_tmp1 = ((scalar_t(1) / scalar_t(2)))*pow_2(grad_u[1]) + ((scalar_t(1) / scalar_t(2)))*pow_2(weak_obj_tmp0);
        const scalar_t weak_obj_tmp2 = grad_u[0] + scalar_t(1);
        const scalar_t weak_obj_tmp3 = ((scalar_t(1) / scalar_t(2)))*pow_2(grad_u[2]) + ((scalar_t(1) / scalar_t(2)))*pow_2(weak_obj_tmp2);
        value[lane] += qw * jacobian_determinant_lane0 * (((scalar_t(1) / scalar_t(2)))*lmbda*pow_2(weak_obj_tmp1 + weak_obj_tmp3 + scalar_t(-1)) + mu*(pow_2(weak_obj_tmp1 + (scalar_t(-1) / scalar_t(2))) + pow_2(weak_obj_tmp3 + (scalar_t(-1) / scalar_t(2))) + scalar_t(2)*pow_2(((scalar_t(1) / scalar_t(2)))*grad_u[1]*weak_obj_tmp2 + ((scalar_t(1) / scalar_t(2)))*grad_u[2]*weak_obj_tmp0)));
        }
    }
}

template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void saint_venant_kirchhoff_d2_tensor_product_gradient_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        const scalar_t weak_mat_tmp1 = ((scalar_t(1) / scalar_t(2)))*pow_2(grad_u[2]) + ((scalar_t(1) / scalar_t(2)))*pow_2(weak_mat_tmp0);
        const scalar_t weak_mat_tmp2 = grad_u[3] + scalar_t(1);
        const scalar_t weak_mat_tmp3 = ((scalar_t(1) / scalar_t(2)))*pow_2(grad_u[1]) + ((scalar_t(1) / scalar_t(2)))*pow_2(weak_mat_tmp2);
        const scalar_t weak_mat_tmp4 = lmbda*(weak_mat_tmp1 + weak_mat_tmp3 + scalar_t(-1));
        const scalar_t weak_mat_tmp5 = ((scalar_t(1) / scalar_t(2)))*grad_u[1]*weak_mat_tmp0 + ((scalar_t(1) / scalar_t(2)))*grad_u[2]*weak_mat_tmp2;
        const scalar_t weak_mat_tmp6 = scalar_t(2)*grad_u[1];
        ...
        const scalar_t weak_mat_tmp10 = scalar_t(2)*grad_u[2];
        const scalar_t weak_mat_tmp11 = scalar_t(2)*weak_mat_tmp2;
        material[0] = mu*(weak_mat_tmp5*weak_mat_tmp6 + weak_mat_tmp7*weak_mat_tmp8) + weak_mat_tmp0*weak_mat_tmp4;
        material[1] = grad_u[1]*weak_mat_tmp4 + mu*(weak_mat_tmp5*weak_mat_tmp8 + weak_mat_tmp6*weak_mat_tmp9);
        material[2] = grad_u[2]*weak_mat_tmp4 + mu*(weak_mat_tmp10*weak_mat_tmp7 + weak_mat_tmp11*weak_mat_tmp5);
        material[3] = mu*(weak_mat_tmp10*weak_mat_tmp5 + weak_mat_tmp11*weak_mat_tmp9) + weak_mat_tmp2*weak_mat_tmp4;
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void saint_venant_kirchhoff_d2_tensor_product_apply_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            scalar_t loperand[4];
        scalar_t material[4];
        const scalar_t weak_mat_tmp0 = grad_u[1]*grad_u[2];
        const scalar_t weak_mat_tmp1 = grad_u[3] + scalar_t(1);
        ...
        const scalar_t weak_mat_tmp5 = grad_u[1]*weak_mat_tmp2;
        const scalar_t weak_mat_tmp6 = grad_u[2]*weak_mat_tmp1;
        const scalar_t weak_mat_tmp7 = lmbda*weak_mat_tmp5 + mu*(scalar_t(2)*weak_mat_tmp5 + weak_mat_tmp6);
        const scalar_t weak_mat_tmp8 = grad_u[2]*weak_mat_tmp2;
        const scalar_t weak_mat_tmp9 = grad_u[1]*weak_mat_tmp1;
        ...
        const scalar_t weak_mat_tmp12 = pow_2(grad_u[1]);
        const scalar_t weak_mat_tmp13 = pow_2(grad_u[2]);
        ...
        material[0] = trial_grad[0]*(lmbda*weak_mat_tmp11 + mu*(scalar_t(3)*weak_mat_tmp11 + weak_mat_tmp14) + weak_mat_tmp16) + trial_grad[1]*weak_mat_tmp7 + trial_grad[2]*weak_mat_tmp10 + trial_grad[3]*weak_mat_tmp4;
        material[1] = trial_grad[0]*weak_mat_tmp7 + trial_grad[1]*(lmbda*weak_mat_tmp12 + mu*(scalar_t(3)*weak_mat_tmp12 + weak_mat_tmp19) + weak_mat_tmp16) + trial_grad[2]*weak_mat_tmp17 + trial_grad[3]*weak_mat_tmp18;
        material[2] = trial_grad[0]*weak_mat_tmp10 + trial_grad[1]*weak_mat_tmp17 + trial_grad[2]*(lmbda*weak_mat_tmp13 + mu*(scalar_t(3)*weak_mat_tmp13 + weak_mat_tmp19) + weak_mat_tmp16) + trial_grad[3]*weak_mat_tmp20;
        material[3] = trial_grad[0]*weak_mat_tmp4 + trial_grad[1]*weak_mat_tmp18 + trial_grad[2]*weak_mat_tmp20 + trial_grad[3]*(lmbda*weak_mat_tmp15 + mu*(weak_mat_tmp14 + scalar_t(3)*weak_mat_tmp15) + weak_mat_tmp16);
        loperand[0] = qw * (material[0] * jacobian_adjugate_lane0 + material[1] * jacobian_adjugate_lane1);
        loperand[1] = qw * (material[0] * jacobian_adjugate_lane2 + material[1] * jacobian_adjugate_lane3);
        loperand[2] = qw * (material[2] * jacobian_adjugate_lane0 + material[3] * jacobian_adjugate_lane1);
        loperand[3] = qw * (material[2] * jacobian_adjugate_lane2 + material[3] * jacobian_adjugate_lane3);
