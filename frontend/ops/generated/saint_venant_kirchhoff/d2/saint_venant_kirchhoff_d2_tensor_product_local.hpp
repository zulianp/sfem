...
static SFEM_INLINE void saint_venant_kirchhoff_d2_tensor_product_objective_block(
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    value[step * value_stride + lane] += qw * det_lane0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(weak_obj_tmp0 + weak_obj_tmp1) + mu*(pow_2(weak_obj_tmp0) + pow_2(weak_obj_tmp1) + s_t(2)*pow_2(((s_t(1) / s_t(2)))*gu[1]*(gu[0] + s_t(1)) + ((s_t(1) / s_t(2)))*gu[2]*(gu[3] + s_t(1)))));
      }
    }
  }
}
...
static SFEM_INLINE void saint_venant_kirchhoff_d2_tensor_product_gradient_block(
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    const s_t weak_mat_tmp4 = gu[3] + s_t(1);
    const s_t weak_mat_tmp5 = ((s_t(1) / s_t(2)))*gu[1]*weak_mat_tmp0 + ((s_t(1) / s_t(2)))*gu[2]*weak_mat_tmp4;
    const s_t weak_mat_tmp6 = s_t(2)*gu[1];
...
static SFEM_INLINE void saint_venant_kirchhoff_d2_tensor_product_apply_block(
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    const s_t weak_mat_tmp5 = gu[1]*weak_mat_tmp2;
    const s_t weak_mat_tmp6 = gu[2]*weak_mat_tmp1;
    const s_t weak_mat_tmp7 = lmbda*weak_mat_tmp5 + mu*(s_t(2)*weak_mat_tmp5 + weak_mat_tmp6);
    const s_t weak_mat_tmp8 = gu[2]*weak_mat_tmp2;
    const s_t weak_mat_tmp9 = gu[1]*weak_mat_tmp1;
    ...
    const s_t weak_mat_tmp12 = pow_2(gu[1]);
    const s_t weak_mat_tmp13 = pow_2(gu[2]);
    const s_t weak_mat_tmp14 = weak_mat_tmp12 + weak_mat_tmp13 + s_t(-1);
    const s_t weak_mat_tmp15 = lmbda*(((s_t(1) / s_t(2)))*pow_2(gu[0]) + gu[0] + ((s_t(1) / s_t(2)))*pow_2(gu[3]) + gu[3] + ((s_t(1) / s_t(2)))*weak_mat_tmp12 + ((s_t(1) / s_t(2)))*weak_mat_tmp13);
