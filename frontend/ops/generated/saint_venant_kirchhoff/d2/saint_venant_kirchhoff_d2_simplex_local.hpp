...
static SFEM_INLINE void saint_venant_kirchhoff_d2_simplex_objective_block(
  ...
  static_assert(VS > 0, "VS must be positive");
    ...
    value[step * value_stride + lane] += qw * det_lane0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(weak_obj_tmp0 + weak_obj_tmp1) + mu*(pow_2(weak_obj_tmp0) + pow_2(weak_obj_tmp1) + s_t(2)*pow_2(((s_t(1) / s_t(2)))*gu1*(gu0 + s_t(1)) + ((s_t(1) / s_t(2)))*gu2*(gu3 + s_t(1)))));
        }
      }
    }
}
