#ifndef SAINT_VENANT_KIRCHHOFF_D2_SIMPLEX_LOCAL_HPP
#define SAINT_VENANT_KIRCHHOFF_D2_SIMPLEX_LOCAL_HPP
#include <math.h>
#include <stddef.h>
...
static SFEM_INLINE void saint_venant_kirchhoff_d2_simplex_objective_block(
        const int nelems,
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        const scalar_t weak_obj_tmp0 = grad_u3 + scalar_t(1);
        const scalar_t weak_obj_tmp1 = ((scalar_t(1) / scalar_t(2)))*pow_2(grad_u1) + ((scalar_t(1) / scalar_t(2)))*pow_2(weak_obj_tmp0);
        const scalar_t weak_obj_tmp2 = grad_u0 + scalar_t(1);
        const scalar_t weak_obj_tmp3 = ((scalar_t(1) / scalar_t(2)))*pow_2(grad_u2) + ((scalar_t(1) / scalar_t(2)))*pow_2(weak_obj_tmp2);
        value[lane] += qw * jacobian_determinant_lane0 * (((scalar_t(1) / scalar_t(2)))*lmbda*pow_2(weak_obj_tmp1 + weak_obj_tmp3 + scalar_t(-1)) + mu*(pow_2(weak_obj_tmp1 + (scalar_t(-1) / scalar_t(2))) + pow_2(weak_obj_tmp3 + (scalar_t(-1) / scalar_t(2))) + scalar_t(2)*pow_2(((scalar_t(1) / scalar_t(2)))*grad_u1*weak_obj_tmp2 + ((scalar_t(1) / scalar_t(2)))*grad_u2*weak_obj_tmp0)));
            }
        }
}

template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void saint_venant_kirchhoff_d2_simplex_tri3_objective_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void saint_venant_kirchhoff_d2_simplex_gradient_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        const scalar_t weak_mat_tmp1 = ((scalar_t(1) / scalar_t(2)))*pow_2(grad_u2) + ((scalar_t(1) / scalar_t(2)))*pow_2(weak_mat_tmp0);
        const scalar_t weak_mat_tmp2 = grad_u3 + scalar_t(1);
        const scalar_t weak_mat_tmp3 = ((scalar_t(1) / scalar_t(2)))*pow_2(grad_u1) + ((scalar_t(1) / scalar_t(2)))*pow_2(weak_mat_tmp2);
        const scalar_t weak_mat_tmp4 = lmbda*(weak_mat_tmp1 + weak_mat_tmp3 + scalar_t(-1));
        const scalar_t weak_mat_tmp5 = ((scalar_t(1) / scalar_t(2)))*grad_u1*weak_mat_tmp0 + ((scalar_t(1) / scalar_t(2)))*grad_u2*weak_mat_tmp2;
        const scalar_t weak_mat_tmp6 = scalar_t(2)*grad_u1;
        const scalar_t weak_mat_tmp7 = weak_mat_tmp1 + (scalar_t(-1) / scalar_t(2));
        const scalar_t weak_mat_tmp8 = scalar_t(2)*weak_mat_tmp0;
        const scalar_t weak_mat_tmp9 = weak_mat_tmp3 + (scalar_t(-1) / scalar_t(2));
        const scalar_t weak_mat_tmp10 = scalar_t(2)*grad_u2;
        const scalar_t weak_mat_tmp11 = scalar_t(2)*weak_mat_tmp2;
        const scalar_t material0 = mu*(weak_mat_tmp5*weak_mat_tmp6 + weak_mat_tmp7*weak_mat_tmp8) + weak_mat_tmp0*weak_mat_tmp4;
        const scalar_t material1 = grad_u1*weak_mat_tmp4 + mu*(weak_mat_tmp5*weak_mat_tmp8 + weak_mat_tmp6*weak_mat_tmp9);
        const scalar_t material2 = grad_u2*weak_mat_tmp4 + mu*(weak_mat_tmp10*weak_mat_tmp7 + weak_mat_tmp11*weak_mat_tmp5);
        const scalar_t material3 = mu*(weak_mat_tmp10*weak_mat_tmp5 + weak_mat_tmp11*weak_mat_tmp9) + weak_mat_tmp2*weak_mat_tmp4;
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void saint_venant_kirchhoff_d2_simplex_tri3_gradient_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void saint_venant_kirchhoff_d2_simplex_apply_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        const scalar_t weak_mat_tmp0 = grad_u1*grad_u2;
        const scalar_t weak_mat_tmp1 = grad_u3 + scalar_t(1);
        const scalar_t weak_mat_tmp2 = grad_u0 + scalar_t(1);
        const scalar_t weak_mat_tmp3 = weak_mat_tmp1*weak_mat_tmp2;
        const scalar_t weak_mat_tmp4 = lmbda*weak_mat_tmp3 + mu*weak_mat_tmp0;
        const scalar_t weak_mat_tmp5 = grad_u1*weak_mat_tmp2;
        const scalar_t weak_mat_tmp6 = grad_u2*weak_mat_tmp1;
        const scalar_t weak_mat_tmp7 = lmbda*weak_mat_tmp5 + mu*(scalar_t(2)*weak_mat_tmp5 + weak_mat_tmp6);
        const scalar_t weak_mat_tmp8 = grad_u2*weak_mat_tmp2;
        const scalar_t weak_mat_tmp9 = grad_u1*weak_mat_tmp1;
        const scalar_t weak_mat_tmp10 = lmbda*weak_mat_tmp8 + mu*(scalar_t(2)*weak_mat_tmp8 + weak_mat_tmp9);
        const scalar_t weak_mat_tmp11 = pow_2(weak_mat_tmp2);
        const scalar_t weak_mat_tmp12 = pow_2(grad_u1);
        const scalar_t weak_mat_tmp13 = pow_2(grad_u2);
        const scalar_t weak_mat_tmp14 = weak_mat_tmp12 + weak_mat_tmp13 + scalar_t(-1);
        const scalar_t weak_mat_tmp15 = pow_2(weak_mat_tmp1);
        const scalar_t weak_mat_tmp16 = lmbda*(((scalar_t(1) / scalar_t(2)))*weak_mat_tmp11 + ((scalar_t(1) / scalar_t(2)))*weak_mat_tmp12 + ((scalar_t(1) / scalar_t(2)))*weak_mat_tmp13 + ((scalar_t(1) / scalar_t(2)))*weak_mat_tmp15 + scalar_t(-1));
        const scalar_t weak_mat_tmp17 = lmbda*weak_mat_tmp0 + mu*weak_mat_tmp3;
        const scalar_t weak_mat_tmp18 = lmbda*weak_mat_tmp9 + mu*(weak_mat_tmp8 + scalar_t(2)*weak_mat_tmp9);
        const scalar_t weak_mat_tmp19 = weak_mat_tmp11 + weak_mat_tmp15 + scalar_t(-1);
        const scalar_t weak_mat_tmp20 = lmbda*weak_mat_tmp6 + mu*(weak_mat_tmp5 + scalar_t(2)*weak_mat_tmp6);
        const scalar_t material0 = trial_grad0*(lmbda*weak_mat_tmp11 + mu*(scalar_t(3)*weak_mat_tmp11 + weak_mat_tmp14) + weak_mat_tmp16) + trial_grad1*weak_mat_tmp7 + trial_grad2*weak_mat_tmp10 + trial_grad3*weak_mat_tmp4;
        const scalar_t material1 = trial_grad0*weak_mat_tmp7 + trial_grad1*(lmbda*weak_mat_tmp12 + mu*(scalar_t(3)*weak_mat_tmp12 + weak_mat_tmp19) + weak_mat_tmp16) + trial_grad2*weak_mat_tmp17 + trial_grad3*weak_mat_tmp18;
        const scalar_t material2 = trial_grad0*weak_mat_tmp10 + trial_grad1*weak_mat_tmp17 + trial_grad2*(lmbda*weak_mat_tmp13 + mu*(scalar_t(3)*weak_mat_tmp13 + weak_mat_tmp19) + weak_mat_tmp16) + trial_grad3*weak_mat_tmp20;
        const scalar_t material3 = trial_grad0*weak_mat_tmp4 + trial_grad1*weak_mat_tmp18 + trial_grad2*weak_mat_tmp20 + trial_grad3*(lmbda*weak_mat_tmp15 + mu*(weak_mat_tmp14 + scalar_t(3)*weak_mat_tmp15) + weak_mat_tmp16);
        const scalar_t loperand0 = qw * (material0 * jacobian_adjugate_lane0 + material1 * jacobian_adjugate_lane1);
        const scalar_t loperand1 = qw * (material0 * jacobian_adjugate_lane2 + material1 * jacobian_adjugate_lane3);
        const scalar_t loperand2 = qw * (material2 * jacobian_adjugate_lane0 + material3 * jacobian_adjugate_lane1);
        const scalar_t loperand3 = qw * (material2 * jacobian_adjugate_lane2 + material3 * jacobian_adjugate_lane3);
...
template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
static SFEM_INLINE void saint_venant_kirchhoff_d2_simplex_tri3_apply_block(
        const int nelems,
        const ptrdiff_t geometry_stride,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate0,
        const scalar_t *const SFEM_RESTRICT jacobian_adjugate1,
