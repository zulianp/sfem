...
#pragma omp for schedule(static)
        for (ptrdiff_t pack = 0; pack < n_packs; ++pack) {
            ...
            for (ptrdiff_t evbegin = e_start; evbegin < e_end; evbegin += VECTOR_SIZE) {
                ...
                const scalar_t *block_u_streams[N_SHAPE * N_FIELD_COMPONENTS] = {block_u_data[0], block_u_data[1], block_u_data[2], block_u_data[3], block_u_data[4], block_u_data[5], block_u_data[6], block_u_data[7]};
...
template <typename scalar_t>
static SFEM_INLINE void saint_venant_kirchhoff_proteus_quad4_hessian_isoparametric_mesh_soa_scatter_bsr(
...
#pragma omp parallel for schedule(static) reduction(|:unsupported_matrix_format)
    for (ptrdiff_t element = 0; element < nelements; ++element) {
        ...
        if constexpr (FORMAT == 1) {
            saint_venant_kirchhoff_proteus_quad4_hessian_isoparametric_mesh_soa_scatter_bsr(ev, element_matrix, rowptr, colidx, values);
        } else {
