...
static int saint_venant_kirchhoff_total_tri3_merit_patch(
        ...
        for (int lane_e = 0; lane_e < ne; ++lane_e) {
          {
            ...
            for (int lane = 0; lane < nsteps; ++lane) {
              ...
              const s_t residual_tmp1 = ((s_t(1) / s_t(2)))*pow_2(residual_tmp0) + ((s_t(1) / s_t(2)))*pow_2(u1_grad_0);
              const s_t residual_tmp2 = u1_grad_1 + s_t(1);
              const s_t residual_tmp3 = ((s_t(1) / s_t(2)))*pow_2(residual_tmp2) + ((s_t(1) / s_t(2)))*pow_2(u0_grad_1);
              const s_t residual_tmp4 = lmbda*(residual_tmp1 + residual_tmp3 + s_t(-1));
              const s_t residual_tmp5 = ((s_t(1) / s_t(2)))*residual_tmp0*u0_grad_1 + ((s_t(1) / s_t(2)))*residual_tmp2*u1_grad_0;
              const s_t residual_tmp6 = s_t(2)*u0_grad_1;
              const s_t residual_tmp7 = residual_tmp1 + (s_t(-1) / s_t(2));
              const s_t residual_tmp8 = s_t(2)*residual_tmp0;
              const s_t residual_tmp9 = residual_tmp3 + (s_t(-1) / s_t(2));
              const s_t residual_tmp10 = s_t(2)*u1_grad_0;
              const s_t residual_tmp11 = s_t(2)*residual_tmp2;
              const s_t grad_coeff0_0 = mu*(residual_tmp5*residual_tmp6 + residual_tmp7*residual_tmp8) + residual_tmp0*residual_tmp4;
              const s_t grad_coeff0_1 = mu*(residual_tmp5*residual_tmp8 + residual_tmp6*residual_tmp9) + residual_tmp4*u0_grad_1;
              const s_t grad_coeff1_0 = mu*(residual_tmp10*residual_tmp7 + residual_tmp11*residual_tmp5) + residual_tmp4*u1_grad_0;
              const s_t grad_coeff1_1 = mu*(residual_tmp10*residual_tmp5 + residual_tmp11*residual_tmp9) + residual_tmp2*residual_tmp4;
...
extern "C" int saint_venant_kirchhoff_total_tri3_merit_patch_a_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::saint_venant_kirchhoff_total_tri3_merit_patch<double, geom_t, 1, 3, 16>(n_owned_nodes, n2e_ptr, n2e_idx, n2e_local, elements, points, (const double *const *)grad_ref, (const double *)q_weight, lmbda, mu, nsteps, (const double *)steps, (const double *)x, (const double *)h, (const double *)accumulator, (double *)merit);
    }
    case (int)sizeof(float): {
        return sfem::codegen::saint_venant_kirchhoff_total_tri3_merit_patch<float, geom_t, 1, 3, 16>(n_owned_nodes, n2e_ptr, n2e_idx, n2e_local, elements, points, (const float *const *)grad_ref, (const float *)q_weight, lmbda, mu, nsteps, (const float *)steps, (const float *)x, (const float *)h, (const float *)accumulator, (float *)merit);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("saint_venant_kirchhoff_total_tri3_merit_patch_a_msoa", -1, (int)scalar_bytes);
}
