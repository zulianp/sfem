...
#include "sfem_GeneratedNeumannGeneral_cuda_c_abi.hpp"
...
namespace sfem {
  namespace {
    ...
    smesh::block_idx_t block_id_for_domain(const smesh::Mesh &mesh,
      ...
      SFEM_ERROR("GPUGeneratedNeumannGeneral: mesh block pointer not found in mesh.blocks()\n");
      return 0;
    }
    ...
    std::shared_ptr<smesh::Sideset> sideset_from_yaml(
      ...
      if (!is_sideset && node.has_child("type")) {
        SFEM_ERROR("GPUGeneratedNeumannGeneral neumann condition requires type=sideset\n");
        return nullptr;
      }

      if (is_file || node.has_child("path")) {
        if (!node.has_child("path")) {
          SFEM_ERROR("GPUGeneratedNeumannGeneral file sideset condition requires path\n");
          return nullptr;
        }
      ...
      if (is_expr || (node.has_child("parent") && node.has_child("lfi"))) {
        if (!node["parent"].is_seq() || !node["lfi"].is_seq()) {
          SFEM_ERROR("GPUGeneratedNeumannGeneral expr sideset condition requires parent/lfi sequences\n");
          return nullptr;
        }

        const ptrdiff_t size = node["parent"].num_children();
        if (node["lfi"].num_children() != size) {
          SFEM_ERROR("GPUGeneratedNeumannGeneral expr sideset parent/lfi length mismatch\n");
          return nullptr;
        }
      ...
      SFEM_ERROR("GPUGeneratedNeumannGeneral neumann condition requires format=file or format=expr\n");
      return nullptr;
    }
#endif  // SFEM_ENABLE_RYAML
  }  // namespace

  class GPUGeneratedNeumannGeneral::Impl {
  public:
  ...
  std::unique_ptr<Op> GPUGeneratedNeumannGeneral::create(const std::shared_ptr<FunctionSpace> &space) {
    ...
    if (space->block_size() != expected_block_size) {
      SFEM_ERROR("GPUGeneratedNeumannGeneral requires block_size=%ld\n",
                       static_cast<long>(expected_block_size));
      return nullptr;
    }
    auto op = std::make_unique<GPUGeneratedNeumannGeneral>(space);
    op->initialize();
    return op;
  }

  GPUGeneratedNeumannGeneral::GPUGeneratedNeumannGeneral(const std::shared_ptr<FunctionSpace> &space)
    : impl_(std::make_unique<Impl>(space)) {}
  GPUGeneratedNeumannGeneral::~GPUGeneratedNeumannGeneral() = default;

  ptrdiff_t GPUGeneratedNeumannGeneral::n_dofs_domain() const { return impl_->space->n_dofs(); }
  ptrdiff_t GPUGeneratedNeumannGeneral::n_dofs_image() const { return impl_->space->n_dofs(); }

    double GPUGeneratedNeumannGeneral::flops_value() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    size_t GPUGeneratedNeumannGeneral::memory_traffic_bytes_value() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    double GPUGeneratedNeumannGeneral::flops_gradient() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    size_t GPUGeneratedNeumannGeneral::memory_traffic_bytes_gradient() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    double GPUGeneratedNeumannGeneral::flops_apply() const {
    double total = 0;
    if (!impl_->domains) {
      return total;
    }
    ...
    size_t GPUGeneratedNeumannGeneral::memory_traffic_bytes_apply() const {
    size_t total = 0;
    if (!impl_->domains) {
      return total;
    }
  ...
  int GPUGeneratedNeumannGeneral::initialize(const std::vector<std::string> &block_names) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeumannGeneral::initialize");
    impl_->domains = std::make_shared<MultiDomainOp>(impl_->space, block_names);
    {
      ...
      if (!dof_graph ||
        ...
        SFEM_ERROR("GPUGeneratedNeumannGeneral::initialize: the dof graph is malformed; the assembly kernels assume it is not\n");
        return SFEM_FAILURE;
      }
    }
  ...
  void GPUGeneratedNeumannGeneral::add_sideset(const std::shared_ptr<smesh::Sideset> &sideset) {
  ...
  void GPUGeneratedNeumannGeneral::add_sideset(const std::shared_ptr<smesh::Sideset> &sideset,
    ...
    SFEM_TRACE_SCOPE("GPUGeneratedNeumannGeneral::add_sideset");
  ...
  void GPUGeneratedNeumannGeneral::add_condition(const NeumannConditions::Condition &condition) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeumannGeneral::add_condition");
    impl_->conditions.push_back(condition);
  }

  int GPUGeneratedNeumannGeneral::gradient(const real_t *const, real_t *const out) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeumannGeneral::gradient");
      ...
      for (const auto &condition : impl_->conditions) {
        ...
        switch (domain.element_type) {
          ...
          real_t *const RSTR u_out[2] = {out + 0, out + 1};
            status |= cu_neumann_general_edgeshell2_boundary_residual_2d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), element_connectivity(domain), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[2], condition.values->data()[3], condition.values->data()[4], condition.values->data()[6], condition.values->data()[7], FIELD_STRIDE, u_out[0], u_out[1], stream);
            break;
          }
                    case smesh::QUAD4: {
            static constexpr ptrdiff_t FIELD_STRIDE = 2;
          ...
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= cu_neumann_general_trishell3_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), element_connectivity(domain), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], condition.values->data()[3], condition.values->data()[4], condition.values->data()[5], condition.values->data()[6], condition.values->data()[7], condition.values->data()[8], condition.values->data()[9], condition.values->data()[10], condition.values->data()[11], FIELD_STRIDE, u_out[0], u_out[1], u_out[2], stream);
            break;
          }
                    case smesh::TET10: {
            static constexpr ptrdiff_t FIELD_STRIDE = 3;
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= cu_neumann_general_trishell6_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), element_connectivity(domain), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], condition.values->data()[3], condition.values->data()[4], condition.values->data()[5], condition.values->data()[6], condition.values->data()[7], condition.values->data()[8], condition.values->data()[9], condition.values->data()[10], condition.values->data()[11], FIELD_STRIDE, u_out[0], u_out[1], u_out[2], stream);
            break;
          }
                    case smesh::HEX8: {
            static constexpr ptrdiff_t FIELD_STRIDE = 3;
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= cu_neumann_general_quadshell4_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), element_connectivity(domain), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], condition.values->data()[3], condition.values->data()[4], condition.values->data()[5], condition.values->data()[6], condition.values->data()[7], condition.values->data()[8], condition.values->data()[9], condition.values->data()[10], condition.values->data()[11], FIELD_STRIDE, u_out[0], u_out[1], u_out[2], stream);
            break;
          }
          ...
          real_t *const RSTR u_out[2] = {out + 0, out + 1};
            ...
            static constexpr ptrdiff_t FIELD_STRIDE = 3;
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= cu_neumann_general_proteus_quadshell4_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), element_connectivity(domain), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], condition.values->data()[3], condition.values->data()[4], condition.values->data()[5], condition.values->data()[6], condition.values->data()[7], condition.values->data()[8], condition.values->data()[9], condition.values->data()[10], condition.values->data()[11], FIELD_STRIDE, u_out[0], u_out[1], u_out[2], stream);
            break;
          }
          default:
            SFEM_ERROR("GPUGeneratedNeumannGeneral does not support element type %d\n",
                                   domain.element_type);
            return SFEM_FAILURE;
        }
      }
  ...
  int GPUGeneratedNeumannGeneral::apply(const real_t *const,
                      const real_t *const,
                      real_t *const) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeumannGeneral::apply");
    return SFEM_SUCCESS;
  }
...
#pragma omp parallel for reduction(+ : gx, gh)
  ...
  int GPUGeneratedNeumannGeneral::hessian_crs(const real_t *const,
              ...
              const idx_t *const,
              real_t *const) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeumannGeneral::hessian_crs");
    return SFEM_SUCCESS;
  }

  void GPUGeneratedNeumannGeneral::set_field(const char *,
                           const std::shared_ptr<Buffer<real_t>> &,
                           const int) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeumannGeneral::set_field");
  }

  void GPUGeneratedNeumannGeneral::set_option(const std::string &, const bool) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeumannGeneral::set_option");
  }

  void GPUGeneratedNeumannGeneral::set_value_in_block(const std::string &block_name,
                  const std::string &var_name,
                  const real_t value) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeumannGeneral::set_value_in_block");
    impl_->domains->set_value_in_block(block_name, var_name, value);
  }

#ifdef SFEM_ENABLE_RYAML
  std::shared_ptr<Op> GPUGeneratedNeumannGeneral::create_from_yaml(const std::shared_ptr<FunctionSpace> &space,
                                                 const ryml::ConstNodeRef             &node) {
    SFEM_TRACE_SCOPE("GPUGeneratedNeumannGeneral::create_from_yaml");
    auto ret = std::make_shared<GPUGeneratedNeumannGeneral>(space);
