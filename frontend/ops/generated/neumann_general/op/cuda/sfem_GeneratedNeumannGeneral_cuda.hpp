...
namespace sfem {
  class GPUGeneratedNeumannGeneral final : public Op {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GPUGeneratedNeumannGeneral(const std::shared_ptr<FunctionSpace> &space);
    ~GPUGeneratedNeumannGeneral() override;

    const char *name() const override { return "GPUGeneratedNeumannGeneral"; }
