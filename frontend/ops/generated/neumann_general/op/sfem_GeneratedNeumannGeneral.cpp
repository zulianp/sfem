...
namespace sfem {
  ...
  int GeneratedNeumannGeneral::initialize(const std::vector<std::string> &block_names) {
    ...
    impl_->domains = std::make_shared<MultiDomainOp>(impl_->space, block_names);
    {
      ...
      if (!dof_graph ||
        ...
        SFEM_ERROR("GeneratedNeumannGeneral::initialize: the dof graph is malformed; the assembly kernels assume it is not\n");
        return SFEM_FAILURE;
      }
    }
      ...
      for (const auto &condition : impl_->conditions) {
        ...
        switch (domain.element_type) {
          ...
          real_t *const RSTR u_out[2] = {out + 0, out + 1};
            status |= neumann_general_edgeshell2_boundary_residual_2d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), element_connectivity(domain), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[2], condition.values->data()[3], condition.values->data()[4], condition.values->data()[6], condition.values->data()[7], FIELD_STRIDE, u_out[0], u_out[1]);
            break;
          }
                    case smesh::QUAD4: {
            static constexpr ptrdiff_t FIELD_STRIDE = 2;
          ...
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= neumann_general_trishell3_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), element_connectivity(domain), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], condition.values->data()[3], condition.values->data()[4], condition.values->data()[5], condition.values->data()[6], condition.values->data()[7], condition.values->data()[8], condition.values->data()[9], condition.values->data()[10], condition.values->data()[11], FIELD_STRIDE, u_out[0], u_out[1], u_out[2]);
            break;
          }
                    case smesh::TET10: {
            static constexpr ptrdiff_t FIELD_STRIDE = 3;
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= neumann_general_trishell6_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), element_connectivity(domain), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], condition.values->data()[3], condition.values->data()[4], condition.values->data()[5], condition.values->data()[6], condition.values->data()[7], condition.values->data()[8], condition.values->data()[9], condition.values->data()[10], condition.values->data()[11], FIELD_STRIDE, u_out[0], u_out[1], u_out[2]);
            break;
          }
                    case smesh::HEX8: {
            static constexpr ptrdiff_t FIELD_STRIDE = 3;
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= neumann_general_quadshell4_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), element_connectivity(domain), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], condition.values->data()[3], condition.values->data()[4], condition.values->data()[5], condition.values->data()[6], condition.values->data()[7], condition.values->data()[8], condition.values->data()[9], condition.values->data()[10], condition.values->data()[11], FIELD_STRIDE, u_out[0], u_out[1], u_out[2]);
            break;
          }
          ...
          real_t *const RSTR u_out[2] = {out + 0, out + 1};
            ...
            static constexpr ptrdiff_t FIELD_STRIDE = 3;
          real_t *const RSTR u_out[3] = {out + 0, out + 1, out + 2};
            status |= neumann_general_proteus_quadshell4_boundary_residual_3d_ss_soa(domain.element_type, real_type, sideset->size(), mesh->n_nodes(), element_connectivity(domain), sideset->parent()->data(), sideset->lfi()->data(), points, condition.values->data()[0], condition.values->data()[1], condition.values->data()[2], condition.values->data()[3], condition.values->data()[4], condition.values->data()[5], condition.values->data()[6], condition.values->data()[7], condition.values->data()[8], condition.values->data()[9], condition.values->data()[10], condition.values->data()[11], FIELD_STRIDE, u_out[0], u_out[1], u_out[2]);
            break;
          }
          default:
  ...
  int GeneratedNeumannGeneral::value(const real_t *x, real_t *const out) {
...
#pragma omp parallel for reduction(+ : acc)
  ...
  int GeneratedNeumannGeneral::value_steps(const real_t *x,
              const real_t *h,
              const int nsteps,
              const real_t *const steps,
              real_t *const out) {
    SFEM_TRACE_SCOPE("GeneratedNeumannGeneral::value_steps");
    if (nsteps <= 0) {
      return SFEM_SUCCESS;
    }
