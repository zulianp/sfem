...
namespace sfem {
  class GeneratedLinearElasticity final : public Op, public TimeSteppable {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GeneratedLinearElasticity(const std::shared_ptr<FunctionSpace> &space);
    ~GeneratedLinearElasticity() override;

    const char *name() const override { return "GeneratedLinearElasticity"; }
