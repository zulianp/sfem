...
namespace sfem {
    namespace {
        ...
        int cache_affine_geometry(const std::shared_ptr<FunctionSpace> &space, MultiDomainOp &domains) {
            ...
            const bool needs_jacobian_aos = true || true;
            for (auto &entry : domains.domains()) {
    ...
    double GeneratedLinearElasticity::flops_value() const {
        ...
        impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics =
                            linear_elasticity_objective_2d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total +=
            ...
            if (dim == 3) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics =
                            linear_elasticity_objective_3d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total +=
                                impl_->objective_uses_affine
                                        ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements)
    ...
    double GeneratedLinearElasticity::flops_gradient() const {
        ...
        impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics =
                            linear_elasticity_gradient_2d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total +=
    ...
    double GeneratedLinearElasticity::flops_apply() const {
        ...
        impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics =
                            linear_elasticity_apply_2d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total +=
    ...
    int GeneratedLinearElasticity::initialize(const std::vector<std::string> &block_names) {
        ...
        for (auto &entry : impl_->domains->domains()) {
            ...
            if (needs_affine_geometry) {
                ...
                if ((impl_->gradient_uses_affine && true) || (impl_->apply_uses_affine && true)) {
                    cache->jacobian_aos =
                            smesh::JacobianAdjugateAndDeterminant::create_AoS(mesh, smesh::MEMORY_SPACE_HOST, block_id);
                    if (!cache->jacobian_aos) {
                        return SFEM_FAILURE;
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (impl_->gradient_uses_affine && impl_->space->has_packed_mesh()) {
                ...
                if (packed_block >= 0) {
                    ...
                    if (dim == 2) {
                        ...
                        if (impl_->use_packed_two_pass) {
                            return linear_elasticity_gradient_packed_two_pass_3d_affine_mesh_soa(
                                    domain.element_type,
                                    packed->n_packs(packed_block),
                                    packed->n_elements_per_pack(packed_block),
                                    domain.block->n_elements(),
                        ...
                        return linear_elasticity_gradient_packed_3d_affine_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
            ...
            if (!impl_->gradient_uses_affine && impl_->space->has_packed_mesh()) {
                ...
                if (packed_block >= 0) {
                    ...
                    if (dim == 2) {
                        if (impl_->use_packed_two_pass) {
                            return linear_elasticity_gradient_packed_two_pass_2d_isoparametric_mesh_soa(
                                    domain.element_type,
                                    packed->n_packs(packed_block),
                                    packed->n_elements_per_pack(packed_block),
                                    domain.block->n_elements(),
                        ...
                        return linear_elasticity_gradient_packed_2d_isoparametric_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
            ...
            if (dim == 2) {
                if (impl_->gradient_uses_affine) {
                    return linear_elasticity_gradient_2d_affine_mesh_soa(domain.element_type,
                                                                         domain.block->n_elements(),
                                                                         mesh->n_nodes(),
                ...
                return linear_elasticity_gradient_2d_isoparametric_mesh_soa(domain.element_type,
                                                                            domain.block->n_elements(),
                                                                            mesh->n_nodes(),
                                                                            domain.block->elements()->data(),
                                                                            points,
                ...
                if (impl_->gradient_uses_affine) {
                    if (adjugate_aos) {
                        return linear_elasticity_gradient_3d_affine_mesh_soa_aos_unit(
                                domain.element_type,
                                domain.block->n_elements(),
                                mesh->n_nodes(),
                                domain.block->elements()->data(),
                    ...
                    return linear_elasticity_gradient_3d_affine_mesh_soa(domain.element_type,
                                                                         domain.block->n_elements(),
                                                                         mesh->n_nodes(),
                                                                         domain.block->elements()->data(),
                                                                         adjugate[0],
                ...
                return linear_elasticity_gradient_3d_isoparametric_mesh_soa(domain.element_type,
                                                                            domain.block->n_elements(),
                                                                            mesh->n_nodes(),
                                                                            domain.block->elements()->data(),
                                                                            points,
    ...
    int GeneratedLinearElasticity::apply(const real_t *const x, const real_t *const h, real_t *const out) {
            ...
            if (dim == 2) {
                if (impl_->apply_uses_affine) {
                    if (impl_->space->has_packed_mesh()) {
                        ...
                        if (packed_block >= 0) {
                            ...
                            auto ghost_reduce_dest = packed->ghost_reduce_dest(packed_block);
                            if (impl_->use_packed_two_pass) {
                                return linear_elasticity_apply_packed_two_pass_2d_affine_mesh_soa(
                                        domain.element_type,
                                        packed->n_packs(packed_block),
                                        packed->n_elements_per_pack(packed_block),
                                        domain.block->n_elements(),
                    ...
                    return linear_elasticity_apply_2d_affine_mesh_soa(domain.element_type,
                                                                      domain.block->n_elements(),
                                                                      mesh->n_nodes(),
                                                                      domain.block->elements()->data(),
                                                                      adjugate[0],
                ...
                if (impl_->space->has_packed_mesh()) {
                    ...
                    if (packed_block >= 0) {
                        ...
                        return linear_elasticity_apply_packed_2d_isoparametric_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
                ...
                return linear_elasticity_apply_2d_isoparametric_mesh_soa(domain.element_type,
                                                                         domain.block->n_elements(),
                                                                         mesh->n_nodes(),
                                                                         domain.block->elements()->data(),
                                                                         points,
                ...
                if (impl_->apply_uses_affine) {
                    if (impl_->space->has_packed_mesh()) {
                        ...
                        if (packed_block >= 0) {
                            ...
                            auto ghost_reduce_dest = packed->ghost_reduce_dest(packed_block);
                            if (impl_->use_packed_two_pass) {
                                return linear_elasticity_apply_packed_two_pass_3d_affine_mesh_soa(
                                        domain.element_type,
                                        packed->n_packs(packed_block),
                                        packed->n_elements_per_pack(packed_block),
                                        domain.block->n_elements(),
                            ...
                            return linear_elasticity_apply_packed_3d_affine_mesh_soa(
                                    domain.element_type,
                                    packed->n_packs(packed_block),
                                    packed->n_elements_per_pack(packed_block),
                                    domain.block->n_elements(),
                    ...
                    return linear_elasticity_apply_3d_affine_mesh_soa(domain.element_type,
                                                                      domain.block->n_elements(),
                                                                      mesh->n_nodes(),
                                                                      domain.block->elements()->data(),
                                                                      adjugate[0],
                ...
                if (impl_->space->has_packed_mesh()) {
                    ...
                    if (packed_block >= 0) {
                        ...
                        return linear_elasticity_apply_packed_3d_isoparametric_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
                ...
                return linear_elasticity_apply_3d_isoparametric_mesh_soa(domain.element_type,
                                                                         domain.block->n_elements(),
                                                                         mesh->n_nodes(),
                                                                         domain.block->elements()->data(),
                                                                         points,
    ...
    int GeneratedLinearElasticity::value(const real_t *x, real_t *const out) {
        ...
        *out        = 0;
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                if (impl_->objective_uses_affine) {
                    status = linear_elasticity_objective_2d_affine_mesh_soa(domain.element_type,
                                                                            nelements,
                                                                            mesh->n_nodes(),
                                                                            domain.block->elements()->data(),
                                                                            adjugate[0],
                    ...
                    status = linear_elasticity_objective_2d_isoparametric_mesh_soa(domain.element_type,
                                                                                   nelements,
                                                                                   mesh->n_nodes(),
                                                                                   domain.block->elements()->data(),
                                                                                   points,
                ...
                if (impl_->objective_uses_affine) {
                    status = linear_elasticity_objective_3d_affine_mesh_soa(domain.element_type,
                                                                            nelements,
                                                                            mesh->n_nodes(),
                                                                            domain.block->elements()->data(),
                                                                            adjugate[0],
                    ...
                    status = linear_elasticity_objective_3d_isoparametric_mesh_soa(domain.element_type,
                                                                                   nelements,
                                                                                   mesh->n_nodes(),
                                                                                   domain.block->elements()->data(),
                                                                                   points,
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (impl_->objective_uses_affine && impl_->space->has_packed_mesh()) {
                ...
                if (packed_block >= 0) {
                    ...
                    const int dim             = mesh->spatial_dimension();
                    if (dim == 2) {
                        status = linear_elasticity_objective_steps_packed_2d_affine_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
                        ...
                        status = linear_elasticity_objective_steps_packed_3d_affine_mesh_soa(
                                domain.element_type,
                                packed->n_packs(packed_block),
                                packed->n_elements_per_pack(packed_block),
                                domain.block->n_elements(),
            ...
            if (status == SFEM_FAILURE) {
                ...
                if (dim == 2) {
                    if (impl_->objective_uses_affine) {
                        status = linear_elasticity_objective_steps_2d_affine_mesh_soa(
                                domain.element_type,
                                nelements,
                                mesh->n_nodes(),
                                domain.block->elements()->data(),
                        ...
                        status = linear_elasticity_objective_steps_2d_isoparametric_mesh_soa(
                                domain.element_type,
                                nelements,
                                mesh->n_nodes(),
                                domain.block->elements()->data(),
                    ...
                    if (impl_->objective_uses_affine) {
                        ...
                        status = linear_elasticity_objective_steps_3d_isoparametric_mesh_soa(
                                domain.element_type,
                                nelements,
                                mesh->n_nodes(),
                                domain.block->elements()->data(),
