...
namespace sfem {
  class GPUGeneratedLinearElasticity final : public Op, public TimeSteppable {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GPUGeneratedLinearElasticity(const std::shared_ptr<FunctionSpace> &space);
    ~GPUGeneratedLinearElasticity() override;

    const char *name() const override { return "GPUGeneratedLinearElasticity"; }
