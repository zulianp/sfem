...
static int linear_elasticity_total_tri3_merit_patch(
        ...
        for (int lane_e = 0; lane_e < ne; ++lane_e) {
          {
            ...
            for (int lane = 0; lane < nsteps; ++lane) {
              ...
              const s_t residual_tmp2 = ((s_t(1) / s_t(2)))*lmbda*(residual_tmp0 + residual_tmp1);
              const s_t residual_tmp3 = mu*(u0_grad_1 + u1_grad_0);
              const s_t grad_coeff0_0 = mu*residual_tmp0 + residual_tmp2;
              const s_t grad_coeff0_1 = residual_tmp3;
              const s_t grad_coeff1_0 = residual_tmp3;
              const s_t grad_coeff1_1 = mu*residual_tmp1 + residual_tmp2;
...
extern "C" int linear_elasticity_total_tri3_merit_patch_a_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::linear_elasticity_total_tri3_merit_patch<double, geom_t, 1, 3, 16>(n_owned_nodes, n2e_ptr, n2e_idx, n2e_local, elements, points, (const double *const *)grad_ref, (const double *)q_weight, lmbda, mu, nsteps, (const double *)steps, (const double *)x, (const double *)h, (const double *)accumulator, (double *)merit);
    }
    case (int)sizeof(float): {
        return sfem::codegen::linear_elasticity_total_tri3_merit_patch<float, geom_t, 1, 3, 16>(n_owned_nodes, n2e_ptr, n2e_idx, n2e_local, elements, points, (const float *const *)grad_ref, (const float *)q_weight, lmbda, mu, nsteps, (const float *)steps, (const float *)x, (const float *)h, (const float *)accumulator, (float *)merit);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("linear_elasticity_total_tri3_merit_patch_a_msoa", -1, (int)scalar_bytes);
}
