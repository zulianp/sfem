...
static SFEM_INLINE void linear_elasticity_d2_simplex_objective_block(
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        value[lane] += qw * jacobian_determinant_lane0 * (((scalar_t(1) / scalar_t(2)))*lmbda*pow_2(grad_u0 + grad_u3) + mu*(pow_2(grad_u0) + pow_2(grad_u3) + scalar_t(2)*pow_2(((scalar_t(1) / scalar_t(2)))*grad_u1 + ((scalar_t(1) / scalar_t(2)))*grad_u2)));
            }
        }
}
...
static SFEM_INLINE void linear_elasticity_d2_simplex_gradient_block(
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        const scalar_t weak_mat_tmp1 = scalar_t(2)*grad_u3;
        const scalar_t weak_mat_tmp2 = ((scalar_t(1) / scalar_t(2)))*lmbda*(weak_mat_tmp0 + weak_mat_tmp1);
        const scalar_t weak_mat_tmp3 = mu*(grad_u1 + grad_u2);
        const scalar_t material0 = mu*weak_mat_tmp0 + weak_mat_tmp2;
        const scalar_t material1 = weak_mat_tmp3;
        const scalar_t material2 = weak_mat_tmp3;
        const scalar_t material3 = mu*weak_mat_tmp1 + weak_mat_tmp2;
...
static SFEM_INLINE void linear_elasticity_d2_simplex_apply_block(
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        const scalar_t weak_mat_tmp0 = lmbda + scalar_t(2)*mu;
        const scalar_t weak_mat_tmp1 = mu*trial_grad1 + mu*trial_grad2;
        const scalar_t material0 = lmbda*trial_grad3 + trial_grad0*weak_mat_tmp0;
        const scalar_t material1 = weak_mat_tmp1;
        const scalar_t material2 = weak_mat_tmp1;
        const scalar_t material3 = lmbda*trial_grad0 + trial_grad3*weak_mat_tmp0;
        const scalar_t loperand0 = qw * (material0 * jacobian_adjugate_lane0 + material1 * jacobian_adjugate_lane1);
        const scalar_t loperand1 = qw * (material0 * jacobian_adjugate_lane2 + material1 * jacobian_adjugate_lane3);
        const scalar_t loperand2 = qw * (material2 * jacobian_adjugate_lane0 + material3 * jacobian_adjugate_lane1);
        const scalar_t loperand3 = qw * (material2 * jacobian_adjugate_lane2 + material3 * jacobian_adjugate_lane3);
