...
static SFEM_INLINE int linear_elasticity_tet10_energy_element_coords_soa(
    ...
    for (ptrdiff_t evbegin = 0; evbegin < nelements; evbegin += VECTOR_SIZE) {
        ...
        scalar_t block_jacobian_adjugate8[N_QP * VECTOR_SIZE];
        scalar_t block_jacobian_determinant0[N_QP * VECTOR_SIZE];
        const scalar_t *const grad_ref_x = sfem::codegen::linear_elasticity_tet10_isoparametric_reference_data<scalar_t>::grad_ref_x();
        const scalar_t *const grad_ref_y = sfem::codegen::linear_elasticity_tet10_isoparametric_reference_data<scalar_t>::grad_ref_y();
        const scalar_t *const grad_ref_z = sfem::codegen::linear_elasticity_tet10_isoparametric_reference_data<scalar_t>::grad_ref_z();
        for (int q = 0; q < N_QP; ++q) {
