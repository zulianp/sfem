...
static SFEM_INLINE void linear_elasticity_d3_simplex_objective_block(
  ...
  static_assert(VS > 0, "VS must be positive");
    ...
    value[step * value_stride + lane] += qw * det_lane0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(gu0 + gu4 + gu8) + mu*(pow_2(gu0) + pow_2(gu4) + pow_2(gu8) + s_t(2)*pow_2(((s_t(1) / s_t(2)))*gu1 + ((s_t(1) / s_t(2)))*gu3) + s_t(2)*pow_2(((s_t(1) / s_t(2)))*gu2 + ((s_t(1) / s_t(2)))*gu6) + s_t(2)*pow_2(((s_t(1) / s_t(2)))*gu5 + ((s_t(1) / s_t(2)))*gu7)));
        }
      }
    }
}
