...
static SFEM_INLINE void linear_elasticity_d3_tensor_product_objective_block(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        value[lane] += qw * jacobian_determinant_lane0 * (((scalar_t(1) / scalar_t(2)))*lmbda*pow_2(grad_u[0] + grad_u[4] + grad_u[8]) + mu*(pow_2(grad_u[0]) + pow_2(grad_u[4]) + pow_2(grad_u[8]) + scalar_t(2)*pow_2(((scalar_t(1) / scalar_t(2)))*grad_u[1] + ((scalar_t(1) / scalar_t(2)))*grad_u[3]) + scalar_t(2)*pow_2(((scalar_t(1) / scalar_t(2)))*grad_u[2] + ((scalar_t(1) / scalar_t(2)))*grad_u[6]) + scalar_t(2)*pow_2(((scalar_t(1) / scalar_t(2)))*grad_u[5] + ((scalar_t(1) / scalar_t(2)))*grad_u[7])));
        }
    }
}
...
static SFEM_INLINE void linear_elasticity_d3_tensor_product_gradient_block(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        const scalar_t weak_mat_tmp1 = scalar_t(2)*grad_u[4];
        const scalar_t weak_mat_tmp2 = scalar_t(2)*grad_u[8];
        const scalar_t weak_mat_tmp3 = ((scalar_t(1) / scalar_t(2)))*lmbda*(weak_mat_tmp0 + weak_mat_tmp1 + weak_mat_tmp2);
        const scalar_t weak_mat_tmp4 = mu*(grad_u[1] + grad_u[3]);
        const scalar_t weak_mat_tmp5 = mu*(grad_u[2] + grad_u[6]);
        const scalar_t weak_mat_tmp6 = mu*(grad_u[5] + grad_u[7]);
...
static SFEM_INLINE void linear_elasticity_d3_tensor_product_apply_block(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        const scalar_t weak_mat_tmp1 = scalar_t(2)*trial_grad[4];
        const scalar_t weak_mat_tmp2 = scalar_t(2)*trial_grad[8];
        const scalar_t weak_mat_tmp3 = ((scalar_t(1) / scalar_t(2)))*lmbda*(weak_mat_tmp0 + weak_mat_tmp1 + weak_mat_tmp2);
        const scalar_t weak_mat_tmp4 = mu*(trial_grad[1] + trial_grad[3]);
        const scalar_t weak_mat_tmp5 = mu*(trial_grad[2] + trial_grad[6]);
        const scalar_t weak_mat_tmp6 = mu*(trial_grad[5] + trial_grad[7]);
        material[0] = mu*weak_mat_tmp0 + weak_mat_tmp3;
        material[1] = weak_mat_tmp4;
        material[2] = weak_mat_tmp5;
        material[3] = weak_mat_tmp4;
