...
static SFEM_INLINE void linear_elasticity_d3_tensor_product_apply_block(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            scalar_t loperand[9];
        scalar_t material[9];
        const scalar_t weak_mat_tmp0 = lmbda*trial_grad[4];
        const scalar_t weak_mat_tmp1 = lmbda*trial_grad[8];
        const scalar_t weak_mat_tmp2 = lmbda + scalar_t(2)*mu;
        const scalar_t weak_mat_tmp3 = mu*trial_grad[1] + mu*trial_grad[3];
        const scalar_t weak_mat_tmp4 = mu*trial_grad[2] + mu*trial_grad[6];
        const scalar_t weak_mat_tmp5 = lmbda*trial_grad[0];
        const scalar_t weak_mat_tmp6 = mu*trial_grad[5] + mu*trial_grad[7];
        material[0] = trial_grad[0]*weak_mat_tmp2 + weak_mat_tmp0 + weak_mat_tmp1;
        material[1] = weak_mat_tmp3;
        material[2] = weak_mat_tmp4;
        material[3] = weak_mat_tmp3;
        material[4] = trial_grad[4]*weak_mat_tmp2 + weak_mat_tmp1 + weak_mat_tmp5;
        material[5] = weak_mat_tmp6;
        material[6] = weak_mat_tmp4;
        material[7] = weak_mat_tmp6;
        material[8] = trial_grad[8]*weak_mat_tmp2 + weak_mat_tmp0 + weak_mat_tmp5;
        loperand[0] = qw * (material[0] * jacobian_adjugate_lane0 + material[1] * jacobian_adjugate_lane1 + material[2] * jacobian_adjugate_lane2);
        loperand[1] = qw * (material[0] * jacobian_adjugate_lane3 + material[1] * jacobian_adjugate_lane4 + material[2] * jacobian_adjugate_lane5);
        loperand[2] = qw * (material[0] * jacobian_adjugate_lane6 + material[1] * jacobian_adjugate_lane7 + material[2] * jacobian_adjugate_lane8);
        loperand[3] = qw * (material[3] * jacobian_adjugate_lane0 + material[4] * jacobian_adjugate_lane1 + material[5] * jacobian_adjugate_lane2);
