...
static SFEM_INLINE void linear_elasticity_d3_tensor_product_objective_block(
    ...
    for (int q = 0; q < NQ; ++q) {
        ...
        value[lane] += qw * jacobian_determinant_lane0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(grad_u[0] + grad_u[4] + grad_u[8]) + mu*(pow_2(grad_u[0]) + pow_2(grad_u[4]) + pow_2(grad_u[8]) + s_t(2)*pow_2(((s_t(1) / s_t(2)))*grad_u[1] + ((s_t(1) / s_t(2)))*grad_u[3]) + s_t(2)*pow_2(((s_t(1) / s_t(2)))*grad_u[2] + ((s_t(1) / s_t(2)))*grad_u[6]) + s_t(2)*pow_2(((s_t(1) / s_t(2)))*grad_u[5] + ((s_t(1) / s_t(2)))*grad_u[7])));
        }
    }
}
...
static SFEM_INLINE void linear_elasticity_d3_tensor_product_gradient_block(
    ...
    for (int q = 0; q < NQ; ++q) {
        ...
        const s_t weak_mat_tmp1 = s_t(2)*grad_u[4];
        const s_t weak_mat_tmp2 = s_t(2)*grad_u[8];
        const s_t weak_mat_tmp3 = ((s_t(1) / s_t(2)))*lmbda*(weak_mat_tmp0 + weak_mat_tmp1 + weak_mat_tmp2);
        const s_t weak_mat_tmp4 = mu*(grad_u[1] + grad_u[3]);
        const s_t weak_mat_tmp5 = mu*(grad_u[2] + grad_u[6]);
        const s_t weak_mat_tmp6 = mu*(grad_u[5] + grad_u[7]);
