...
#pragma omp parallel for schedule(static)
    for (ptrdiff_t evbegin = 0; evbegin < nelements; evbegin += VECTOR_SIZE) {
        ...
        laplace_d2_simplex_residual_block_contiguous<scalar_t, N_QP, N_SHAPE, VECTOR_SIZE>(nelems, 0, block_affine_geometry_streams[4], block_adjugate, affine_shape, affine_grad_ref_x, affine_grad_ref_y, affine_q_weight, block_current, kappa, block_output);
...
#pragma omp parallel for schedule(static)
    for (ptrdiff_t evbegin = 0; evbegin < nelements; evbegin += VECTOR_SIZE) {
        ...
        laplace_d2_simplex_jacobian_action_block_contiguous<scalar_t, N_QP, N_SHAPE, VECTOR_SIZE>(nelems, 0, block_affine_geometry_streams[4], block_adjugate, affine_shape, affine_grad_ref_x, affine_grad_ref_y, affine_q_weight, block_direction, kappa, block_output);
...
template <typename scalar_t>
static SFEM_INLINE int laplace_tri6_jacobian_action_packed_isoparametric_mesh_soa_impl(
        ...
        for (ptrdiff_t pack = 0; pack < n_packs; ++pack) {
            ...
            for (ptrdiff_t element = e_start; element < e_end; ++element) {
                ...
                for (int shape = 0; shape < N_SHAPE; ++shape) {
                    ...
                    const uint16_t field_packed_node = elements[shape][element];
                    for (int d = 0; d < DIM; ++d) {
                ...
                for (int q = 0; q < N_QP; ++q) {
                    const int lane = 0;
                    const scalar_t J00 = block_coordinates[0][0] * isoparametric_grad_ref_x[q * N_SHAPE + 0] + block_coordinates[2][0] * isoparametric_grad_ref_x[q * N_SHAPE + 1] + block_coordinates[4][0] * isoparametric_grad_ref_x[q * N_SHAPE + 2] + block_coordinates[6][0] * isoparametric_grad_ref_x[q * N_SHAPE + 3] + block_coordinates[8][0] * isoparametric_grad_ref_x[q * N_SHAPE + 4] + block_coordinates[10][0] * isoparametric_grad_ref_x[q * N_SHAPE + 5];
                    const scalar_t J01 = block_coordinates[0][0] * isoparametric_grad_ref_y[q * N_SHAPE + 0] + block_coordinates[2][0] * isoparametric_grad_ref_y[q * N_SHAPE + 1] + block_coordinates[4][0] * isoparametric_grad_ref_y[q * N_SHAPE + 2] + block_coordinates[6][0] * isoparametric_grad_ref_y[q * N_SHAPE + 3] + block_coordinates[8][0] * isoparametric_grad_ref_y[q * N_SHAPE + 4] + block_coordinates[10][0] * isoparametric_grad_ref_y[q * N_SHAPE + 5];
                    const scalar_t J10 = block_coordinates[1][0] * isoparametric_grad_ref_x[q * N_SHAPE + 0] + block_coordinates[3][0] * isoparametric_grad_ref_x[q * N_SHAPE + 1] + block_coordinates[5][0] * isoparametric_grad_ref_x[q * N_SHAPE + 2] + block_coordinates[7][0] * isoparametric_grad_ref_x[q * N_SHAPE + 3] + block_coordinates[9][0] * isoparametric_grad_ref_x[q * N_SHAPE + 4] + block_coordinates[11][0] * isoparametric_grad_ref_x[q * N_SHAPE + 5];
                    const scalar_t J11 = block_coordinates[1][0] * isoparametric_grad_ref_y[q * N_SHAPE + 0] + block_coordinates[3][0] * isoparametric_grad_ref_y[q * N_SHAPE + 1] + block_coordinates[5][0] * isoparametric_grad_ref_y[q * N_SHAPE + 2] + block_coordinates[7][0] * isoparametric_grad_ref_y[q * N_SHAPE + 3] + block_coordinates[9][0] * isoparametric_grad_ref_y[q * N_SHAPE + 4] + block_coordinates[11][0] * isoparametric_grad_ref_y[q * N_SHAPE + 5];
                    geometry_jacobian_adjugate_and_determinant_2<scalar_t>(
                            J00, J01, J10, J11, block_adjugate_streams, block_determinant, q * VECTOR_SIZE + lane);
                }
...
extern "C" int laplace_tri6_jacobian_action_packed_isoparametric_mesh_soa(
    ...
    return sfem::codegen::laplace_tri6_jacobian_action_packed_isoparametric_mesh_soa_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, kappa, direction_stride, u_direction, out_stride, u_out);
}
...
static SFEM_INLINE void laplace_tri6_hessian_crs_isoparametric_mesh_soa_find_cols(
...
template <typename scalar_t>
static SFEM_INLINE int laplace_tri6_hessian_crs_isoparametric_mesh_soa_scatter_crs(
    ...
    for (int i = 0; i < N_SHAPE; ++i) {
        ...
        laplace_tri6_hessian_crs_isoparametric_mesh_soa_find_cols(ev, cols, lenrow, ks);
        for (int j = 0; j < N_SHAPE; ++j) {
            if (ks[j] < 0 || ks[j] >= lenrow || cols[ks[j]] != ev[j]) {
                if (valid_graph) {
                    std::fprintf(stderr, "laplace_tri6_hessian_crs_isoparametric_mesh_soa_scatter_crs missing graph entry (%ld, %ld)\n", (long)ev[i], (long)ev[j]);
                }
...
static SFEM_INLINE idx_t laplace_tri6_hessian_crs_isoparametric_mesh_soa_packed_global_node(
...
template <typename scalar_t>
static SFEM_INLINE int laplace_tri6_hessian_crs_isoparametric_mesh_soa_discover_packed_crs_entries(
        const idx_t *const SFEM_RESTRICT ev,
        const count_t *const SFEM_RESTRICT rowptr,
        const idx_t *const SFEM_RESTRICT colidx,
    ...
    for (int i = 0; i < N_SHAPE; ++i) {
        ...
        for (int j = 0; j < N_SHAPE; ++j) {
            if (ks[j] < 0 || ks[j] >= lenrow || cols[ks[j]] != ev[j]) {
                if (valid_graph) {
                    std::fprintf(stderr, "laplace_tri6_hessian_crs_isoparametric_mesh_soa_discover_packed_crs_entries missing graph entry (%ld, %ld)\n", (long)ev[i], (long)ev[j]);
                }
                entries[i * N_SHAPE + j] = row_begin;
                valid_graph = false;
            } else {
...
template <typename scalar_t>
static SFEM_INLINE void laplace_tri6_hessian_crs_isoparametric_mesh_soa_scatter_packed_crs_entries(
        const scalar_t *const SFEM_RESTRICT element_matrix,
...
template <typename scalar_t>
static SFEM_INLINE int laplace_tri6_hessian_crs_isoparametric_mesh_soa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const SFEM_RESTRICT elements,
        const geom_t *const *const SFEM_RESTRICT points,
...
#pragma omp parallel for schedule(static) reduction(|:invalid_matrix_graph)
    for (ptrdiff_t element = 0; element < nelements; ++element) {
        ...
        invalid_matrix_graph |= (laplace_tri6_hessian_crs_isoparametric_mesh_soa_scatter_crs(ev, element_matrix, rowptr, colidx, values) != SFEM_SUCCESS);
    }
...
template <typename scalar_t>
static SFEM_INLINE int laplace_tri6_hessian_crs_isoparametric_mesh_soa_packed_discover_impl(
        const ptrdiff_t n_packs,
        const ptrdiff_t n_elements_per_pack,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
...
#pragma omp parallel for schedule(static) reduction(|:invalid_matrix_graph)
    for (ptrdiff_t pack = 0; pack < n_packs; ++pack) {
        ...
        for (ptrdiff_t element = e_start; element < e_end; ++element) {
            idx_t ev[N_SHAPE];
            for (int shape = 0; shape < N_SHAPE; ++shape) {
                ev[shape] = laplace_tri6_hessian_crs_isoparametric_mesh_soa_packed_global_node(elements[shape][element], pack, owned_nodes_ptr, ghost_ptr, ghost_idx);
            }
            count_t *const entries = &packed_element_entries[element * N_SHAPE * N_SHAPE];
            invalid_matrix_graph |= (laplace_tri6_hessian_crs_isoparametric_mesh_soa_discover_packed_crs_entries<scalar_t>(ev, rowptr, colidx, entries) != SFEM_SUCCESS);
        }
    }
    return invalid_matrix_graph ? SFEM_FAILURE : SFEM_SUCCESS;
}
...
template <typename scalar_t>
static SFEM_INLINE int laplace_tri6_hessian_crs_isoparametric_mesh_soa_packed_fill_impl(
        const ptrdiff_t n_packs,
        const ptrdiff_t n_elements_per_pack,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
...
#pragma omp for schedule(static)
        for (ptrdiff_t pack = 0; pack < n_packs; ++pack) {
            ...
            laplace_tri6_hessian_crs_isoparametric_mesh_soa_scatter_packed_crs_entries(element_matrix, entries, values);
            }
        }
    }
    return SFEM_SUCCESS;
...
extern "C" int laplace_tri6_hessian_crs_isoparametric_mesh_soa(
    ...
    return sfem::codegen::laplace_tri6_hessian_crs_isoparametric_mesh_soa_impl<double>(nelements, nnodes, elements, points, kappa, rowptr, colidx, values);
}
...
extern "C" int laplace_tri6_hessian_crs_packed_one_pass_isoparametric_mesh_soa(
    ...
    return sfem::codegen::laplace_tri6_hessian_crs_isoparametric_mesh_soa_packed_fill_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, kappa, packed_element_entries, values);
}
...
extern "C" int laplace_tri6_hessian_crs_packed_two_pass_isoparametric_mesh_soa(
    ...
    const int graph_status = sfem::codegen::laplace_tri6_hessian_crs_isoparametric_mesh_soa_packed_discover_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, rowptr, colidx, packed_element_entries);
...
extern "C" int laplace_tri6_hessian_crs_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::laplace_tri6_hessian_crs_isoparametric_mesh_soa_impl<float>(nelements, nnodes, elements, points, kappa, rowptr, colidx, values);
}
...
extern "C" int laplace_tri6_hessian_crs_packed_one_pass_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::laplace_tri6_hessian_crs_isoparametric_mesh_soa_packed_fill_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, kappa, packed_element_entries, values);
}
...
extern "C" int laplace_tri6_hessian_crs_packed_two_pass_isoparametric_mesh_soa_float(
    ...
    const int graph_status = sfem::codegen::laplace_tri6_hessian_crs_isoparametric_mesh_soa_packed_discover_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, rowptr, colidx, packed_element_entries);
    if (graph_status != SFEM_SUCCESS) return graph_status;
    return sfem::codegen::laplace_tri6_hessian_crs_isoparametric_mesh_soa_packed_fill_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, kappa, packed_element_entries, values);
}
...
template <typename scalar_t>
static SFEM_INLINE void laplace_tri6_hessian_coo_triplet_isoparametric_mesh_soa_scatter_coo_triplets(
        const idx_t *const SFEM_RESTRICT ev,
        const scalar_t *const SFEM_RESTRICT element_matrix,
...
template <typename scalar_t>
static SFEM_INLINE int laplace_tri6_hessian_coo_triplet_isoparametric_mesh_soa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const SFEM_RESTRICT elements,
        const geom_t *const *const SFEM_RESTRICT points,
...
#pragma omp parallel for schedule(static)
    for (ptrdiff_t element = 0; element < nelements; ++element) {
        ...
        laplace_tri6_hessian_coo_triplet_isoparametric_mesh_soa_scatter_coo_triplets(ev, element_matrix, element, rows, cols, values);
    }
...
extern "C" int laplace_tri6_hessian_coo_triplet_isoparametric_mesh_soa(
    ...
    return sfem::codegen::laplace_tri6_hessian_coo_triplet_isoparametric_mesh_soa_impl<double>(nelements, nnodes, elements, points, kappa, rows, cols, values);
}
...
extern "C" int laplace_tri6_hessian_coo_triplet_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::laplace_tri6_hessian_coo_triplet_isoparametric_mesh_soa_impl<float>(nelements, nnodes, elements, points, kappa, rows, cols, values);
}
...
template <typename scalar_t>
static SFEM_INLINE int laplace_tri6_hessian_dia_isoparametric_mesh_soa_scatter_dia(
        const idx_t *const SFEM_RESTRICT ev,
        const scalar_t *const SFEM_RESTRICT element_matrix,
        const ptrdiff_t nnodes,
        ...
        for (int j = 0; j < N_SHAPE; ++j) {
            ...
            if (diagonal == ndiag) {
                if (valid_diagonal_offsets) {
                    std::fprintf(stderr, "laplace_tri6_hessian_dia_isoparametric_mesh_soa_scatter_dia missing diagonal offset %d\n", offset);
                }
...
template <typename scalar_t>
static SFEM_INLINE int laplace_tri6_hessian_dia_isoparametric_mesh_soa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const SFEM_RESTRICT elements,
        const geom_t *const *const SFEM_RESTRICT points,
...
#pragma omp parallel for schedule(static) reduction(|:invalid_matrix_graph)
    for (ptrdiff_t element = 0; element < nelements; ++element) {
        ...
        invalid_matrix_graph |= (laplace_tri6_hessian_dia_isoparametric_mesh_soa_scatter_dia(ev, element_matrix, nnodes, diag_offsets, ndiag, values) != SFEM_SUCCESS);
    }
...
extern "C" int laplace_tri6_hessian_dia_isoparametric_mesh_soa(
    ...
    return sfem::codegen::laplace_tri6_hessian_dia_isoparametric_mesh_soa_impl<double>(nelements, nnodes, elements, points, kappa, diag_offsets, ndiag, values);
}
...
extern "C" int laplace_tri6_hessian_dia_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::laplace_tri6_hessian_dia_isoparametric_mesh_soa_impl<float>(nelements, nnodes, elements, points, kappa, diag_offsets, ndiag, values);
}
