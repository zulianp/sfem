...
static SFEM_INLINE void laplace_quad4_hessian_crs_isoparametric_mesh_soa_find_cols(
...
template <typename scalar_t>
static SFEM_INLINE int laplace_quad4_hessian_crs_isoparametric_mesh_soa_scatter_crs(
    ...
    for (int i = 0; i < N_SHAPE; ++i) {
        ...
        laplace_quad4_hessian_crs_isoparametric_mesh_soa_find_cols(ev, cols, lenrow, ks);
        for (int j = 0; j < N_SHAPE; ++j) {
            if (ks[j] < 0 || ks[j] >= lenrow || cols[ks[j]] != ev[j]) {
                if (valid_graph) {
                    std::fprintf(stderr, "laplace_quad4_hessian_crs_isoparametric_mesh_soa_scatter_crs missing graph entry (%ld, %ld)\n", (long)ev[i], (long)ev[j]);
                }
...
static SFEM_INLINE idx_t laplace_quad4_hessian_crs_isoparametric_mesh_soa_packed_global_node(
...
template <typename scalar_t>
static SFEM_INLINE int laplace_quad4_hessian_crs_isoparametric_mesh_soa_discover_packed_crs_entries(
        const idx_t *const SFEM_RESTRICT ev,
        const count_t *const SFEM_RESTRICT rowptr,
        const idx_t *const SFEM_RESTRICT colidx,
    ...
    for (int i = 0; i < N_SHAPE; ++i) {
        ...
        for (int j = 0; j < N_SHAPE; ++j) {
            if (ks[j] < 0 || ks[j] >= lenrow || cols[ks[j]] != ev[j]) {
                if (valid_graph) {
                    std::fprintf(stderr, "laplace_quad4_hessian_crs_isoparametric_mesh_soa_discover_packed_crs_entries missing graph entry (%ld, %ld)\n", (long)ev[i], (long)ev[j]);
                }
                entries[i * N_SHAPE + j] = row_begin;
                valid_graph = false;
            } else {
...
template <typename scalar_t>
static SFEM_INLINE void laplace_quad4_hessian_crs_isoparametric_mesh_soa_scatter_packed_crs_entries(
        const scalar_t *const SFEM_RESTRICT element_matrix,
...
template <typename scalar_t>
static SFEM_INLINE int laplace_quad4_hessian_crs_isoparametric_mesh_soa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const SFEM_RESTRICT elements,
        const geom_t *const *const SFEM_RESTRICT points,
...
#pragma omp parallel for schedule(static) reduction(|:invalid_matrix_graph)
    for (ptrdiff_t element = 0; element < nelements; ++element) {
        ...
        for (int trial_local = 0; trial_local < 4; ++trial_local) {
            ...
            laplace_d2_tensor_product_jacobian_action_block<scalar_t, N_QP, N_SHAPE, VECTOR_SIZE>(1, 1, block_determinant, block_adjugate, isoparametric_shape_1d, isoparametric_grad_1d, isoparametric_q_weight_1d, block_direction_streams, kappa, block_output_streams);
        ...
        invalid_matrix_graph |= (laplace_quad4_hessian_crs_isoparametric_mesh_soa_scatter_crs(ev, element_matrix, rowptr, colidx, values) != SFEM_SUCCESS);
    }
...
template <typename scalar_t>
static SFEM_INLINE int laplace_quad4_hessian_crs_isoparametric_mesh_soa_packed_discover_impl(
...
#pragma omp parallel for schedule(static) reduction(|:invalid_matrix_graph)
    for (ptrdiff_t pack = 0; pack < n_packs; ++pack) {
        ...
        for (ptrdiff_t element = e_start; element < e_end; ++element) {
            idx_t ev[N_SHAPE];
            for (int shape = 0; shape < N_SHAPE; ++shape) {
                ev[shape] = laplace_quad4_hessian_crs_isoparametric_mesh_soa_packed_global_node(elements[shape][element], pack, owned_nodes_ptr, ghost_ptr, ghost_idx);
            }
            count_t *const entries = &packed_element_entries[element * N_SHAPE * N_SHAPE];
            invalid_matrix_graph |= (laplace_quad4_hessian_crs_isoparametric_mesh_soa_discover_packed_crs_entries<scalar_t>(ev, rowptr, colidx, entries) != SFEM_SUCCESS);
        }
    }
    return invalid_matrix_graph ? SFEM_FAILURE : SFEM_SUCCESS;
}
...
template <typename scalar_t>
static SFEM_INLINE int laplace_quad4_hessian_crs_isoparametric_mesh_soa_packed_fill_impl(
        const ptrdiff_t n_packs,
        const ptrdiff_t n_elements_per_pack,
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
...
#pragma omp for schedule(static)
        ...
        geometry_jacobian_adjugate_and_determinant<scalar_t, DIM, N_QP, VECTOR_SIZE>(
            ...
            laplace_quad4_hessian_crs_isoparametric_mesh_soa_scatter_packed_crs_entries(element_matrix, entries, values);
            }
        }
...
extern "C" int laplace_quad4_hessian_crs_isoparametric_mesh_soa(
    ...
    return sfem::codegen::laplace_quad4_hessian_crs_isoparametric_mesh_soa_impl<double>(nelements, nnodes, elements, points, kappa, rowptr, colidx, values);
}
...
extern "C" int laplace_quad4_hessian_crs_packed_one_pass_isoparametric_mesh_soa(
    ...
    return sfem::codegen::laplace_quad4_hessian_crs_isoparametric_mesh_soa_packed_fill_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, kappa, packed_element_entries, values);
}
...
extern "C" int laplace_quad4_hessian_crs_packed_two_pass_isoparametric_mesh_soa(
    ...
    const int graph_status = sfem::codegen::laplace_quad4_hessian_crs_isoparametric_mesh_soa_packed_discover_impl<double>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, rowptr, colidx, packed_element_entries);
...
extern "C" int laplace_quad4_hessian_crs_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::laplace_quad4_hessian_crs_isoparametric_mesh_soa_impl<float>(nelements, nnodes, elements, points, kappa, rowptr, colidx, values);
}
...
extern "C" int laplace_quad4_hessian_crs_packed_one_pass_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::laplace_quad4_hessian_crs_isoparametric_mesh_soa_packed_fill_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, kappa, packed_element_entries, values);
}
...
extern "C" int laplace_quad4_hessian_crs_packed_two_pass_isoparametric_mesh_soa_float(
    ...
    const int graph_status = sfem::codegen::laplace_quad4_hessian_crs_isoparametric_mesh_soa_packed_discover_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, rowptr, colidx, packed_element_entries);
    if (graph_status != SFEM_SUCCESS) return graph_status;
    return sfem::codegen::laplace_quad4_hessian_crs_isoparametric_mesh_soa_packed_fill_impl<float>(n_packs, n_elements_per_pack, nelements, nnodes, max_nodes_per_pack, elements, owned_nodes_ptr, n_shared_nodes, ghost_ptr, ghost_idx, points, kappa, packed_element_entries, values);
}
...
template <typename scalar_t>
static SFEM_INLINE void laplace_quad4_hessian_coo_triplet_isoparametric_mesh_soa_scatter_coo_triplets(
        const idx_t *const SFEM_RESTRICT ev,
        const scalar_t *const SFEM_RESTRICT element_matrix,
...
template <typename scalar_t>
static SFEM_INLINE int laplace_quad4_hessian_coo_triplet_isoparametric_mesh_soa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const SFEM_RESTRICT elements,
        const geom_t *const *const SFEM_RESTRICT points,
...
#pragma omp parallel for schedule(static)
    for (ptrdiff_t element = 0; element < nelements; ++element) {
        ...
        laplace_quad4_hessian_coo_triplet_isoparametric_mesh_soa_scatter_coo_triplets(ev, element_matrix, element, rows, cols, values);
    }
...
extern "C" int laplace_quad4_hessian_coo_triplet_isoparametric_mesh_soa(
    ...
    return sfem::codegen::laplace_quad4_hessian_coo_triplet_isoparametric_mesh_soa_impl<double>(nelements, nnodes, elements, points, kappa, rows, cols, values);
}
...
extern "C" int laplace_quad4_hessian_coo_triplet_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::laplace_quad4_hessian_coo_triplet_isoparametric_mesh_soa_impl<float>(nelements, nnodes, elements, points, kappa, rows, cols, values);
}
...
template <typename scalar_t>
static SFEM_INLINE int laplace_quad4_hessian_dia_isoparametric_mesh_soa_scatter_dia(
        const idx_t *const SFEM_RESTRICT ev,
        const scalar_t *const SFEM_RESTRICT element_matrix,
        const ptrdiff_t nnodes,
        ...
        for (int j = 0; j < N_SHAPE; ++j) {
            ...
            if (diagonal == ndiag) {
                if (valid_diagonal_offsets) {
                    std::fprintf(stderr, "laplace_quad4_hessian_dia_isoparametric_mesh_soa_scatter_dia missing diagonal offset %d\n", offset);
                }
...
template <typename scalar_t>
static SFEM_INLINE int laplace_quad4_hessian_dia_isoparametric_mesh_soa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t nnodes,
        idx_t **const SFEM_RESTRICT elements,
        const geom_t *const *const SFEM_RESTRICT points,
...
#pragma omp parallel for schedule(static) reduction(|:invalid_matrix_graph)
    for (ptrdiff_t element = 0; element < nelements; ++element) {
        ...
        invalid_matrix_graph |= (laplace_quad4_hessian_dia_isoparametric_mesh_soa_scatter_dia(ev, element_matrix, nnodes, diag_offsets, ndiag, values) != SFEM_SUCCESS);
    }
...
extern "C" int laplace_quad4_hessian_dia_isoparametric_mesh_soa(
    ...
    return sfem::codegen::laplace_quad4_hessian_dia_isoparametric_mesh_soa_impl<double>(nelements, nnodes, elements, points, kappa, diag_offsets, ndiag, values);
}
...
extern "C" int laplace_quad4_hessian_dia_isoparametric_mesh_soa_float(
    ...
    return sfem::codegen::laplace_quad4_hessian_dia_isoparametric_mesh_soa_impl<float>(nelements, nnodes, elements, points, kappa, diag_offsets, ndiag, values);
}
