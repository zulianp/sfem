...
#include "../../cuda/laplace_d2_tensor_product_local.cuh"
...
static const KernelDiagnostics laplace_quad4_objective_soa_diagnostics_data = {
  "laplace_quad4_objective_soa",
  "QUAD4",
  2,
  4,
  4,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_laplace_quad4_objective_soa_diagnostics(void) {
  return &sfem::codegen::laplace_quad4_objective_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
__global__ void laplace_quad4_objective_steps_i_msoa_impl(
...
extern "C" int cu_laplace_quad4_objective_steps_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        ...
        sfem::codegen::laplace_quad4_objective_steps_i_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, kappa, u_stride, (const double *)ux, h_stride, (const double *)hx, nsteps, (const double *)steps, (double *)value);
        return sfem::codegen::launch_status("laplace_quad4_objective_steps_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::laplace_quad4_objective_steps_i_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, kappa, u_stride, (const float *)ux, h_stride, (const float *)hx, nsteps, (const float *)steps, (float *)value);
        return sfem::codegen::launch_status("laplace_quad4_objective_steps_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("laplace_quad4_objective_steps_i_msoa", -1, (int)scalar_bytes);
}
...
static const KernelDiagnostics laplace_quad4_gradient_soa_diagnostics_data = {
  "laplace_quad4_gradient_soa",
  "QUAD4",
  2,
  4,
  4,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_laplace_quad4_gradient_soa_diagnostics(void) {
  return &sfem::codegen::laplace_quad4_gradient_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
__global__ void laplace_quad4_gradient_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
  ...
  for (ptrdiff_t evb = (ptrdiff_t)blockIdx.x * blockDim.x + threadIdx.x; evb < nelements; evb += (ptrdiff_t)blockDim.x * gridDim.x) {
    ...
    const s_t *const bu_streams[NS * NC] = {bu_data[0], bu_data[1], bu_data[3], bu_data[2]};
    s_t *const bout_streams[NS * NC] = {bout_data[0], bout_data[1], bout_data[3], bout_data[2]};
...
extern "C" int cu_laplace_quad4_gradient_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::laplace_quad4_gradient_i_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, kappa, u_stride, (const double *)ux, out_stride, (double *)outx);
        return sfem::codegen::launch_status("laplace_quad4_gradient_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::laplace_quad4_gradient_i_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, kappa, u_stride, (const float *)ux, out_stride, (float *)outx);
        return sfem::codegen::launch_status("laplace_quad4_gradient_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("laplace_quad4_gradient_i_msoa", -1, (int)scalar_bytes);
}
...
static const KernelDiagnostics laplace_quad4_apply_soa_diagnostics_data = {
  "laplace_quad4_apply_soa",
  "QUAD4",
  2,
  4,
  4,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_laplace_quad4_apply_soa_diagnostics(void) {
  return &sfem::codegen::laplace_quad4_apply_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
__global__ void laplace_quad4_apply_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
extern "C" int cu_laplace_quad4_apply_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::laplace_quad4_apply_i_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, kappa, h_stride, (const double *)hx, out_stride, (double *)outx);
        return sfem::codegen::launch_status("laplace_quad4_apply_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::laplace_quad4_apply_i_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, kappa, h_stride, (const float *)hx, out_stride, (float *)outx);
        return sfem::codegen::launch_status("laplace_quad4_apply_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("laplace_quad4_apply_i_msoa", -1, (int)scalar_bytes);
}
