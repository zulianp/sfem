...
namespace sfem {
  class GPUGeneratedLaplace final : public Op, public TimeSteppable {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GPUGeneratedLaplace(const std::shared_ptr<FunctionSpace> &space);
    ~GPUGeneratedLaplace() override;

    const char *name() const override { return "GPUGeneratedLaplace"; }
