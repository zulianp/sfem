...
namespace sfem {
  class GeneratedLaplace final : public Op, public TimeSteppable {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GeneratedLaplace(const std::shared_ptr<FunctionSpace> &space);
    ~GeneratedLaplace() override;

    const char *name() const override { return "GeneratedLaplace"; }
