...
static SFEM_INLINE void laplace_d3_tensor_product_objective_block(
    ...
    tensor_gradient<s_t, NQ, NS, VS, 3, 1>(nelems, shape_1d, grad_1d, u_streams, 0, &grad_u_ref_q[0 * NQ * 3 * VS]);
    for (int q = 0; q < NQ; ++q) {
        const int qx = q % NQ1;
        ...
        value[lane] += qw * jacobian_determinant_lane0 * (((s_t(1) / s_t(2)))*kappa*(pow_2(grad_u[0]) + pow_2(grad_u[1]) + pow_2(grad_u[2])));
        }
    }
}
