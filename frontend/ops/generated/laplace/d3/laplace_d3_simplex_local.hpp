    ...
    static_assert(VS > 0, "VS must be positive");
        for (int q = 0; q < NQ; ++q) {
            ...
            for (int shape = 0; shape < NS; ++shape) {
                ...
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    grad_u_ref2_values[lane] += u_streams[shape * 1 + 0][lane] * grad_ref_z[q * NS + shape];
                }
            }
            #pragma omp simd
            for (int lane = 0; lane < nelems; ++lane) {
        ...
        value[lane] += qw * jacobian_determinant_lane0 * (((s_t(1) / s_t(2)))*kappa*(pow_2(grad_u0) + pow_2(grad_u1) + pow_2(grad_u2)));
            }
        }
}
...
static SFEM_INLINE void laplace_d3_simplex_tet4_objective_block(
    ...
    static_assert(VS > 0, "VS must be positive");
        { const int q = 0;  // constant-P1 simplex
            ...
            const s_t grad_u_ref2 = -(u_streams[0 * 1 + 0][lane]) + u_streams[3 * 1 + 0][lane];
            const s_t inv_jacobian_determinant = s_t(1) / jacobian_determinant_lane0;
            const s_t grad_u0 = (grad_u_ref0 * jacobian_adjugate_lane0 + grad_u_ref1 * jacobian_adjugate_lane3 + grad_u_ref2 * jacobian_adjugate_lane6) * inv_jacobian_determinant;
            const s_t grad_u1 = (grad_u_ref0 * jacobian_adjugate_lane1 + grad_u_ref1 * jacobian_adjugate_lane4 + grad_u_ref2 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
            const s_t grad_u2 = (grad_u_ref0 * jacobian_adjugate_lane2 + grad_u_ref1 * jacobian_adjugate_lane5 + grad_u_ref2 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
...
static SFEM_INLINE void laplace_d3_simplex_tet4_metric_objective_block(
        ...
        const s_t *const SFEM_RESTRICT geom_metric3,
        const s_t *const SFEM_RESTRICT geom_metric4,
        const s_t *const SFEM_RESTRICT geom_metric5,
        const s_t *const SFEM_RESTRICT q_weight,
        const s_t kappa,
        const s_t *const SFEM_RESTRICT u_streams[NS * 1],
        s_t *const SFEM_RESTRICT value
    ...
    static_assert(VS > 0, "VS must be positive");
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            const s_t geom_metric_lane3 = geom_metric3[geometry_offset];
            const s_t geom_metric_lane4 = geom_metric4[geometry_offset];
            const s_t geom_metric_lane5 = geom_metric5[geometry_offset];
...
static SFEM_INLINE void laplace_d3_simplex_gradient_block(
    ...
    static_assert(VS > 0, "VS must be positive");
        ...
        const s_t material2 = grad_u2*kappa;
