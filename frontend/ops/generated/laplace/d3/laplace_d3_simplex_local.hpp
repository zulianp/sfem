...
namespace sfem {
    namespace codegen {
        ...
        static SFEM_INLINE void laplace_d3_simplex_residual_block(const int                           nelems,
...
#pragma omp simd
                    for (int lane = 0; lane < nelems; ++lane) {
                        ...
                        const scalar_t  adj7            = adjugate[7][geometry_offset];
                        const scalar_t  adj8            = adjugate[8][geometry_offset];
                        const scalar_t  test_grad0 =
                                (grad_ref_x[q * N_SHAPE + test] * adj0 + grad_ref_y[q * N_SHAPE + test] * adj3 +
                                 grad_ref_z[q * N_SHAPE + test] * adj6) /
                                det;
                        const scalar_t test_grad1 =
                                (grad_ref_x[q * N_SHAPE + test] * adj1 + grad_ref_y[q * N_SHAPE + test] * adj4 +
                                 grad_ref_z[q * N_SHAPE + test] * adj7) /
                                det;
                        const scalar_t test_grad2 =
                                (grad_ref_x[q * N_SHAPE + test] * adj2 + grad_ref_y[q * N_SHAPE + test] * adj5 +
                                 grad_ref_z[q * N_SHAPE + test] * adj8) /
                                det;
                        output[test * N_FIELDS + 0][lane] +=
                                q_weight[q] * det *
                                (grad_coeff0_0_values[lane] * test_grad0 + grad_coeff0_1_values[lane] * test_grad1 +
                                 grad_coeff0_2_values[lane] * test_grad2);
                    }
                }
            }
        }
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
        static SFEM_INLINE void laplace_d3_simplex_tet4_residual_block(const int                           nelems,
                                                                       const ptrdiff_t                     geometry_stride,
...
#pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    ...
                    const scalar_t  u_metric_grad_0_ref_value = geom_metric00 * u_grad_0_ref_value +
                                                               geom_metric01 * u_grad_1_ref_value +
                                                               geom_metric02 * u_grad_2_ref_value;
                    const scalar_t u_metric_grad_1_ref_value = geom_metric01 * u_grad_0_ref_value +
                                                               geom_metric11 * u_grad_1_ref_value +
                                                               geom_metric12 * u_grad_2_ref_value;
                    const scalar_t u_metric_grad_2_ref_value = geom_metric02 * u_grad_0_ref_value +
                                                               geom_metric12 * u_grad_1_ref_value +
                                                               geom_metric22 * u_grad_2_ref_value;
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
        static SFEM_INLINE void laplace_d3_simplex_jacobian_action_block(const int                           nelems,
                                                                         const ptrdiff_t                     geometry_stride,
                                                                         const scalar_t *const SFEM_RESTRICT determinant,
                                                                         const scalar_t *const SFEM_RESTRICT adjugate[9],
                                                                         const scalar_t *const SFEM_RESTRICT shape,
...
#pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    ...
                    const scalar_t  u_direction_grad_0 =
                            (u_direction_grad_0_ref * adj0 + u_direction_grad_1_ref * adj3 + u_direction_grad_2_ref * adj6) / det;
                    const scalar_t u_direction_grad_1 =
                            (u_direction_grad_0_ref * adj1 + u_direction_grad_1_ref * adj4 + u_direction_grad_2_ref * adj7) / det;
                    const scalar_t u_direction_grad_2 =
                            (u_direction_grad_0_ref * adj2 + u_direction_grad_1_ref * adj5 + u_direction_grad_2_ref * adj8) / det;
...
#pragma omp simd
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE>
        static SFEM_INLINE void laplace_d3_simplex_tet4_jacobian_action_block(const int                           nelems,
                                                                              const ptrdiff_t                     geometry_stride,
                                                                              const scalar_t *const SFEM_RESTRICT geom_metric[6],
                                                                              const scalar_t *const SFEM_RESTRICT q_weight,
                                                                              const scalar_t *const SFEM_RESTRICT
...
#pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    ...
                    const scalar_t  geom_metric12                = (q_weight[q] * (kappa)) * geom_metric[4][geometry_offset];
                    const scalar_t  geom_metric22                = (q_weight[q] * (kappa)) * geom_metric[5][geometry_offset];
                    const scalar_t  u_direction_metric_grad_0_ref_value = geom_metric00 * u_direction_grad_0_ref_value +
                                                                         geom_metric01 * u_direction_grad_1_ref_value +
                                                                         geom_metric02 * u_direction_grad_2_ref_value;
                    const scalar_t u_direction_metric_grad_1_ref_value = geom_metric01 * u_direction_grad_0_ref_value +
                                                                         geom_metric11 * u_direction_grad_1_ref_value +
                                                                         geom_metric12 * u_direction_grad_2_ref_value;
                    const scalar_t u_direction_metric_grad_2_ref_value = geom_metric02 * u_direction_grad_0_ref_value +
                                                                         geom_metric12 * u_direction_grad_1_ref_value +
                                                                         geom_metric22 * u_direction_grad_2_ref_value;
                    output[0][lane] += -(u_direction_metric_grad_0_ref_value)-u_direction_metric_grad_1_ref_value -
                                       u_direction_metric_grad_2_ref_value;
