...
static int laplace_total_tet4_merit_patch(
...
#pragma omp parallel
  {
    ...
    s_t pm_state_grad[NQ * 3 * VS];
    s_t pm_direction_grad[NQ * 3 * VS];
          ...
          for (int lane = 0; lane < ne; ++lane) {
            ...
            for (int c = 0; c < NC; ++c) {
              for (int d = 0; d < ND; ++d) {
                ...
                pm_state_grad[(q * 3 + c * ND + d) * VS + lane] = mapped / det;
              }
            }
            ...
            for (int c = 0; c < NC; ++c) {
              for (int d = 0; d < ND; ++d) {
                ...
                pm_direction_grad[(q * 3 + c * ND + d) * VS + lane] = mapped / det;
              }
            }
        ...
        for (int lane_e = 0; lane_e < ne; ++lane_e) {
          {
            ...
            for (int lane = 0; lane < nsteps; ++lane) {
              ...
              const s_t u_grad_0 = pm_state_grad[(q * 3 + 0) * VS + lane_e] + alpha * pm_direction_grad[(q * 3 + 0) * VS + lane_e];
              const s_t u_grad_1 = pm_state_grad[(q * 3 + 1) * VS + lane_e] + alpha * pm_direction_grad[(q * 3 + 1) * VS + lane_e];
              const s_t u_grad_2 = pm_state_grad[(q * 3 + 2) * VS + lane_e] + alpha * pm_direction_grad[(q * 3 + 2) * VS + lane_e];
              const s_t grad_coeff0_0 = kappa*u_grad_0;
              const s_t grad_coeff0_1 = kappa*u_grad_1;
              const s_t grad_coeff0_2 = kappa*u_grad_2;
...
extern "C" int laplace_total_tet4_merit_patch_a_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::laplace_total_tet4_merit_patch<double, geom_t, 1, 4, 16>(n_owned_nodes, n2e_ptr, n2e_idx, n2e_local, elements, points, (const double *const *)grad_ref, (const double *)q_weight, kappa, nsteps, (const double *)steps, (const double *)x, (const double *)h, (const double *)accumulator, (double *)merit);
    }
    case (int)sizeof(float): {
        return sfem::codegen::laplace_total_tet4_merit_patch<float, geom_t, 1, 4, 16>(n_owned_nodes, n2e_ptr, n2e_idx, n2e_local, elements, points, (const float *const *)grad_ref, (const float *)q_weight, kappa, nsteps, (const float *)steps, (const float *)x, (const float *)h, (const float *)accumulator, (float *)merit);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("laplace_total_tet4_merit_patch_a_msoa", -1, (int)scalar_bytes);
}
