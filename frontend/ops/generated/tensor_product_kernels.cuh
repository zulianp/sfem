#ifndef SFEM_CODEGEN_TENSOR_PRODUCT_KERNELS_CUH
#define SFEM_CODEGEN_TENSOR_PRODUCT_KERNELS_CUH
            ...
            for (int sy = 0; sy < S; ++sy) {
                {
                    ...
                    for (int sx = 0; sx < S; ++sx) {
                        ...
                        const scalar_t u = streams[shape * 2 + component][0];
                        v += u * shape_1d[qx * S + sx];
                        gx += u * grad_1d[qx * S + sx];
                    }
                    const int i = (qx * S + sy) * VECTOR_SIZE + 0;
                    value_x[i] = v;
                    grad_x[i] = gx;
                }
            }
        ...
        for (int qy = 0; qy < Q; ++qy) {
            for (int qx = 0; qx < Q; ++qx) {
                const int q = qx + Q * qy;
                {
                    ...
                    gradient[(q * 2 + 0) * VECTOR_SIZE + 0] = gx;
                    gradient[(q * 2 + 1) * VECTOR_SIZE + 0] = gy;
                }
            }
        }
    }
            ...
            for (int sy = 0; sy < S; ++sy) {
                {
                    ...
                    for (int qy = 0; qy < Q; ++qy) {
                        const int q = qx + Q * qy;
                        tx += flux[(q * 2 + 0) * VECTOR_SIZE + 0] * shape_1d[qy * S + sy];
                        ty += flux[(q * 2 + 1) * VECTOR_SIZE + 0] * grad_1d[qy * S + sy];
                    }
                    const int i = (qx * S + sy) * VECTOR_SIZE + 0;
                    stage_x[i] = tx;
                    stage_y[i] = ty;
        ...
        for (int sy = 0; sy < S; ++sy) {
            for (int sx = 0; sx < S; ++sx) {
                const int shape = sx + S * sy;
                {
                    ...
                    out_streams[shape * 2 + component][0] += value;
                }
            }
        }
    }
                ...
                for (int sz = 0; sz < S; ++sz) {
                    {
                        ...
                        for (int sx = 0; sx < S; ++sx) {
                            ...
                            const scalar_t u = streams[shape * 3 + component][0];
                            v += u * shape_1d[qx * S + sx];
                            gx += u * grad_1d[qx * S + sx];
                        }
                        const int i = ((qx * S + sy) * S + sz) * VECTOR_SIZE + 0;
                        value_x[i] = v;
                        grad_x[i] = gx;
                    }
                }
                ...
                for (int sz = 0; sz < S; ++sz) {
                    {
                        ...
                        const int j = ((qx * Q + qy) * S + sz) * VECTOR_SIZE + 0;
                        value_xy[j] = v;
                        grad_x_xy[j] = gx;
                        grad_y_xy[j] = gy;
                    }
            ...
            for (int qy = 0; qy < Q; ++qy) {
                for (int qx = 0; qx < Q; ++qx) {
                    const int q = qx + Q * (qy + Q * qz);
                    {
                        ...
                        gradient[(q * 3 + 0) * VECTOR_SIZE + 0] = gx;
                        gradient[(q * 3 + 1) * VECTOR_SIZE + 0] = gy;
                        gradient[(q * 3 + 2) * VECTOR_SIZE + 0] = gz;
                    }
                }
            }
        }
                ...
                for (int sz = 0; sz < S; ++sz) {
                    {
                        ...
                        for (int qz = 0; qz < Q; ++qz) {
                            const int q = qx + Q * (qy + Q * qz);
                            tx += flux[(q * 3 + 0) * VECTOR_SIZE + 0] * shape_1d[qz * S + sz];
                            ty += flux[(q * 3 + 1) * VECTOR_SIZE + 0] * shape_1d[qz * S + sz];
                            tz += flux[(q * 3 + 2) * VECTOR_SIZE + 0] * grad_1d[qz * S + sz];
                        }
                        const int i = ((qx * Q + qy) * S + sz) * VECTOR_SIZE + 0;
                        stage_x[i] = tx;
                        stage_y[i] = ty;
                        stage_z[i] = tz;
                    }
                ...
                for (int sz = 0; sz < S; ++sz) {
                    {
                        ...
                        const int j = ((qx * S + sy) * S + sz) * VECTOR_SIZE + 0;
                        stage_xy_x[j] = tx;
                        stage_xy_y[j] = ty;
                        stage_xy_z[j] = tz;
                    }
            ...
            for (int sy = 0; sy < S; ++sy) {
                for (int sx = 0; sx < S; ++sx) {
                    const int shape = sx + S * (sy + S * sz);
                    {
                        ...
                        out_streams[shape * 3 + component][0] += value;
                    }
                }
            }
        }
    ...
    static __host__ __device__ __forceinline__ void evaluate(
        ...
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int sy = 0; sy < S; ++sy) {
            {
                ...
                for (int sx = 0; sx < S; ++sx) {
                    const int s = sx + S * sy;
                    const scalar_t u = streams[s * N_FIELDS + f][0];
                    v += u * shape_1d[qx * S + sx];
                    g += u * grad_1d[qx * S + sx];
                }
                const int i = ((f * Q + qx) * S + sy) * VECTOR_SIZE + 0;
                vx[i] = v;
                gx[i] = g;
            }
        }
        for (int f = 0; f < N_FIELDS; ++f) for (int qy = 0; qy < Q; ++qy) for (int qx = 0; qx < Q; ++qx) {
            const int q = qx + Q * qy;
            {
                ...
                value[(f * N_QP + q) * VECTOR_SIZE + 0] = v;
                gradient[((f * N_QP + q) * 2 + 0) * VECTOR_SIZE + 0] = g0;
                gradient[((f * N_QP + q) * 2 + 1) * VECTOR_SIZE + 0] = g1;
            }
        }
    }
        ...
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int sy = 0; sy < S; ++sy) {
            {
                ...
                for (int sx = 0; sx < S; ++sx) {
                    const int s = sx + S * sy;
                    v += streams[s * N_FIELDS + f][0] * shape_1d[qx * S + sx];
                }
                vx[((f * Q + qx) * S + sy) * VECTOR_SIZE + 0] = v;
            }
        }
        for (int f = 0; f < N_FIELDS; ++f) for (int qy = 0; qy < Q; ++qy) for (int qx = 0; qx < Q; ++qx) {
            const int q = qx + Q * qy;
            {
                scalar_t v = scalar_t(0);
                for (int sy = 0; sy < S; ++sy) {
                    v += vx[((f * Q + qx) * S + sy) * VECTOR_SIZE + 0] * shape_1d[qy * S + sy];
                }
                value[(f * N_QP + q) * VECTOR_SIZE + 0] = v;
            }
        }
        ...
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int sy = 0; sy < S; ++sy) {
            {
                ...
                for (int qy = 0; qy < Q; ++qy) {
                    const int q = qx + Q * qy;
                    a += value_coeff[(f * N_QP + q) * VECTOR_SIZE + 0] * shape_1d[qy * S + sy]
                       + grad_coeff[((f * N_QP + q) * 2 + 1) * VECTOR_SIZE + 0] * grad_1d[qy * S + sy];
                    b += grad_coeff[((f * N_QP + q) * 2 + 0) * VECTOR_SIZE + 0] * shape_1d[qy * S + sy];
                }
                const int i = ((f * Q + qx) * S + sy) * VECTOR_SIZE + 0;
                sv[i] = a;
                sg[i] = b;
        ...
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int sy = 0; sy < S; ++sy) {
            {
                ...
                sv[((f * Q + qx) * S + sy) * VECTOR_SIZE + 0] = a;
            }
        }
        for (int f = 0; f < N_FIELDS; ++f) for (int sy = 0; sy < S; ++sy) for (int sx = 0; sx < S; ++sx) {
            const int s = sx + S * sy;
            {
                scalar_t v = scalar_t(0);
                for (int qx = 0; qx < Q; ++qx) {
                    v += sv[((f * Q + qx) * S + sy) * VECTOR_SIZE + 0] * shape_1d[qx * S + sx];
                }
                output[s * N_FIELDS + f][0] += v;
            }
        }
    ...
    static __host__ __device__ __forceinline__ void evaluate(
        ...
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int sy = 0; sy < S; ++sy) for (int sz = 0; sz < S; ++sz) {
            {
                ...
                const int i = (((f * Q + qx) * S + sy) * S + sz) * VECTOR_SIZE + 0;
                vx[i] = v;
                gx[i] = g;
            }
        }
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int qy = 0; qy < Q; ++qy) for (int sz = 0; sz < S; ++sz) {
            {
                ...
                const int j = (((f * Q + qx) * Q + qy) * S + sz) * VECTOR_SIZE + 0;
                vxy[j] = v;
                g0xy[j] = g0;
                g1xy[j] = g1;
            }
        ...
        for (int f = 0; f < N_FIELDS; ++f) for (int qz = 0; qz < Q; ++qz) for (int qy = 0; qy < Q; ++qy) for (int qx = 0; qx < Q; ++qx) {
            const int q = qx + Q * (qy + Q * qz);
            {
                ...
                value[(f * N_QP + q) * VECTOR_SIZE + 0] = v;
                gradient[((f * N_QP + q) * 3 + 0) * VECTOR_SIZE + 0] = g0;
                gradient[((f * N_QP + q) * 3 + 1) * VECTOR_SIZE + 0] = g1;
                gradient[((f * N_QP + q) * 3 + 2) * VECTOR_SIZE + 0] = g2;
            }
        }
    }
        ...
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int sy = 0; sy < S; ++sy) for (int sz = 0; sz < S; ++sz) {
            {
                ...
                vx[(((f * Q + qx) * S + sy) * S + sz) * VECTOR_SIZE + 0] = v;
            }
        }
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int qy = 0; qy < Q; ++qy) for (int sz = 0; sz < S; ++sz) {
            {
                scalar_t v = scalar_t(0);
                for (int sy = 0; sy < S; ++sy) {
                    v += vx[(((f * Q + qx) * S + sy) * S + sz) * VECTOR_SIZE + 0] * shape_1d[qy * S + sy];
                }
                vxy[(((f * Q + qx) * Q + qy) * S + sz) * VECTOR_SIZE + 0] = v;
            }
        }
        for (int f = 0; f < N_FIELDS; ++f) for (int qz = 0; qz < Q; ++qz) for (int qy = 0; qy < Q; ++qy) for (int qx = 0; qx < Q; ++qx) {
            const int q = qx + Q * (qy + Q * qz);
            {
                scalar_t v = scalar_t(0);
                for (int sz = 0; sz < S; ++sz) {
                    v += vxy[(((f * Q + qx) * Q + qy) * S + sz) * VECTOR_SIZE + 0] * shape_1d[qz * S + sz];
                }
                value[(f * N_QP + q) * VECTOR_SIZE + 0] = v;
            }
        }
        ...
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int qy = 0; qy < Q; ++qy) for (int sz = 0; sz < S; ++sz) {
            {
                ...
                for (int qz = 0; qz < Q; ++qz) {
                    const int q = qx + Q * (qy + Q * qz);
                    a += value_coeff[(f * N_QP + q) * VECTOR_SIZE + 0] * shape_1d[qz * S + sz]
                       + grad_coeff[((f * N_QP + q) * 3 + 2) * VECTOR_SIZE + 0] * grad_1d[qz * S + sz];
                    b += grad_coeff[((f * N_QP + q) * 3 + 0) * VECTOR_SIZE + 0] * shape_1d[qz * S + sz];
                    c += grad_coeff[((f * N_QP + q) * 3 + 1) * VECTOR_SIZE + 0] * shape_1d[qz * S + sz];
                }
                const int i = (((f * Q + qx) * Q + qy) * S + sz) * VECTOR_SIZE + 0;
                z0[i] = a;
                z1[i] = b;
                z2[i] = c;
            }
        ...
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int sy = 0; sy < S; ++sy) for (int sz = 0; sz < S; ++sz) {
            {
                ...
                const int j = (((f * Q + qx) * S + sy) * S + sz) * VECTOR_SIZE + 0;
                yz0[j] = a;
                yz1[j] = b;
            }
        }
        ...
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int qy = 0; qy < Q; ++qy) for (int sz = 0; sz < S; ++sz) {
            {
                ...
                z0[(((f * Q + qx) * Q + qy) * S + sz) * VECTOR_SIZE + 0] = a;
            }
        }
        for (int f = 0; f < N_FIELDS; ++f) for (int qx = 0; qx < Q; ++qx) for (int sy = 0; sy < S; ++sy) for (int sz = 0; sz < S; ++sz) {
            {
                scalar_t a = scalar_t(0);
                for (int qy = 0; qy < Q; ++qy) {
                    a += z0[(((f * Q + qx) * Q + qy) * S + sz) * VECTOR_SIZE + 0] * shape_1d[qy * S + sy];
                }
                yz0[(((f * Q + qx) * S + sy) * S + sz) * VECTOR_SIZE + 0] = a;
            }
        }
        for (int f = 0; f < N_FIELDS; ++f) for (int sz = 0; sz < S; ++sz) for (int sy = 0; sy < S; ++sy) for (int sx = 0; sx < S; ++sx) {
            const int s = sx + S * (sy + S * sz);
            {
                scalar_t v = scalar_t(0);
                for (int qx = 0; qx < Q; ++qx) {
                    v += yz0[(((f * Q + qx) * S + sy) * S + sz) * VECTOR_SIZE + 0] * shape_1d[qx * S + sx];
                }
                output[s * N_FIELDS + f][0] += v;
            }
        }
