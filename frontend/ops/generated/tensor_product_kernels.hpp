...
namespace sfem {
    namespace codegen {
        ...
        static constexpr int integer_root(const int value, const int exponent) { return integer_root_search(value, exponent, 1); }
        ...
        struct TensorProductWeakOps<scalar_t, N_QP, N_SHAPE, VECTOR_SIZE, 2> {
            ...
            static SFEM_INLINE void gradient_impl(const int                           nelems,
...
#pragma omp simd
            ...
            static SFEM_INLINE void gradient(const int                           nelems,
                                             const scalar_t *const SFEM_RESTRICT shape_1d,
                                             const scalar_t *const SFEM_RESTRICT grad_1d,
            ...
            static SFEM_INLINE void gradient_contiguous(const int                           nelems,
                                                        const scalar_t *const SFEM_RESTRICT shape_1d,
                                                        const scalar_t *const SFEM_RESTRICT grad_1d,
            ...
            static SFEM_INLINE void test(const int                           nelems,
                                         const scalar_t *const SFEM_RESTRICT shape_1d,
                                         const scalar_t *const SFEM_RESTRICT grad_1d,
...
#pragma omp simd
                        for (int lane = 0; lane < nelems; ++lane) {
                            ...
                            for (int qx = 0; qx < Q; ++qx) {
                                const int i = (qx * S + sy) * VECTOR_SIZE + lane;
                                value += stage_x[i] * grad_1d[qx * S + sx] + stage_y[i] * shape_1d[qx * S + sx];
                            }
...
#pragma omp simd
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE, int DIM>
        static SFEM_INLINE void tensor_gradient(const int                           nelems,
                                                const scalar_t *const SFEM_RESTRICT shape_1d,
                                                const scalar_t *const SFEM_RESTRICT grad_1d,
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE, int DIM>
        static SFEM_INLINE void tensor_gradient_contiguous(const int                           nelems,
                                                           const scalar_t *const SFEM_RESTRICT shape_1d,
                                                           const scalar_t *const SFEM_RESTRICT grad_1d,
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE, int DIM>
        static SFEM_INLINE void tensor_test(const int                           nelems,
                                            const scalar_t *const SFEM_RESTRICT shape_1d,
                                            const scalar_t *const SFEM_RESTRICT grad_1d,
                                            const scalar_t *const SFEM_RESTRICT flux,
        ...
        struct TensorProductResidualOps<scalar_t, N_QP, N_SHAPE, VECTOR_SIZE, 2> {
            ...
            static SFEM_INLINE void evaluate(const int                           nelems,
...
#pragma omp simd
            ...
            template <int N_FIELDS>
            static SFEM_INLINE void evaluate_contiguous(const int             nelems,
                                                        const scalar_t *const shape_1d,
                                                        const scalar_t *const grad_1d,
...
#pragma omp simd
            ...
            template <int N_FIELDS>
            static SFEM_INLINE void evaluate_value(const int                           nelems,
                                                   const scalar_t *const               shape_1d,
                                                   const scalar_t *const SFEM_RESTRICT streams[N_FIELDS * N_SHAPE],
                                                   scalar_t *const                     value) {
                static constexpr int Q = integer_root(N_QP, 2);
...
#pragma omp simd
            ...
            template <int N_FIELDS>
            static SFEM_INLINE void evaluate_value_contiguous(const int             nelems,
                                                              const scalar_t *const shape_1d,
                                                              const scalar_t        streams[N_FIELDS * N_SHAPE][VECTOR_SIZE],
                                                              scalar_t *const       value) {
                static constexpr int Q = integer_root(N_QP, 2);
...
#pragma omp simd
            ...
            template <int N_FIELDS>
            static SFEM_INLINE void integrate(const int                     nelems,
                                              const scalar_t *const         shape_1d,
                                              const scalar_t *const         grad_1d,
...
#pragma omp simd
            ...
            template <int N_FIELDS>
            static SFEM_INLINE void integrate_contiguous(const int             nelems,
                                                         const scalar_t *const shape_1d,
                                                         const scalar_t *const grad_1d,
                                                         const scalar_t *const value_coeff,
                                                         const scalar_t *const grad_coeff,
...
#pragma omp simd
            ...
            template <int N_FIELDS>
            static SFEM_INLINE void integrate_value(const int                     nelems,
                                                    const scalar_t *const         shape_1d,
                                                    const scalar_t *const         value_coeff,
                                                    scalar_t *const SFEM_RESTRICT output[N_FIELDS * N_SHAPE]) {
                static constexpr int Q = integer_root(N_QP, 2);
...
#pragma omp simd
            ...
            template <int N_FIELDS>
            static SFEM_INLINE void integrate_value_contiguous(const int             nelems,
                                                               const scalar_t *const shape_1d,
                                                               const scalar_t *const value_coeff,
                                                               scalar_t              output[N_FIELDS * N_SHAPE][VECTOR_SIZE]) {
                static constexpr int Q = integer_root(N_QP, 2);
...
#pragma omp simd
        ...
        static SFEM_INLINE void tensor_evaluate(const int                           nelems,
                                                const scalar_t *const               shape_1d,
                                                const scalar_t *const               grad_1d,
                                                const scalar_t *const SFEM_RESTRICT streams[N_FIELDS * N_SHAPE],
                                                scalar_t *const                     value,
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE, int DIM, int N_FIELDS>
        static SFEM_INLINE void tensor_evaluate_contiguous(const int             nelems,
                                                           const scalar_t *const shape_1d,
                                                           const scalar_t *const grad_1d,
                                                           const scalar_t        streams[N_FIELDS * N_SHAPE][VECTOR_SIZE],
                                                           scalar_t *const       value,
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE, int DIM, int N_FIELDS>
        static SFEM_INLINE void tensor_evaluate_value(const int                           nelems,
                                                      const scalar_t *const               shape_1d,
                                                      const scalar_t *const SFEM_RESTRICT streams[N_FIELDS * N_SHAPE],
                                                      scalar_t *const                     value) {
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE, int DIM, int N_FIELDS>
        static SFEM_INLINE void tensor_evaluate_value_contiguous(const int             nelems,
                                                                 const scalar_t *const shape_1d,
                                                                 const scalar_t        streams[N_FIELDS * N_SHAPE][VECTOR_SIZE],
                                                                 scalar_t *const       value) {
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE, int DIM, int N_FIELDS>
        static SFEM_INLINE void tensor_integrate(const int                     nelems,
                                                 const scalar_t *const         shape_1d,
                                                 const scalar_t *const         grad_1d,
                                                 const scalar_t *const         value_coeff,
                                                 const scalar_t *const         grad_coeff,
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE, int DIM, int N_FIELDS>
        static SFEM_INLINE void tensor_integrate_contiguous(const int             nelems,
                                                            const scalar_t *const shape_1d,
                                                            const scalar_t *const grad_1d,
                                                            const scalar_t *const value_coeff,
                                                            const scalar_t *const grad_coeff,
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE, int DIM, int N_FIELDS>
        static SFEM_INLINE void tensor_integrate_value(const int                     nelems,
                                                       const scalar_t *const         shape_1d,
                                                       const scalar_t *const         value_coeff,
                                                       scalar_t *const SFEM_RESTRICT output[N_FIELDS * N_SHAPE]) {
        ...
        template <typename scalar_t, int N_QP, int N_SHAPE, int VECTOR_SIZE, int DIM, int N_FIELDS>
        static SFEM_INLINE void tensor_integrate_value_contiguous(const int             nelems,
                                                                  const scalar_t *const shape_1d,
                                                                  const scalar_t *const value_coeff,
                                                                  scalar_t              output[N_FIELDS * N_SHAPE][VECTOR_SIZE]) {
