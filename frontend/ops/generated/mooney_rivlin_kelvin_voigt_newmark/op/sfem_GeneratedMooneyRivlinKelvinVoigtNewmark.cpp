        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            switch (domain.element_type) {
                case smesh::TRI3: {
                    ...
                    int status = mooney_rivlin_kelvin_voigt_newmark_elastic_gradient_2d_affine_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, storage[0], storage[1], 2, state + 0, state + 1, 2, out + 0, out + 1);
                    if (status != SFEM_SUCCESS) return status;
                    return mooney_rivlin_kelvin_voigt_newmark_viscous_residual_2d_affine_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, storage[3], storage[2], storage[4], 2, u_data[0], u_data[1], 2, u_old_data[0], u_old_data[1], 2, u_out[0], u_out[1]);
                }
                case smesh::TET4: {
                ...
                case smesh::PROTEUS_QUAD4: {
                    ...
                    const real_t *const SFEM_RESTRICT u_old_data[2] = {previous + 0, previous + 1};
                    real_t *const SFEM_RESTRICT u_out[2] = {out + 0, out + 1};
                    int status = mooney_rivlin_kelvin_voigt_newmark_elastic_gradient_2d_isoparametric_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], storage[1], 2, state + 0, state + 1, 2, out + 0, out + 1);
                    if (status != SFEM_SUCCESS) return status;
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            switch (domain.element_type) {
                case smesh::TRI3: {
                    ...
                    real_t *const SFEM_RESTRICT u_out[2] = {out + 0, out + 1};
                    int status = mooney_rivlin_kelvin_voigt_newmark_elastic_apply_2d_affine_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, storage[0], storage[1], 2, state + 0, state + 1, 2, direction + 0, direction + 1, 2, out + 0, out + 1);
                    if (status != SFEM_SUCCESS) return status;
                    return mooney_rivlin_kelvin_voigt_newmark_viscous_jacobian_action_2d_affine_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, storage[3], storage[2], storage[4], 2, u_data[0], u_data[1], 2, u_old_data[0], u_old_data[1], 2, u_direction_data[0], u_direction_data[1], 2, u_out[0], u_out[1]);
                }
                case smesh::TET4: {
                    static constexpr ptrdiff_t FIELD_STRIDE = 3;
                ...
                case smesh::PROTEUS_QUAD4: {
                    ...
                    const real_t *const SFEM_RESTRICT u_direction_data[2] = {direction + 0, direction + 1};
                    real_t *const SFEM_RESTRICT u_out[2] = {out + 0, out + 1};
                    int status = mooney_rivlin_kelvin_voigt_newmark_elastic_apply_2d_isoparametric_mesh_soa(domain.element_type, real_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], storage[1], 2, state + 0, state + 1, 2, direction + 0, direction + 1, 2, out + 0, out + 1);
                    if (status != SFEM_SUCCESS) return status;
