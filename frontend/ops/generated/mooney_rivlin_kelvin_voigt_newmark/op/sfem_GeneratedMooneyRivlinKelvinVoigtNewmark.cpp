...
namespace sfem {
    ...
    double GeneratedMooneyRivlinKelvinVoigtNewmark::flops_value() const {
        ...
        impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (dim == 2) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics = mooney_rivlin_kelvin_voigt_newmark_elastic_objective_2d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
            ...
            if (dim == 3) {
                {
                    const sfem::codegen::KernelDiagnostics *const diagnostics = mooney_rivlin_kelvin_voigt_newmark_elastic_objective_3d_soa_diagnostics(domain.element_type);
                    if (diagnostics) {
                        total += impl_->objective_uses_affine ? sfem::codegen::KernelDiagnostics_total_flops_affine_mesh(diagnostics, nelements) : sfem::codegen::KernelDiagnostics_total_flops_isoparametric_mesh(diagnostics, nelements);
                    }
                }
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            switch (domain.element_type) {
                ...
                case smesh::TET4: {
                    ...
                    if (status != SFEM_SUCCESS) return status;
                    return impl_->residual_uses_affine ? mooney_rivlin_kelvin_voigt_newmark_viscous_residual_3d_affine_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], adjugate[4], adjugate[5], adjugate[6], adjugate[7], adjugate[8], determinant, storage[3], storage[2], storage[4], 3, u_data[0], u_data[1], u_data[2], 3, u_old_data[0], u_old_data[1], u_old_data[2], 3, u_out[0], u_out[1], u_out[2]) : mooney_rivlin_kelvin_voigt_newmark_viscous_residual_3d_isoparametric_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[3], storage[2], storage[4], 3, u_data[0], u_data[1], u_data[2], 3, u_old_data[0], u_old_data[1], u_old_data[2], 3, u_out[0], u_out[1], u_out[2]);
                }
                case smesh::TET10: {
                    static constexpr ptrdiff_t FIELD_STRIDE = 3;
                    const real_t *const SFEM_RESTRICT u_data[3] = {state + 0, state + 1, state + 2};
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            switch (domain.element_type) {
                case smesh::TRI3: {
                    ...
                    real_t *const SFEM_RESTRICT u_out[2] = {out + 0, out + 1};
                    int status = impl_->apply_uses_affine ? mooney_rivlin_kelvin_voigt_newmark_elastic_apply_2d_affine_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, storage[0], storage[1], 2, state + 0, state + 1, 2, direction + 0, direction + 1, 2, out + 0, out + 1) : mooney_rivlin_kelvin_voigt_newmark_elastic_apply_2d_isoparametric_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], storage[1], 2, state + 0, state + 1, 2, direction + 0, direction + 1, 2, out + 0, out + 1);
                    if (status != SFEM_SUCCESS) return status;
        ...
        return impl_->domains->iterate([&](const OpDomain &domain) {
            ...
            if (impl_->objective_uses_affine) {
                auto jacobian = std::static_pointer_cast<smesh::JacobianAdjugateAndDeterminant>(
                        domain.user_data);
                if (!jacobian) {
                    SFEM_ERROR("GeneratedMooneyRivlinKelvinVoigtNewmark affine objective requires cached geometry\n");
                    return SFEM_FAILURE;
                }
                adjugate = reinterpret_cast<const geom_t *const *>(
                        jacobian->jacobian_adjugate_SoA()->data());
            ...
            switch (domain.element_type) {
                case smesh::TRI3:
                    status = impl_->objective_uses_affine ? mooney_rivlin_kelvin_voigt_newmark_elastic_objective_2d_affine_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, storage[0], storage[1], 2, state + 0, state + 1, impl_->element_values.get()) : mooney_rivlin_kelvin_voigt_newmark_elastic_objective_2d_isoparametric_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], storage[1], 2, state + 0, state + 1, impl_->element_values.get());
                    break;
                case smesh::TRI6:
                    status = impl_->objective_uses_affine ? mooney_rivlin_kelvin_voigt_newmark_elastic_objective_2d_affine_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], determinant, storage[0], storage[1], 2, state + 0, state + 1, impl_->element_values.get()) : mooney_rivlin_kelvin_voigt_newmark_elastic_objective_2d_isoparametric_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], storage[1], 2, state + 0, state + 1, impl_->element_values.get());
                    break;
                ...
                case smesh::PROTEUS_QUAD4:
                    ...
                    break;
                case smesh::TET4:
                    status = impl_->objective_uses_affine ? mooney_rivlin_kelvin_voigt_newmark_elastic_objective_3d_affine_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], adjugate[4], adjugate[5], adjugate[6], adjugate[7], adjugate[8], determinant, storage[0], storage[1], 3, state + 0, state + 1, state + 2, impl_->element_values.get()) : mooney_rivlin_kelvin_voigt_newmark_elastic_objective_3d_isoparametric_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], storage[1], 3, state + 0, state + 1, state + 2, impl_->element_values.get());
                    break;
                case smesh::TET10:
                    status = impl_->objective_uses_affine ? mooney_rivlin_kelvin_voigt_newmark_elastic_objective_3d_affine_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), adjugate[0], adjugate[1], adjugate[2], adjugate[3], adjugate[4], adjugate[5], adjugate[6], adjugate[7], adjugate[8], determinant, storage[0], storage[1], 3, state + 0, state + 1, state + 2, impl_->element_values.get()) : mooney_rivlin_kelvin_voigt_newmark_elastic_objective_3d_isoparametric_mesh_soa(domain.element_type, domain.block->n_elements(), mesh->n_nodes(), domain.block->elements()->data(), points, storage[0], storage[1], 3, state + 0, state + 1, state + 2, impl_->element_values.get());
                    break;
