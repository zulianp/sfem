...
__global__ void mooney_rivlin_kelvin_voigt_newmark_viscous_hex8_hessian_crs_i_msoa_impl(
  ...
  for (ptrdiff_t element = (ptrdiff_t)blockIdx.x * blockDim.x + threadIdx.x; element < nelements; element += (ptrdiff_t)blockDim.x * gridDim.x) {
    ...
    for (int trial_local = 0; trial_local < 24; ++trial_local) {
      ...
      mooney_rivlin_kelvin_voigt_newmark_viscous_d3_tensor_product_jacobian_action_block_contiguous<s_t, NQ, NS, VS>(1, 1, bdeterminant, badjugate, isoparametric_shape_1d, isoparametric_grad_1d, isoparametric_q_weight_1d, bcurrent, bprevious, bdirection, eta_b, eta_s, u_dt_shift, boutput);
