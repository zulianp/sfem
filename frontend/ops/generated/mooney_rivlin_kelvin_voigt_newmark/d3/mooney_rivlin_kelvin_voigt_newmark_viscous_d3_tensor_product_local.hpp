...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_viscous_d3_tensor_product_residual_block(
    ...
    for (int q = 0; q < NQ; ++q) {
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            const s_t u0_grad_2_ref = current_grad_ref[((0 * NQ + q) * ND + 2) * VS + lane];
            ...
            const s_t u0_old_grad_2_ref = previous_grad_ref[((0 * NQ + q) * ND + 2) * VS + lane];
            ...
            const s_t u1_grad_2_ref = current_grad_ref[((1 * NQ + q) * ND + 2) * VS + lane];
            ...
            const s_t u1_old_grad_2_ref = previous_grad_ref[((1 * NQ + q) * ND + 2) * VS + lane];
            ...
            const s_t u2_grad_0_ref = current_grad_ref[((2 * NQ + q) * ND + 0) * VS + lane];
            ...
            const s_t u2_old_grad_0_ref = previous_grad_ref[((2 * NQ + q) * ND + 0) * VS + lane];
            ...
            grad_coeff_ref[((2 * NQ + q) * ND + 0) * VS + lane] = qw * (adj0 * grad_coeff2_0 + adj1 * grad_coeff2_1 + adj2 * grad_coeff2_2);
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_viscous_d3_tensor_product_jacobian_action_block(
    ...
    for (int q = 0; q < NQ; ++q) {
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            const s_t u0_direction_grad_2_ref = direction_grad_ref[((0 * NQ + q) * ND + 2) * VS + lane];
            ...
            const s_t u1_direction_grad_2_ref = direction_grad_ref[((1 * NQ + q) * ND + 2) * VS + lane];
            ...
            const s_t u2_old_grad_1 = (u2_old_grad_0_ref * adj1 + u2_old_grad_1_ref * adj4 + u2_old_grad_2_ref * adj7) / det;
            const s_t u2_old_grad_2 = (u2_old_grad_0_ref * adj2 + u2_old_grad_1_ref * adj5 + u2_old_grad_2_ref * adj8) / det;
            const s_t u2_direction_grad_0_ref = direction_grad_ref[((2 * NQ + q) * ND + 0) * VS + lane];
