...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_viscous_d3_tensor_product_residual_block(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            const scalar_t u0_grad_2_ref = current_grad_ref[((0 * N_QP + q) * DIM + 2) * VECTOR_SIZE + lane];
            ...
            const scalar_t u0_old_grad_2_ref = previous_grad_ref[((0 * N_QP + q) * DIM + 2) * VECTOR_SIZE + lane];
            ...
            const scalar_t u1_grad_2_ref = current_grad_ref[((1 * N_QP + q) * DIM + 2) * VECTOR_SIZE + lane];
            ...
            const scalar_t u1_old_grad_2_ref = previous_grad_ref[((1 * N_QP + q) * DIM + 2) * VECTOR_SIZE + lane];
            ...
            const scalar_t u2_grad_0_ref = current_grad_ref[((2 * N_QP + q) * DIM + 0) * VECTOR_SIZE + lane];
            const scalar_t u2_grad_1_ref = current_grad_ref[((2 * N_QP + q) * DIM + 1) * VECTOR_SIZE + lane];
            const scalar_t u2_grad_2_ref = current_grad_ref[((2 * N_QP + q) * DIM + 2) * VECTOR_SIZE + lane];
            ...
            const scalar_t u2_old_grad_0_ref = previous_grad_ref[((2 * N_QP + q) * DIM + 0) * VECTOR_SIZE + lane];
            const scalar_t u2_old_grad_1_ref = previous_grad_ref[((2 * N_QP + q) * DIM + 1) * VECTOR_SIZE + lane];
            const scalar_t u2_old_grad_2_ref = previous_grad_ref[((2 * N_QP + q) * DIM + 2) * VECTOR_SIZE + lane];
            ...
            value_coeff[(2 * N_QP + q) * VECTOR_SIZE + lane] = scalar_t(0);
            grad_coeff_ref[((2 * N_QP + q) * DIM + 0) * VECTOR_SIZE + lane] = qw * (adj0 * grad_coeff2_0 + adj1 * grad_coeff2_1 + adj2 * grad_coeff2_2);
            grad_coeff_ref[((2 * N_QP + q) * DIM + 1) * VECTOR_SIZE + lane] = qw * (adj3 * grad_coeff2_0 + adj4 * grad_coeff2_1 + adj5 * grad_coeff2_2);
            grad_coeff_ref[((2 * N_QP + q) * DIM + 2) * VECTOR_SIZE + lane] = qw * (adj6 * grad_coeff2_0 + adj7 * grad_coeff2_1 + adj8 * grad_coeff2_2);
        }
    }
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_viscous_d3_tensor_product_jacobian_action_block(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            const scalar_t u0_direction_grad_2_ref = direction_grad_ref[((0 * N_QP + q) * DIM + 2) * VECTOR_SIZE + lane];
            ...
            const scalar_t u1_direction_grad_2_ref = direction_grad_ref[((1 * N_QP + q) * DIM + 2) * VECTOR_SIZE + lane];
            ...
            const scalar_t u2_old_grad_1 = (u2_old_grad_0_ref * adj1 + u2_old_grad_1_ref * adj4 + u2_old_grad_2_ref * adj7) / det;
            const scalar_t u2_old_grad_2 = (u2_old_grad_0_ref * adj2 + u2_old_grad_1_ref * adj5 + u2_old_grad_2_ref * adj8) / det;
            const scalar_t u2_direction_grad_0_ref = direction_grad_ref[((2 * N_QP + q) * DIM + 0) * VECTOR_SIZE + lane];
            const scalar_t u2_direction_grad_1_ref = direction_grad_ref[((2 * N_QP + q) * DIM + 1) * VECTOR_SIZE + lane];
            const scalar_t u2_direction_grad_2_ref = direction_grad_ref[((2 * N_QP + q) * DIM + 2) * VECTOR_SIZE + lane];
