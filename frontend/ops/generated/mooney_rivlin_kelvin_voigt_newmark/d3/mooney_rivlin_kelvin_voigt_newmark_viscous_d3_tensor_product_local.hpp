#ifndef MOONEY_RIVLIN_KELVIN_VOIGT_NEWMARK_VISCOUS_D3_TENSOR_PRODUCT_LOCAL_HPP
#define MOONEY_RIVLIN_KELVIN_VOIGT_NEWMARK_VISCOUS_D3_TENSOR_PRODUCT_LOCAL_HPP
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_viscous_d3_tensor_product_residual_block(
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    for (int lane = 0; lane < ne; ++lane) {
      ...
      const s_t u0_grad_0_ref = current_grad_ref[(q * ND) * VS + lane];
      const s_t u0_grad_1_ref = current_grad_ref[(q * ND + 1) * VS + lane];
      const s_t u0_grad_2_ref = current_grad_ref[(q * ND + 2) * VS + lane];
      ...
      const s_t u0_old_grad_0_ref = previous_grad_ref[(q * ND) * VS + lane];
      const s_t u0_old_grad_1_ref = previous_grad_ref[(q * ND + 1) * VS + lane];
      const s_t u0_old_grad_2_ref = previous_grad_ref[(q * ND + 2) * VS + lane];
      ...
      const s_t u1_grad_0_ref = current_grad_ref[((NQ + q) * ND) * VS + lane];
      const s_t u1_grad_1_ref = current_grad_ref[((NQ + q) * ND + 1) * VS + lane];
      const s_t u1_grad_2_ref = current_grad_ref[((NQ + q) * ND + 2) * VS + lane];
      ...
      const s_t u1_old_grad_0_ref = previous_grad_ref[((NQ + q) * ND) * VS + lane];
      const s_t u1_old_grad_1_ref = previous_grad_ref[((NQ + q) * ND + 1) * VS + lane];
      const s_t u1_old_grad_2_ref = previous_grad_ref[((NQ + q) * ND + 2) * VS + lane];
      ...
      const s_t u2_grad_0_ref = current_grad_ref[((2 * NQ + q) * ND) * VS + lane];
      const s_t u2_grad_1_ref = current_grad_ref[((2 * NQ + q) * ND + 1) * VS + lane];
      const s_t u2_grad_2_ref = current_grad_ref[((2 * NQ + q) * ND + 2) * VS + lane];
      ...
      const s_t u2_old_grad_0_ref = previous_grad_ref[((2 * NQ + q) * ND) * VS + lane];
      const s_t u2_old_grad_1_ref = previous_grad_ref[((2 * NQ + q) * ND + 1) * VS + lane];
      const s_t u2_old_grad_2_ref = previous_grad_ref[((2 * NQ + q) * ND + 2) * VS + lane];
      ...
      value_coeff[q * VS + lane] = s_t(0);
      grad_coeff_ref[(q * ND) * VS + lane] = qw * (adj0 * grad_coeff0_0 + adj1 * grad_coeff0_1 + adj2 * grad_coeff0_2);
      grad_coeff_ref[(q * ND + 1) * VS + lane] = qw * (adj3 * grad_coeff0_0 + adj4 * grad_coeff0_1 + adj5 * grad_coeff0_2);
      grad_coeff_ref[(q * ND + 2) * VS + lane] = qw * (adj6 * grad_coeff0_0 + adj7 * grad_coeff0_1 + adj8 * grad_coeff0_2);
      value_coeff[(NQ + q) * VS + lane] = s_t(0);
      grad_coeff_ref[((NQ + q) * ND) * VS + lane] = qw * (adj0 * grad_coeff1_0 + adj1 * grad_coeff1_1 + adj2 * grad_coeff1_2);
      grad_coeff_ref[((NQ + q) * ND + 1) * VS + lane] = qw * (adj3 * grad_coeff1_0 + adj4 * grad_coeff1_1 + adj5 * grad_coeff1_2);
      grad_coeff_ref[((NQ + q) * ND + 2) * VS + lane] = qw * (adj6 * grad_coeff1_0 + adj7 * grad_coeff1_1 + adj8 * grad_coeff1_2);
      value_coeff[(2 * NQ + q) * VS + lane] = s_t(0);
      grad_coeff_ref[((2 * NQ + q) * ND) * VS + lane] = qw * (adj0 * grad_coeff2_0 + adj1 * grad_coeff2_1 + adj2 * grad_coeff2_2);
      grad_coeff_ref[((2 * NQ + q) * ND + 1) * VS + lane] = qw * (adj3 * grad_coeff2_0 + adj4 * grad_coeff2_1 + adj5 * grad_coeff2_2);
      grad_coeff_ref[((2 * NQ + q) * ND + 2) * VS + lane] = qw * (adj6 * grad_coeff2_0 + adj7 * grad_coeff2_1 + adj8 * grad_coeff2_2);
    }
  }
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_viscous_d3_tensor_product_jacobian_action_block(
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    for (int lane = 0; lane < ne; ++lane) {
      ...
      const s_t u0_old_grad_1 = (u0_old_grad_0_ref * adj1 + u0_old_grad_1_ref * adj4 + u0_old_grad_2_ref * adj7) / det;
      const s_t u0_old_grad_2 = (u0_old_grad_0_ref * adj2 + u0_old_grad_1_ref * adj5 + u0_old_grad_2_ref * adj8) / det;
      const s_t u0_direction_grad_0_ref = direction_grad_ref[(q * ND) * VS + lane];
      const s_t u0_direction_grad_1_ref = direction_grad_ref[(q * ND + 1) * VS + lane];
      const s_t u0_direction_grad_2_ref = direction_grad_ref[(q * ND + 2) * VS + lane];
      ...
      const s_t u1_old_grad_1 = (u1_old_grad_0_ref * adj1 + u1_old_grad_1_ref * adj4 + u1_old_grad_2_ref * adj7) / det;
      const s_t u1_old_grad_2 = (u1_old_grad_0_ref * adj2 + u1_old_grad_1_ref * adj5 + u1_old_grad_2_ref * adj8) / det;
      const s_t u1_direction_grad_0_ref = direction_grad_ref[((NQ + q) * ND) * VS + lane];
      const s_t u1_direction_grad_1_ref = direction_grad_ref[((NQ + q) * ND + 1) * VS + lane];
      const s_t u1_direction_grad_2_ref = direction_grad_ref[((NQ + q) * ND + 2) * VS + lane];
      ...
      const s_t u2_old_grad_1 = (u2_old_grad_0_ref * adj1 + u2_old_grad_1_ref * adj4 + u2_old_grad_2_ref * adj7) / det;
      const s_t u2_old_grad_2 = (u2_old_grad_0_ref * adj2 + u2_old_grad_1_ref * adj5 + u2_old_grad_2_ref * adj8) / det;
      const s_t u2_direction_grad_0_ref = direction_grad_ref[((2 * NQ + q) * ND) * VS + lane];
      const s_t u2_direction_grad_1_ref = direction_grad_ref[((2 * NQ + q) * ND + 1) * VS + lane];
      const s_t u2_direction_grad_2_ref = direction_grad_ref[((2 * NQ + q) * ND + 2) * VS + lane];
