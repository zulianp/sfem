...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d3_tensor_product_objective_block(
    ...
    s_t grad_u_ref_q[NQ * 9 * VS];
    tensor_gradient<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, u_streams, 0, &grad_u_ref_q[0 * NQ * 3 * VS]);
    tensor_gradient<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, u_streams, 1, &grad_u_ref_q[1 * NQ * 3 * VS]);
    tensor_gradient<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, u_streams, 2, &grad_u_ref_q[2 * NQ * 3 * VS]);
    for (int q = 0; q < NQ; ++q) {
        const int qx = q % NQ1;
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            s_t grad_u_ref[9];
            grad_u_ref[0] = grad_u_ref_q[((0 * NQ + q) * 3 + 0) * VS + lane];
            grad_u_ref[1] = grad_u_ref_q[((0 * NQ + q) * 3 + 1) * VS + lane];
            grad_u_ref[2] = grad_u_ref_q[((0 * NQ + q) * 3 + 2) * VS + lane];
            grad_u_ref[3] = grad_u_ref_q[((1 * NQ + q) * 3 + 0) * VS + lane];
            grad_u_ref[4] = grad_u_ref_q[((1 * NQ + q) * 3 + 1) * VS + lane];
            grad_u_ref[5] = grad_u_ref_q[((1 * NQ + q) * 3 + 2) * VS + lane];
            grad_u_ref[6] = grad_u_ref_q[((2 * NQ + q) * 3 + 0) * VS + lane];
            grad_u_ref[7] = grad_u_ref_q[((2 * NQ + q) * 3 + 1) * VS + lane];
            grad_u_ref[8] = grad_u_ref_q[((2 * NQ + q) * 3 + 2) * VS + lane];
            s_t grad_u[9];
        ...
        const s_t weak_obj_tmp0 = grad_u[8] + s_t(1);
        const s_t weak_obj_tmp1 = grad_u[1]*grad_u[3]*weak_obj_tmp0;
        const s_t weak_obj_tmp2 = grad_u[4] + s_t(1);
        const s_t weak_obj_tmp3 = grad_u[2]*grad_u[6]*weak_obj_tmp2;
        const s_t weak_obj_tmp4 = grad_u[0] + s_t(1);
        const s_t weak_obj_tmp5 = grad_u[5]*grad_u[7]*weak_obj_tmp4;
        const s_t weak_obj_tmp6 = pow_2(grad_u[1]) + pow_2(grad_u[7]) + pow_2(weak_obj_tmp2);
        const s_t weak_obj_tmp7 = pow_2(grad_u[2]) + pow_2(grad_u[5]) + pow_2(weak_obj_tmp0);
        const s_t weak_obj_tmp8 = pow_2(grad_u[3]) + pow_2(grad_u[6]) + pow_2(weak_obj_tmp4);
        const s_t weak_obj_tmp9 = weak_obj_tmp6 + weak_obj_tmp7 + weak_obj_tmp8;
        value[lane] += qw * jacobian_determinant_lane0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(grad_u[1]*grad_u[5]*grad_u[6] + grad_u[2]*grad_u[3]*grad_u[7] + weak_obj_tmp0*weak_obj_tmp2*weak_obj_tmp4 - weak_obj_tmp1 - weak_obj_tmp3 - weak_obj_tmp5 + s_t(-1)) + mu*(-s_t(6)*grad_u[1]*grad_u[5]*grad_u[6] - s_t(6)*grad_u[2]*grad_u[3]*grad_u[7] - s_t(6)*weak_obj_tmp0*weak_obj_tmp2*weak_obj_tmp4 + s_t(6)*weak_obj_tmp1 + s_t(6)*weak_obj_tmp3 + s_t(6)*weak_obj_tmp5 - (s_t(1) / s_t(2))*pow_2(weak_obj_tmp6) - (s_t(1) / s_t(2))*pow_2(weak_obj_tmp7) - (s_t(1) / s_t(2))*pow_2(weak_obj_tmp8) + ((s_t(1) / s_t(2)))*pow_2(weak_obj_tmp9) + weak_obj_tmp9 - pow_2(grad_u[1]*grad_u[2] + grad_u[5]*weak_obj_tmp2 + grad_u[7]*weak_obj_tmp0) - pow_2(grad_u[1]*weak_obj_tmp4 + grad_u[3]*weak_obj_tmp2 + grad_u[6]*grad_u[7]) - pow_2(grad_u[2]*weak_obj_tmp4 + grad_u[3]*grad_u[5] + grad_u[6]*weak_obj_tmp0)));
        }
    }
}
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d3_tensor_product_gradient_block(
    ...
    for (int q = 0; q < NQ; ++q) {
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            s_t loperand[9];
        s_t material[9];
        const s_t weak_mat_tmp0 = s_t(2)*grad_u[5];
        const s_t weak_mat_tmp1 = grad_u[4] + s_t(1);
        const s_t weak_mat_tmp2 = grad_u[8] + s_t(1);
        const s_t weak_mat_tmp3 = grad_u[0] + s_t(1);
        const s_t weak_mat_tmp4 = grad_u[5]*grad_u[7];
        const s_t weak_mat_tmp5 = ((s_t(1) / s_t(2)))*lmbda*(-grad_u[1]*grad_u[3]*weak_mat_tmp2 + grad_u[1]*grad_u[5]*grad_u[6] + grad_u[2]*grad_u[3]*grad_u[7] - grad_u[2]*grad_u[6]*weak_mat_tmp1 + weak_mat_tmp1*weak_mat_tmp2*weak_mat_tmp3 - weak_mat_tmp3*weak_mat_tmp4 + s_t(-1));
        const s_t weak_mat_tmp6 = grad_u[1]*weak_mat_tmp3 + grad_u[3]*weak_mat_tmp1 + grad_u[6]*grad_u[7];
        const s_t weak_mat_tmp7 = s_t(2)*grad_u[1];
        const s_t weak_mat_tmp8 = grad_u[2]*weak_mat_tmp3 + grad_u[3]*grad_u[5] + grad_u[6]*weak_mat_tmp2;
        const s_t weak_mat_tmp9 = s_t(2)*grad_u[2];
        const s_t weak_mat_tmp10 = pow_2(grad_u[3]) + pow_2(grad_u[6]) + pow_2(weak_mat_tmp3);
        const s_t weak_mat_tmp11 = s_t(2)*weak_mat_tmp3;
        const s_t weak_mat_tmp12 = pow_2(grad_u[1]) + pow_2(grad_u[7]) + pow_2(weak_mat_tmp1);
        const s_t weak_mat_tmp13 = pow_2(grad_u[2]) + pow_2(grad_u[5]) + pow_2(weak_mat_tmp2);
        const s_t weak_mat_tmp14 = weak_mat_tmp10 + weak_mat_tmp12 + weak_mat_tmp13;
        const s_t weak_mat_tmp15 = s_t(2)*grad_u[3];
        const s_t weak_mat_tmp16 = grad_u[1]*grad_u[2] + grad_u[5]*weak_mat_tmp1 + grad_u[7]*weak_mat_tmp2;
        const s_t weak_mat_tmp17 = s_t(2)*grad_u[6];
        const s_t weak_mat_tmp18 = s_t(6)*grad_u[2];
        const s_t weak_mat_tmp19 = s_t(2)*weak_mat_tmp1;
        const s_t weak_mat_tmp20 = s_t(2)*grad_u[7];
        const s_t weak_mat_tmp21 = s_t(6)*grad_u[1];
        const s_t weak_mat_tmp22 = s_t(2)*weak_mat_tmp2;
        material[0] = mu*(s_t(2)*grad_u[0] - s_t(6)*weak_mat_tmp1*weak_mat_tmp2 - weak_mat_tmp10*weak_mat_tmp11 + weak_mat_tmp11*weak_mat_tmp14 + s_t(6)*weak_mat_tmp4 - weak_mat_tmp6*weak_mat_tmp7 - weak_mat_tmp8*weak_mat_tmp9 + s_t(2)) + weak_mat_tmp5*(-grad_u[7]*weak_mat_tmp0 + s_t(2)*weak_mat_tmp1*weak_mat_tmp2);
        material[1] = mu*(s_t(2)*grad_u[1]*weak_mat_tmp14 + s_t(2)*grad_u[1] + s_t(6)*grad_u[3]*weak_mat_tmp2 - s_t(6)*grad_u[5]*grad_u[6] - weak_mat_tmp11*weak_mat_tmp6 - weak_mat_tmp12*weak_mat_tmp7 - weak_mat_tmp16*weak_mat_tmp9) + weak_mat_tmp5*(s_t(2)*grad_u[5]*grad_u[6] - weak_mat_tmp15*weak_mat_tmp2);
        material[2] = mu*(s_t(2)*grad_u[2]*weak_mat_tmp14 + s_t(2)*grad_u[2] - s_t(6)*grad_u[3]*grad_u[7] + s_t(6)*grad_u[6]*weak_mat_tmp1 - weak_mat_tmp11*weak_mat_tmp8 - weak_mat_tmp13*weak_mat_tmp9 - weak_mat_tmp16*weak_mat_tmp7) + weak_mat_tmp5*(grad_u[7]*weak_mat_tmp15 - weak_mat_tmp1*weak_mat_tmp17);
        material[3] = mu*(s_t(6)*grad_u[1]*weak_mat_tmp2 + s_t(2)*grad_u[3]*weak_mat_tmp14 + s_t(2)*grad_u[3] - grad_u[7]*weak_mat_tmp18 - weak_mat_tmp0*weak_mat_tmp8 - weak_mat_tmp10*weak_mat_tmp15 - weak_mat_tmp19*weak_mat_tmp6) + weak_mat_tmp5*(s_t(2)*grad_u[2]*grad_u[7] - weak_mat_tmp2*weak_mat_tmp7);
        material[4] = mu*(s_t(2)*grad_u[4] + grad_u[6]*weak_mat_tmp18 - weak_mat_tmp0*weak_mat_tmp16 - weak_mat_tmp12*weak_mat_tmp19 + weak_mat_tmp14*weak_mat_tmp19 - weak_mat_tmp15*weak_mat_tmp6 - s_t(6)*weak_mat_tmp2*weak_mat_tmp3 + s_t(2)) + weak_mat_tmp5*(-grad_u[6]*weak_mat_tmp9 + s_t(2)*weak_mat_tmp2*weak_mat_tmp3);
        material[5] = mu*(s_t(2)*grad_u[5]*weak_mat_tmp14 + s_t(2)*grad_u[5] - grad_u[6]*weak_mat_tmp21 + s_t(6)*grad_u[7]*weak_mat_tmp3 - weak_mat_tmp0*weak_mat_tmp13 - weak_mat_tmp15*weak_mat_tmp8 - weak_mat_tmp16*weak_mat_tmp19) + weak_mat_tmp5*(grad_u[6]*weak_mat_tmp7 - weak_mat_tmp20*weak_mat_tmp3);
        material[6] = mu*(s_t(6)*grad_u[2]*weak_mat_tmp1 - grad_u[5]*weak_mat_tmp21 + s_t(2)*grad_u[6]*weak_mat_tmp14 + s_t(2)*grad_u[6] - weak_mat_tmp10*weak_mat_tmp17 - weak_mat_tmp20*weak_mat_tmp6 - weak_mat_tmp22*weak_mat_tmp8) + weak_mat_tmp5*(grad_u[5]*weak_mat_tmp7 - weak_mat_tmp1*weak_mat_tmp9);
        material[7] = mu*(-grad_u[3]*weak_mat_tmp18 + s_t(6)*grad_u[5]*weak_mat_tmp3 + s_t(2)*grad_u[7]*weak_mat_tmp14 + s_t(2)*grad_u[7] - weak_mat_tmp12*weak_mat_tmp20 - weak_mat_tmp16*weak_mat_tmp22 - weak_mat_tmp17*weak_mat_tmp6) + weak_mat_tmp5*(grad_u[3]*weak_mat_tmp9 - weak_mat_tmp0*weak_mat_tmp3);
        material[8] = mu*(grad_u[3]*weak_mat_tmp21 + s_t(2)*grad_u[8] - s_t(6)*weak_mat_tmp1*weak_mat_tmp3 - weak_mat_tmp13*weak_mat_tmp22 + weak_mat_tmp14*weak_mat_tmp22 - weak_mat_tmp16*weak_mat_tmp20 - weak_mat_tmp17*weak_mat_tmp8 + s_t(2)) + weak_mat_tmp5*(-grad_u[3]*weak_mat_tmp7 + s_t(2)*weak_mat_tmp1*weak_mat_tmp3);
    ...
    tensor_test<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, &loperand_q[0 * NQ * 3 * VS], out_streams, 0);
    tensor_test<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, &loperand_q[1 * NQ * 3 * VS], out_streams, 1);
    tensor_test<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, &loperand_q[2 * NQ * 3 * VS], out_streams, 2);
}
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d3_tensor_product_apply_block(
    ...
    s_t loperand_q[NQ * 9 * VS];
    tensor_gradient<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, u_streams, 0, &grad_u_ref_q[0 * NQ * 3 * VS]);
    tensor_gradient<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, h_streams, 0, &grad_h_ref_q[0 * NQ * 3 * VS]);
    tensor_gradient<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, u_streams, 1, &grad_u_ref_q[1 * NQ * 3 * VS]);
    tensor_gradient<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, h_streams, 1, &grad_h_ref_q[1 * NQ * 3 * VS]);
    tensor_gradient<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, u_streams, 2, &grad_u_ref_q[2 * NQ * 3 * VS]);
    tensor_gradient<s_t, NQ, NS, VS, 3, 3>(nelems, shape_1d, grad_1d, h_streams, 2, &grad_h_ref_q[2 * NQ * 3 * VS]);
    for (int q = 0; q < NQ; ++q) {
        const int qx = q % NQ1;
        const int qy = (q / NQ1) % NQ1;
        const int qz = q / (NQ1 * NQ1);
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            s_t loperand[9];
        s_t material[9];
        const s_t weak_mat_tmp0 = s_t(2)*grad_u[6];
        const s_t weak_mat_tmp1 = grad_u[7]*weak_mat_tmp0;
        const s_t weak_mat_tmp2 = grad_u[4] + s_t(1);
        const s_t weak_mat_tmp3 = s_t(2)*grad_u[3];
        const s_t weak_mat_tmp4 = weak_mat_tmp2*weak_mat_tmp3;
        const s_t weak_mat_tmp5 = mu*(-weak_mat_tmp1 - weak_mat_tmp4);
        const s_t weak_mat_tmp6 = grad_u[8] + s_t(1);
        const s_t weak_mat_tmp7 = grad_u[3]*weak_mat_tmp6;
        const s_t weak_mat_tmp8 = grad_u[5]*grad_u[6] - weak_mat_tmp7;
        const s_t weak_mat_tmp9 = grad_u[5]*grad_u[7];
        ...
        const s_t weak_mat_tmp14 = grad_u[5]*weak_mat_tmp3;
        const s_t weak_mat_tmp15 = weak_mat_tmp0*weak_mat_tmp6;
        const s_t weak_mat_tmp16 = mu*(-weak_mat_tmp14 - weak_mat_tmp15);
        const s_t weak_mat_tmp17 = grad_u[3]*grad_u[7];
        const s_t weak_mat_tmp18 = grad_u[6]*weak_mat_tmp2;
        const s_t weak_mat_tmp19 = weak_mat_tmp17 - weak_mat_tmp18;
        const s_t weak_mat_tmp20 = s_t(2)*grad_u[2];
        const s_t weak_mat_tmp21 = grad_u[5]*weak_mat_tmp20;
        const s_t weak_mat_tmp22 = s_t(2)*grad_u[1];
        const s_t weak_mat_tmp23 = weak_mat_tmp2*weak_mat_tmp22;
        const s_t weak_mat_tmp24 = mu*(-weak_mat_tmp21 - weak_mat_tmp23);
        const s_t weak_mat_tmp25 = grad_u[1]*weak_mat_tmp6;
        const s_t weak_mat_tmp26 = grad_u[2]*grad_u[7] - weak_mat_tmp25;
        const s_t weak_mat_tmp27 = grad_u[7]*weak_mat_tmp22;
        const s_t weak_mat_tmp28 = weak_mat_tmp20*weak_mat_tmp6;
        const s_t weak_mat_tmp29 = mu*(-weak_mat_tmp27 - weak_mat_tmp28);
        const s_t weak_mat_tmp30 = grad_u[1]*grad_u[5];
        const s_t weak_mat_tmp31 = grad_u[2]*weak_mat_tmp2;
        const s_t weak_mat_tmp32 = weak_mat_tmp30 - weak_mat_tmp31;
        const s_t weak_mat_tmp33 = s_t(2)*pow_2(grad_u[5]);
        const s_t weak_mat_tmp34 = s_t(2)*pow_2(weak_mat_tmp6) + s_t(2);
        const s_t weak_mat_tmp35 = weak_mat_tmp33 + weak_mat_tmp34;
        const s_t weak_mat_tmp36 = s_t(2)*pow_2(grad_u[7]);
        ...
        const s_t weak_mat_tmp40 = grad_u[1]*grad_u[6];
        const s_t weak_mat_tmp41 = grad_u[0] + s_t(1);
        const s_t weak_mat_tmp42 = grad_u[7]*weak_mat_tmp41;
        const s_t weak_mat_tmp43 = weak_mat_tmp40 - weak_mat_tmp42;
        const s_t weak_mat_tmp44 = s_t(6)*grad_u[7];
        const s_t weak_mat_tmp45 = grad_u[2]*grad_u[3];
        const s_t weak_mat_tmp46 = s_t(2)*weak_mat_tmp45;
        const s_t weak_mat_tmp47 = grad_u[5]*weak_mat_tmp41;
        const s_t weak_mat_tmp48 = lmbda*(grad_u[1]*grad_u[5]*grad_u[6] - grad_u[1]*weak_mat_tmp7 + grad_u[2]*grad_u[3]*grad_u[7] - grad_u[2]*weak_mat_tmp18 + weak_mat_tmp2*weak_mat_tmp41*weak_mat_tmp6 - weak_mat_tmp41*weak_mat_tmp9 + s_t(-1));
        const s_t weak_mat_tmp49 = grad_u[7]*weak_mat_tmp48;
        ...
        const s_t weak_mat_tmp65 = s_t(6)*grad_u[4] + s_t(6);
        ...
        const s_t weak_mat_tmp80 = s_t(2)*pow_2(grad_u[3]);
        const s_t weak_mat_tmp81 = s_t(2)*pow_2(grad_u[6]);
        const s_t weak_mat_tmp82 = weak_mat_tmp80 + weak_mat_tmp81;
        const s_t weak_mat_tmp83 = s_t(6)*grad_u[6];
        const s_t weak_mat_tmp84 = s_t(2)*weak_mat_tmp31;
        const s_t weak_mat_tmp85 = grad_u[6]*weak_mat_tmp48;
        const s_t weak_mat_tmp86 = mu*(s_t(4)*grad_u[1]*grad_u[5] - weak_mat_tmp83 - weak_mat_tmp84) + weak_mat_tmp85;
        const s_t weak_mat_tmp87 = s_t(2)*weak_mat_tmp42;
        const s_t weak_mat_tmp88 = mu*(s_t(4)*grad_u[1]*grad_u[6] - weak_mat_tmp52 - weak_mat_tmp87) + weak_mat_tmp54;
        const s_t weak_mat_tmp89 = s_t(6)*grad_u[3];
        const s_t weak_mat_tmp90 = -s_t(2)*grad_u[2]*grad_u[7];
        const s_t weak_mat_tmp91 = grad_u[3]*weak_mat_tmp48;
        ...
        const s_t weak_mat_tmp101 = mu*(s_t(4)*grad_u[2]*grad_u[3] - weak_mat_tmp100 - weak_mat_tmp44) + weak_mat_tmp49;
        const s_t weak_mat_tmp102 = s_t(2)*weak_mat_tmp25;
        const s_t weak_mat_tmp103 = mu*(s_t(4)*grad_u[2]*grad_u[7] - weak_mat_tmp102 - weak_mat_tmp89) + weak_mat_tmp91;
        ...
        const s_t weak_mat_tmp116 = s_t(2)*pow_2(grad_u[2]);
        const s_t weak_mat_tmp117 = weak_mat_tmp116 + weak_mat_tmp34;
        const s_t weak_mat_tmp118 = s_t(2)*pow_2(grad_u[1]);
        const s_t weak_mat_tmp119 = weak_mat_tmp118 + weak_mat_tmp36;
        const s_t weak_mat_tmp120 = s_t(6)*grad_u[2];
        const s_t weak_mat_tmp121 = grad_u[2]*weak_mat_tmp48;
        const s_t weak_mat_tmp122 = mu*(s_t(4)*grad_u[3]*grad_u[7] - weak_mat_tmp120 - weak_mat_tmp96) + weak_mat_tmp121;
        const s_t weak_mat_tmp123 = s_t(6)*grad_u[1];
        const s_t weak_mat_tmp124 = grad_u[1]*weak_mat_tmp48;
        ...
        const s_t weak_mat_tmp127 = grad_u[2]*weak_mat_tmp22;
        const s_t weak_mat_tmp128 = mu*(-weak_mat_tmp127 - weak_mat_tmp74);
        const s_t weak_mat_tmp129 = grad_u[6]*weak_mat_tmp3;
        ...
        const s_t weak_mat_tmp134 = s_t(6)*grad_u[0] + s_t(6);
        ...
        const s_t weak_mat_tmp139 = mu*(s_t(4)*grad_u[5]*grad_u[6] - weak_mat_tmp123 - weak_mat_tmp69) + weak_mat_tmp124;
