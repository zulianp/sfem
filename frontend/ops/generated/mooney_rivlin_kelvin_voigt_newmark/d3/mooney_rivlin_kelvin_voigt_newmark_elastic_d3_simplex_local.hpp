...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d3_simplex_objective_block(
    ...
    static_assert(VS > 0, "VS must be positive");
        for (int q = 0; q < NQ; ++q) {
            ...
            s_t grad_u_ref4_values[VS];
            s_t grad_u_ref5_values[VS];
            s_t grad_u_ref6_values[VS];
            s_t grad_u_ref7_values[VS];
            s_t grad_u_ref8_values[VS];
            #pragma omp simd
            ...
            #pragma omp simd
            for (int lane = 0; lane < nelems; ++lane) {
                grad_u_ref4_values[lane] = s_t(0);
            }
            #pragma omp simd
            for (int lane = 0; lane < nelems; ++lane) {
                grad_u_ref5_values[lane] = s_t(0);
            }
            #pragma omp simd
            for (int lane = 0; lane < nelems; ++lane) {
                grad_u_ref6_values[lane] = s_t(0);
            }
            #pragma omp simd
            for (int lane = 0; lane < nelems; ++lane) {
                grad_u_ref7_values[lane] = s_t(0);
            }
            #pragma omp simd
            for (int lane = 0; lane < nelems; ++lane) {
                grad_u_ref8_values[lane] = s_t(0);
            }
            for (int shape = 0; shape < NS; ++shape) {
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    grad_u_ref0_values[lane] += u_streams[shape * 3 + 0][lane] * grad_ref_x[q * NS + shape];
                }
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    grad_u_ref1_values[lane] += u_streams[shape * 3 + 0][lane] * grad_ref_y[q * NS + shape];
                }
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    grad_u_ref2_values[lane] += u_streams[shape * 3 + 0][lane] * grad_ref_z[q * NS + shape];
                }
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    grad_u_ref3_values[lane] += u_streams[shape * 3 + 1][lane] * grad_ref_x[q * NS + shape];
                }
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    grad_u_ref4_values[lane] += u_streams[shape * 3 + 1][lane] * grad_ref_y[q * NS + shape];
                }
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    grad_u_ref5_values[lane] += u_streams[shape * 3 + 1][lane] * grad_ref_z[q * NS + shape];
                }
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    grad_u_ref6_values[lane] += u_streams[shape * 3 + 2][lane] * grad_ref_x[q * NS + shape];
                }
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    grad_u_ref7_values[lane] += u_streams[shape * 3 + 2][lane] * grad_ref_y[q * NS + shape];
                }
                #pragma omp simd
                for (int lane = 0; lane < nelems; ++lane) {
                    grad_u_ref8_values[lane] += u_streams[shape * 3 + 2][lane] * grad_ref_z[q * NS + shape];
                }
            }
            #pragma omp simd
            for (int lane = 0; lane < nelems; ++lane) {
            ...
            const s_t jacobian_adjugate_lane4 = jacobian_adjugate4[geometry_offset];
            const s_t jacobian_adjugate_lane5 = jacobian_adjugate5[geometry_offset];
            const s_t jacobian_adjugate_lane6 = jacobian_adjugate6[geometry_offset];
            const s_t jacobian_adjugate_lane7 = jacobian_adjugate7[geometry_offset];
            const s_t jacobian_adjugate_lane8 = jacobian_adjugate8[geometry_offset];
            ...
            const s_t grad_u_ref4 = grad_u_ref4_values[lane];
            const s_t grad_u_ref5 = grad_u_ref5_values[lane];
            const s_t grad_u_ref6 = grad_u_ref6_values[lane];
            const s_t grad_u_ref7 = grad_u_ref7_values[lane];
            const s_t grad_u_ref8 = grad_u_ref8_values[lane];
        const s_t inv_jacobian_determinant = s_t(1) / jacobian_determinant_lane0;
        const s_t grad_u0 = (grad_u_ref0 * jacobian_adjugate_lane0 + grad_u_ref1 * jacobian_adjugate_lane3 + grad_u_ref2 * jacobian_adjugate_lane6) * inv_jacobian_determinant;
        const s_t grad_u1 = (grad_u_ref0 * jacobian_adjugate_lane1 + grad_u_ref1 * jacobian_adjugate_lane4 + grad_u_ref2 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
        const s_t grad_u2 = (grad_u_ref0 * jacobian_adjugate_lane2 + grad_u_ref1 * jacobian_adjugate_lane5 + grad_u_ref2 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
        const s_t grad_u3 = (grad_u_ref3 * jacobian_adjugate_lane0 + grad_u_ref4 * jacobian_adjugate_lane3 + grad_u_ref5 * jacobian_adjugate_lane6) * inv_jacobian_determinant;
        const s_t grad_u4 = (grad_u_ref3 * jacobian_adjugate_lane1 + grad_u_ref4 * jacobian_adjugate_lane4 + grad_u_ref5 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
        const s_t grad_u5 = (grad_u_ref3 * jacobian_adjugate_lane2 + grad_u_ref4 * jacobian_adjugate_lane5 + grad_u_ref5 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
        const s_t grad_u6 = (grad_u_ref6 * jacobian_adjugate_lane0 + grad_u_ref7 * jacobian_adjugate_lane3 + grad_u_ref8 * jacobian_adjugate_lane6) * inv_jacobian_determinant;
        const s_t grad_u7 = (grad_u_ref6 * jacobian_adjugate_lane1 + grad_u_ref7 * jacobian_adjugate_lane4 + grad_u_ref8 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
        const s_t grad_u8 = (grad_u_ref6 * jacobian_adjugate_lane2 + grad_u_ref7 * jacobian_adjugate_lane5 + grad_u_ref8 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
        const s_t weak_obj_tmp0 = grad_u8 + s_t(1);
        const s_t weak_obj_tmp1 = grad_u1*grad_u3*weak_obj_tmp0;
        const s_t weak_obj_tmp2 = grad_u4 + s_t(1);
        const s_t weak_obj_tmp3 = grad_u2*grad_u6*weak_obj_tmp2;
        const s_t weak_obj_tmp4 = grad_u0 + s_t(1);
        const s_t weak_obj_tmp5 = grad_u5*grad_u7*weak_obj_tmp4;
        const s_t weak_obj_tmp6 = pow_2(grad_u1) + pow_2(grad_u7) + pow_2(weak_obj_tmp2);
        const s_t weak_obj_tmp7 = pow_2(grad_u2) + pow_2(grad_u5) + pow_2(weak_obj_tmp0);
        const s_t weak_obj_tmp8 = pow_2(grad_u3) + pow_2(grad_u6) + pow_2(weak_obj_tmp4);
        const s_t weak_obj_tmp9 = weak_obj_tmp6 + weak_obj_tmp7 + weak_obj_tmp8;
        value[lane] += qw * jacobian_determinant_lane0 * (((s_t(1) / s_t(2)))*lmbda*pow_2(grad_u1*grad_u5*grad_u6 + grad_u2*grad_u3*grad_u7 + weak_obj_tmp0*weak_obj_tmp2*weak_obj_tmp4 - weak_obj_tmp1 - weak_obj_tmp3 - weak_obj_tmp5 + s_t(-1)) + mu*(-s_t(6)*grad_u1*grad_u5*grad_u6 - s_t(6)*grad_u2*grad_u3*grad_u7 - s_t(6)*weak_obj_tmp0*weak_obj_tmp2*weak_obj_tmp4 + s_t(6)*weak_obj_tmp1 + s_t(6)*weak_obj_tmp3 + s_t(6)*weak_obj_tmp5 - (s_t(1) / s_t(2))*pow_2(weak_obj_tmp6) - (s_t(1) / s_t(2))*pow_2(weak_obj_tmp7) - (s_t(1) / s_t(2))*pow_2(weak_obj_tmp8) + ((s_t(1) / s_t(2)))*pow_2(weak_obj_tmp9) + weak_obj_tmp9 - pow_2(grad_u1*grad_u2 + grad_u5*weak_obj_tmp2 + grad_u7*weak_obj_tmp0) - pow_2(grad_u1*weak_obj_tmp4 + grad_u3*weak_obj_tmp2 + grad_u6*grad_u7) - pow_2(grad_u2*weak_obj_tmp4 + grad_u3*grad_u5 + grad_u6*weak_obj_tmp0)));
            }
        }
}
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d3_simplex_tet4_objective_block(
    ...
    static_assert(VS > 0, "VS must be positive");
        { const int q = 0;  // constant-P1 simplex
            ...
            const s_t grad_u_ref1 = -(u_streams[0 * 3 + 0][lane]) + u_streams[2 * 3 + 0][lane];
            const s_t grad_u_ref2 = -(u_streams[0 * 3 + 0][lane]) + u_streams[3 * 3 + 0][lane];
            const s_t grad_u_ref3 = -(u_streams[0 * 3 + 1][lane]) + u_streams[1 * 3 + 1][lane];
            const s_t grad_u_ref4 = -(u_streams[0 * 3 + 1][lane]) + u_streams[2 * 3 + 1][lane];
            const s_t grad_u_ref5 = -(u_streams[0 * 3 + 1][lane]) + u_streams[3 * 3 + 1][lane];
            const s_t grad_u_ref6 = -(u_streams[0 * 3 + 2][lane]) + u_streams[1 * 3 + 2][lane];
            const s_t grad_u_ref7 = -(u_streams[0 * 3 + 2][lane]) + u_streams[2 * 3 + 2][lane];
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d3_simplex_gradient_block(
    ...
    static_assert(VS > 0, "VS must be positive");
        ...
        const s_t grad_u7 = (grad_u_ref6 * jacobian_adjugate_lane1 + grad_u_ref7 * jacobian_adjugate_lane4 + grad_u_ref8 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
        const s_t grad_u8 = (grad_u_ref6 * jacobian_adjugate_lane2 + grad_u_ref7 * jacobian_adjugate_lane5 + grad_u_ref8 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
        const s_t weak_mat_tmp0 = s_t(2)*grad_u5;
        const s_t weak_mat_tmp1 = grad_u4 + s_t(1);
        const s_t weak_mat_tmp2 = grad_u8 + s_t(1);
        const s_t weak_mat_tmp3 = grad_u0 + s_t(1);
        const s_t weak_mat_tmp4 = grad_u5*grad_u7;
        const s_t weak_mat_tmp5 = ((s_t(1) / s_t(2)))*lmbda*(-grad_u1*grad_u3*weak_mat_tmp2 + grad_u1*grad_u5*grad_u6 + grad_u2*grad_u3*grad_u7 - grad_u2*grad_u6*weak_mat_tmp1 + weak_mat_tmp1*weak_mat_tmp2*weak_mat_tmp3 - weak_mat_tmp3*weak_mat_tmp4 + s_t(-1));
        const s_t weak_mat_tmp6 = grad_u1*weak_mat_tmp3 + grad_u3*weak_mat_tmp1 + grad_u6*grad_u7;
        const s_t weak_mat_tmp7 = s_t(2)*grad_u1;
        const s_t weak_mat_tmp8 = grad_u2*weak_mat_tmp3 + grad_u3*grad_u5 + grad_u6*weak_mat_tmp2;
        const s_t weak_mat_tmp9 = s_t(2)*grad_u2;
        const s_t weak_mat_tmp10 = pow_2(grad_u3) + pow_2(grad_u6) + pow_2(weak_mat_tmp3);
        const s_t weak_mat_tmp11 = s_t(2)*weak_mat_tmp3;
        const s_t weak_mat_tmp12 = pow_2(grad_u1) + pow_2(grad_u7) + pow_2(weak_mat_tmp1);
        const s_t weak_mat_tmp13 = pow_2(grad_u2) + pow_2(grad_u5) + pow_2(weak_mat_tmp2);
        const s_t weak_mat_tmp14 = weak_mat_tmp10 + weak_mat_tmp12 + weak_mat_tmp13;
        const s_t weak_mat_tmp15 = s_t(2)*grad_u3;
        const s_t weak_mat_tmp16 = grad_u1*grad_u2 + grad_u5*weak_mat_tmp1 + grad_u7*weak_mat_tmp2;
        const s_t weak_mat_tmp17 = s_t(2)*grad_u6;
        const s_t weak_mat_tmp18 = s_t(6)*grad_u2;
        const s_t weak_mat_tmp19 = s_t(2)*weak_mat_tmp1;
        const s_t weak_mat_tmp20 = s_t(2)*grad_u7;
        const s_t weak_mat_tmp21 = s_t(6)*grad_u1;
        const s_t weak_mat_tmp22 = s_t(2)*weak_mat_tmp2;
        const s_t material0 = mu*(s_t(2)*grad_u0 - s_t(6)*weak_mat_tmp1*weak_mat_tmp2 - weak_mat_tmp10*weak_mat_tmp11 + weak_mat_tmp11*weak_mat_tmp14 + s_t(6)*weak_mat_tmp4 - weak_mat_tmp6*weak_mat_tmp7 - weak_mat_tmp8*weak_mat_tmp9 + s_t(2)) + weak_mat_tmp5*(-grad_u7*weak_mat_tmp0 + s_t(2)*weak_mat_tmp1*weak_mat_tmp2);
        const s_t material1 = mu*(s_t(2)*grad_u1*weak_mat_tmp14 + s_t(2)*grad_u1 + s_t(6)*grad_u3*weak_mat_tmp2 - s_t(6)*grad_u5*grad_u6 - weak_mat_tmp11*weak_mat_tmp6 - weak_mat_tmp12*weak_mat_tmp7 - weak_mat_tmp16*weak_mat_tmp9) + weak_mat_tmp5*(s_t(2)*grad_u5*grad_u6 - weak_mat_tmp15*weak_mat_tmp2);
        const s_t material2 = mu*(s_t(2)*grad_u2*weak_mat_tmp14 + s_t(2)*grad_u2 - s_t(6)*grad_u3*grad_u7 + s_t(6)*grad_u6*weak_mat_tmp1 - weak_mat_tmp11*weak_mat_tmp8 - weak_mat_tmp13*weak_mat_tmp9 - weak_mat_tmp16*weak_mat_tmp7) + weak_mat_tmp5*(grad_u7*weak_mat_tmp15 - weak_mat_tmp1*weak_mat_tmp17);
        const s_t material3 = mu*(s_t(6)*grad_u1*weak_mat_tmp2 + s_t(2)*grad_u3*weak_mat_tmp14 + s_t(2)*grad_u3 - grad_u7*weak_mat_tmp18 - weak_mat_tmp0*weak_mat_tmp8 - weak_mat_tmp10*weak_mat_tmp15 - weak_mat_tmp19*weak_mat_tmp6) + weak_mat_tmp5*(s_t(2)*grad_u2*grad_u7 - weak_mat_tmp2*weak_mat_tmp7);
        const s_t material4 = mu*(s_t(2)*grad_u4 + grad_u6*weak_mat_tmp18 - weak_mat_tmp0*weak_mat_tmp16 - weak_mat_tmp12*weak_mat_tmp19 + weak_mat_tmp14*weak_mat_tmp19 - weak_mat_tmp15*weak_mat_tmp6 - s_t(6)*weak_mat_tmp2*weak_mat_tmp3 + s_t(2)) + weak_mat_tmp5*(-grad_u6*weak_mat_tmp9 + s_t(2)*weak_mat_tmp2*weak_mat_tmp3);
        const s_t material5 = mu*(s_t(2)*grad_u5*weak_mat_tmp14 + s_t(2)*grad_u5 - grad_u6*weak_mat_tmp21 + s_t(6)*grad_u7*weak_mat_tmp3 - weak_mat_tmp0*weak_mat_tmp13 - weak_mat_tmp15*weak_mat_tmp8 - weak_mat_tmp16*weak_mat_tmp19) + weak_mat_tmp5*(grad_u6*weak_mat_tmp7 - weak_mat_tmp20*weak_mat_tmp3);
        const s_t material6 = mu*(s_t(6)*grad_u2*weak_mat_tmp1 - grad_u5*weak_mat_tmp21 + s_t(2)*grad_u6*weak_mat_tmp14 + s_t(2)*grad_u6 - weak_mat_tmp10*weak_mat_tmp17 - weak_mat_tmp20*weak_mat_tmp6 - weak_mat_tmp22*weak_mat_tmp8) + weak_mat_tmp5*(grad_u5*weak_mat_tmp7 - weak_mat_tmp1*weak_mat_tmp9);
        const s_t material7 = mu*(-grad_u3*weak_mat_tmp18 + s_t(6)*grad_u5*weak_mat_tmp3 + s_t(2)*grad_u7*weak_mat_tmp14 + s_t(2)*grad_u7 - weak_mat_tmp12*weak_mat_tmp20 - weak_mat_tmp16*weak_mat_tmp22 - weak_mat_tmp17*weak_mat_tmp6) + weak_mat_tmp5*(grad_u3*weak_mat_tmp9 - weak_mat_tmp0*weak_mat_tmp3);
        const s_t material8 = mu*(grad_u3*weak_mat_tmp21 + s_t(2)*grad_u8 - s_t(6)*weak_mat_tmp1*weak_mat_tmp3 - weak_mat_tmp13*weak_mat_tmp22 + weak_mat_tmp14*weak_mat_tmp22 - weak_mat_tmp16*weak_mat_tmp20 - weak_mat_tmp17*weak_mat_tmp8 + s_t(2)) + weak_mat_tmp5*(-grad_u3*weak_mat_tmp7 + s_t(2)*weak_mat_tmp1*weak_mat_tmp3);
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d3_simplex_apply_block(
    ...
    static_assert(VS > 0, "VS must be positive");
        ...
        const s_t inv_jacobian_determinant = s_t(1) / jacobian_determinant_lane0;
        const s_t grad_u0 = (grad_u_ref0 * jacobian_adjugate_lane0 + grad_u_ref1 * jacobian_adjugate_lane3 + grad_u_ref2 * jacobian_adjugate_lane6) * inv_jacobian_determinant;
        const s_t trial_grad0 = (grad_h_ref0 * jacobian_adjugate_lane0 + grad_h_ref1 * jacobian_adjugate_lane3 + grad_h_ref2 * jacobian_adjugate_lane6) * inv_jacobian_determinant;
        const s_t grad_u1 = (grad_u_ref0 * jacobian_adjugate_lane1 + grad_u_ref1 * jacobian_adjugate_lane4 + grad_u_ref2 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
        const s_t trial_grad1 = (grad_h_ref0 * jacobian_adjugate_lane1 + grad_h_ref1 * jacobian_adjugate_lane4 + grad_h_ref2 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
        const s_t grad_u2 = (grad_u_ref0 * jacobian_adjugate_lane2 + grad_u_ref1 * jacobian_adjugate_lane5 + grad_u_ref2 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
        const s_t trial_grad2 = (grad_h_ref0 * jacobian_adjugate_lane2 + grad_h_ref1 * jacobian_adjugate_lane5 + grad_h_ref2 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
        const s_t grad_u3 = (grad_u_ref3 * jacobian_adjugate_lane0 + grad_u_ref4 * jacobian_adjugate_lane3 + grad_u_ref5 * jacobian_adjugate_lane6) * inv_jacobian_determinant;
        const s_t trial_grad3 = (grad_h_ref3 * jacobian_adjugate_lane0 + grad_h_ref4 * jacobian_adjugate_lane3 + grad_h_ref5 * jacobian_adjugate_lane6) * inv_jacobian_determinant;
        const s_t grad_u4 = (grad_u_ref3 * jacobian_adjugate_lane1 + grad_u_ref4 * jacobian_adjugate_lane4 + grad_u_ref5 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
        const s_t trial_grad4 = (grad_h_ref3 * jacobian_adjugate_lane1 + grad_h_ref4 * jacobian_adjugate_lane4 + grad_h_ref5 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
        const s_t grad_u5 = (grad_u_ref3 * jacobian_adjugate_lane2 + grad_u_ref4 * jacobian_adjugate_lane5 + grad_u_ref5 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
        const s_t trial_grad5 = (grad_h_ref3 * jacobian_adjugate_lane2 + grad_h_ref4 * jacobian_adjugate_lane5 + grad_h_ref5 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
        const s_t grad_u6 = (grad_u_ref6 * jacobian_adjugate_lane0 + grad_u_ref7 * jacobian_adjugate_lane3 + grad_u_ref8 * jacobian_adjugate_lane6) * inv_jacobian_determinant;
        const s_t trial_grad6 = (grad_h_ref6 * jacobian_adjugate_lane0 + grad_h_ref7 * jacobian_adjugate_lane3 + grad_h_ref8 * jacobian_adjugate_lane6) * inv_jacobian_determinant;
        const s_t grad_u7 = (grad_u_ref6 * jacobian_adjugate_lane1 + grad_u_ref7 * jacobian_adjugate_lane4 + grad_u_ref8 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
        const s_t trial_grad7 = (grad_h_ref6 * jacobian_adjugate_lane1 + grad_h_ref7 * jacobian_adjugate_lane4 + grad_h_ref8 * jacobian_adjugate_lane7) * inv_jacobian_determinant;
        const s_t grad_u8 = (grad_u_ref6 * jacobian_adjugate_lane2 + grad_u_ref7 * jacobian_adjugate_lane5 + grad_u_ref8 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
        const s_t trial_grad8 = (grad_h_ref6 * jacobian_adjugate_lane2 + grad_h_ref7 * jacobian_adjugate_lane5 + grad_h_ref8 * jacobian_adjugate_lane8) * inv_jacobian_determinant;
        const s_t weak_mat_tmp0 = s_t(2)*grad_u6;
        const s_t weak_mat_tmp1 = grad_u7*weak_mat_tmp0;
        const s_t weak_mat_tmp2 = grad_u4 + s_t(1);
        const s_t weak_mat_tmp3 = s_t(2)*grad_u3;
        const s_t weak_mat_tmp4 = weak_mat_tmp2*weak_mat_tmp3;
        const s_t weak_mat_tmp5 = mu*(-weak_mat_tmp1 - weak_mat_tmp4);
        const s_t weak_mat_tmp6 = grad_u8 + s_t(1);
        const s_t weak_mat_tmp7 = grad_u3*weak_mat_tmp6;
        const s_t weak_mat_tmp8 = grad_u5*grad_u6 - weak_mat_tmp7;
        const s_t weak_mat_tmp9 = grad_u5*grad_u7;
        ...
        const s_t weak_mat_tmp14 = grad_u5*weak_mat_tmp3;
        const s_t weak_mat_tmp15 = weak_mat_tmp0*weak_mat_tmp6;
        const s_t weak_mat_tmp16 = mu*(-weak_mat_tmp14 - weak_mat_tmp15);
        const s_t weak_mat_tmp17 = grad_u3*grad_u7;
        const s_t weak_mat_tmp18 = grad_u6*weak_mat_tmp2;
        const s_t weak_mat_tmp19 = weak_mat_tmp17 - weak_mat_tmp18;
        const s_t weak_mat_tmp20 = s_t(2)*grad_u2;
        const s_t weak_mat_tmp21 = grad_u5*weak_mat_tmp20;
        const s_t weak_mat_tmp22 = s_t(2)*grad_u1;
        const s_t weak_mat_tmp23 = weak_mat_tmp2*weak_mat_tmp22;
        const s_t weak_mat_tmp24 = mu*(-weak_mat_tmp21 - weak_mat_tmp23);
        const s_t weak_mat_tmp25 = grad_u1*weak_mat_tmp6;
        const s_t weak_mat_tmp26 = grad_u2*grad_u7 - weak_mat_tmp25;
        const s_t weak_mat_tmp27 = grad_u7*weak_mat_tmp22;
        const s_t weak_mat_tmp28 = weak_mat_tmp20*weak_mat_tmp6;
        const s_t weak_mat_tmp29 = mu*(-weak_mat_tmp27 - weak_mat_tmp28);
        const s_t weak_mat_tmp30 = grad_u1*grad_u5;
        const s_t weak_mat_tmp31 = grad_u2*weak_mat_tmp2;
        const s_t weak_mat_tmp32 = weak_mat_tmp30 - weak_mat_tmp31;
        const s_t weak_mat_tmp33 = s_t(2)*pow_2(grad_u5);
        const s_t weak_mat_tmp34 = s_t(2)*pow_2(weak_mat_tmp6) + s_t(2);
        const s_t weak_mat_tmp35 = weak_mat_tmp33 + weak_mat_tmp34;
        const s_t weak_mat_tmp36 = s_t(2)*pow_2(grad_u7);
        ...
        const s_t weak_mat_tmp40 = grad_u1*grad_u6;
        const s_t weak_mat_tmp41 = grad_u0 + s_t(1);
        const s_t weak_mat_tmp42 = grad_u7*weak_mat_tmp41;
        const s_t weak_mat_tmp43 = weak_mat_tmp40 - weak_mat_tmp42;
        const s_t weak_mat_tmp44 = s_t(6)*grad_u7;
        const s_t weak_mat_tmp45 = grad_u2*grad_u3;
        const s_t weak_mat_tmp46 = s_t(2)*weak_mat_tmp45;
        const s_t weak_mat_tmp47 = grad_u5*weak_mat_tmp41;
        const s_t weak_mat_tmp48 = lmbda*(grad_u1*grad_u5*grad_u6 - grad_u1*weak_mat_tmp7 + grad_u2*grad_u3*grad_u7 - grad_u2*weak_mat_tmp18 + weak_mat_tmp2*weak_mat_tmp41*weak_mat_tmp6 - weak_mat_tmp41*weak_mat_tmp9 + s_t(-1));
        const s_t weak_mat_tmp49 = grad_u7*weak_mat_tmp48;
        ...
        const s_t weak_mat_tmp65 = s_t(6)*grad_u4 + s_t(6);
        ...
        const s_t weak_mat_tmp80 = s_t(2)*pow_2(grad_u3);
        const s_t weak_mat_tmp81 = s_t(2)*pow_2(grad_u6);
        const s_t weak_mat_tmp82 = weak_mat_tmp80 + weak_mat_tmp81;
        const s_t weak_mat_tmp83 = s_t(6)*grad_u6;
        const s_t weak_mat_tmp84 = s_t(2)*weak_mat_tmp31;
        const s_t weak_mat_tmp85 = grad_u6*weak_mat_tmp48;
        const s_t weak_mat_tmp86 = mu*(s_t(4)*grad_u1*grad_u5 - weak_mat_tmp83 - weak_mat_tmp84) + weak_mat_tmp85;
        const s_t weak_mat_tmp87 = s_t(2)*weak_mat_tmp42;
        const s_t weak_mat_tmp88 = mu*(s_t(4)*grad_u1*grad_u6 - weak_mat_tmp52 - weak_mat_tmp87) + weak_mat_tmp54;
        const s_t weak_mat_tmp89 = s_t(6)*grad_u3;
        const s_t weak_mat_tmp90 = -s_t(2)*grad_u2*grad_u7;
        const s_t weak_mat_tmp91 = grad_u3*weak_mat_tmp48;
        ...
        const s_t weak_mat_tmp101 = mu*(s_t(4)*grad_u2*grad_u3 - weak_mat_tmp100 - weak_mat_tmp44) + weak_mat_tmp49;
        const s_t weak_mat_tmp102 = s_t(2)*weak_mat_tmp25;
        const s_t weak_mat_tmp103 = mu*(s_t(4)*grad_u2*grad_u7 - weak_mat_tmp102 - weak_mat_tmp89) + weak_mat_tmp91;
        ...
        const s_t weak_mat_tmp116 = s_t(2)*pow_2(grad_u2);
        const s_t weak_mat_tmp117 = weak_mat_tmp116 + weak_mat_tmp34;
        const s_t weak_mat_tmp118 = s_t(2)*pow_2(grad_u1);
        const s_t weak_mat_tmp119 = weak_mat_tmp118 + weak_mat_tmp36;
        const s_t weak_mat_tmp120 = s_t(6)*grad_u2;
        const s_t weak_mat_tmp121 = grad_u2*weak_mat_tmp48;
        const s_t weak_mat_tmp122 = mu*(s_t(4)*grad_u3*grad_u7 - weak_mat_tmp120 - weak_mat_tmp96) + weak_mat_tmp121;
        const s_t weak_mat_tmp123 = s_t(6)*grad_u1;
        const s_t weak_mat_tmp124 = grad_u1*weak_mat_tmp48;
        ...
        const s_t weak_mat_tmp127 = grad_u2*weak_mat_tmp22;
        const s_t weak_mat_tmp128 = mu*(-weak_mat_tmp127 - weak_mat_tmp74);
        const s_t weak_mat_tmp129 = grad_u6*weak_mat_tmp3;
        ...
        const s_t weak_mat_tmp134 = s_t(6)*grad_u0 + s_t(6);
        ...
        const s_t weak_mat_tmp139 = mu*(s_t(4)*grad_u5*grad_u6 - weak_mat_tmp123 - weak_mat_tmp69) + weak_mat_tmp124;
