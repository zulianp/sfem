...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d2_simplex_objective_block(
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        const scalar_t weak_obj_tmp0 = grad_u1*grad_u2;
        const scalar_t weak_obj_tmp1 = grad_u0 + scalar_t(1);
        const scalar_t weak_obj_tmp2 = grad_u3 + scalar_t(1);
        const scalar_t weak_obj_tmp3 = pow_2(grad_u1);
        const scalar_t weak_obj_tmp4 = pow_2(grad_u2);
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d2_simplex_gradient_block(
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        const scalar_t weak_mat_tmp2 = lmbda*(-grad_u1*grad_u2 + weak_mat_tmp0*weak_mat_tmp1 + scalar_t(-1));
        const scalar_t weak_mat_tmp3 = grad_u1*weak_mat_tmp1 + grad_u2*weak_mat_tmp0;
        const scalar_t weak_mat_tmp4 = scalar_t(2)*grad_u1;
        const scalar_t weak_mat_tmp5 = pow_2(grad_u2) + pow_2(weak_mat_tmp1);
        ...
        const scalar_t material0 = mu*(scalar_t(4)*grad_u0 - scalar_t(6)*grad_u3 + scalar_t(2)*weak_mat_tmp1*weak_mat_tmp8 - weak_mat_tmp3*weak_mat_tmp4 - weak_mat_tmp5*weak_mat_tmp6 + scalar_t(-2)) + weak_mat_tmp0*weak_mat_tmp2;
        const scalar_t material1 = -grad_u2*weak_mat_tmp2 + mu*(scalar_t(4)*grad_u1 + scalar_t(6)*grad_u2 - weak_mat_tmp3*weak_mat_tmp6 - weak_mat_tmp4*weak_mat_tmp7 + weak_mat_tmp4*weak_mat_tmp8);
        const scalar_t material2 = -grad_u1*weak_mat_tmp2 + mu*(scalar_t(6)*grad_u1 + scalar_t(4)*grad_u2 - weak_mat_tmp10*weak_mat_tmp3 - weak_mat_tmp5*weak_mat_tmp9 + weak_mat_tmp8*weak_mat_tmp9);
        const scalar_t material3 = mu*(-scalar_t(6)*grad_u0 + scalar_t(4)*grad_u3 + scalar_t(2)*weak_mat_tmp0*weak_mat_tmp8 - weak_mat_tmp10*weak_mat_tmp7 - weak_mat_tmp3*weak_mat_tmp9 + scalar_t(-2)) + weak_mat_tmp1*weak_mat_tmp2;
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d2_simplex_apply_block(
    ...
    static_assert(VECTOR_SIZE > 0, "VECTOR_SIZE must be positive");
        ...
        const scalar_t weak_mat_tmp4 = -grad_u2*weak_mat_tmp1 - grad_u2*weak_mat_tmp3;
        const scalar_t weak_mat_tmp5 = -grad_u1*weak_mat_tmp1 - grad_u1*weak_mat_tmp3;
        ...
        const scalar_t weak_mat_tmp9 = grad_u1*grad_u2;
        ...
        const scalar_t weak_mat_tmp12 = pow_2(grad_u2);
        const scalar_t weak_mat_tmp13 = lmbda*weak_mat_tmp7;
        const scalar_t weak_mat_tmp14 = weak_mat_tmp2*weak_mat_tmp7;
        const scalar_t weak_mat_tmp15 = -grad_u2*weak_mat_tmp13 - grad_u2*weak_mat_tmp14;
        ...
        const scalar_t weak_mat_tmp17 = pow_2(grad_u1);
        const scalar_t weak_mat_tmp18 = -grad_u1*weak_mat_tmp13 - grad_u1*weak_mat_tmp14;
        const scalar_t weak_mat_tmp19 = pow_2(weak_mat_tmp7);
        const scalar_t material0 = trial_grad0*(lmbda*weak_mat_tmp6 + mu*(scalar_t(2)*weak_mat_tmp6 + scalar_t(4))) + trial_grad1*weak_mat_tmp4 + trial_grad2*weak_mat_tmp5 + trial_grad3*weak_mat_tmp11;
        const scalar_t material1 = trial_grad0*weak_mat_tmp4 + trial_grad1*(lmbda*weak_mat_tmp12 + mu*(scalar_t(2)*weak_mat_tmp12 + scalar_t(4))) + trial_grad2*weak_mat_tmp16 + trial_grad3*weak_mat_tmp15;
        const scalar_t material2 = trial_grad0*weak_mat_tmp5 + trial_grad1*weak_mat_tmp16 + trial_grad2*(lmbda*weak_mat_tmp17 + mu*(scalar_t(2)*weak_mat_tmp17 + scalar_t(4))) + trial_grad3*weak_mat_tmp18;
        const scalar_t material3 = trial_grad0*weak_mat_tmp11 + trial_grad1*weak_mat_tmp15 + trial_grad2*weak_mat_tmp18 + trial_grad3*(lmbda*weak_mat_tmp19 + mu*(scalar_t(2)*weak_mat_tmp19 + scalar_t(4)));
        const scalar_t loperand0 = qw * (material0 * jacobian_adjugate_lane0 + material1 * jacobian_adjugate_lane1);
        const scalar_t loperand1 = qw * (material0 * jacobian_adjugate_lane2 + material1 * jacobian_adjugate_lane3);
        const scalar_t loperand2 = qw * (material2 * jacobian_adjugate_lane0 + material3 * jacobian_adjugate_lane1);
        const scalar_t loperand3 = qw * (material2 * jacobian_adjugate_lane2 + material3 * jacobian_adjugate_lane3);
