...
#include "../mooney_rivlin_kelvin_voigt_newmark_elastic_d2_tensor_product_hessian.hpp"
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_proteus_quad4_hessian_i_msoa_find_cols(
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_proteus_quad4_hessian_i_msoa_scatter_bsr(
  ...
  for (int i = 0; i < NS; ++i) {
    ...
    mooney_rivlin_kelvin_voigt_newmark_elastic_proteus_quad4_hessian_i_msoa_find_cols(ev, cols, lenrow, ks);
    for (int j = 0; j < NS; ++j) {
...
static int mooney_rivlin_kelvin_voigt_newmark_elastic_proteus_quad4_hessian_i_msoa_assemble_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const *const RSTR points,
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t element = 0; element < nelements; ++element) {
    ...
    mooney_rivlin_kelvin_voigt_newmark_elastic_d2_tensor_product_direct_hessian_tensor_product_element_matrix<s_t, NQ, NS, VS>(badj0, badj1, badj2, badj3, bdet0, isoparametric_shape_1d, isoparametric_grad_1d, isoparametric_q_weight_1d, lmbda, mu, bu_data, element_matrix);

    if constexpr (FORMAT == 1) {
      mooney_rivlin_kelvin_voigt_newmark_elastic_proteus_quad4_hessian_i_msoa_scatter_bsr(ev, element_matrix, rowptr, colidx, values);
    }
  }
...
extern "C" int mooney_rivlin_kelvin_voigt_newmark_elastic_proteus_quad4_hessian_bsr_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_proteus_quad4_hessian_i_msoa_assemble_impl<double, geom_t, 1>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, rowptr, colidx, (double *)values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
    }
    case (int)sizeof(float): {
        return sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_proteus_quad4_hessian_i_msoa_assemble_impl<float, geom_t, 1>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, rowptr, colidx, (float *)values, nullptr, 0, 0, nullptr, nullptr, nullptr, nullptr);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_newmark_elastic_proteus_quad4_hessian_bsr_i_msoa", -1, (int)scalar_bytes);
}
