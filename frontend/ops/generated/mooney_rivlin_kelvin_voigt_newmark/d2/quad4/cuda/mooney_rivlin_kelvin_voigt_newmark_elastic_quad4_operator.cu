...
static const KernelDiagnostics mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_objective_soa_diagnostics_data = {
  "mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_objective_soa",
  "QUAD4",
  2,
  4,
  4,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_objective_soa_diagnostics(void) {
  return &sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_objective_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
__global__ void mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_objective_steps_i_msoa_impl(
  ...
  for (ptrdiff_t evb = (ptrdiff_t)blockIdx.x * blockDim.x + threadIdx.x; evb < nelements; evb += (ptrdiff_t)blockDim.x * gridDim.x) {
    ...
    mooney_rivlin_kelvin_voigt_newmark_elastic_d2_tensor_product_objective_block<s_t, NQ, NS, VS>(ne, VS, badj0, badj1, badj2, badj3, bdet0, isoparametric_shape_1d, isoparametric_grad_1d, isoparametric_q_weight_1d, lmbda, mu, bu_streams, bh_streams, nsteps, steps, nelements, &value[evb]);
  }
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_objective_steps_i_msoa(
  ...
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_objective_steps_i_msoa", -1, (int)scalar_bytes);
}
...
static const KernelDiagnostics mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_soa_diagnostics_data = {
  "mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_soa",
  "QUAD4",
  2,
  4,
  4,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_soa_diagnostics(void) {
  return &sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
__global__ void mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_i_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, out_stride, (double *)outx, (double *)outy);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_i_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, out_stride, (float *)outx, (float *)outy);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_gradient_i_msoa", -1, (int)scalar_bytes);
}
...
static const KernelDiagnostics mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_soa_diagnostics_data = {
  "mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_soa",
  "QUAD4",
  2,
  4,
  4,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_soa_diagnostics(void) {
  return &sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_soa_diagnostics_data;
}
...
template <typename s_t, typename g_t, int VS>
__global__ void mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_i_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const *const RSTR points,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_i_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, h_stride, (const double *)hx, (const double *)hy, out_stride, (double *)outx, (double *)outy);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_i_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, h_stride, (const float *)hx, (const float *)hy, out_stride, (float *)outx, (float *)outy);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_i_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_newmark_elastic_quad4_apply_i_msoa", -1, (int)scalar_bytes);
}
