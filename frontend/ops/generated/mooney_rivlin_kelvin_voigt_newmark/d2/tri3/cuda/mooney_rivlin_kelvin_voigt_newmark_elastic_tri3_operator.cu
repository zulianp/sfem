...
template <typename s_t, typename g_t, int VS>
__global__ void mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_objective_a_msoa_impl(
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_objective_a_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        ...
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_objective_a_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, (double *)value);
        return SFEM_SUCCESS;
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_objective_a_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, (float *)value);
        return SFEM_SUCCESS;
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_objective_a_msoa", -1, (int)scalar_bytes);
}
