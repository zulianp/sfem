...
#include "../../cuda/mooney_rivlin_kelvin_voigt_newmark_elastic_d2_simplex_local.cuh"
...
template <typename s_t, typename g_t, int VS>
__global__ void mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_objective_steps_a_msoa_impl(
...
template <typename s_t, typename g_t, int VS>
__global__ void mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_gradient_a_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const RSTR g_adj0,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_gradient_a_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_gradient_a_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, out_stride, (double *)outx, (double *)outy);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_gradient_a_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_gradient_a_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, out_stride, (float *)outx, (float *)outy);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_gradient_a_msoa_impl");
    }
    default:
      break;
...
template <typename s_t, typename g_t, int VS>
__global__ void mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_apply_a_msoa_impl(
        const ptrdiff_t nelements,
        const ptrdiff_t,
        idx_t **const RSTR elements,
        const g_t *const RSTR g_adj0,
...
extern "C" int cu_mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_apply_a_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_apply_a_msoa_impl<double, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, lmbda, mu, u_stride, (const double *)ux, (const double *)uy, h_stride, (const double *)hx, (const double *)hy, out_stride, (double *)outx, (double *)outy);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_apply_a_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_apply_a_msoa_impl<float, geom_t, 1><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, lmbda, mu, u_stride, (const float *)ux, (const float *)uy, h_stride, (const float *)hx, (const float *)hy, out_stride, (float *)outx, (float *)outy);
        return sfem::codegen::launch_status("mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_apply_a_msoa_impl");
    }
    default:
      break;
