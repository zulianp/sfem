...
static SFEM_INLINE int mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_energy_element_coords_soa(
    ...
    for (ptrdiff_t evbegin = 0; evbegin < nelements; evbegin += VECTOR_SIZE) {
        ...
        scalar_t block_jacobian_adjugate3[N_QP * VECTOR_SIZE];
        scalar_t block_jacobian_determinant0[N_QP * VECTOR_SIZE];
        const scalar_t *const grad_ref_x = sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_isoparametric_reference_data<scalar_t>::grad_ref_x();
        const scalar_t *const grad_ref_y = sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_elastic_tri3_isoparametric_reference_data<scalar_t>::grad_ref_y();
        for (int q = 0; q < N_QP; ++q) {
