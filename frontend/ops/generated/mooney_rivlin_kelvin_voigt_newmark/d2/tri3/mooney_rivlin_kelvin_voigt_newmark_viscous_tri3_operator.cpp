...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
    ...
    mooney_rivlin_kelvin_voigt_newmark_viscous_d2_simplex_residual_block_contiguous<s_t, NQ, NS, VS>(ne, 0, bageom_streams[4], badjugate, affine_shape, affine_grad_ref_x, affine_grad_ref_y, affine_q_weight, bcurrent, bprevious, eta_b, eta_s, newmark_velocity_alpha, boutput);
...
extern "C" int mooney_rivlin_kelvin_voigt_newmark_viscous_tri3_jacobian_action_esoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_viscous_d2_simplex_jacobian_action_block<double, 1, 3, 16>(ne, geometry_stride, (const double *)determinant, (const double *const *)adjugate, sfem::codegen::ref_tri3_q1<double>::shape(), sfem::codegen::ref_tri3_q1<double>::grad_ref_x(), sfem::codegen::ref_tri3_q1<double>::grad_ref_y(), sfem::codegen::quad_tri_q1<double>::q_weight(), (const double *const *)current, (const double *const *)previous, (const double *const *)direction, eta_b, eta_s, newmark_velocity_alpha, (double *const *)output);
        return SFEM_SUCCESS;
    }
    case (int)sizeof(float): {
        sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_viscous_d2_simplex_jacobian_action_block<float, 1, 3, 16>(ne, geometry_stride, (const float *)determinant, (const float *const *)adjugate, sfem::codegen::ref_tri3_q1<float>::shape(), sfem::codegen::ref_tri3_q1<float>::grad_ref_x(), sfem::codegen::ref_tri3_q1<float>::grad_ref_y(), sfem::codegen::quad_tri_q1<float>::q_weight(), (const float *const *)current, (const float *const *)previous, (const float *const *)direction, eta_b, eta_s, newmark_velocity_alpha, (float *const *)output);
        return SFEM_SUCCESS;
    }
    default:
      break;
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
    ...
    mooney_rivlin_kelvin_voigt_newmark_viscous_d2_simplex_jacobian_action_block_contiguous<s_t, NQ, NS, VS>(ne, 0, bageom_streams[4], badjugate, affine_shape, affine_grad_ref_x, affine_grad_ref_y, affine_q_weight, bcurrent, bprevious, bdirection, eta_b, eta_s, newmark_velocity_alpha, boutput);
...
static SFEM_INLINE int mooney_rivlin_kelvin_voigt_newmark_viscous_tri3_hessian_crs_i_msoa_impl(
  ...
  const s_t *const isoparametric_shape = sfem::codegen::ref_tri3_q1<s_t>::shape();
...
#pragma omp parallel for schedule(static)
  for (ptrdiff_t element = 0; element < nelements; ++element) {
    ...
    for (int trial_local = 0; trial_local < 6; ++trial_local) {
      ...
      mooney_rivlin_kelvin_voigt_newmark_viscous_d2_simplex_jacobian_action_block_contiguous<s_t, NQ, NS, VS>(1, 1, bdeterminant, badjugate, isoparametric_shape, isoparametric_grad_ref_x, isoparametric_grad_ref_y, isoparametric_q_weight, bcurrent, bprevious, bdirection, eta_b, eta_s, newmark_velocity_alpha, boutput);
      for (int test_local = 0; test_local < 6; ++test_local) {
        const int test = row_tensor_stream(test_local);
        element_matrix[test_local * 6 + trial_local] = boutput[test][0];
      }
    }
...
extern "C" int mooney_rivlin_kelvin_voigt_newmark_viscous_tri3_hessian_bsr_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
      return sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_viscous_tri3_hessian_crs_i_msoa_impl<double>(nelements, nnodes, elements, points, eta_b, eta_s, newmark_velocity_alpha, current_stride, (const double *)u0, (const double *)u1, previous_stride, (const double *)u0_old, (const double *)u1_old, rowptr, colidx, (double *)values);
    }
    case (int)sizeof(float): {
      return sfem::codegen::mooney_rivlin_kelvin_voigt_newmark_viscous_tri3_hessian_crs_i_msoa_impl<float>(nelements, nnodes, elements, points, eta_b, eta_s, newmark_velocity_alpha, current_stride, (const float *)u0, (const float *)u1, previous_stride, (const float *)u0_old, (const float *)u1_old, rowptr, colidx, (float *)values);
    }
    default:
      break;
  }
