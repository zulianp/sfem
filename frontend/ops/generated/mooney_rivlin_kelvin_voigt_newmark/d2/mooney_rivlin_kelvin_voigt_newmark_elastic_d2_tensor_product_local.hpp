#ifndef MOONEY_RIVLIN_KELVIN_VOIGT_NEWMARK_ELASTIC_D2_TENSOR_PRODUCT_LOCAL_HPP
#define MOONEY_RIVLIN_KELVIN_VOIGT_NEWMARK_ELASTIC_D2_TENSOR_PRODUCT_LOCAL_HPP
#include <math.h>
#include <stddef.h>
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d2_tensor_product_objective_block(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        const scalar_t weak_obj_tmp0 = grad_u[1]*grad_u[2];
        const scalar_t weak_obj_tmp1 = grad_u[0] + scalar_t(1);
        const scalar_t weak_obj_tmp2 = grad_u[3] + scalar_t(1);
        const scalar_t weak_obj_tmp3 = pow_2(grad_u[1]);
        const scalar_t weak_obj_tmp4 = pow_2(grad_u[2]);
        const scalar_t weak_obj_tmp5 = pow_2(weak_obj_tmp1);
        const scalar_t weak_obj_tmp6 = pow_2(weak_obj_tmp2);
        const scalar_t weak_obj_tmp7 = weak_obj_tmp3 + weak_obj_tmp6;
        const scalar_t weak_obj_tmp8 = weak_obj_tmp4 + weak_obj_tmp5;
        value[lane] += qw * jacobian_determinant_lane0 * (((scalar_t(1) / scalar_t(2)))*lmbda*pow_2(-weak_obj_tmp0 + weak_obj_tmp1*weak_obj_tmp2 + scalar_t(-1)) + mu*(scalar_t(6)*weak_obj_tmp0 - scalar_t(6)*weak_obj_tmp1*weak_obj_tmp2 + scalar_t(2)*weak_obj_tmp3 + scalar_t(2)*weak_obj_tmp4 + scalar_t(2)*weak_obj_tmp5 + scalar_t(2)*weak_obj_tmp6 - (scalar_t(1) / scalar_t(2))*pow_2(weak_obj_tmp7) - (scalar_t(1) / scalar_t(2))*pow_2(weak_obj_tmp8) + ((scalar_t(1) / scalar_t(2)))*pow_2(weak_obj_tmp7 + weak_obj_tmp8) - pow_2(grad_u[1]*weak_obj_tmp1 + grad_u[2]*weak_obj_tmp2) + scalar_t(1)));
        }
    }
}
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d2_tensor_product_gradient_block(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        const scalar_t weak_mat_tmp2 = lmbda*(-grad_u[1]*grad_u[2] + weak_mat_tmp0*weak_mat_tmp1 + scalar_t(-1));
        const scalar_t weak_mat_tmp3 = grad_u[1]*weak_mat_tmp1 + grad_u[2]*weak_mat_tmp0;
        const scalar_t weak_mat_tmp4 = scalar_t(2)*grad_u[1];
        const scalar_t weak_mat_tmp5 = pow_2(grad_u[2]) + pow_2(weak_mat_tmp1);
        const scalar_t weak_mat_tmp6 = scalar_t(2)*weak_mat_tmp1;
        const scalar_t weak_mat_tmp7 = pow_2(grad_u[1]) + pow_2(weak_mat_tmp0);
        const scalar_t weak_mat_tmp8 = weak_mat_tmp5 + weak_mat_tmp7;
        const scalar_t weak_mat_tmp9 = scalar_t(2)*grad_u[2];
        const scalar_t weak_mat_tmp10 = scalar_t(2)*weak_mat_tmp0;
        material[0] = mu*(scalar_t(4)*grad_u[0] - scalar_t(6)*grad_u[3] + scalar_t(2)*weak_mat_tmp1*weak_mat_tmp8 - weak_mat_tmp3*weak_mat_tmp4 - weak_mat_tmp5*weak_mat_tmp6 + scalar_t(-2)) + weak_mat_tmp0*weak_mat_tmp2;
        material[1] = -grad_u[2]*weak_mat_tmp2 + mu*(scalar_t(4)*grad_u[1] + scalar_t(6)*grad_u[2] - weak_mat_tmp3*weak_mat_tmp6 - weak_mat_tmp4*weak_mat_tmp7 + weak_mat_tmp4*weak_mat_tmp8);
        material[2] = -grad_u[1]*weak_mat_tmp2 + mu*(scalar_t(6)*grad_u[1] + scalar_t(4)*grad_u[2] - weak_mat_tmp10*weak_mat_tmp3 - weak_mat_tmp5*weak_mat_tmp9 + weak_mat_tmp8*weak_mat_tmp9);
        material[3] = mu*(-scalar_t(6)*grad_u[0] + scalar_t(4)*grad_u[3] + scalar_t(2)*weak_mat_tmp0*weak_mat_tmp8 - weak_mat_tmp10*weak_mat_tmp7 - weak_mat_tmp3*weak_mat_tmp9 + scalar_t(-2)) + weak_mat_tmp1*weak_mat_tmp2;
...
static SFEM_INLINE void mooney_rivlin_kelvin_voigt_newmark_elastic_d2_tensor_product_apply_block(
    ...
    for (int q = 0; q < N_QP; ++q) {
        ...
        scalar_t material[4];
        const scalar_t weak_mat_tmp0 = grad_u[3] + scalar_t(1);
        const scalar_t weak_mat_tmp1 = lmbda*weak_mat_tmp0;
        const scalar_t weak_mat_tmp2 = scalar_t(2)*mu;
        const scalar_t weak_mat_tmp3 = weak_mat_tmp0*weak_mat_tmp2;
        const scalar_t weak_mat_tmp4 = -grad_u[2]*weak_mat_tmp1 - grad_u[2]*weak_mat_tmp3;
        const scalar_t weak_mat_tmp5 = -grad_u[1]*weak_mat_tmp1 - grad_u[1]*weak_mat_tmp3;
        const scalar_t weak_mat_tmp6 = pow_2(weak_mat_tmp0);
        const scalar_t weak_mat_tmp7 = grad_u[0] + scalar_t(1);
        const scalar_t weak_mat_tmp8 = weak_mat_tmp0*weak_mat_tmp7;
        const scalar_t weak_mat_tmp9 = grad_u[1]*grad_u[2];
        const scalar_t weak_mat_tmp10 = lmbda*(weak_mat_tmp0*weak_mat_tmp7 - weak_mat_tmp9 + scalar_t(-1));
        const scalar_t weak_mat_tmp11 = lmbda*weak_mat_tmp8 + mu*(scalar_t(4)*weak_mat_tmp0*weak_mat_tmp7 - scalar_t(2)*weak_mat_tmp9 + scalar_t(-6)) + weak_mat_tmp10;
        const scalar_t weak_mat_tmp12 = pow_2(grad_u[2]);
        const scalar_t weak_mat_tmp13 = lmbda*weak_mat_tmp7;
        const scalar_t weak_mat_tmp14 = weak_mat_tmp2*weak_mat_tmp7;
        const scalar_t weak_mat_tmp15 = -grad_u[2]*weak_mat_tmp13 - grad_u[2]*weak_mat_tmp14;
        const scalar_t weak_mat_tmp16 = lmbda*weak_mat_tmp9 + mu*(-scalar_t(2)*weak_mat_tmp8 + scalar_t(4)*weak_mat_tmp9 + scalar_t(6)) - weak_mat_tmp10;
        const scalar_t weak_mat_tmp17 = pow_2(grad_u[1]);
        const scalar_t weak_mat_tmp18 = -grad_u[1]*weak_mat_tmp13 - grad_u[1]*weak_mat_tmp14;
        const scalar_t weak_mat_tmp19 = pow_2(weak_mat_tmp7);
        material[0] = trial_grad[0]*(lmbda*weak_mat_tmp6 + mu*(scalar_t(2)*weak_mat_tmp6 + scalar_t(4))) + trial_grad[1]*weak_mat_tmp4 + trial_grad[2]*weak_mat_tmp5 + trial_grad[3]*weak_mat_tmp11;
        material[1] = trial_grad[0]*weak_mat_tmp4 + trial_grad[1]*(lmbda*weak_mat_tmp12 + mu*(scalar_t(2)*weak_mat_tmp12 + scalar_t(4))) + trial_grad[2]*weak_mat_tmp16 + trial_grad[3]*weak_mat_tmp15;
        material[2] = trial_grad[0]*weak_mat_tmp5 + trial_grad[1]*weak_mat_tmp16 + trial_grad[2]*(lmbda*weak_mat_tmp17 + mu*(scalar_t(2)*weak_mat_tmp17 + scalar_t(4))) + trial_grad[3]*weak_mat_tmp18;
        material[3] = trial_grad[0]*weak_mat_tmp11 + trial_grad[1]*weak_mat_tmp15 + trial_grad[2]*weak_mat_tmp18 + trial_grad[3]*(lmbda*weak_mat_tmp19 + mu*(scalar_t(2)*weak_mat_tmp19 + scalar_t(4)));
        loperand[0] = qw * (material[0] * jacobian_adjugate_lane0 + material[1] * jacobian_adjugate_lane1);
        loperand[1] = qw * (material[0] * jacobian_adjugate_lane2 + material[1] * jacobian_adjugate_lane3);
        loperand[2] = qw * (material[2] * jacobian_adjugate_lane0 + material[3] * jacobian_adjugate_lane1);
        loperand[3] = qw * (material[2] * jacobian_adjugate_lane2 + material[3] * jacobian_adjugate_lane3);
