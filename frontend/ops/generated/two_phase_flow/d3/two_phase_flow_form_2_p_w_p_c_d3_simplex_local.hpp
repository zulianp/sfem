...
static SFEM_INLINE void two_phase_flow_form_2_p_w_p_c_d3_simplex_jacobian_action_block(
    ...
    for (int q = 0; q < NQ; ++q) {
        ...
        for (int lane = 0; lane < nelems; ++lane) {
            ...
            const s_t residual_tmp5 = S_res + s_t(-1);
            const s_t residual_tmp6 = p_c_direction*rho_w0/dt;
            const s_t residual_tmp7 = pow_m1(residual_tmp2);
            const s_t residual_tmp8 = pow_m1(residual_tmp0);
            const s_t residual_tmp9 = exp(kappa_T*(p_w - p_wr));
            const s_t residual_tmp10 = residual_tmp1*residual_tmp3*residual_tmp7*residual_tmp8*residual_tmp9;
            const s_t residual_tmp11 = K_0*p_w_grad_0 + K_1*p_w_grad_1 + K_2*p_w_grad_2;
            const s_t residual_tmp12 = residual_tmp4*residual_tmp5;
            const s_t residual_tmp13 = S_res - residual_tmp12;
            const s_t residual_tmp14 = pow(residual_tmp13, (s_t(-1) / s_t(2)));
            const s_t residual_tmp15 = dt*residual_tmp10*residual_tmp12*residual_tmp14;
            const s_t residual_tmp16 = pow(residual_tmp13, pow_m1(C_kw1));
            const s_t residual_tmp17 = s_t(1) - residual_tmp16;
            const s_t residual_tmp18 = pow(residual_tmp17, C_kw1);
            const s_t residual_tmp19 = residual_tmp18 + s_t(-1);
            const s_t residual_tmp20 = ((s_t(1) / s_t(2)))*pow_2(residual_tmp19);
        ...
        for (int test = 0; test < NS; ++test) {
            ...
            for (int lane = 0; lane < nelems; ++lane) {
                ...
                output[test * NC + 0][lane] += q_weight[q] * det * (value_coeff0_values[lane] * test_value + grad_coeff0_0_values[lane] * test_grad0 + grad_coeff0_1_values[lane] * test_grad1 + grad_coeff0_2_values[lane] * test_grad2);
            }
        }
    }
}
