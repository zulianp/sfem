...
__host__ __device__ __forceinline__ void two_phase_flow_form_1_p_c_d3_tensor_product_residual_block(
    const int ne,
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    s_t *const RSTR grad_coeff_ref_q1_2 = &grad_coeff_ref[((NQ + q) * ND + 2) * VS];
    {
      ...
      grad_coeff_ref_q0_0[0] = s_t(0);
      grad_coeff_ref_q0_1[0] = s_t(0);
      grad_coeff_ref_q0_2[0] = s_t(0);
...
template <typename s_t, int NQ, int NS, int VS>
__host__ __device__ __forceinline__ void two_phase_flow_form_1_p_c_d3_tensor_product_residual_block_contiguous(
    const int ne,
    const ptrdiff_t geometry_stride,
    const s_t *const RSTR determinant,
    const s_t *const RSTR adjugate[9],
