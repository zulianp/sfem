...
template <typename scalar_t>
static SFEM_INLINE count_t two_phase_flow_form_2_p_c_p_c_tet4_hessian_crs_isoparametric_mesh_soa_find_col(
        const idx_t row,
        const idx_t col,
...
static SFEM_INLINE int two_phase_flow_form_2_p_c_p_c_tet4_hessian_crs_isoparametric_mesh_soa_scatter_crs(
    ...
    for (int i = 0; i < N_SHAPE; ++i) {
        ...
        for (int j = 0; j < N_SHAPE; ++j) {
            const count_t entry = two_phase_flow_form_2_p_c_p_c_tet4_hessian_crs_isoparametric_mesh_soa_find_col<scalar_t>(ev[i], ev[j], rowptr, colidx);
            if (entry == row_end) {
                if (valid_graph) {
