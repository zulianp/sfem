...
static SFEM_INLINE int two_phase_flow_form_1_p_w_tri3_residual_affine_mesh_soa_impl(
    ...
    (void)nnodes;
    const s_t *const affine_shape = sfem::codegen::two_phase_flow_form_1_p_w_tri3_affine_reference_data<s_t>::shape();
    const s_t *const affine_grad_ref_x = sfem::codegen::two_phase_flow_form_1_p_w_tri3_affine_reference_data<s_t>::grad_ref_x();
    const s_t *const affine_grad_ref_y = sfem::codegen::two_phase_flow_form_1_p_w_tri3_affine_reference_data<s_t>::grad_ref_y();
    const s_t *const affine_q_weight = sfem::codegen::two_phase_flow_form_1_p_w_tri3_affine_reference_data<s_t>::q_weight();
...
#pragma omp parallel for schedule(static)
    for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
        ...
        two_phase_flow_form_1_p_w_d2_simplex_residual_block_contiguous<s_t, NQ, NS, VS>(nelems, 0, bageom_streams[4], block_adjugate, affine_shape, affine_grad_ref_x, affine_grad_ref_y, affine_q_weight, block_current, block_previous, C_kw1, K_0, K_1, K_2, K_3, P_r, S_res, dt, kappa_T, m, mu_w, p_wr, porosity, rho_w0, block_output);
...
#pragma omp parallel for schedule(static)
    for (ptrdiff_t evb = 0; evb < nelements; evb += VS) {
        ...
        two_phase_flow_form_1_p_w_d2_simplex_jacobian_action_block_contiguous<s_t, NQ, NS, VS>(nelems, 0, bageom_streams[0], affine_shape, affine_q_weight, block_output);
