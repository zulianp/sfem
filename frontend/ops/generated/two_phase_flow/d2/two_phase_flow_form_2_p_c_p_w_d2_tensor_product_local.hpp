#ifndef TWO_PHASE_FLOW_FORM_2_P_C_P_W_D2_TENSOR_PRODUCT_LOCAL_HPP
#define TWO_PHASE_FLOW_FORM_2_P_C_P_W_D2_TENSOR_PRODUCT_LOCAL_HPP
...
template <typename s_t, int NQ, int NS, int VS>
static SFEM_INLINE void two_phase_flow_form_2_p_c_p_w_d2_tensor_product_residual_block_contiguous(
    const int ne,
    const ptrdiff_t geometry_stride,
    const s_t *const RSTR determinant,
    const s_t *const RSTR shape_1d,
...
static SFEM_INLINE void two_phase_flow_form_2_p_c_p_w_d2_tensor_product_jacobian_action_block(
  ...
  for (int q = 0; q < NQ; ++q) {
    ...
    for (int lane = 0; lane < ne; ++lane) {
      ...
      const s_t residual_tmp9 = residual_tmp8*(-K_0*p_c_grad_0 - K_1*p_c_grad_1);
...
template <typename s_t, int NQ, int NS, int VS>
static SFEM_INLINE void two_phase_flow_form_2_p_c_p_w_d2_tensor_product_jacobian_action_block_contiguous(
    const int ne,
    const ptrdiff_t geometry_stride,
    const s_t *const RSTR determinant,
    const s_t *const RSTR adjugate[4],
