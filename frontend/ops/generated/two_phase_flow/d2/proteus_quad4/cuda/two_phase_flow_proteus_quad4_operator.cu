...
static const KernelDiagnostics two_phase_flow_proteus_quad4_jacobian_p_w_p_w_diagnostics_data = {
  "two_phase_flow_proteus_quad4_jacobian_p_w_p_w",
  "PROTEUS_QUAD4",
  2,
  16,
  4,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_two_phase_flow_proteus_quad4_jacobian_p_w_p_w_diagnostics(void) {
  return &sfem::codegen::two_phase_flow_proteus_quad4_jacobian_p_w_p_w_diagnostics_data;
}

namespace sfem {
namespace codegen {

static const KernelDiagnostics two_phase_flow_proteus_quad4_jacobian_p_w_p_c_diagnostics_data = {
  "two_phase_flow_proteus_quad4_jacobian_p_w_p_c",
  "PROTEUS_QUAD4",
  2,
  16,
  4,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_two_phase_flow_proteus_quad4_jacobian_p_w_p_c_diagnostics(void) {
  return &sfem::codegen::two_phase_flow_proteus_quad4_jacobian_p_w_p_c_diagnostics_data;
}

namespace sfem {
namespace codegen {

static const KernelDiagnostics two_phase_flow_proteus_quad4_jacobian_p_c_p_w_diagnostics_data = {
  "two_phase_flow_proteus_quad4_jacobian_p_c_p_w",
  "PROTEUS_QUAD4",
  2,
  16,
  4,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_two_phase_flow_proteus_quad4_jacobian_p_c_p_w_diagnostics(void) {
  return &sfem::codegen::two_phase_flow_proteus_quad4_jacobian_p_c_p_w_diagnostics_data;
}

namespace sfem {
namespace codegen {

static const KernelDiagnostics two_phase_flow_proteus_quad4_jacobian_p_c_p_c_diagnostics_data = {
  "two_phase_flow_proteus_quad4_jacobian_p_c_p_c",
  "PROTEUS_QUAD4",
  2,
  16,
  4,
...
extern "C" const sfem::codegen::KernelDiagnostics *cu_two_phase_flow_proteus_quad4_jacobian_p_c_p_c_diagnostics(void) {
  return &sfem::codegen::two_phase_flow_proteus_quad4_jacobian_p_c_p_c_diagnostics_data;
}
...
extern "C" int cu_two_phase_flow_proteus_quad4_residual_esoa(
  ...
  return sfem::codegen::unsupported_dispatch("two_phase_flow_proteus_quad4_residual_esoa", -1, (int)scalar_bytes);
}
...
__global__ void two_phase_flow_proteus_quad4_residual_a_msoa_impl(
...
extern "C" int cu_two_phase_flow_proteus_quad4_residual_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t nelements,
  ...
  return sfem::codegen::unsupported_dispatch("two_phase_flow_proteus_quad4_residual_a_msoa", -1, (int)scalar_bytes);
}
...
__global__ void two_phase_flow_proteus_quad4_residual_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
...
extern "C" int cu_two_phase_flow_proteus_quad4_residual_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::two_phase_flow_proteus_quad4_residual_i_msoa_impl<double><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, C_ka1, C_ka2, C_kw1, K_0, K_1, K_2, K_3, M_c, P_r, R, S_res, T, Z, dt, kappa_T, m, mu_c, mu_w, p_wr, porosity, rho_w0, current_stride, (const double *)p_w, (const double *)p_c, previous_stride, (const double *)p_w_old, (const double *)p_c_old, out_stride, (double *)p_w_out, (double *)p_c_out);
        return sfem::codegen::launch_status("two_phase_flow_proteus_quad4_residual_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
  ...
  return sfem::codegen::unsupported_dispatch("two_phase_flow_proteus_quad4_residual_i_msoa", -1, (int)scalar_bytes);
}
...
extern "C" int cu_two_phase_flow_proteus_quad4_residual_i_maos(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return cu_two_phase_flow_proteus_quad4_residual_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const double *)parameters)[0], ((const double *)parameters)[1], ((const double *)parameters)[2], ((const double *)parameters)[3], ((const double *)parameters)[4], ((const double *)parameters)[5], ((const double *)parameters)[6], ((const double *)parameters)[7], ((const double *)parameters)[8], ((const double *)parameters)[9], ((const double *)parameters)[10], ((const double *)parameters)[11], ((const double *)parameters)[12], ((const double *)parameters)[13], ((const double *)parameters)[14], ((const double *)parameters)[15], ((const double *)parameters)[16], ((const double *)parameters)[17], ((const double *)parameters)[18], ((const double *)parameters)[19], ((const double *)parameters)[20], 2, (const double *)current + 0, (const double *)current + 1, 2, (const double *)previous + 0, (const double *)previous + 1, 2, (double *)output + 0, (double *)output + 1, stream);
    }
    case (int)sizeof(float): {
        return cu_two_phase_flow_proteus_quad4_residual_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const float *)parameters)[0], ((const float *)parameters)[1], ((const float *)parameters)[2], ((const float *)parameters)[3], ((const float *)parameters)[4], ((const float *)parameters)[5], ((const float *)parameters)[6], ((const float *)parameters)[7], ((const float *)parameters)[8], ((const float *)parameters)[9], ((const float *)parameters)[10], ((const float *)parameters)[11], ((const float *)parameters)[12], ((const float *)parameters)[13], ((const float *)parameters)[14], ((const float *)parameters)[15], ((const float *)parameters)[16], ((const float *)parameters)[17], ((const float *)parameters)[18], ((const float *)parameters)[19], ((const float *)parameters)[20], 2, (const float *)current + 0, (const float *)current + 1, 2, (const float *)previous + 0, (const float *)previous + 1, 2, (float *)output + 0, (float *)output + 1, stream);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("two_phase_flow_proteus_quad4_residual_i_maos", -1, (int)scalar_bytes);
}

extern "C" int cu_two_phase_flow_proteus_quad4_jacobian_action_esoa(
    const int scalar_bytes,
    const int ne,
    const ptrdiff_t geometry_stride,
    const void *const RSTR determinant,
  ...
  return sfem::codegen::unsupported_dispatch("two_phase_flow_proteus_quad4_jacobian_action_esoa", -1, (int)scalar_bytes);
}
...
template <typename s_t, typename g_t>
__global__ void two_phase_flow_proteus_quad4_jacobian_action_a_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const g_t *const RSTR g_adj0,
...
extern "C" int cu_two_phase_flow_proteus_quad4_jacobian_action_a_msoa(
    const int scalar_bytes,
    const ptrdiff_t nelements,
    const ptrdiff_t nnodes,
    idx_t **const RSTR elements,
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::two_phase_flow_proteus_quad4_jacobian_action_a_msoa_impl<double, geom_t><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, C_ka1, C_ka2, C_kw1, K_0, K_1, K_2, K_3, M_c, P_r, R, S_res, T, Z, dt, kappa_T, m, mu_c, mu_w, p_wr, porosity, rho_w0, current_stride, (const double *)p_w, (const double *)p_c, direction_stride, (const double *)p_w_direction, (const double *)p_c_direction, out_stride, (double *)p_w_out, (double *)p_c_out);
        return sfem::codegen::launch_status("two_phase_flow_proteus_quad4_jacobian_action_a_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::two_phase_flow_proteus_quad4_jacobian_action_a_msoa_impl<float, geom_t><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, g_adj0, g_adj1, g_adj2, g_adj3, g_det0, C_ka1, C_ka2, C_kw1, K_0, K_1, K_2, K_3, M_c, P_r, R, S_res, T, Z, dt, kappa_T, m, mu_c, mu_w, p_wr, porosity, rho_w0, current_stride, (const float *)p_w, (const float *)p_c, direction_stride, (const float *)p_w_direction, (const float *)p_c_direction, out_stride, (float *)p_w_out, (float *)p_c_out);
        return sfem::codegen::launch_status("two_phase_flow_proteus_quad4_jacobian_action_a_msoa_impl");
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("two_phase_flow_proteus_quad4_jacobian_action_a_msoa", -1, (int)scalar_bytes);
}
...
template <typename s_t>
__global__ void two_phase_flow_proteus_quad4_jacobian_action_i_msoa_impl(
    const ptrdiff_t nelements,
    const ptrdiff_t,
    idx_t **const RSTR elements,
    const geom_t *const *const RSTR points,
...
extern "C" int cu_two_phase_flow_proteus_quad4_jacobian_action_i_msoa(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
        sfem::codegen::two_phase_flow_proteus_quad4_jacobian_action_i_msoa_impl<double><<<grid_size, block_size, 0, (cudaStream_t)stream>>>(nelements, nnodes, elements, points, C_ka1, C_ka2, C_kw1, K_0, K_1, K_2, K_3, M_c, P_r, R, S_res, T, Z, dt, kappa_T, m, mu_c, mu_w, p_wr, porosity, rho_w0, current_stride, (const double *)p_w, (const double *)p_c, direction_stride, (const double *)p_w_direction, (const double *)p_c_direction, out_stride, (double *)p_w_out, (double *)p_c_out);
        return sfem::codegen::launch_status("two_phase_flow_proteus_quad4_jacobian_action_i_msoa_impl");
    }
    case (int)sizeof(float): {
        const int block_size = 256;
        const int grid_size = (int)((nelements + block_size - 1) / block_size);
  ...
  return sfem::codegen::unsupported_dispatch("two_phase_flow_proteus_quad4_jacobian_action_i_msoa", -1, (int)scalar_bytes);
}
...
extern "C" int cu_two_phase_flow_proteus_quad4_jacobian_action_i_maos(
  ...
  switch (scalar_bytes) {
    case (int)sizeof(double): {
        return cu_two_phase_flow_proteus_quad4_jacobian_action_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const double *)parameters)[0], ((const double *)parameters)[1], ((const double *)parameters)[2], ((const double *)parameters)[3], ((const double *)parameters)[4], ((const double *)parameters)[5], ((const double *)parameters)[6], ((const double *)parameters)[7], ((const double *)parameters)[8], ((const double *)parameters)[9], ((const double *)parameters)[10], ((const double *)parameters)[11], ((const double *)parameters)[12], ((const double *)parameters)[13], ((const double *)parameters)[14], ((const double *)parameters)[15], ((const double *)parameters)[16], ((const double *)parameters)[17], ((const double *)parameters)[18], ((const double *)parameters)[19], ((const double *)parameters)[20], 2, (const double *)current + 0, (const double *)current + 1, 2, (const double *)direction + 0, (const double *)direction + 1, 2, (double *)output + 0, (double *)output + 1, stream);
    }
    case (int)sizeof(float): {
        return cu_two_phase_flow_proteus_quad4_jacobian_action_i_msoa(scalar_bytes, nelements, nnodes, elements, points, ((const float *)parameters)[0], ((const float *)parameters)[1], ((const float *)parameters)[2], ((const float *)parameters)[3], ((const float *)parameters)[4], ((const float *)parameters)[5], ((const float *)parameters)[6], ((const float *)parameters)[7], ((const float *)parameters)[8], ((const float *)parameters)[9], ((const float *)parameters)[10], ((const float *)parameters)[11], ((const float *)parameters)[12], ((const float *)parameters)[13], ((const float *)parameters)[14], ((const float *)parameters)[15], ((const float *)parameters)[16], ((const float *)parameters)[17], ((const float *)parameters)[18], ((const float *)parameters)[19], ((const float *)parameters)[20], 2, (const float *)current + 0, (const float *)current + 1, 2, (const float *)direction + 0, (const float *)direction + 1, 2, (float *)output + 0, (float *)output + 1, stream);
    }
    default:
      break;
  }
  return sfem::codegen::unsupported_dispatch("two_phase_flow_proteus_quad4_jacobian_action_i_maos", -1, (int)scalar_bytes);
}
