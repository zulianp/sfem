...
namespace sfem {
  class GPUGeneratedTwoPhaseFlow final : public Op {
  public:
    static std::unique_ptr<Op> create(const std::shared_ptr<FunctionSpace> &space);

    explicit GPUGeneratedTwoPhaseFlow(const std::shared_ptr<FunctionSpace> &space);
    ~GPUGeneratedTwoPhaseFlow() override;

    const char *name() const override { return "GPUGeneratedTwoPhaseFlow"; }
