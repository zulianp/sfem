...
#pragma omp parallel for
    ...
    std::unique_ptr<SSMeshContactSurface> SSMeshContactSurface::create(const std::shared_ptr<FunctionSpace> &space,
        ...
        auto &ssmesh   = space->semi_structured_mesh();
        auto  ss_sides = sfem::create_host_buffer<idx_t>((ssmesh.level() + 1) * (ssmesh.level() + 1), sideset->parent()->size());
        ...
        ssquad4_to_standard_quad4_mesh(ssmesh.level(), sideset->parent()->size(), ss_sides->data(), sides->data());
