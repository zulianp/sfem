...
inline int semi_structured_level(const Mesh &mesh) {
    return smesh::proteus_hex_micro_elements_per_dim(mesh.element_type(0));
}

inline idx_t **semi_structured_element_data(Mesh &mesh) {
    return mesh.elements(0)->data();
}

inline geom_t **semi_structured_point_data(Mesh &mesh) {
...
inline ptrdiff_t semi_structured_interior_start(const Mesh &mesh) {
    ...
    const ptrdiff_t n_interior_per_element =
            level > 1 ? static_cast<ptrdiff_t>(level - 1) * (level - 1) * (level - 1) : 0;
...
inline int semi_structured_apply_hierarchical_renumbering(Mesh &mesh) {
    return smesh::semistructured_hierarchical_renumbering(
            mesh.element_type(0), semi_structured_level(mesh), mesh.n_nodes(), mesh.elements(0));
}
