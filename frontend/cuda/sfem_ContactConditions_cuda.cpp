...
public:
    ...
    GPUContactConditions(const std::shared_ptr<ContactConditions> &dc)
        ...
        conditions = (boundary_condition_t *)malloc(n_conditions *
                                                              sizeof(boundary_condition_t));

        for (int d = 0; d < n_conditions; d++) {
            boundary_conditions_host_to_device(&h_buffer[d],
                                               &conditions[d]);
        }
    }
    ...
    ~GPUContactConditions() {
        d_destroy_conditions(n_conditions, conditions);
    }
};
